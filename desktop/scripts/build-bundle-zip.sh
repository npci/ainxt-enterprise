...
#   * The bundle DOES embed the AiNxt CLI binary from ../ainxt-cli/dist/release
#     and resources/code-skills (see build.extraResources in package.json).
#
# Usage:
#   cd desktop
...
if [[ "$TARGET" == "asar-swap" ]]; then
  ...
  for f in playwrightManager.js cliManager.js; do [[ -f "$f" ]] && cp -f "$f" "$STAGE/"; done
  ...
  if [[ -f "ainxt-windows-x64.exe" ]]; then
    mkdir -p "$RES/bin"; cp ainxt-windows-x64.exe "$RES/bin/ainxt-windows-x64.exe"
...
if [[ "${SKIP_PRECHECKS:-0}" != "1" ]]; then
  ...
  # CLI binary is embedded via build.extraResources — build fails without it.
  CLI_DIR="$REPO_ROOT/ainxt-cli/dist/release"
  if ! ls "$CLI_DIR"/$CLI_GLOB >/dev/null 2>&1; then
    die "AiNxt CLI binary not found: $CLI_DIR/$CLI_GLOB
     electron-builder embeds it (build.extraResources). Build the CLI first, or
     place the release binary there. See ainxt-cli build docs."
  fi
