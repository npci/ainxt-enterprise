 ...
 * "computer use" — built on Playwright rather than Anthropic's stubbed native
 * modules.
 *
 * SECURITY (AiNxt):
...
function _log(action, detail) {
  ...
  console.log(`[cowork-browser] ${action}${detail ? " · " + detail : ""}`);
}
...
// Content-Security-Policy for all local HTML responses served by this module.
const _CSP_HEADER =
...
function _audit(opts, action, target, allowed, reason) {
  ...
  try {
    _gwPost(opts, "/cowork/computer-use/audit", {
