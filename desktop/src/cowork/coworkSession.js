...
// Tells the agent it is an office assistant wired to AiNxt connectors / KB / docs
...
const OFFICE_PROMPT =
  "You are AiNxt Buddy — an AI OFFICE ASSISTANT for an AiNxt employee (a NON-engineer). " +
...
class CoworkOfficeSession {
  ...
  _injectMcpIntoConfigLegacy() {
    ...
    try {
      ...
      const allow = this.role && Array.isArray(this.role.allowed_connectors) ? this.role.allowed_connectors : [];
      if (allow.length) coworkArgs.push("--header", `x-cowork-allowed-tools: ${allow.join(",")}`);
      ...
      execFileSync(bin.command, [...bin.args, ...coworkArgs], {
    ...
    try {
      if (this.localMcpPort) {
        ...
        execFileSync(bin.command, [
          ...
          "ainxt_desktop", `http://127.0.0.1:${this.localMcpPort}/sse?surface=cowork${root}`,
  ...
  async _spawn() {
    ...
    const cwd = granted
      ? this.cwd
      : (this._scratchDir && fs.existsSync(this._scratchDir) ? this._scratchDir : fs.mkdtempSync(path.join(os.tmpdir(), "ainxt-cowork-")));
    ...
    // x-ainxt-surface: cowork header so the gateway tags model_usages rows
  ...
  async _spawnStreamLegacy(cwd, granted) {
    ...
    // Also inject x-ainxt-surface: cowork and x-ainxt-conv-id into config.toml
    ...
    try {
      ...
      if (this.localMcpPort) {
        ...
        execFileSync(bin.command, [
          ...
          "--scope", "user",
          "ainxt_desktop",
          `http://127.0.0.1:${this.localMcpPort}/sse?surface=cowork${root}`,
        ], { env: { ...process.env }, timeout: 5000, stdio: "pipe" });
        added.push("ainxt_desktop");
  ...
  _removeMcpFromConfig() {
    ...
    // Also sweep the legacy "ainxt_cowork" name for users upgrading from a
    // previous build — otherwise their config.toml keeps a stale entry and the
    // CLI advertises every Cowork tool twice (once under each prefix). The
    // `ainxt mcp remove` call is tolerant of "not present", so this is safe
    // to run unconditionally.
    if (!toRemove.includes("ainxt_cowork")) toRemove.push("ainxt_cowork");
  ...
  // Write x-ainxt-surface: cowork into [models.extra_headers] in config.toml
  ...
  _injectSurfaceHeader() {
    try {
      this._injectExtraHeader("x-ainxt-surface", "cowork");
      _trace("SURFACE_HEADER_INJECT", "x-ainxt-surface: cowork ensured in config.toml");
    } catch (err) {
