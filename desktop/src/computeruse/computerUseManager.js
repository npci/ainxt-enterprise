...
"use strict";
/**
 * Native OS computer-use for Buddy (AiNxt).
 *
 * Lets the Buddy agent control the machine — move/click the mouse, type, press
...
async function _confirm(action, detail) {
  try {
    const { response } = await dialog.showMessageBox({
      ...
      defaultId: 0,
      cancelId: 0,
      title: "Buddy wants to control your computer",
      message: `Allow Buddy to ${action}?`,
...
async function _audit(opts, action, target, allowed, blockReason, findings, redacted) {
  ...
  try {
    ...
    const body = JSON.stringify({
      session_id: opts.sessionId || "buddy", action, target: target || "",
