...
"use strict";
/**
 * DispatchPoller — the DESKTOP half of Buddy "Dispatch" (mobile/web → desktop).
 *
 ...
 * claims them one at a time, runs them through a headless Buddy agent session,
 ...
 *     and is never logged.
 */
const { BuddySessionManager } = require("./buddySession");
...
class DispatchPoller {
  ...
  constructor(deps = {}) {
    ...
    this.mgr = new BuddySessionManager((sid, ev) => {
  ...
  // Run one dispatched prompt through a headless Buddy session; resolve the outcome.
  async _run(dispatch, base, jwt) {
    ...
    if (!id || !created.ready) {
      ...
      return { status: "failed", error: "Could not start a Buddy session on the desktop." };
    }
