...
"use strict";
/**
 * Buddy auth helpers. The CLI owns the real auth lifecycle (JWT in
 * ~/.ainxt/config.json, auto-refresh). Buddy only needs to know whether a token
...
const _apiKeySecret = _makeSecret("buddyApiKeyEnc");
 ...
 * Buddy's gateway-side tools (connector MCP, memory, run_code, doc-gen) must
...
/**
 * Resolve the best WORKING gateway token for a Buddy session. Prefers the CLI
