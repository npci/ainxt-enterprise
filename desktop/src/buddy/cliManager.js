...
// See buddySession.js's _realToolName for the full explanation: the ACP CLI
...
class CliSession {
  constructor(id, cwd, emit, resumeId) {
    ...
    this._permissionMode = "default"; // "default" | "acceptEdits" | "plan" — see setPermissionMode()
  ...
  start() {
    ...
    // Kept for the mid-session auth-failure check in the stderr handler below —
    // avoids re-resolving (and potentially re-answering differently) mid-session.
    this._cliPath = bin.command;
    ...
    this.proc.stderr.on("data", (d) => {
      ...
      if (text) {
        ...
        // Mid-session auth failure (token revoked/expired after this session
        // already started — the "buddy:create" pre-flight check in main.js
        // only covers session START). Re-signing-in from here is intentionally
        // NOT automated (see main.js's buddy:create handler comment): ainxt
        // login is interactive and a headless spawn has nowhere to receive the
        // pasted token. Surface a clear, CLI-specific message instead of the
        // raw stderr as a generic notice.
        if (/\b401\b/i.test(text) || /unauthorized/i.test(text) || /not logged in/i.test(text) || /no credentials/i.test(text)) {
          const loginCmd = this._cliPath ? `"${this._cliPath}" login` : "ainxt login";
          this.emit(this.id, {
            type: "error",
            code: "auth_required",
            msg: `AiNxt CLI sign-in expired. Open a terminal, run ${loginCmd}, then try again in Buddy.`,
          });
          return;
        }
  ...
  _handleSdkMessage(msg) {
    ...
    switch (method) {
      ...
      case "session/update": {
        ...
        // tool_use_result (old) OR tool_call_update with status completed (v0.2.101+)
          ...
          if (status === "completed") {
            ...
            // Surface WHY it failed (see buddySession.js for the full rationale) —
      ...
      case "agent/confirm": {
        ...
        _trace("PERM_CHECK", { via: "agent/confirm", mode: this._permissionMode, reqId, toolName });
        // "Auto-accept edits" applies here too — this is the older confirm
        // protocol variant (pre session/request_permission); some CLI builds
        // use this one instead, so it must be gated the same way or "Auto-
        // accept" silently does nothing for anyone still on this path.
        if (this._permissionMode === "acceptEdits" && reqId !== undefined && reqId !== null) {
      ...
      case "_ainxt.dev/session_notification": {
        ...
        // /compact sends no agent_message_chunk text, so without this the
        // final "result" carries an empty _streamBuffer and the chat just
        // goes blank after the spinner — surface what actually happened.
        else if (upd.sessionUpdate === "auto_compact_completed") {
          const before = upd.tokens_before, after = upd.tokens_after;
          const text = upd.summary_preview
            ? `\n\n🗜️ *Context compacted* (${before} → ${after} tokens)\n\n${upd.summary_preview}\n\n`
            : (before === after
              ? `\n\n🗜️ *Nothing to compact* — conversation is already small (${before} tokens).\n\n`
              : `\n\n🗜️ *Context compacted*: ${before} → ${after} tokens.\n\n`);
          this._deltasSinceAssistant = true;
      ...
      case "session/request_permission": {
        ...
        _trace("PERM_CHECK", { via: "session/request_permission", mode: this._permissionMode, reqId, toolName, options });
        // "Auto-accept edits" mode: the CLI's own bypassPermissions flag was
        // rejected at spawn time because it silently auto-executes without
        // ever emitting this request at all (see spawn args) — so this is
        // enforced here instead, by answering immediately without showing
        // the Allow/Don't Allow dialog. "default" and "plan" still confirm
        // every tool call; "plan" only changes exit_plan_mode's behavior below.
        if (this._permissionMode === "acceptEdits" && reqId !== undefined && reqId !== null) {
          const chosen = options.find((o) => o.kind === "allow_once" || o.optionId === "allow-once")
            || { optionId: "allow-once" };
      ...
      // "default"/"acceptEdits": auto-approve so the agent proceeds immediately
      // (as before). "plan" mode's whole point is to stop here and let the user
      // actually review before execution starts, so it waits for a real Allow/
      // Don't Allow answer instead of auto-approving.
      case "_ainxt.dev/exit_plan_mode": {
        const reqId = msg.id;
        const planContent = params.planContent || "";
        if (this._permissionMode === "plan") {
          if (reqId !== undefined && reqId !== null) {
            this._pendingPerms.set(reqId, { isPlanApproval: true });
          }
          // Show the plan as a normal (markdown-rendered) chat message — the
          // permission bar itself only has room for a one-line inline detail,
          // not a multi-paragraph plan.
          if (planContent) {
  ...
  _runLegacy(task, model) {
    ...
    const proc = spawn(bin.command, args, {
      cwd,
      env: { ...process.env, FORCE_COLOR: "0", AINXT_IS_BUDDY: "1", ...TLS_ENV },
    });
    this.proc = proc;
  ...
  respondConfirm(requestId, answer) {
    ...
    // ── Plan-mode approval — { approved: bool }, not the tool-permission shape ──
    if (perm && perm.isPlanApproval) {
      this._writeRpcResponse(requestId, { approved: allowed });
      return;
    }
    ...
    if (perm && perm.isPermissionRequest) {
      ...
      let chosen;
      if (!allowed) {
        chosen = options.find((o) => o.kind === "reject_once" || o.optionId === "reject-once")
        ...
        chosen = options.find((o) => o.kind === "allow_once" || o.optionId === "allow-once")
          || { optionId: "allow-once" };
      }
  ...
  interrupt() {
    ...
    // NEW CLI (ACP): the agent method is `session/cancel` (a client→agent
    // notification carrying sessionId), confirmed from the CLI binary's agent
    // method table (session/new · load · set_mode · prompt · cancel · set_model)
    // — see buddySession.js's interrupt() for the same fix applied earlier.
    // `notifications/cancelled` is not a method the ACP agent handles at all,
    // so Stop never actually cancelled the turn AND the UI stayed "busy"
    // forever (no result was ever emitted for it). Two fixes:
    //   1) Send the correct `session/cancel` notification.
    //   2) Immediately clear busy + emit an "interrupted" result so the Stop
    //      button releases right away instead of waiting on the CLI.
    // We deliberately do NOT kill the process (unlike streamjson): the ACP
    // agent is a persistent JSON-RPC server reused for the NEXT prompt, and
    // there's no re-spawn path for it — killing it would wedge every future
  ...
  async setPermissionMode(mode) {
    // The CLI itself has no runtime mode-change RPC and its own bypassPermissions
    // flag silently auto-executes everything without ever asking (see the spawn
    // args above) — unsafe to send at spawn either. So "default" / "acceptEdits" /
    // "plan" are enforced entirely on the desktop side: session/request_permission,
    // agent/confirm, and exit_plan_mode below branch on this._permissionMode
    // instead of always prompting / always auto-approving.
    this._permissionMode = mode || "default";
    _trace("SET_PERMISSION_MODE", { sessionId: this.id, mode: this._permissionMode });
    return true;
  }
