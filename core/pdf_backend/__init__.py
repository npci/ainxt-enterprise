...
files, with roughly 99 call sites and no abstraction. PyMuPDF was **AGPL-3.0**,
which is one-way incompatible with this project's Apache-2.0 licence: Apache
code may be absorbed into an AGPL work, but AGPL code may not be redistributed
under Apache terms. This package removed the direct binding so call sites use
...
------------------------------------
This is a real trade, stated plainly rather than buried. PyMuPDF's
`pymupdf4llm` contributed heading inference, page batching and repeated
header/footer stripping to Markdown extraction. Without it, `core.document_parser`
...
#: The only backend: permissively-licensed pdfium. PyMuPDF (AGPL-3.0) has
#: been removed from this project entirely — see NOTICE.
...
def _selected() -> str:
    ...
    if name == "pymupdf":
        raise PdfBackendError(
            "%s=pymupdf is no longer supported: PyMuPDF (AGPL-3.0) has been "
            "removed from this project. Unset %s or set it to 'pdfium'."
...
def to_markdown(path: Any, **kwargs: Any) -> str:
    ...
    Batching/header options that the removed pymupdf4llm rung used to accept
    are rejected rather than ignored, because a caller asking for a page range
