...
def sdlc_stage_hint(stage: str, default: str = "complex") -> str:
    ...
    Invalid hints fall back to the code default. solution → complex when ENABLE_OPUS
    is off, so callers never need to special-case Opus availability.
    """
    base = SDLC_STAGE_MODEL_DEFAULTS.get(stage, default)
    env  = os.getenv(f"SDLC_MODEL_{stage.upper()}")
...
def cli_model_for_tier(hint: str) -> str:
    ...
    _tier_to_model = {
        "solution":  CLAUDE_OPUS_MODEL,
        "complex":   CLAUDE_PRIMARY_MODEL,
        "haiku":     CLAUDE_HAIKU,
        "medium":    OPENAI_CODING_MODEL,
        "deep":      OPENAI_LATEST_MODEL,
    }
    model_id = _tier_to_model.get((hint or "").strip().lower(), CLAUDE_PRIMARY_MODEL)
...
def openai_model_for_tier(hint: str) -> tuple[str, bool]:
    ...
    _openai_tiers = {
        "deep":   OPENAI_LATEST_MODEL,
        "medium": OPENAI_CODING_MODEL,
        "mini":   OPENAI_SIMPLE_MODEL,
        "simple": OPENAI_SIMPLE_MODEL,
    }
    if h in _openai_tiers:
        return _openai_tiers[h], False
    return OPENAI_LATEST_MODEL, True
...
def tier_cost_per_1m(hint: str) -> tuple[float, float]:
    ...
    hint to its concrete model id and reads the canonical MODEL_COST_PER_1M.
    Reads ENABLE_OPUS at call time for the solution tier. Local/simple → (0, 0).
    Unknown hints fall back to the Sonnet (complex) rate, never $0, so an
    unrecognised tier over-bills rather than silently under-bills.
    """
    h = (hint or "").strip().lower()
    if h in ("local", "simple"):
        return (0.0, 0.0)
    _map = {
        "solution": CLAUDE_OPUS_MODEL if os.getenv("ENABLE_OPUS", "true").lower() in ("true", "1", "yes") else CLAUDE_PRIMARY_MODEL,
        "complex":  CLAUDE_PRIMARY_MODEL,
        "haiku":    CLAUDE_HAIKU,
        "medium":   OPENAI_CODING_MODEL,
        "deep":     OPENAI_LATEST_MODEL,
    ...
    return MODEL_COST_PER_1M.get(_map.get(h, CLAUDE_PRIMARY_MODEL), MODEL_COST_PER_1M.get(CLAUDE_PRIMARY_MODEL, (0.0, 0.0)))
