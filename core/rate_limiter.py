...
# ── Feature flag ──────────────────────────────────────────────────────────────
# RATE_LIMIT_ENABLED=true   → rate limiting is active (default — safe for prod).
    ...
    logger.warning(
        ...
        extra={
            ...
            "reason": "RATE_LIMIT_ENABLED env var is false — all rate limiting bypassed. "
                      "Rate limiting is ENABLED by default; set RATE_LIMIT_ENABLED=false only in dev/test.",
        },
    )
...
def _get_redis():
    ...
    When Redis is unavailable and RATE_LIMIT_ENABLED is True, callers fall
    back to the in-process _fallback_counters dict rather than skipping the
    check entirely (fail-closed on a single node).
    """
    try:
        ...
        if RATE_LIMIT_ENABLED:
            logger.warning(
                ...
                extra={
                    ...
                    "reason": "Redis unreachable — falling back to in-process counter. "
                              "Rate limiting is ENABLED by default; limits are enforced "
                              "per-process only (non-distributed) until Redis recovers.",
                },
            )
        return None
...
#: POST /auth/forgot-password — 10 per 15 minutes per IP
# This endpoint is unauthenticated, takes no token or ownership proof, and
# overwrites the account password outright, so unthrottled it lets anyone who
# knows an email address lock that user out on demand. It needs *a* limit.
#
# But it is the single reset path for everyone — admins included, there is no
# separate admin reset endpoint — and scope can only be "ip" here because
# "user"/"ip+user" need an authenticated user_id this endpoint does not have.
# Behind a corporate NAT every employee shares one egress IP, so too tight a
# limit means one person's retries lock out the whole office, including the
# admin trying to recover their own account. 10/15min blunts the abuse while
# leaving room for a shared egress and an admin running a few resets.
AUTH_FORGOT_PASSWORD = RateLimitConfig(
    limit=10,
    window_seconds=900,
