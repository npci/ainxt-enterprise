...
def parse_pdf(path: str) -> str:
    ...
    Markdown extraction ladder, best fidelity first:
      1. markitdown (MIT) — via core.pdf_backend.to_markdown(). No heading
         inference, no batching, no header/footer strip
         See core/pdf_backend/pdfium_backend.py.
...
def parse_image(path: str, filename: str) -> str:
    """Describe an image with Gemini vision; placeholder only if that truly fails.
    ...
    "[Image: <name> — vision unavailable]" on deployments that talk to Gemini
    directly, even with a working GEMINI_API_KEY. That placeholder then became the
    attachment's parsed_text / image_description / image_caption, which is what the
    model saw on turn 1 and on every follow-up turn ("vision isn't loading on my
    end"). The failure was invisible because the fallback is a plausible string
    ...
    _fallback = f"[Image: {filename} — vision unavailable]"
    try:
        ...
        from gateway_gemini import generate_with_image as _vision
        _text = (_vision(
        ...
        # generate_with_image reports failure in-band as a plain sentence rather
        # than raising, so those have to be caught here or they get persisted as
        # if they were a real description of the image.
        if not _text:
            logger.error(f"parse_image: vision returned empty text for {filename!r}")
            return _fallback
        if _text.startswith(("Error generating response", "Request blocked")):
            logger.error(f"parse_image: vision failed for {filename!r}: {_text}")
            return _fallback

        _via = "proxy" if os.environ.get("LLM_PROXY_URL", "").strip() else "direct"
        logger.info(
            f"parse_image: vision OK ({_via}) for {filename!r} ({len(_text)} chars)"
        )
        return _text
    except Exception as e:
        # ERROR, not warning: the caller stores this placeholder as the image's
        # description, so the model is left blind for the rest of the chat.
        logger.error(f"parse_image: vision unavailable for {filename!r} ({e}); using fallback")
        return _fallback
