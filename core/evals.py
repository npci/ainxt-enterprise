...
# Set EVAL_ENABLED=true in .env to enable LLM-as-judge evaluation calls.
#
# EVAL_ENABLED=false (OSS default) — all eval calls are skipped. No extra LLM
#                    cost, no extra latency per chat message. Safe for local dev
#                    and for OSS deployments that don't need quality monitoring.
# EVAL_ENABLED=true — LLM-as-judge runs after every chat
#                    response for quality monitoring across the organisation.
EVAL_ENABLED: bool = os.getenv("EVAL_ENABLED", "false").lower() not in ("false", "0", "no")
