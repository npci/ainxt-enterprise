...
def validate_register_request(body: Any) -> Tuple[bool, Dict[str, List[str]], Dict[str, Any]]:
    ...
    - password → intentionally NOT run through XSS/character-set checks (a password
                 is never rendered and restricting its charset would weaken entropy);
                 only a minimum-length check is enforced here as defense-in-depth
                 (the client already enforces this, but the server must not rely on it).
