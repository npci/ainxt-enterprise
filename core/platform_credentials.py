...
def extract_gitlab_pat(stored_value: str) -> str:
    ...
    the BARE token only. This returns that bare token regardless of format:
      ...
      1. If a ``glpat-``/``gloas-``/``glptt-`` prefixed substring is present,
         ...
         Note: ``.`` is deliberately included in the character class because
         GitLab's current PAT format can embed a literal ``.`` (e.g. a
         ``.NN.NN``-style suffix). Omitting it caused the match to stop at the
         first dot and silently return a truncated token, which GitLab then
         rejected with a 401 indistinguishable from a real auth failure.
    ...
    if not v:
        return v
    import re
    # Include '.' in the character class: the new routable GitLab PAT format is
    # glpat-<base>.<ver>.<crc> (contains dots). Excluding '.' truncated such a
    # token at the first dot, yielding a short/invalid token → GitLab HTTP 401
    # on branch/clone/MR operations.
