...
encodes every rule in the brand contract supplied above — the 16:9 canvas, margins, palette, type
...
**The maximums above are enforced by silent truncation.** Pass 9 bullets and only
the first 6 reach the slide; pass 20 table rows and only the first 7 do — with no
error and a clean build. If you have more content than a pattern holds, split it
across slides yourself.
...
**Never invent numbers.** If the user gave you no figures, do not manufacture KPIs,
percentages or chart values to fill `metric` and `table`. Carry the argument with
`evidence`, `split` and `statement` instead. A confident-looking fabricated number is
the worst thing a deck can contain.
...
6. Any figure, percentage or chart value the user never gave you? → remove it.
7. Any list longer than its pattern's maximum (it would be silently cut)? → split it.
8. Placeholder text — `REPLACE`, `TODO`, `Lorem`, `Sample` — all gone?
9. Does the script end in `d.save()`?
