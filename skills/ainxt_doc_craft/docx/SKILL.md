---
name: docx-craft
description: Build brand-compliant .docx (and PDF, via export) with docx-js. Read with DOCXJS.md and SKELETON.js.
license: Apache-2.0
...
You are producing a document that will be circulated and relied on. Obey
`INTERNAL_BRAND.md` (supplied above this file) for every colour, size and prohibition.
This file tells you **how to compose**. `DOCXJS.md` gives you the API facts.
`SKELETON.js` is a working file you may fill in rather than starting from nothing.
...
7. Every font size a value from the brand type scale, expressed in **half-points**
   (11pt → `size: 22`)?
8. Every spacing value a multiple of 80 twips (4pt)?
9. Footer present with page number and classification?
10. Logo guarded by an existence check so a missing file cannot throw?
11. Placeholder text — `Lorem`, `TODO`, `REPLACE`, `Sample` — all gone?
12. Does `Packer.toBuffer` complete before the process exits?
