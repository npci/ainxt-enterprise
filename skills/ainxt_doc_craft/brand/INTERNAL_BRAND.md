---
name: internal-brand
description: Worked example of a real, filled-in enterprise brand contract for document artifacts (docx, pptx, xlsx, pdf), based on AiNxt's own visual identity. Copy this pattern for your own organisation — see brand/BRAND.md for the blank OSS default.
license: MIT
---

# Example Enterprise Document Identity Contract

This is a **worked example**, not a live requirement — it shows what a real,
filled-in brand contract looks like, using AiNxt's own visual identity as the
sample content. Copy this pattern into your own file (e.g.
`brand/<YourOrg>_BRAND.md`) and point `DOC_BRAND_FILE` at it in your `.env` to
apply your organisation's own colors, fonts and rules the same way. Every
artifact produced under an active brand contract follows its visual identity
and governs docx, pptx, xlsx and pdf equally — where a format recipe conflicts
with the active contract, **the contract wins**.
