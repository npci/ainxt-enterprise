...
#
# app.api.triggers lives under AgentStudio/backend and pulls apscheduler +
# AgentStudio config, so the whole module is skipped cleanly if those deps
...
_ABS_BACKEND = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "AgentStudio", "backend")
)
...
triggers = pytest.importorskip(
    "app.api.triggers",
    reason="AgentStudio backend deps (apscheduler/config) not installed",
)
