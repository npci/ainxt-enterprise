...
def _ensure_abstudio_on_path() -> None:
    """Prepend ``<repo>/ABStudio/backend`` to sys.path so ``app.*`` imports work.
    ...
    abs_backend = os.path.join(repo_root, "ABStudio", "backend")
...
def dispatch_due_triggers() -> dict:
    ...
    try:
        ...
        # Gateway startup (gateway.py) prepends ABStudio/backend to sys.path so
