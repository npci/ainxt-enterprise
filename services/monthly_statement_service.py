...
def _resolve_monthly_budget(db: Session, user: User, tier_default: float) -> float:
    """
    Prefer an explicit row in budget_configs (user-level wins over band-level),
    fall back to the tier default we derived from ad_level.
    """
    row = (
    ...
    if user.ad_level is not None:
        row = (
            db.query(BudgetConfig)
            .filter(BudgetConfig.band_level == int(user.ad_level))
            .first()
        )
        if row is not None:
            return float(row.monthly_limit_usd)
