...
async def lifespan(app: FastAPI):
    ...
    if NOMIC_EMBED_URL:
        try:
            ...
            # Startup probe — one real embed call to confirm the configured
            # endpoint is reachable and returning the expected vector dimension.
            probe_nomic = await asyncio.wait_for(
                _nomic.embed(["startup probe: nomic connectivity check"]),
                timeout=30.0,
            )
...
class RerankResponse(BaseModel):
    results:    list[dict]
    latency_ms: float = 0.0
    # model/degraded reflect reranker.reranker_info() at call time — lets
    # callers (models/hybrid_retriever.py) tell "the configured CrossEncoder
    # scored these" apart from "the model never loaded, this is a plain RRF
    # fusion score" instead of silently treating both as equally reliable.
    model:      str  = ""
    degraded:   bool = False
...
async def embed(req: EmbedRequest):
    ...
    try:
        if req.provider == "openai":
            # Unlike _nomic, _openai is ALWAYS constructed at startup (see
            # lifespan()) regardless of whether OPENAI_API_KEY is set, so
            # "is it None" can never catch a missing key here — without this
            # explicit check, a request would sail through to a real HTTP
            # call to OpenAI with an empty Bearer token and fail with a raw
            # 401 from OpenAI's side, surfaced as an opaque 500 with none of
            # the "what do I set" guidance the Nomic branch below gives.
            if not OPENAI_API_KEY:
                raise HTTPException(
                    status_code=503,
                    detail=(
                        "OpenAI embedder not configured — set OPENAI_API_KEY "
                        "(and optionally OPENAI_EMBED_MODEL) in .env and "
                        "restart the embed service"
                    ),
                )
            if _openai is None:
...
async def rerank(req: RerankRequest):
    ...
    from services.embed_svc.reranker import rerank as _rerank, reranker_info as _reranker_info
    ...
    latency_ms = (time.perf_counter() - t0) * 1000
    _info = _reranker_info()
    return RerankResponse(
        results=results,
        latency_ms=round(latency_ms, 2),
        model=_info["model"],
        degraded=_info["fallback"],
    )
...
async def health():
    ...
    # ── Nomic (custom endpoint) embed probe ─────────────────────────
    nomic_svc_ok  = False
    nomic_svc_dim = 0
    nomic_svc_err = ""
    if _nomic:
    ...
    return {
        ...
        # Nomic embed probe (custom / remote OpenAI-compatible endpoint)
