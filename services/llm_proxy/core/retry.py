...
async def retry_llm_async(fn, *args, max_attempts: int = 3, base_delay: float = 1.0, **kwargs):
    """Async version of retry_llm — uses asyncio.sleep for non-blocking backoff."""
    import asyncio
    last_exc = None
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            delay = base_delay * (2 ** (attempt - 1))
            logger.warning(
                f"retry_llm_async: attempt {attempt}/{max_attempts} failed "
                f"({e}), retrying in {delay:.1f}s"
            )
