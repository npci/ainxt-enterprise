...
from store.budget_store import reset_usage, update_cached_cost_limit, BASE_COST_USD
...
#: Base allocation every user resets to. Mirrors store.budget_store.BASE_COST_USD
#: — default $50, configurable via the BUDGET_BASE_COST_USD env var.
_RESET_BASE_USD = Decimal(str(BASE_COST_USD))
...
def _snapshot_one_user(user_id: str, period_yyyymm: str) -> str:
    ...
    try:
        ...
        # Flat reset: base returns to _RESET_BASE_USD and the pooled extra
        # (all of it HOD-granted) is fully drained back to zero. Nothing
        # carries over month to month.
        new_total = _RESET_BASE_USD
        ...
        #
        # Base returns to the platform base allocation (_RESET_BASE_USD,
        # default $50, configurable via BUDGET_BASE_COST_USD) for EVERY user
        # — no band differentiation applies to the cost dimension.
        # _get_band_allocation is kept only for the audit `default_limit_usd`
        # snapshot column, preserving its historical shape.
        #
        # Extra is fully flattened to $0 — HOD-granted extra does NOT carry
        # over month to month.
        #
        # This write MUST stay inside the `if inserted:` branch — a re-run
        # for the same period returns "skipped" above, so the reset lands
        # exactly once per period. Applying it outside would double-reset.
        cfg.base_cost_usd    = _RESET_BASE_USD
        cfg.extra_cost_usd   = _ZERO
        # monthly_limit_usd is the audit's closing_limit_usd next period AND
        # the sole input to monthly-statement utilisation.
        ...
        update_cached_cost_limit(
            ...
            new_extra_cost_usd=float(_ZERO),
        )
...
def _render_pre_reset_html(
    ...
    <html><body style="font-family:Segoe UI,Arial,sans-serif;color:#222;">
      ...
      <p style="margin-top:16px;">After the reset:
        <ul>
          <li>Your base budget allocation returns to the platform default of {_fmt_usd(_RESET_BASE_USD)}.</li>
    ...
    text_body = (
        ...
        f"After the reset your counters will be zeroed, your base returns to "
        f"{_fmt_usd(_RESET_BASE_USD)},\n"
...
def _render_post_reset_html(
    ...
    html_body = f"""
      <html><body style="font-family:Segoe UI,Arial,sans-serif;color:#222;">
        ...
        <p style="margin-top:16px;color:#555;font-size:12px;">
          Every user has been reset to a {_fmt_usd(_RESET_BASE_USD)} base allocation, and any HOD-approved extra
          budget from last month has been cleared.
        </p>
    ...
    text_body = (
        ...
        f"Every user has been reset to a {_fmt_usd(_RESET_BASE_USD)} base allocation, and any HOD-approved extra\n"
               f"budget from last month has been cleared.\n\n"
               f"— AiNxt Platform\n"
    )
    return html_body, text_body
