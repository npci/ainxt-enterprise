    ...
    *, doc_id: str, chat_id: Optional[str], user_id: str,
    ...
    try:
        ...
        try:
            ...
            if existing:
                return
            # chat_attachments.chat_id is FK-constrained to chats(id), and the
            # chats row is created lazily in a background thread by
            # gateway.py::_save_chat_messages — which this mirror can outrun on
            # the first turn of a new chat. See db/chat_rows.ensure_chat_row.
            from db.chat_rows import ensure_chat_row
            ensure_chat_row(db, str(chat_id), str(user_id))
