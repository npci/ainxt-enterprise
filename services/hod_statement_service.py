...
def generate_and_send_hod_bulk(
    ...
    No-op (logged) when HOD_APPROVAL_ENABLED is False — flat/admin-only
    deployments have no department/HOD hierarchy, so there is nothing to
    digest and no HOD inbox to send it to. Per plan, this is suppressed
    entirely rather than redirected to an "admin rollup" equivalent, since no
    such equivalent exists today.
    """
    from core.config import HOD_APPROVAL_ENABLED
    if not HOD_APPROVAL_ENABLED:
        logger.info(
            "hod_statement: generate_and_send_hod_bulk skipped — "
            "HOD_APPROVAL_ENABLED is False (flat/admin-only mode)."
        )
        return {
            "ok": True, "period": f"{year:04d}-{month:02d}",
            "total_departments": 0, "sent": 0, "skipped": 0, "failed": 0,
        }
