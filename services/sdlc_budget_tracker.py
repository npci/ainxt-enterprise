...
# ── Flat-mode (HOD_APPROVAL_ENABLED=False) preflight ─────────────────────────

def _check_org_budget(run_id: str, who: str) -> tuple[bool, str]:
    """
    Flat-mode preflight: gate against the org-wide monthly cap
    (services/org_budget_governor.py) instead of a per-HOD allocation cap.
    No department lookup, no HOD email, no ainxt.department_hod_mapping
    involved. Mirrors check_hod_budget()'s HOD-mode enforcement semantics:
    advisory-only (log + allow) when HOD_CAP_ENFORCEMENT_ENABLED is false.
    """
    from services.hod_budget_governor import _enforcement_enabled
    from services.org_budget_governor import get_org_cap_status

    enforce = _enforcement_enabled()

    try:
        status = get_org_cap_status()
    except Exception as exc:
        logger.warning(
            "sdlc_budget: get_org_cap_status failed (allowing run): %s", exc
        )
        return True, ""

    if enforce and status.remaining_usd <= Decimal("0.00"):
        return False, (
            f"SDLC run blocked: org-wide monthly budget exhausted. "
            f"Used: ${float(status.consumed_usd):.2f} of ${float(status.cap_usd):.2f} "
            f"for period {status.period_yyyymm}. Budget resets on "
            f"{status.resets_on.isoformat()}. Contact your administrator to "
            "raise the org-wide cap."
        )

    logger.info(
        "sdlc_budget: preflight OK (flat mode) remaining=$%.2f enforce=%s run=%s user=%s",
        float(status.remaining_usd), enforce, run_id, who,
    )
    return True, ""
...
def check_hod_budget(user_id: str, run_id: str, user_email: str = "") -> tuple[bool, str]:
    ...
    Flat mode (HOD_APPROVAL_ENABLED=False, the default): skips department/HOD
    lookup entirely (steps 1-2 below) and gates against the org-wide cap
    instead — no hod_email is involved at all, and sdlc_runs.hod_email stays
    NULL for the run (finalize_run_budget() then reserves org-wide spend
    instead of an HOD's). HOD mode (True) runs the existing steps 1-2
    unchanged, including the existing per-user "no department/no HOD
    mapping" hard-block/advisory-only behaviour.
    """
    if not HOD_APPROVAL_ENABLED:
        return _check_org_budget(run_id, user_email or user_id or "unknown")
...
def _stamp_hod_ledger_id(run_id: str, ledger_id: str) -> None:
    """Persist a ledger id (HOD or org-wide) onto sdlc_runs.hod_ledger_id for
    traceability. The column name predates the flat-mode/org-cap path but is
    reused as-is — it is simply "the ledger row this run's spend was charged
    against", whichever ledger that is.
    """
    db2 = SessionLocal()
    try:
        db2.execute(
...
def finalize_run_budget(run_id: str) -> None:
    ...
    if not hod_email:
        # Flat mode (HOD_APPROVAL_ENABLED=False) never writes hod_email at
        # preflight — reserve against the org-wide cap instead, if the flag
        # is actually off (guards against a stale/uninitialised run row from
        # before this flag existed, or a run that skipped preflight entirely).
        if not HOD_APPROVAL_ENABLED and cost_usd > 0:
            try:
                from services.org_budget_governor import reserve_org_spend
                result = reserve_org_spend("sdlc_run", cost_usd, run_id=run_id)
                ledger_id = result.get("ledger_id")
            except Exception as exc:
                logger.error(
                    "sdlc_budget: reserve_org_spend failed run=%s cost=%.4f: %s",
                    run_id, cost_usd, exc,
                )
                return
            if ledger_id:
                _stamp_hod_ledger_id(run_id, ledger_id)
            logger.info(
                "sdlc_budget: finalize OK (flat mode/org cap) run=%s cost=$%.4f ledger_id=%s",
                run_id, cost_usd, ledger_id,
            )
            return
        logger.info(
