...
class BudgetConfig(Base):
    """
    Per-user or per-band budget configuration.
    Overrides: user-level row wins over band-level row.
    Band defaults: A1-B2 = $30/month, C1+ = $50/month.
    """
    __tablename__ = "budget_configs"

    id                    = Column(UUID(as_uuid=False), primary_key=True, default=_uuid)
    # Either user_id OR band_level set — not both
    ...
    monthly_limit_usd     = Column(Float,       nullable=False, default=30.0)
    base_cost_usd         = Column(Numeric(12, 6), nullable=False, default=50.0)
    extra_cost_usd        = Column(Numeric(12, 6), nullable=False, default=0.0)
    # 10x-winner carryover. extra_cost_usd stays the POOLED extra total;
    # winner_extra_usd is the winner-origin slice of it, and the remainder
    # (extra_cost_usd - winner_extra_usd) is HOD-granted money. At the monthly
    # reset the HOD portion is drained first, so an unspent winner balance
    # carries over month to month until it is exhausted.
    winner_extra_usd      = Column(Numeric(12, 6), nullable=False, default=0.0)
    # 'YYYY-MM' of the most recent winner award; NULL once the balance hits 0.
    # Backs the server-side one-award-per-period guard.
    winner_origin_period  = Column(String(7),   nullable=True)
