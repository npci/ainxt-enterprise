...
export default defineConfig( ( { command } ) => ( {
  ...
  resolve: {
    alias: {
      // @abs → ABStudio/frontend/src
      // Allows ai-ui to import ABStudio components without moving files:
      //   import BuildStudio from '@abs/BuildStudio.jsx'
      '@abs': path.resolve( __dirname, '../ABStudio/frontend/src' ),
    },
    // Deduplicate shared packages so ABStudio components (resolved from
    // ABStudio/frontend/src/) use the SAME React/ReactDOM/Zustand instances
    // as ai-ui. Without this, Rollup tries to resolve 'react' relative to
    // ABStudio/frontend/ where there is no node_modules, causing build errors.
    dedupe: [
      'react',
      'react-dom',
      'react/jsx-runtime',
    ...
    // specifiers from ABStudio source files (which have no local node_modules).
    modules: [
  ...
  css: {
    postcss: {
      plugins: [
        prefixSelector( {
          prefix: '[data-ac]',
          // Only scope CSS that belongs to ABStudio — skip ai-ui's own files
          ...
          transform( prefix, selector, prefixedSelector, filePath ) {
            ...
            if ( !norm.includes( 'ABStudio/frontend/src' ) ) return selector;
