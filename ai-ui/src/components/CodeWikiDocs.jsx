...
function WikiCard({ job, onOpen, onRegenerate, onRetry, onDelete, regenerating, retrying, deleting }) {
  const busy = job.status === "pending" || job.status === "running";
  const failed = job.status === "failed";
  return (
    <div
      onClick={() => onOpen(job)}
      className="border border-gray-200 rounded-xl p-4 hover:bg-indigo-50 hover:shadow-sm transition bg-white group flex flex-col h-full cursor-pointer"
    >
      ...
      >
        ...
        <button
          ...
          disabled
          title="Regenerate (temporarily disabled)"
          className="p-1.5 rounded text-indigo-600 opacity-40 cursor-not-allowed"
        >
          <RefreshCw size={13} />
        </button>
        <button
...
export default function CodeWikiDocs() {
  ...
  const handleRegenerate = async (name) => {
    ...
    try {
      ...
      if (
        existing_modules_to_regenerate.length === 0 &&
        new_modules_to_create.length === 0 &&
        modules_to_remove.length === 0 &&
        !overview_should_regen
      ) {
        window.alert("Documentation is already up to date.");
        return;
      }

      let msg = "The following documentation changes will be performed:\n\n";
      if (existing_modules_to_regenerate.length) {
        msg += "Regenerate modules:\n" + existing_modules_to_regenerate.join("\n") + "\n\n";
      }
      if (new_modules_to_create.length) {
        msg += "Create modules:\n" + new_modules_to_create.join("\n") + "\n\n";
      }
      if (modules_to_remove.length) {
        msg += "Remove modules:\n" + modules_to_remove.join("\n") + "\n\n";
      }
      if (overview_should_regen) {
        msg += "Overview.md will be regenerated.\n\n";
      }
