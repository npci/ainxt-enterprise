...
import { Database, User, Sparkles, Trash2, Plus } from "lucide-react";
  ...
  return (
    <div className="mt-1">
      <div className="h-1 w-full bg-gray-100 rounded-full overflow-hidden">
        <div className={`h-full ${pct > 90 ? "bg-red-400" : "bg-gray-400"}`} style={{ width: `${pct}%` }} />
      </div>
      <div className="text-[11px] text-gray-400 mt-0.5">{used.toLocaleString()} / {max.toLocaleString()} chars ({pct}%)</div>
    </div>
  );
}
...
export default function Memory() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-xl font-semibold text-indigo-700 mb-1">Memory</h1>
        ...
        {TABS.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-1.5 px-3 py-2 text-sm transition border-b-2 -mb-px ${
                tab === t.id ? "border-indigo-600 text-indigo-700 font-medium" : "border-transparent text-gray-400 hover:text-gray-600"
              }`}>
      ...
      {tab === "memories" && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-gray-500">{entries.length} saved from your conversations</p>
            <div className="flex gap-2">
              <button onClick={loadMemories} className="px-2 py-1 rounded-md text-xs bg-gray-100 text-gray-500 hover:bg-gray-200">Refresh</button>
              {entries.length > 0 && (
                <button onClick={clearMemories} className="px-2 py-1 rounded-md text-xs bg-red-100 text-red-600 hover:bg-red-200">Clear all</button>
              )}
            </div>
          </div>
          {memLoading ? (
            ...
            <p className="text-sm text-gray-400 py-8 text-center">No memories yet. AiNxt saves key facts from your chats automatically.</p>
          ) : (
            <ul className="space-y-2">
              {entries.map(e => (
                <li key={e.id} className="group flex items-start justify-between gap-3 border border-gray-200 rounded-lg p-3 hover:bg-gray-50">
        ...
        <div className="space-y-5">
          <div>
            ...
            <textarea value={about} maxLength={CI_MAX} onChange={e => { setAbout(e.target.value); setCiSaved(false); }}
              ...
              className="w-full border border-gray-200 rounded-lg p-3 text-sm text-gray-900 bg-white h-28 resize-none focus:outline-none focus:ring-2 focus:ring-gray-300" />
          ...
          <div className="flex items-center gap-3">
            ...
            {ciSaved && <span className="text-xs text-green-600">Saved — applied to every new message.</span>}
          </div>
        </div>
      )}
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select value={prefs.tone || ""} onChange={e => setPref("tone", e.target.value)}
                className="w-full border border-gray-200 rounded-lg p-2 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-gray-300">
          ...
          <div>
            ...
            <textarea value={prefs.email_signature || ""} maxLength={SIG_MAX}
              ...
              className="w-full border border-gray-200 rounded-lg p-3 text-sm text-gray-900 bg-white h-20 resize-none focus:outline-none focus:ring-2 focus:ring-gray-300" />
          ...
          <div className="flex items-center gap-3">
            ...
            {prefsErr && <span className="text-xs text-red-500">{prefsErr}</span>}
          </div>

          {/* Saved notes (agent-learned + self-added durable facts) */}
          <div className="pt-4 border-t border-gray-100">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Saved notes <span className="text-xs text-gray-400 font-normal">({notes.length}/{NOTES_CAP})</span>
            </label>
            <p className="text-xs text-gray-400 mb-2">Durable facts Buddy remembers about you. You and the agent can add or remove these.</p>
            <div className="flex gap-2 mb-2">
              <input type="text" value={newNote} maxLength={NOTE_MAX} onChange={e => setNewNote(e.target.value)}
                ...
                className="flex-1 border border-gray-200 rounded-lg p-2 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-gray-300 disabled:bg-gray-50" />
              <button onClick={addNote} disabled={!newNote.trim() || notes.length >= NOTES_CAP}
                className="px-3 py-2 text-sm bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 disabled:opacity-40 flex items-center gap-1">
                <Plus size={14} /> Add
              </button>
            </div>
            {noteErr && <p className="text-xs text-red-500 mb-2">{noteErr}</p>}
            {notes.length === 0 ? (
        ...
        {rows.map(([k, v], i) => (
          <div key={i} className="flex gap-2">
            <input type="text" value={k} onChange={e => update(i, "k", e.target.value)} placeholder="alias"
              className="flex-1 border border-gray-200 rounded-lg p-2 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-gray-300" />
