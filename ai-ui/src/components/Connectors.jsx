...
const ENABLED_CONNECTORS = new Set(["microsoft_365", "gitlab", "jira_connector"]);
  ...
  const handleConnect = async (connector) => {
    ...
    try {
      ...
      if (!popup) {
        ...
        setOauthMessages(p => ({
          ...p,
          [connectorName]: { success: false, text: "Popup blocked. Please allow popups and click Connect again." },
        }));
        return;
      }
      ...
      const timer = setInterval(async () => {
        ...
        const closed = (() => { try { return popup.closed; } catch { return false; } })();
        if (closed) {
          await load();
