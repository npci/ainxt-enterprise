...
// `dynamicMeta` is a modelId → {label, provider, color} map built from the
// live GET /all-models catalogue (the admin "LLM Providers" screen's data —
// see the fetch in ModelGovernance() below). Checked before the static
// PROVIDER_META table so an admin-added model (a newer Claude/GPT/Gemini
// version, an openai_compatible/OpenRouter model, or an Ollama model) shows
// its real display name/provider immediately, without needing a code change
// here. Final fallback never displays the literal "Unknown" — it derives a
// generic-but-correctly-labeled entry straight from the model id.
function providerMeta(modelId, dynamicMeta = {}) {
  ...
  return (
    dynamicMeta[modelId] ||
    PROVIDER_META[modelId] ||
    { label: modelId, provider: modelId, color: 'bg-gray-100 text-gray-600' }
  )
}
...
function ModelsSection({ models, onToast, dynamicMeta }) {
  ...
  async function toggleUser(userId, modelId) {
    ...
    onToast?.(`${providerMeta(modelId, dynamicMeta).label} ${newAllowed ? 'enabled' : 'disabled'} for user`)
  }
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      {models.map(modelId => {
        const meta   = providerMeta(modelId, dynamicMeta)
...
export default function ModelGovernance() {
  ...
  // Display-metadata source: GET /all-models (the admin "LLM Providers"
  // screen's live catalogue) — every model it returns is looked up here
  // first, ahead of the static PROVIDER_META table, so a model an admin
  // adds/syncs shows its real label/provider instead of falling through to
  // a generic-looking entry.
  useEffect(() => {
    authFetch(`${API}/all-models`)
      ...
      .catch(() => { /* offline — providerMeta() falls back to PROVIDER_META / model id */ })
  }, [])
