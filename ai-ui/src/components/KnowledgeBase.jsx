...
export default function KnowledgeBase({
  ...
  // kbConfigured: null = still checking, true = embed-svc reachable,
  // false = not set up (or unreachable) — drives the setup banner below.
  // Checked once per page visit, not polled — this isn't meant to catch
  // embed-svc going down mid-session, just "was setup ever run at all".
  const [kbConfigured, setKbConfigured] = useState(null);
  useEffect(() => {
    // The gateway only serves /health under the API_BASE prefix
    // (/ainxt/v1/api/health) — see gateway.py's `_v1` router mount.
    // No auth needed, matches how install.sh/doctor.sh check it.
    fetch(`${API_BASE}/health`)
      .then(r => r.json())
      .then(d => setKbConfigured(d?.checks?.embed_svc === "ok"))
  ...
  async function handleUpload(files) {
    ...
    setScopeWarn(false);
    // The backend's `namespace` form field is a strict slug
    // (^[a-zA-Z0-9_-]{1,64}$ — store/docs_store.py upload_doc), but the
    // domain picker allows spaces/punctuation (validateIdentifier above is
    // deliberately looser, matching core/security_validation.py's identifier
    // allow-list for domain/spec_version — NOT the namespace regex). Sending
    // the raw domain straight through as the namespace 500s the upload for
    // any domain name containing a space (e.g. "Online Settlements") —
    // confirmed via live testing. Slugify it for the namespace field only;
    // `specDomain` itself is still sent unslugified under the `domain` key.
    const ns = specDomain.trim().toLowerCase()
      .replace(/[^a-z0-9_-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 64);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-white">
      ...
      {/* ── SETUP BANNER — shown when embed-svc (parsing/embedding/
          reranking) hasn't been set up, e.g. the user said "no" during
          ./install.sh. Non-blocking: the rest of the page still renders
          underneath, this is informational, not a full-page gate. */}
      {kbConfigured === false && (
        <div className="flex items-center gap-2 px-6 py-2.5 bg-amber-50 border-b border-amber-200 flex-shrink-0 text-sm text-amber-800">
          <ShieldOff size={15} className="text-amber-600 flex-shrink-0" />
          <span>
            Knowledge Base isn't set up yet — parsing, embedding, and reranking need a one-time setup.
            Run <code className="px-1 py-0.5 bg-amber-100 rounded font-mono text-xs">./kb-setup.sh</code> in
            your terminal, or see <code className="px-1 py-0.5 bg-amber-100 rounded font-mono text-xs">docs/KB_SETUP.md</code>.
          </span>
        </div>
      )}
