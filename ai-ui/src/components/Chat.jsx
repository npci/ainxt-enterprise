...
export default function Chat({
  ...
  // ── Fetch all available models + user's allowed models on mount ──────────
  useEffect(() => {
    ...
    authFetch(`${API}/chats/${activeChatId}/messages`)
      ...
      .then(data => {
        const loaded = (data.messages || []).map(m => {
          ...
          // Classify BEFORE cleaning — stripAttachmentContext removes the 🖼 marker.
          const _isImage = _hasAttach && /🖼/.test(m.content || "");
 ...
 const ok = await confirm({ title: "Delete chat", message: `Delete this "${title || 'Chat'}"? This cannot be undone.`, confirmLabel: "Delete" });
  ...
  async function sendMessage() {
    ...
    if (selectedModel === "veo-3.1-generate-preview") {
      ...
      // Backend clamps to [4, 8] (Veo API hard limit; routers/chat_router.py).
