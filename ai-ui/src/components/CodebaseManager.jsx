...
export default function CodebaseManager({ user }) {
  ...
  async function rejectRequest(reqId) {
    ...
    await authFetch(`${API}/index/requests/${reqId}/reject`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ note: reviewNote }),
    });
  ...
  async function deleteRepo(slug, name) {
    ...
    if (!ok) return;
   try {
     const res =  await authFetch(`${API}/index/repos/${slug}`, { method: "DELETE" });
  ...
  async function reindex(slug) {
    await authFetch(`${API}/index/repos/${slug}/reindex`, { method: "POST" });
