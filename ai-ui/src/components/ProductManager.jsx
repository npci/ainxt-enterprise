...
export default function ProductManager({ user }) {
  ...
  async function removeRepo(repoName) {
    if (!selected) return;
    try {
      // res.ok was never checked — a rejected removal (e.g. dept gate 403)
      // still reloaded the detail view with no error shown, indistinguishable
      // from a successful removal.
      const res = await authFetch(`${API}/products/${selected.id}/repos/${encodeURIComponent(repoName)}`, { method: "DELETE" });
      if (!res.ok && res.status !== 204) {
        const d = await res.json().catch(() => ({}));
        throw new Error(d.detail || "Remove repo failed");
      }
      loadDetail(selected.id);
    } catch (err) {
      setError(err.message || "Remove repo failed");
    }
  }
