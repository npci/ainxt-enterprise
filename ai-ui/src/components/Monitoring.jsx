...
export default function Monitoring({ user }) {
  // Monitoring is part of the admin-only "Observe" section — the App.jsx
  // route guard already redirects non-admins away before this component
  // ever mounts, but this check stays as defense-in-depth (e.g. if the
  // component is ever rendered outside that route).
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-gray-400">
        ...
        <p className="text-xs mt-1 text-center max-w-xs">
          Monitoring is available to admins only.
        </p>
      </div>
    );
  }
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {checkList.map(([svc, info]) => {
                const ok = (() => {
                  if (info === "ok" || info === "connected") return true;
                  if (typeof info === "string") {
                    if (info.startsWith("ok") || info.startsWith("connected")) return true;
                    if (/\d+ monitored, 0 open/.test(info)) return true;
                    return false;
                  }
                  if (typeof info === "object" && info !== null) {
                    if (info.status === "ok") return true;
                    // kv nested object: {DB0: {ok: true}, ...}
                    return Object.values(info).every(v => v?.ok === true);
                  }
                  return false;
                })();
                const disabled = typeof info === "string" && info.startsWith("not configured");
                return (
                  <div key={svc} className={`rounded-xl border p-3 flex items-center gap-3 ${ok ? "bg-green-50 border-green-200" : disabled ? "bg-gray-50 border-gray-200" : "bg-red-50 border-red-200"}`}>
                    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${ok ? "bg-green-500" : disabled ? "bg-gray-400" : "bg-red-500 animate-pulse"}`} />
                    ...
                    <span className={`ml-auto text-xs font-bold ${ok ? "text-green-600" : disabled ? "text-gray-400" : "text-red-600"}`}>
                      {ok ? "OK" : disabled ? "OFF" : "DOWN"}
                    </span>
                  </div>
                );
              })}
