...
// (URL-safe base64), built the same way as the rotated/short keys below.
//
// Generated at runtime rather than written as a literal: these tests only need
// *a* valid 32-byte key, never one specific value, and a hardcoded base64
// string is indistinguishable from a real leaked credential to a secret
// scanner. Randomising it also keeps the suite honest — nothing can quietly
// depend on a fixed key.
...
const testKey = toB64Url(crypto.getRandomValues(new Uint8Array(32)));
...
describe("piiCrypto — key configured (the shipping configuration)", () => {
  it("round-trips a value through encrypt -> decrypt", async () => {
    const { encryptPii, decryptPii } = await loadModule({ key: testKey });
  ...
  it("piiKeyMissing is false when a valid key is present", async () => {
    const { piiKeyMissing } = await loadModule({ key: testKey });
  ...
  it("a wrong-but-valid key fails safe instead of leaking ciphertext", async () => {
    ...
    const good = await loadModule({ key: testKey });
    const token = await good.encryptPii("alice@example.com", true);

    const otherKey = toB64Url(new Uint8Array(32).fill(7));
  ...
  it("ignores a key that is not 32 bytes and reports the build as broken", async () => {
    const shortKey = toB64Url(new Uint8Array(16).fill(3));
