...
#
# Allocation model:
#   - Each user gets a total allocation (default $50, configurable via
...
from core.config import RDB_BUDGET, POSTGRES_SCHEMA as _DB_SCHEMA
from core.kv import get_kv, KVError
from core.logger import logger

# Platform base budget allocation — the amount every user is seeded with /
# reset to monthly. Configurable via BUDGET_BASE_COST_USD (default $50);
# mirrors db.models._BASE_COST_USD_DEFAULT / db.migrate._BASE_COST_USD_DDL_DEFAULT.
# Defined here (near the top) so it's available as a default parameter value
# for set_budget() below, rather than the previous mismatched literal 30.0.
BASE_COST_USD             = float(_os.getenv("BUDGET_BASE_COST_USD",      "50.0"))
...
def _pg_get_budget(user_id: str) -> Optional[dict]:
    ...
    try:
        ...
        cost  = float(row[0]) if row[0] is not None else float(row[3] or BASE_COST_USD)
        tok   = int(row[1])   if row[1] is not None else 500_000
        reqs  = int(row[2])   if row[2] is not None else 1_000
        base  = float(row[4]) if row[4] is not None else BASE_COST_USD
        extra = float(row[5]) if row[5] is not None else max(0.0, cost - BASE_COST_USD)
        return {
            "user_id":              user_id,
...
def _pg_upsert_budget(user_id: str, max_tokens: int, max_requests: int,
                      ...
                      extra_cost_usd: Optional[float] = None) -> None:
        ...
        cur.execute("""
            INSERT INTO budget_configs
                (id, user_id, monthly_limit_usd, max_tokens_total,
                 ...
                 base_cost_usd, extra_cost_usd, created_at, updated_at)
...
def set_budget(user_id: str, max_cost_usd_per_day: float = 0.0,
               ...
               max_cost_usd_total: float = BASE_COST_USD,
               model_limits: Optional[dict] = None,
               base_cost_usd: Optional[float] = None,
               extra_cost_usd: Optional[float] = None) -> dict:
    ...
    need to set/adjust the split explicitly (approval increments, monthly
    reset) should pass both.
    """
...
def get_budget(user_id: str) -> Optional[dict]:
    ...
    if rc:
        try:
            ...
            if data:
                _cost_total = float(data.get("max_cost_usd_total") or data.get("max_cost_usd_per_day", BASE_COST_USD))
...
def update_cached_cost_limit(user_id: str, new_cost_usd_total: float,
                              ...
                              new_extra_cost_usd: Optional[float] = None) -> None:
    """Refresh max_cost_usd_total (and optionally base/extra) in the
...
#   BUDGET_BASE_COST_USD     — everyone's "base" budget allocation, the amount
#                              every user is seeded with / reset to monthly,
#                              before any extra top-ups (default $50)
#                              (BASE_COST_USD itself is defined near the top
#                              of this file, above set_budget())
...
def resolve_delegates_for_hod(hod_email: str) -> List[str]:
    ...
    try:
        ...
        try:
            rows = db.execute(
                _text(
                    f'SELECT "delegated_to" FROM {_DB_SCHEMA}.department_hod_mapping '
        ...
        try:
            ...
            rows = db.execute(
                _text(
                    ...
                    f'FROM {_DB_SCHEMA}.department_hod_mapping '
...
def _get_sole_admin_email() -> Optional[str]:
    """Return the lowercased email of the single active ``admin`` user, or
    None if there is no active admin at all.

    Used by `resolve_approvers_for_request()` below, which is the SHARED
    routing helper behind `auth/rbac.py::resolve_request_approvers()` —
    consumed by Products, KB documents, Codebase-index, and Cowork
    submissions. Those flows intentionally keep the "exactly one admin"
    assumption: an admin's own submission auto-approves instantly at
    submission time (see `auth/rbac.py::is_sole_active_admin`), so there is
    no "other admin" to route a *pending* item to in flat mode. If multiple
    admin rows exist anyway (not the expected deployment shape for these
    flows), the first one found is used deterministically; this function
    does not attempt multi-admin routing.

    NOTE: budget-increase requests do NOT use this function — see
    `_list_all_active_admin_emails()` / `resolve_budget_approvers_for_request()`
    below, which fans out to every active admin instead of just one.
    """
    try:
        from db.database import SessionLocal
        ...
        logger.warning(f"BudgetStore._get_sole_admin_email: {e}")
        return None


def resolve_approvers_for_request(requester_email: str) -> dict:
    """Resolve the full approver set for `requester_email`'s request.

    HOD_APPROVAL_ENABLED=true (department/HOD hierarchy, existing behaviour):
    the HOD (from users.hod_email) plus every delegatee the HOD has nominated.

    HOD_APPROVAL_ENABLED=false (flat/admin-only, default): the single admin
    account is the approver — there is no department/HOD hierarchy, and no
    multi-admin pool, to resolve against (flat-mode deployments are assumed
    to have exactly one admin for these flows). Returned in the same shape
    as the HOD-mode result (`hod_email` + `delegatees`, delegatees always
    empty in flat mode) so every downstream consumer (`auth/rbac.py::
    is_request_approver`, product-catalog/KB/codebase approval-notify) needs
    no changes: they already treat `hod_email` as "primary approver" and
    independently OR in `is_admin(current_user)`.

    If the requester IS the admin, there is nobody to route to — callers
    should treat this the same as "unroutable" (in practice these three
    flows already auto-approve the admin's own submission at creation time,
    so this function is only reached for non-admin submitters).

    NOTE: this is the SHARED resolver behind Products/KB/Codebase/Cowork
    approval-notify routing (via `auth/rbac.py::resolve_request_approvers`).
    Budget-increase requests use a separate, dedicated resolver —
    `resolve_budget_approvers_for_request()` below — which fans out to ALL
    active admins (self-approval preserved + multi-admin broadcast) instead
    of just one, since budget explicitly needs that behaviour while the
    other three flows deliberately keep the single-admin auto-approve
    shortcut.

    Returns:
        {
          "hod_email":   str  | None,  # None if unroutable (no HOD / no admin)
          "delegatees":  list[str],    # always [] in flat mode
        }
    """
    if not requester_email:
    ...
    if not HOD_APPROVAL_ENABLED:
        admin_email = _get_sole_admin_email()
        if not admin_email or admin_email == requester_email.strip().lower():
            return {"hod_email": None, "delegatees": []}
        return {"hod_email": admin_email, "delegatees": []}
...
def _list_all_active_admin_emails() -> List[str]:
    """Return the lowercased emails of every active ``admin`` user, in a
    deterministic order (earliest-created first).

    Used ONLY by `resolve_budget_approvers_for_request()` below, to route
    flat-mode ("HOD_APPROVAL_ENABLED=false") budget-increase requests to ALL
    active admins, not just one — unlike `_get_sole_admin_email()` above,
    which the other maker-checker flows (Products/KB/Codebase/Cowork) still
    use.
    """
    try:
        from db.database import SessionLocal
        ...
        logger.warning(f"BudgetStore._list_all_active_admin_emails: {e}")
        return []


def resolve_budget_approvers_for_request(requester_email: str) -> dict:
    """Resolve the full approver set for `requester_email`'s BUDGET-increase
    request specifically. Dedicated resolver — NOT shared with Products/KB/
    Codebase/Cowork (see `resolve_approvers_for_request()` above for those).

    HOD_APPROVAL_ENABLED=true (department/HOD hierarchy): identical to
    `resolve_approvers_for_request()` — the HOD plus every delegatee.

    HOD_APPROVAL_ENABLED=false (flat/admin-only, default): ALL active admins
    are approvers — there is no department/HOD hierarchy to resolve against,
    but a flat-mode deployment may have any number of admins. Returned in
    the same shape as the HOD-mode result (`hod_email` + `delegatees`) so
    every downstream consumer (`request_budget_increase`'s fan-out,
    `approve_budget_request`, `reject_budget_request`) needs no changes —
    they already handle N routed approvers via the `delegatees` list,
    first-wins, supersede-the-rest.

    NOTE on self-approval: the caller (routers/budget_router.py::
    request_increase) never calls this function for an admin in flat mode —
    an admin's own request is short-circuited to apply immediately (see
    _apply_self_service_budget_increase), the same as /budget/hod/self-
    increase. Only a non-admin user's request reaches this resolver, so it
    fans out to EVERY active admin unconditionally — whichever admin acts
    first wins and every other admin's pending row is superseded/rejected.

    Returns:
        {
          "hod_email":   str  | None,  # None if unroutable (no admin at all)
          "delegatees":  list[str],    # every other active admin, flat mode
        }
    """
    if not requester_email:
        return {"hod_email": None, "delegatees": []}
    ...
    from core.config import HOD_APPROVAL_ENABLED
    if not HOD_APPROVAL_ENABLED:
        admins = _list_all_active_admin_emails()
        if not admins:
            return {"hod_email": None, "delegatees": []}
        return {"hod_email": admins[0], "delegatees": admins[1:]}
...
def set_hod_delegates(hod_email: str,
    ...
    try:
        ...
        cur.execute(
            f"""
            UPDATE {_DB_SCHEMA}.department_hod_mapping
               SET delegated_to = %s
...
def request_budget_increase(user_id: str, requested_extra_cost_usd: float,
                             ...
                             delegatee_emails: Optional[List[str]] = None) -> dict:
    """
    ...
    ledger_action = "approve_request"

    existing = get_budget(user_id) or {}
    current_base  = float(existing.get("base_cost_usd", BASE_COST_USD))
    ...
    try:
        ...
        cur.execute(f"""
            ...
            FROM   {_DB_SCHEMA}.hod_allocation_ledger
    ...
    try:
        ...
        for row_hod, row_delegatee in approver_rows:
            cur.execute(f"""
                INSERT INTO {_DB_SCHEMA}.hod_allocation_ledger (
...
def get_pending_budget_requests(hod_email: Optional[str] = None,
    ...
    try:
        ...
        where = ["action = 'approve_request'", "status = ANY(%s)"]
    ...
    try:
        ...
        try:
            ...
            if already_resolved is not None:
                ...
                cur.execute(
                    f"SELECT approved_by FROM {_DB_SCHEMA}.hod_allocation_ledger WHERE id = %s",
                    (already_resolved[0],),
                )
            ...
            cur.execute(f"""
                SELECT base_cost_usd, extra_cost_usd, max_cost_usd_total,
                       max_tokens_total, max_requests_total
                FROM   {_DB_SCHEMA}.budget_configs
                WHERE  user_id = %s
                FOR UPDATE
            """, (str(target_user_id),))
            ...
            if cfg_row is not None:
                old_base   = float(cfg_row[0]) if cfg_row[0] is not None else BASE_COST_USD
            ...
            cur.execute(f"""
                UPDATE {_DB_SCHEMA}.hod_allocation_ledger
                   SET status = 'approved',
                       amount_usd = %s,
            ...
            if cfg_row is not None:
                cur.execute(f"""
                    UPDATE {_DB_SCHEMA}.budget_configs
                       SET base_cost_usd      = %s,
                           extra_cost_usd     = %s,
                ...
                cur.execute(f"""
                    INSERT INTO {_DB_SCHEMA}.budget_configs
                        (id, user_id, monthly_limit_usd, max_tokens_total,
