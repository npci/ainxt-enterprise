...
def _make_opener():
    """Return a urllib opener, optionally bypassing HTTP(S) proxy.

    Set GITLAB_NO_PROXY=true to bypass any configured HTTP proxy when
    reaching your GitLab instance (useful when GitLab is on an internal
    network that must be reached directly).

    Set GITLAB_NO_PROXY=false (default) to respect the system proxy
    settings (HTTP_PROXY / HTTPS_PROXY environment variables).
    """
    import urllib.request
    _no_proxy = os.getenv("GITLAB_NO_PROXY", "false").lower() in ("true", "1", "yes")
    if _no_proxy:
...
def _get(path: str) -> dict | list:
    ...
    try:
        ...
        logger.error("GitLab GET %s failed: HTTP error", path)
        return {"error": "http_error"}
    except Exception:
        logger.error("GitLab GET %s failed: unexpected error", path)
...
def _ref_project(row: dict) -> str:
    ...
    suffix ("your-org/payments!7" for an MR, "your-org/payments#7" for an issue), which is
...
def gitlab_post_governance_note(project: str, mr_iid: int, report_md: str) -> str:
    ...
    line anchors (platform rule). Idempotent: finds a PRIOR governance note (by the
