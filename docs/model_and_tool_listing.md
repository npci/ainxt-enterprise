...
This module does not execute agents or workflows itself; its responsibility is **discovery, filtering, and lightweight retrieval**. It sits inside the [gateway](gateway.md) service and reuses authentication, telemetry, and registry infrastructure defined elsewhere.
...
**Dependencies:**
- [core.model_registry](shared_core.md#model_routing) for canonical model constants.
- [gateway_local_llm](local_llm_gateway.md) for in-house hosted models.
- [ClientSourceMiddleware](gateway.md) for `request.state.client_source`.
- [auth.jwt_handler](authentication.md) for Veo user claim resolution.
...
**Dependencies:**
- [mcp.registry](mcp_system.md)
- [auth.rbac](authentication.md)
...
**Dependencies:**
- [models.hybrid_retriever](shared_core.md#model_routing)
...
|-----------------|--------|-------|
| LLM model registry | [core.model_registry](shared_core.md#model_routing) | Canonical model IDs and display labels. |
| Local LLM proxy | [gateway_local_llm](local_llm_gateway.md) | In-house model discovery. |
| MCP tool registry | [mcp.registry](mcp_system.md) | Tool catalog source. |
| Hybrid retriever | [models.hybrid_retriever](shared_core.md#model_routing) | Codebase semantic search. |
| PostgreSQL | [db.database](database.md) | Security scan storage. |
| Auth / RBAC | [auth.rbac](authentication.md), [auth.jwt_handler](authentication.md) | Admin checks and claim resolution. |
| Telemetry | [core.telemetry](shared_core.md#core_infrastructure) | Usage and latency metrics. |
...
- [gateway](gateway.md) — Parent service and routing context.
- [local_llm_gateway](local_llm_gateway.md) — In-house model hosting.
- [mcp_system](mcp_system.md) — Tool registry and MCP infrastructure.
- [shared_core model_routing](shared_core.md#model_routing) — Model registry and hybrid retriever.
- [database](database.md) — PostgreSQL schema and session management.
- [authentication](authentication.md) — RBAC and JWT handling.
