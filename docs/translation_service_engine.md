...
The engine itself accepts FLORES-200 codes. The ISO → FLORES mapping is handled by `config.to_flores()` and covers the same 26 codes documented in [translation_service_api.md](translation_service_api.md):
...
- [translation_service_api.md](translation_service_api.md) — HTTP layer, request validation, and service lifecycle.
