...
- Converts the [`RunRequest.workflow`](app_models.md) into a [`ChainDefinition`](engine_native_engine.md) via [`to_chain`](api_deps.md).
- Builds an [`ExecutionContext`](api_deps.md) and a [`RunUsageTracker`](core_governance.md).
- Streams events from [`NativeEngine.execute`](engine_native_engine.md), collecting the final `output` and `thread_id`.
- Returns a [`RunResponse`](app_models.md) with `status`, `output`, and `thread_id`.
...
- [`engine_native_engine`](engine_native_engine.md) — pure-Python workflow engine that executes the chain.
- [`core_governance`](core_governance.md) — budget checks, audit events, and usage tracking.
- [`app_models`](app_models.md) — `RunRequest`, `ResumeRequest`, `RunResponse`, `Workflow`.
