...
| [core_config](core_config.md) | `REDIS_HOST`, `REDIS_PORT`, `RDB_WORKFLOW`, `RDB_REGISTRY` — KV configuration |
...
`check_governance_sla_reminders()` is invoked daily at 09:00 IST (03:30 UTC) by the platform's cron scheduler (see [worker_orchestration](worker_orchestration.md)). It can also be triggered manually via `POST /governance/sla/remind`.
