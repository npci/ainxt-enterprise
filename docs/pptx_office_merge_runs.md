...
The module is not exposed as a standalone API endpoint. It is imported and called by higher-level scripts in the same skill family, ultimately invoked through the ABStudio skill execution framework (see [`engine_native_engine`](engine_native_engine.md) and [`api_execution`](api_execution.md)).
