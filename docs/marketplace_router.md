...
- Validation and sanitization happen first via `validate_tool_register_request` in [core/security_validation](shared_core.md#core-infrastructure).
...
- **[auth.dependencies](auth.md)** — `get_current_user` for actor resolution.
- **[core.config](shared_core.md#core-infrastructure)** — `RDB_REGISTRY` constant selecting the KV database index.
- **[core.kv](shared_core.md#kv-store)** — `get_kv` and `KVError` for marketplace metadata storage.
- **[core.logger](shared_core.md#core-infrastructure)** — Structured logging.
- **[core.security_validation](shared_core.md#core-infrastructure)** — `validate_tool_register_request` for input sanitization.
- **[db.database / db.models](database.md)** — `SessionLocal` and the `MCPServer` SQLAlchemy model.
- **[mcp.registry](mcp_system.md)** — `mcp_registry` singleton providing `tools` and `skills` sub-registries.
...
- **State enforcement**: `MCPRegistry.execute_tool()` blocks execution of user-registered tools whose `status` is not `PRODUCTION`, routing them through the governance approval flow (see [mcp_governance_router](mcp_governance_router.md)).
...
- **Agent Builder / ABStudio**: The ABStudio frontend lists and selects tools/skills from the marketplace when configuring agents. See [abstudio_backend](abstudio_backend.md) and [abstudio_frontend](abstudio_frontend.md).
- **AI-UI plugin catalog**: `GET /plugins/curated` feeds the CLI/web plugin discover view. The file is produced by [workers/external_sync_worker](workers.md#external-integration-workers).
- **MCP Server Router**: DB-registered and hot-registered tools are also reachable through the SSE/HTTP MCP server endpoints defined in [mcp_server_router](mcp_server_router.md).
- **Governance Router**: Tools that require approval before production use flow through [mcp_governance_router](mcp_governance_router.md).
- **Tool execution**: Runtime invocation of registered tools is handled by `ToolRegistry.execute()` and `MCPRegistry.execute_tool()` in the [mcp_system](mcp_system.md).

---

## 8. Operational notes

- **KV backend selection**: `_get_redis()` uses `core.kv.get_kv(RDB_REGISTRY, decode_responses=True)`. The actual backend (Redis, RustyCluster, etc.) is selected by `REDIS_CLIENT_CONFIG_DB3`; the function name is retained for backwards compatibility.
