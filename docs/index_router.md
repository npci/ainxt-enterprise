...
This is handled by `core.platform_credentials.get_gitlab_token()` and `inject_gitlab_token()`. See [shared_core](shared_core.md) for details on the platform credentials module.
...
The `enqueue_index_job` function in `core.job_queue` acquires a distributed lock scoped to `(repo_name, product_id, branch)` before enqueuing. This prevents duplicate indexing jobs for the same repo/product/branch combination. See [shared_core](shared_core.md) for details on the job queue infrastructure.
...
> See [shared_core](shared_core.md) for the worker infrastructure and [embedding_service](embedding_service.md) for the embedding pipeline.
...
|---|---|---|
| Authentication & RBAC | [shared_core](shared_core.md) → `authentication` | User authentication, role checks, approval permissions |
| Core Infrastructure | [shared_core](shared_core.md) → `core_infrastructure` | Config, logging, KV store, security validation |
| Job Queue | [shared_core](shared_core.md) → `core_infrastructure` | RQ-based job enqueuing with distributed locks |
| Platform Credentials | [shared_core](shared_core.md) → `core_infrastructure` | GitLab PAT resolution and URL injection |
| Inbox Store | [shared_core](shared_core.md) → `store_layer` | Inbox notifications for approvers and submitters |
| Database Models | [shared_core](shared_core.md) → `database` | `IndexRequest`, `User`, `InboxItem` ORM models |
| Index Worker | [workers](workers.md) → `chat_agent_execution_workers` | Background indexing job execution |
| Embedding Service | [embedding_service](embedding_service.md) | Vector embedding generation |
| Gateway | [gateway](gateway.md) → `indexing_and_search` | Gateway-level index endpoints that delegate to this router |
...
Notifications are fire-and-forget — failures are logged as warnings but do not affect the core workflow. See [shared_core](shared_core.md) → `store_layer` for the inbox store implementation.
