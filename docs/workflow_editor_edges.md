...
4. **Maintaining editor state consistency** by delegating all mutations to the central [`workflowStore`](store.md).
...
`AiEdge` reads and writes the following slices of [`workflowStore`](store.md):
...
- **It does not own workflow persistence.** Saving, loading, duplicating, and deleting workflows are handled by the backend via [`api_workflows`](api_workflows.md) and the [`core_workflow_repo`](core_workflow_repo.md).
- **It does not define the data schema.** The backend [`app_models`](app_models.md) module defines the `Edge` model and the overall `Workflow` schema that the frontend mirrors.
- **It does not execute workflows.** Execution is driven by [`api_execution`](api_execution.md) and the [`engine_native_engine`](engine_native_engine.md); the edge component only reflects execution progress through the `activeNodeIds` animation.
...
- **It relies on the central store.** All mutations pass through [`workflowStore`](store.md), which also provides utilities such as `createWorkflowEdgeId`, `hasIllegalCycle`, and `pruneToConnectedSubgraph`.
...
- [`store`](store.md) — Central Zustand store providing `insertNodeOnEdge`, `removeEdge`, `hoveredEdgeId`, and `activeNodeIds`.
- [`app_models`](app_models.md) — Backend Pydantic models including `Edge` and `Workflow`.
- [`api_workflows`](api_workflows.md) — REST endpoints for workflow CRUD operations.
- [`api_execution`](api_execution.md) — REST and streaming endpoints for running workflows.
- [`engine_native_engine`](engine_native_engine.md) — Backend execution engine that determines which nodes are active during a run.
