...
> **Related modules:** [agent_factory_pipeline](agent_factory_pipeline.md) · [api_factories](api_factories.md) · [api_catalog](api_catalog.md) · [core_factory_utils](core_factory_utils.md) · [core_workflow_repo](core_workflow_repo.md) · [core_governance](core_governance.md) · [workflow_factory_pipeline](workflow_factory_pipeline.md) · [skills_feature](skills_feature.md)
...
It is the ABStudio implementation of the **Anthropic skill-creator methodology**: rather than hand-copying the authoring guidance into prompts (where it drifts), the pipeline reads the canonical `skills/ainxt-skills/skill-creator/SKILL.md` from disk at runtime and injects the authoring-relevant sections into every generation prompt. The on-disk file becomes the single source of truth for *how to write a skill*; the pipeline owns *the machinery* (staging, validation, quality loop, packaging).

The module is consumed by three distinct entry points:

| Consumer | Entry point | Purpose |
|---|---|---|
| **Skill Factory Chat** (UI) | `skill_factory_chat` SSE endpoint in [api_factories](api_factories.md) | Multi-turn conversational creation: clarify → plan card → generate → confirm |
| **Agent Factory** (programmatic) | `DynamicSkillGenerator` in [agent_factory_pipeline](agent_factory_pipeline.md) | Headless one-shot generation to fill a capability gap detected during agent assembly |
| **Catalog upload** (import) | `upload_catalog_skill` in [api_catalog](api_catalog.md) | Validates and imports a packaged `.skill`/`.zip` archive using the same validation rules |
...
flowchart TB
    ...
    subgraph External["External"]
        SkillCreatorMD["skills/ainxt-skills/<br/>skill-creator/SKILL.md"]
...
A singleton (`_skill_creator_guidance`) that reads `skills/ainxt-skills/skill-creator/SKILL.md` from disk and caches the **authoring-relevant sections** only. It re-reads the file when its `mtime` changes, so editing the `.md` immediately changes how skills are generated — no code change or redeploy required.
...
> **Note:** `_infer_plan_card_defaults` is duplicated in [workflow_factory_pipeline](workflow_factory_pipeline.md) — both factories share the same Plan Card UX pattern. The skill variant uses a 200-token cap and a skill-specific context string.
...
#### `SkillEvaluator`
LLM-based replacement for the skill-creator's `run_eval.py`. Tests **trigger accuracy** without needing the Claude CLI:
...
Per-skill-name `asyncio.Lock` preventing two concurrent agent creations from generating the same skill simultaneously. Used by `DynamicSkillGenerator` in [agent_factory_pipeline](agent_factory_pipeline.md).
...
Returns a `.skill` (ZIP) file as bytes containing `SKILL.md` plus any bundled scripts/references. Mirrors `package_skill.py` from the skill-creator plugin. Enforces path safety — never lets a `rel_path` escape the skill folder.
...
The `DynamicSkillGenerator` in [agent_factory_pipeline](agent_factory_pipeline.md) uses the same pipeline components but **skips conversational clarification** — the agent factory already knows what skill is needed (gap name + agent blueprint context).
...
The `upload_catalog_skill` endpoint in [api_catalog](api_catalog.md) imports a packaged `.skill`/`.zip` archive using the **same validation rules** as the generation path:
...
See [core_governance](core_governance.md) for the full governance lifecycle.

## 8. LLM Call Topology

Every LLM call in the pipeline routes through `call_factory_llm` in [core_factory_utils](core_factory_utils.md), which:
...
1. **On-disk guidance as single source of truth** — The skill-creator `SKILL.md` is read at runtime, not hardcoded. Editing the spec changes generation behavior with no code change. The pipeline owns the *machinery*; the file owns the *methodology*.
...
skills/ainxt-skills/skill-creator/SKILL.md
```
