...
- [memory_system_chat_summarizer](memory_system_chat_summarizer.md) — Rolling per-chat summaries and LLM-based memory filtering.
- [memory_system_cowork_memory](memory_system_cowork_memory.md) — Cowork personalization preferences and durable agent-remembered facts.
...
- **kv_store** — `RedisMemory` uses `core.kv.get_kv` for Redis/RustyCluster abstraction.
