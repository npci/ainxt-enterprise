...
1. Open a GET request to `/presentation?id=<id>&stream=true&_rsc=<rsc>` via [`presentonFetch`](config.md).
...
1. **Request Construction**  
   The stream URL is built relative to `PRESENTON_BASE` (configured in [`config.js`](config.md)):
   ```javascript
   const url = `/presentation?id=${encodeURIComponent(presentationId)}&stream=true&_rsc=${rsc}`;
   ```
   [`presentonFetch`](config.md) prepends `/presenton` and applies a configurable timeout.
...
|------------|--------|------|
| `presentonFetch` | [`config.js`](config.md) | Performs the actual HTTP request with Presenton base URL and timeout handling. |
