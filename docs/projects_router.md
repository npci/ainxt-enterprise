...
This router is part of the shared API layer and is consumed by the AI UI frontend's [Projects](ai_ui_frontend.md#projects) feature.
...
See [workspace_messages_store](store_layer.md) for persistence details.
...
See [budget_store](store_layer.md) and [budget_router](budget_router.md) for broader budget management.
...
|------------|--------|------|
| `get_current_user` | [auth.dependencies](authentication.md) | JWT-based user extraction. |
| `is_admin` | [auth.rbac](authentication.md) | Admin check for listing all projects. |
| `validate_product_name`, `validate_description`, `validate_repo_name` | [core.security_validation](core_infrastructure.md) | XSS/SQL-injection input sanitization. |
| `logger`, `set_request_id`, `set_chat_context`, `bind_context`, `set_span_id` | [core.logger](core_infrastructure.md) | Structured, context-aware logging. |
| `OPENAI_CODING_MODEL`, `MODEL_COST_PER_1M` | [core.model_registry](core_infrastructure.md) | Default model label and cost rates. |
| `list_projects`, `create_project`, `get_project`, `update_project`, `delete_project` | [store/projects_store](store_layer.md) | Project persistence. |
| `get_messages`, `get_history_for_injection`, `save_messages`, `delete_project_messages` | [store/workspace_messages_store](store_layer.md) | Conversation history persistence. |
| `check_budget` | [store/budget_store](store_layer.md) | Spend gate. |
| `OrchestratorAgent` | [agents.orchestrator](agent_system.md) | Multi-step planning, retrieval, and generation. |
| `model_router` | [models.model_router](model_routing.md) | LLM routing, streaming, and fallback. |
| `SessionLocal`, `ChatAttachment` | [db.database / db.models](database.md) | Attachment text lookup. |
...
- [ai_ui_frontend.md](ai_ui_frontend.md) — Frontend Projects feature that consumes this router.
- [agent_system.md](agent_system.md) — `OrchestratorAgent` planning and execution.
- [model_routing.md](model_routing.md) — `ModelRouter` tier selection and streaming.
- [store_layer.md](store_layer.md) — `projects_store`, `workspace_messages_store`, and `budget_store`.
- [authentication.md](authentication.md) — JWT and RBAC dependencies.
- [core_infrastructure.md](core_infrastructure.md) — Logging, validation, and model registry utilities.
- [database.md](database.md) — SQLAlchemy models and session management.
