...
The workers do **not** contain business logic themselves — they are thin orchestration wrappers that delegate to the [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) agents and the [store_layer](store_layer.md) persistence layer.
...
|---|---|---|
| `agents.sdlc_pipeline` | [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) | All pipeline entry points: `run_feature_pipeline`, `run_bug_pipeline`, `run_pr_review_pipeline`, `resume_*`, `run_governance_*`, `address_pr_review_comments`, `_resolve_gitlab_repo`, `_gov_resolve_gitlab_token`, `run_governance_scan_snapshot` |
| `agents.sdlc_state_machine.CodingStateMachine` | [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) | Reconstructed in `resume_from_stage_job`, `retry_commit_job`, `run_endgate_governance_job` for stage-level resume and governance end-gate execution |
| `agents.sdlc_governance` (engine, config) | [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) | Governance skill selection, fix-prompt building, report rendering, `max_iters()`, `parse_subset()` |
| `agents.sdlc_cli_engine` | [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) | `run_cli` for governance fixer loops in standalone review |
| `store.sdlc_store` | [store_layer](store_layer.md) | `get_run`, `update_run_state`, `add_run_event`, `list_runs`, `SDLCCancelled` |
| `store.sdlc_artifacts` | [store_layer](store_layer.md) | `_store_artifact`, `_load_latest_artifact`, `compute_input_hash` for `GOVERNANCE_REPORT` persistence |
| `core.job_queue` | [core_infrastructure](core_infrastructure.md) | `release_sdlc_slot`, `refresh_sdlc_slot` for dedup/concurrency control |
| `tools.gitlab_tools` | [shared_integrations](shared_integrations.md) | `gitlab_merge_mr`, `gitlab_get_mr_diff`, `gitlab_set_mr_draft`, `gitlab_post_governance_note`, `gitlab_get_project_clone_url`, `set_token` |
| `workers.workspace_sync_worker` | [external_integration_workers](external_integration_workers.md) | `prepare_run_workspace` for governance workspace materialization |
| `db.database` | [database](database.md) | `SessionLocal` for governance product-ID resolution |
...
Worker processes are started by [worker_orchestration](worker_orchestration.md) (`workers/start_workers.py`), which spawns RQ worker processes that consume the SDLC queue. The `expire_stale_hitl_runs` function is registered as a scheduled cron job within the same orchestration layer.

### Event Consumption

SDLC state-change events produced by `store.sdlc_store.update_run_state` are consumed by the [kafka_event_consumer](kafka_event_consumer.md) (`_handle_sdlc_events`) for audit trails and secondary writes.
...
- [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) — Core SDLC pipeline agents, state machine, governance engine, and CLI engine
- [store_layer](store_layer.md) — Run state persistence, stage artifacts, governance findings/approvers
- [core_infrastructure](core_infrastructure.md) — Job queue, logging, config, model registry, platform credentials
- [shared_integrations](shared_integrations.md) — GitLab tools, Jira tools, and other connector adapters
- [external_integration_workers](external_integration_workers.md) — Workspace sync worker for per-run git clone management
- [database](database.md) — SQLAlchemy session management and SDLC run models
- [sdlc_router](sdlc_router.md) — API endpoints that enqueue these worker jobs
- [worker_orchestration](worker_orchestration.md) — RQ worker process startup and cron scheduling
- [kafka_event_consumer](kafka_event_consumer.md) — Consumes SDLC state-change events for audit
