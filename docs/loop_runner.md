...
1. **API**: `POST /loops/{id}/run-stream` (see [api_loops](api_loops.md) / `app/api/loops.py::run_loop_stream_route`) — resolves the loop, goal, and chain, then streams `LoopRunner.execute()` as SSE.
