...
The module is intentionally presentational: it receives a message record and an optional budget object, then decides which chips are relevant and how they should be styled. It is consumed by higher-level chat modules including [`chat`](chat.md), [`threads`](threads.md), and [`projects`](projects.md).
...
|-----------|---------------|-----------|
| `MessageMeta` | Orchestrates chip visibility and renders the meta row. | [`chat`](chat.md), [`threads`](threads.md), [`projects`](projects.md) |
...
| [`chat`](chat.md), [`threads`](threads.md), [`projects`](projects.md) | Parent modules that import and render `MessageMeta`. |
...
- [`budget`](budget.md) (via the `/budget/me` API) supplies monthly spend and limit data.
- The AI coach system, surfaced through [`coach`](coach.md), populates `msg.coachHits` when a message triggers quality rules.
...
- [`projects`](projects.md) — uses `MessageMeta` inside project-based chat.
- [`budget`](budget.md) — provides the monthly budget data consumed by `MonthlyBudgetChip`.
- [`coach`](coach.md) — produces the rule hits consumed by `CoachHitsChip`.
