...
For the backend governance engine, skill execution, and API contract, see [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) and [shared_api_routers/sdlc_router](shared_api_routers.md). For the parent UI that hosts this panel, see [sdlc_pipeline](sdlc_pipeline.md).
...
- It coordinates with the [sdlc_worker](workers.md) background jobs that run the governance scan, fixer loops, and resume operations.
...
- It is distinct from the generic [Governance](governance.md) component (entity-level governance for agents/skills/tools) and the [ModelGovernance](model_governance.md) component.
...
- [shared_api_routers](shared_api_routers.md) — FastAPI routers, including `sdlc_router.py`.
- [workers](workers.md) — Background workers, including `sdlc_worker.py`.
