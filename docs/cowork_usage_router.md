...
- **[auth](auth.md)** — `get_current_user` resolves the JWT-authenticated caller, and `require_admin` restricts analytics and spend-limit management to administrators.
...
- All write/read endpoints require a valid JWT via [auth.dependencies.get_current_user](auth.md).
- Analytics and spend-limit configuration require admin privileges via [auth.rbac.require_admin](auth.md).
...
- **[auth](auth.md)** — Authentication and authorization primitives.
