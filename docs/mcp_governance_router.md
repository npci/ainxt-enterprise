...
- Propagating approved tools into the live [MCP Registry](mcp_system.md).
...
4. **Propagate** — On approval, the router attempts to register the tool with the live [MCP Registry](mcp_system.md) through `MCPRegistry.register_http_tool`. Registry sync failures are logged but do not fail the approval.
...
| Module | Relationship |
|--------|--------------|
| [mcp_system](mcp_system.md) | Provides `MCPRegistry`, `ToolRegistry`, and `SkillRegistry`; the governance router propagates approvals into this registry. |
...
- Downstream, it protects the [MCP Registry](mcp_system.md) from unreviewed tool definitions.
