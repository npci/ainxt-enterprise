...
|-----------------|----------------|
| [model_routing](model_routing.md) | `models.model_router` and `models.classifier` select the model tier and classify query complexity/domain. |
| [core_infrastructure](core_infrastructure.md) | `core.logger`, `core.telemetry`, `core.config`, `core.evals`, and `core.context_compressor` provide logging, tracing, evaluation, and context compression. |
| [shared_integrations](shared_integrations.md) | `connectors.registry`, `connectors.engine`, and `tools.gitlab_tools` / `tools.jira_tools` execute connector and GitLab/Jira calls. |
| [memory_system](memory_system.md) | `memory.postgres_memory` stores tool sequences and retrieves hints for future runs. |
| [advanced_reasoning](advanced_reasoning.md) | `TreeOfThoughts`, `SelfConsistency`, and `ChainOfVerification` are invoked for high-risk or low-confidence runs. |
