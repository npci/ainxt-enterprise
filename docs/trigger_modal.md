...
`TriggerModal` is a self-contained form component defined in `ai-ui/src/components/SDLCPipeline.jsx`. It is rendered by the parent [`sdlc_pipeline`](sdlc_pipeline.md) component when the user clicks **New Pipeline**. After a successful submission it returns the new `run_id` via the `onTriggered` callback so the parent can refresh the run list and begin early-failure polling.
...
Validation is delegated to [`securityValidation`](ai_ui_frontend_utils.md):
...
`TriggerModal` uses [`apiFetch`](config.md) to talk to the SDLC endpoints exposed by [`sdlc_router`](sdlc_router.md). The backend then enqueues work through [`sdlc_worker`](sdlc_pipeline_workers.md).
...
When `useMultiRepoEnabled()` returns `true`, the modal renders the [`DepTable`](dep_table.md) component. `DepTable` automatically fetches declared dependencies from the primary repo's build manifest and merges them with any user-added rows. Each dependency carries:
...
See [`dep_table`](dep_table.md) for the full dependency-table behavior.

---

## Governance Review Option

The **Run Governance Review** checkbox enables post-REVIEW execution of pluggable governance skills (EA, IS, DPDP) over the generated diff. Because the backend does not yet expose a catalog of skill slugs, the subset picker is a free-form comma-separated input. An empty value means "all loaded skills." The backend parsing is handled by `agents/sdlc_governance/config.py::parse_subset` (see [`sdlc_governance`](shared_core_sdlc_pipeline.md#sdlc_governance)).
...
| Module | Relationship |
|--------|--------------|
| [`sdlc_pipeline`](sdlc_pipeline.md) | Parent component that owns the modal visibility and run-list state. |
| [`dep_table`](dep_table.md) | Child component for multi-repo dependency editing. |
| [`config`](config.md) | Provides `apiFetch` for authenticated HTTP requests. |
| [`ai_ui_frontend_utils`](ai_ui_frontend_utils.md) | Provides `validateJiraKey`, `validateRepoName`, `validateBranch`, `validateSummary`. |
| [`sdlc_router`](sdlc_router.md) | Backend router that receives the trigger requests. |
| [`sdlc_pipeline_workers`](sdlc_pipeline_workers.md) | Background workers that execute the triggered pipelines. |
| [`shared_core_sdlc_pipeline`](shared_core_sdlc_pipeline.md) | Shared SDLC agent logic, including governance skill parsing. |
