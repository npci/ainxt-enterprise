...
- **Lifecycle coupling**: `startup()` and `shutdown()` are invoked from [`app_main`](app_main.md) lifespan hooks.
...
All endpoints depend on `require_access` from [`api_deps`](api_deps.md), which resolves the caller to an [`AuthenticatedUser`](app_models.md) and enforces that the user has access to the `agent-chain` framework.
...
|------------|--------|------|
| `AuthenticatedUser` | [`app_models`](app_models.md) | User identity model. |
| `require_access` | [`api_deps`](api_deps.md) | Authentication / framework-access dependency. |
| `postgres_enabled` | [`core_config`](core_config.md) | Feature flag for PostgreSQL backend. |
| `AgentChatStore` | [`checkpoint`](checkpoint.md) | Abstract store interface. |
| `PostgresAgentChatStore` | [`checkpoint`](checkpoint.md) | Production PostgreSQL implementation. |
| `FileAgentChatStore` | [`checkpoint`](checkpoint.md) | Local JSON file implementation. |

### 5.2 Runtime dependencies

| Dependency | Module | Role |
|------------|--------|------|
| `app.main._lifespan` | [`app_main`](app_main.md) | Calls `startup()` / `shutdown()`. |
...
1. **Creation / deployment**: [`api_agents`](api_agents.md) and [`agent_factory_pipeline`](agent_factory_pipeline.md) create and register agents.
...
- [`checkpoint`](checkpoint.md) — `AgentChatStore` implementations.
- [`api_deps`](api_deps.md) — authentication dependencies.
- [`app_models`](app_models.md) — `AuthenticatedUser` and request models.
- [`app_main`](app_main.md) — application lifespan and startup hooks.
