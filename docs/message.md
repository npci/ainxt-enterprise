...
Every chat surface in the platform — the main [Chat](chat.md) interface, [KbChat](kb_chat.md), [Threads](threads.md), [CoworkDesktop](cowork_desktop.md), and [Projects](projects.md) — relies on the exports from this module to render assistant responses consistently.
...
**Canvas integration:** When the backend returns an `artifact_id`, an "Edit in Canvas" button opens [CoworkCanvas](cowork_desktop.md) — a full-screen modal with version history and AI-powered iterative editing (see [CoworkCanvas](cowork_desktop.md) documentation for details).
...
A simpler download component for presentations generated via the Presenton service. Uses `buildExportPayload(presentationId, title)` from [presenton-payload](presenton_lib.md) and `presentonApi.exportPresentation()` from [presenton-api](presenton_lib.md) to fetch the file as a blob, then triggers a browser download with a sanitized filename.
...
A comprehensive map of custom React renderers for every Markdown element type, exported for reuse by [Chat](chat.md), [Threads](threads.md), [CoworkDesktop](cowork_desktop.md), and [Projects](projects.md). Provides consistent indigo-themed styling across all surfaces:
...
| `CoworkCanvas` | [cowork_canvas](cowork_canvas.md) | Full-screen document editing modal with version history and AI revision |
...
| `buildExportPayload` | [presenton-payload](presenton_lib.md) | Builds PPT export request payload |
| `presentonApi` | [presenton-api](presenton_lib.md) | Presenton API client (export, outline, status) |
...
| [KbChat](kb_chat.md) | `Message`, `mdComponents`, `mdUrlTransform`, `parseDocMarkers`, `DocDownloadButton`, `PPTDownloadButton`, `DownloadableImage`, `ExpandableMessageBody` | Knowledge Base chat — mirrors Chat's rendering pipeline |
| [Threads](threads.md) | `mdComponents` | Discussion threads — uses shared markdown renderers for assistant responses |
| [CoworkDesktop](cowork_desktop.md) | `mdComponents` | Desktop cowork — uses shared markdown renderers |
| [Projects](projects.md) | `mdComponents` | Projects — uses shared markdown renderers |
