...
- **[api_execution](api_execution.md)** — Emits `generated_files` arrays on `agent_complete` and `complete` SSE events.
- **[api_documents](api_documents.md)** — Serves generated files at `/generated-files/*` and handles asset extraction.
...
- **Knowledge upload** ([KnowledgeUploadInline](KnowledgeUploadInline.md), [api_kb](api_kb.md)) — Handles ingestion of documents into the knowledge base; not to be confused with generated-file downloads.
- **Document generation** ([doc_download_router](doc_download_router.md), [doc_generator](doc_generator.md)) — Produces DOCX/PPTX/XLSX files on the backend; the resulting download URLs are consumed here.
...
- [api_execution](api_execution.md) — Backend endpoints that emit generated files.
- [api_documents](api_documents.md) — Backend file/asset serving.
