...
| `memory_system_redis_memory` | SESSION / WORKING | Redis | Ephemeral conversation + run history with TTL |
...
> stored. See [shared_integrations](../reference/shared_integrations.md) for the guardrails
> module.
...
The `cowork_admin_router` (see [shared_api_routers](../api/shared_api_routers.md))
...
| [memory_system_chat_summarizer](../chat/memory_system_chat_summarizer.md) | Sibling — LLM gate for cross-chat memory storage |
| [shared_api_routers](../api/shared_api_routers.md) | `cowork_admin_router` exposes this module via REST |
| [core_infrastructure](../infrastructure/core_infrastructure.md) | Provides `core.logger` |
| [database](../storage/database.md) | Owns the `engine` pool and migration DDL |
