...
|------------|--------|---------|
| `authFetch` | [config](config.md) | Authenticated HTTP requests with correlation IDs and credentials. |
| `useToast`, `useConfirm` | [ui_dialog](ui_dialog.md) | Toast notifications and confirmation dialogs. |
| `validateProductName`, `validateSecurity` | [ai_ui_frontend_utils](ai_ui_frontend_utils.md) | Input validation helpers. |
...
| Dependency | Module | Purpose |
|------------|--------|---------|
| `profile_router` | [shared_api_routers](shared_api_routers.md) | CRUD endpoints for profile, tokens, and custom instructions. |
| `api_keys_router` | [shared_api_routers](shared_api_routers.md) | Generation and revocation of IDE API keys. |
| `get_current_user` / `_require_jwt_auth` | [auth](auth.md) | JWT authentication and authorization. |
| `users`, `user_tokens`, `user_api_keys` | [database](database.md) | Persistent storage for profile data and credentials. |

---

## Integration with the Wider System

- **Chat / KbChat**: The `about_user` and `response_style` values saved in the Profile module are injected into chat prompts by the chat backend, giving every conversation persistent user context. See [chat](chat.md) and [kb_chat](kb_chat.md).
- **Connectors / GitLab & Jira**: Stored GitLab and Atlassian tokens are used by connector adapters and tools to act on behalf of the user. See [connectors](connectors.md) and [shared_integrations](shared_integrations.md).
- **Desktop App**: The desktop client can request a plaintext token reveal (audit-logged) to perform `git clone` with the user's stored GitLab PAT. See [desktop_app](desktop_app.md).
- **IDE Plugins**: IDE API keys expose an OpenAI-compatible endpoint so external editors can route completions through AiNxt. See [endpoint_proxy_router](shared_api_routers.md) and [gateway](gateway.md).
