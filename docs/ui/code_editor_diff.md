...
- **Out of scope**:
  ...
  - File-level chrome such as path headers, expand/collapse buttons, or approval actions (see [code](../codebase/code.md), [code_editor](code_editor.md), and [diff_approval](../sdlc/diff_approval.md)).
...
|-----------------|--------------------|-------|
| [code](../codebase/code.md) | `Diff` | renders a proposed file change in the chat stream |
| [code](../codebase/code.md) | `ToolDiff` | renders the result of a `Write`/`MultiEdit` tool call |
...
In [code](../codebase/code.md), the `Diff` component receives a block `b` containing `path`, `added`, `removed`, `isNew`, and `lines`. It renders a header with the file path and change counts, then delegates the body to `DiffLines`:
...
1. **Chat / tool diff cards** ([code](../codebase/code.md)) — lightweight, inline, often transient.
2. **Lite-IDE diff fallback** ([code_editor](code_editor.md)) — integrated into a file editor with tabs and save actions.
3. **Approval gate diff** ([diff_approval](../sdlc/diff_approval.md)) — richer review UI with per-file comments and compile/test badges; uses its own `FileDiff` component rather than `DiffLines`.
...
- [code](../codebase/code.md) — chat-level code rendering, including `Diff` and `ToolDiff`.
- [code_editor](code_editor.md) — lite-IDE file explorer, editor panel, and diff view.
- [diff_approval](../sdlc/diff_approval.md) — SDLC approval gate with verified diffs and review comments.
