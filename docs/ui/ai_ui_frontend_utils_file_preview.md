...
These utilities are consumed by the [document_preview](../documents/document_preview.md) feature, specifically the `DocumentPreviewModal` component and its sub-renderers.
...
| `cachedGetOrFetch(attachmentId)` | Cache-first read with authenticated server fallback via config. |
...
- Falls back to `GET /chat/attachments/{id}/raw` using `authFetch` from config when the local entry is unavailable.
...
- ai_ui_frontend_utils_security — input and form validation

It is consumed primarily by the [document_preview](../documents/document_preview.md) component and indirectly by the [chat](../chat/chat.md) and [kb_chat](../knowledge/kb_chat.md) features that handle file uploads.
...
- [document_preview](../documents/document_preview.md) — primary consumer of these utilities
- ai_ui_frontend_config — provides `authFetch` and `API_BASE`
- [chat](../chat/chat.md) and [kb_chat](../knowledge/kb_chat.md) — upload flows that populate the cache
