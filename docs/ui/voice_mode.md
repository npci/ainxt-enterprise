...
- [gateway](../core/gateway.md) — Exposes `/voice/tts` and `/voice/stt`; the TTS path is consumed by the `ttsApi` wrapper.
- [llm_proxy](../llm/llm_proxy.md) — Hosts `/llm/tts`, the production TTS proxy that reaches OpenAI from the web tier.
