...
The **Sidebar** is the node palette component in the ABStudio workflow editor. It renders a draggable list of node types that users can drop onto the [Canvas](Canvas.md) to build workflows. It is a thin, presentation-focused React component that relies on the [workflowStore](../workflowStore.md) for workflow state and on React Flow's native drag-and-drop contract for node creation.
...
|--------------|-----------|-------------|
| **Agent** | `agent` | Represents an agent invocation step. See [AgentNode](../AgentNode.md). |
| **Condition** | `condition` | Branches execution based on configurable rules. See [ConditionNode](../ConditionNode.md). |
| **Existing Asset** | `subflow` | Embeds or links an existing workflow or agent as a subflow. See [SubflowNode](../SubflowNode.md). |
| **Loop** | `loop` | Iterates a body subgraph until a termination condition is met. See [LoopNode](../LoopNode.md). |
| **End** | `end` | Terminates the workflow. See [EndNode](../EndNode.md). |
...
The Sidebar reads the current workflow nodes from [workflowStore](../workflowStore.md) to determine whether an `end` node already exists:
...
| Dependency | Role |
|------------|------|
| [workflowStore](../workflowStore.md) | Provides the current `nodes` array used to enforce the single-end-node rule. |
...
| Node type components ([AgentNode](../AgentNode.md), [ConditionNode](../ConditionNode.md), [LoopNode](../LoopNode.md), [SubflowNode](../SubflowNode.md), [EndNode](../EndNode.md)) | Render the nodes after they are dropped; the Sidebar only knows their type strings. |
...
- **[ConfigPanel](../infrastructure/ConfigPanel.md)** – the property editor for the selected node.
- **[ChatPanel](../chat/ChatPanel.md)** – the assistant chat used to generate or modify workflows.
- **[DebugLogView](DebugLogView.md)** – execution trace and debug output viewer.
- **[workflowStore](../workflowStore.md)** – the shared Zustand store that holds the workflow graph state.
