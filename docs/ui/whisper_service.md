...
- **[gateway](../core/gateway.md)** — routes `POST /voice/stt` to this service when `WHISPER_SVC_URL` is configured. See [gateway.md](../core/gateway.md) for the broader routing and health/monitoring picture.
- **ai_ui_frontend** — the Voice Mode UI in `ai-ui/src/components/VoiceMode.jsx` initiates the audio capture that ultimately reaches this service.
...
| `WHISPER_SVC_URL` | — | URL the [gateway](../core/gateway.md) uses to proxy STT requests here. |
...
For platform-level health aggregation, see [gateway.md](../core/gateway.md) under health and monitoring.
...
| Service | Role |
|---------|------|
| [gateway](../core/gateway.md) | Proxies voice STT requests and exposes `/voice/stt` to clients. |
| [llm_proxy](../llm/llm_proxy.md) | Handles LLM, TTS, and image generation; Whisper Service is the STT counterpart. |
...
| ai_ui_frontend | Provides the Voice Mode UI that captures audio and sends it through the gateway. |

---

## See Also

- [gateway.md](../core/gateway.md)
- [llm_proxy.md](../llm/llm_proxy.md)
