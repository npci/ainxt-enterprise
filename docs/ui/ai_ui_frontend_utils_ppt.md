...
These utilities are intentionally lightweight and regex-based so that intent classification and parameter extraction can happen synchronously on the client without calling an LLM. They feed the [PPT Wizard](../presentation/ppt_wizard.md) and [PPT Chat](../presentation/ppt_chat.md) experiences and are consumed by the broader [KB Chat](../knowledge/kb_chat.md) flow.
...
| Module | Role |
|--------|------|
| [presenton_router](../presentation/presenton_router.md) | Receives `GenerateRequest` / `OutlineRequest` and orchestrates outline generation and presentation download |
| [presenton_lib](../presentation/presenton_lib.md) | Client-side library that streams outlines, polls status, and builds slide payloads |
...
`ai_ui_frontend_utils_ppt` sits at the boundary between natural user input and structured PPT generation. It is part of the larger ai_ui_frontend application and relies on the following upstream/downstream modules:

- **Upstream:** [ai_ui_frontend_utils_chat_message](ai_ui_frontend_utils_chat_message.md) provides message formatting helpers used by `KbChat.jsx` after intent detection.
- **Upstream:** ai_ui_frontend_utils_security sanitizes user input before it reaches the PPT utilities.
- **Downstream:** [ppt_wizard](../presentation/ppt_wizard.md) consumes extracted parameters to render the step-by-step presentation builder.
- **Downstream:** [presenton_lib](../presentation/presenton_lib.md) and [presenton_router](../presentation/presenton_router.md) perform the actual outline streaming, status polling, and file generation.
...
- ai_ui_frontend – parent frontend application
- [ai_ui_frontend_utils](ai_ui_frontend_utils.md) – parent utilities module
- [ai_ui_frontend_utils_chat_message](ai_ui_frontend_utils_chat_message.md) – message content helpers
- ai_ui_frontend_utils_security – input validation helpers
- [kb_chat](../knowledge/kb_chat.md) – primary consumer of PPT intent detection
- [ppt_wizard](../presentation/ppt_wizard.md) – UI that consumes extracted parameters
- [ppt_chat](../presentation/ppt_chat.md) – chat-based PPT generation UI
- [presenton_lib](../presentation/presenton_lib.md) – client-side PPT generation library
- [presenton_router](../presentation/presenton_router.md) – backend API for PPT generation
