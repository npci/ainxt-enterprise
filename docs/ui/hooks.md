...
This module is intentionally small and focused: it contains no UI markup, only stateful logic that can be composed with presentational components such as [`HoverTooltip`](../ui/common_components.md) and [`HoverTooltip.jsx`](../ui/common_components.md).
...
`useHoverTooltip` is designed to be consumed by presentational components in the frontend. The most direct consumer is the [`HoverTooltip`](../ui/common_components.md) component in the `common_components` module, which combines this hook with tooltip rendering markup.
...
For details on the tooltip UI itself, see [`common_components.md`](../ui/common_components.md).
