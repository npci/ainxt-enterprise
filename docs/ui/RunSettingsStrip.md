...
| Module | Relationship |
| --- | --- |
| workflowStore | Provides `runSubagentsEnabled` and `setRunSubagentsEnabled`. The store persists the run-level policy across the session. |
...
| [engine_native_engine](../agents/engine_native_engine.md) | Resolves the final subagent policy per node using the precedence rules above. |
...
1. Add the new policy value and setter to workflowStore.
...
- workflowStore — client state for the workflow editor, including run-level execution flags.
- [api_execution](../api/api_execution.md) — backend endpoints for running and resuming workflows.
- [engine_native_engine](../agents/engine_native_engine.md) — execution engine that resolves subagent delegation per node.
