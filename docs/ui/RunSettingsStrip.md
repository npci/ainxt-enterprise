...
This precedence is shared with the per-node toggle in [ConfigPanel](../infrastructure/ConfigPanel.md); both surfaces display the same explanatory copy so users learn the behavior consistently.
...
| [ConfigPanel](../infrastructure/ConfigPanel.md) | Hosts the per-node subagent pins (`data.enable_subagents` / `data.disable_subagents`) that take precedence over the run-level flag. |
| [ChatPanel](../chat/ChatPanel.md) | Initiates workflow runs and passes the run-level subagent flag to the execution API. |
| [api_execution](../api/api_execution.md) | Backend route (`run_workflow_stream`) that receives the run-level flag and starts execution. |
| [engine_native_engine](../reference/engine_native_engine.md) | Resolves the final subagent policy per node using the precedence rules above. |
...
- [ConfigPanel](../infrastructure/ConfigPanel.md) — per-node subagent pins and node configuration UI.
- [ChatPanel](../chat/ChatPanel.md) — chat header actions that trigger workflow runs.
- [workflowStore](../workflowStore.md) — client state for the workflow editor, including run-level execution flags.
- [api_execution](../api/api_execution.md) — backend endpoints for running and resuming workflows.
- [engine_native_engine](../reference/engine_native_engine.md) — execution engine that resolves subagent delegation per node.
