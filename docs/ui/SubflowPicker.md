...
- **`single`** — selects one `{ kind, refId, refName }` object (used by the [SubflowNode](../SubflowNode.md) configuration panel).
- **`multi`** — builds a list of such objects (used by the [AgentEditor](../AgentEditor.md) to attach workflows or agents).
...
| ---------- | ------------ |
| [apiFetch](../api_config.md) | HTTP helper used for all backend calls. |
| [SubflowNode](../SubflowNode.md) | Primary consumer in single-mode for subflow node configuration. |
| [AgentEditor](../AgentEditor.md) | Consumer in multi-mode for attaching workflows/agents to an agent. |
| [ConfigPanel](../infrastructure/ConfigPanel.md) | Hosts the picker when a subflow node is selected on the canvas. |
| [workflowStore](../workflowStore.md) | Provides the open workflow ID used for `excludeWorkflowId`. |
