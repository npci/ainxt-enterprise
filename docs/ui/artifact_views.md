...
The `payload` is what gets passed as the `p` prop to stage renderers. See [sdlc_router](../sdlc/sdlc_router.md) for the full API contract.
...
Returns the `VERIFIED_DIFF` artifact (the real, compiled, test-green diff the human approves at the HITL gate). Used by `DiffApprovalPanel` when rendered inside `StageArtifactDrawer` for the `IMPLEMENT` stage. See [sdlc_router](../sdlc/sdlc_router.md) for details.
...
- [sdlc_router](../sdlc/sdlc_router.md) — Backend API router for SDLC endpoints (artifact, verified-diff, governance)
