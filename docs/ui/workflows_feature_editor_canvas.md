...
The Canvas is mounted inside the `EditorShell` layout (see [app_core](../reference/app_core.md)) alongside the [Sidebar](../workflows_feature_editor_sidebar_pickers.md), [ConfigPanel](../workflows_feature_editor_config_panel.md), and [ChatPanel](../chat/workflows_feature_editor_chat_panel.md). It reads and writes all graph state through the shared Zustand-based `workflowStore`, which also powers undo/redo, execution tracking, and the serialization path that sends the workflow definition to the backend `NativeEngine` for execution.
...
During workflow execution, the Canvas provides real-time visual feedback by highlighting active nodes. The `workflowStore.activeNodeIds` array is populated by the [ChatPanel](../chat/workflows_feature_editor_chat_panel.md) as it processes SSE events from the backend `/run-stream` endpoint.
...
For details on the execution engine, see [engine_native_engine](../reference/engine_native_engine.md).
...
| [Sidebar](../workflows_feature_editor_sidebar_pickers.md) | Draggable node palette that feeds the Canvas drop handler |
| [ConfigPanel](../workflows_feature_editor_config_panel.md) | Node configuration panel — opens when `selectedNodeId` is set |
| [ChatPanel](../chat/workflows_feature_editor_chat_panel.md) | Execution/chat surface — drives `activeNodeIds` and `loopProgress` |
| [EditorShell](../reference/app_core.md) (`App.jsx`) | Parent layout — wraps Canvas in `ReactFlowProvider`, manages edit/preview mode |
| [Node Components](../workflows_feature_editor_nodes.md) | Seven custom node renderers |
| [AiEdge](../workflows_feature_editor_edges.md) | Custom edge with insert/delete actions |
| [NativeEngine](../reference/engine_native_engine.md) | Backend graph traversal engine that consumes the Canvas's workflow definition |
...
- [EditorShell & App Core](../reference/app_core.md) — Parent layout, mode switching, workflow name editing
- [Sidebar & Pickers](../workflows_feature_editor_sidebar_pickers.md) — Node palette, SubflowPicker, LoopItemsPicker, RunSettingsStrip
- [ConfigPanel](../workflows_feature_editor_config_panel.md) — Per-node configuration (agent, condition, loop, subflow, evaluation gate)
- [ChatPanel](../chat/workflows_feature_editor_chat_panel.md) — Workflow execution, SSE handling, HITL, debug log
- [DebugLogView](../workflows_feature_editor_debug_log.md) — Execution timeline and per-node event inspection
- [Node Components](../workflows_feature_editor_nodes.md) — Individual node renderers
- [Edge Components](../workflows_feature_editor_edges.md) — AiEdge with insert/delete actions
- [Condition Builders](../workflows_feature_editor_conditions.md) — Condition DSL editors for condition and loop nodes
- [Workflow Store](../storage/store.md) — Zustand store with undo/redo, execution state, graph validation
- [NativeEngine](../reference/engine_native_engine.md) — Backend graph traversal and agent execution engine
- [Workflows Dashboard](../workflows/workflows_feature_dashboard.md) — Workflow list, creation, and card management
- [Workflow Factory Chat](../chat/workflows_feature_factory_chat.md) — AI-powered workflow generation
