...
For details on how code blocks are embedded inside chat messages, see the [message](../chat/message.md) module. For the broader code-editing and diff-rendering experience, see the [code](../codebase/code.md) module. For the main chat orchestration that produces the content being highlighted, see the [chat](../chat/chat.md) module.
...
- **No copy button**: Copy-to-clipboard functionality is intentionally omitted here. Parent components such as `CopyableCodeBlock` in the [message](../chat/message.md) module wrap or replace this component when that behavior is required.
...
- [message](../chat/message.md) — Renders assistant messages and may use `CodeBlock` for fenced code blocks.
- [code](../codebase/code.md) — Provides code editing, diff views, and tool-call rendering.
- [chat](../chat/chat.md) — Main chat interface that streams and displays assistant responses containing code.
- [kb_chat](../knowledge/kb_chat.md) — Knowledge-base chat that can also display highlighted code snippets.
