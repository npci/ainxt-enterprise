...
The **ConfigPanel** is the right-hand configuration sidebar of the ABStudio workflow editor. It renders a context-sensitive form for whichever canvas node is currently selected, letting users edit node data without leaving the visual workflow canvas. It supports all workflow node types defined in `workflowStore`: `start`, `end`, `agent`, `condition`, `subflow`, and `loop`.
...
1. Subscribes to `selectedNodeId`, `nodes`, `edges`, `workflowId`, `workflowKnowledge`, and `activeThreadId` from `workflowStore`.
2. Bootstraps the saved agents and workflows catalogs (via `agentsStore` and `dashboardStore`) so the in-canvas agent name validator can enforce global uniqueness.
3. Fetches the `ABSTUDIO_AGENT_TRIGGERS_ENABLED` feature flag from `/triggers/config`.
4. Uses `useAvailableModels` to populate model dropdowns and auto-configure provider / max-tokens defaults.
...
Callback passed to `GenerateInstructionsModal`. Writes the AI-generated instructions into the selected agent node.
...
|---------|--------|---------------------------|
| **Agent Name** | `name` | Must be unique across other workflow agent nodes, saved agents, and saved workflows. Validated with `validateEntityName`. |
| **Instructions** | `instructions` | System prompt for the agent. Can be AI-generated via `GenerateInstructionsModal`. |
| **Model** | `modelName`, `provider`, `apiKey`, `baseUrl` | Routed through the AiNxt gateway. Auto-caps `maxTokens` via `getMaxTokensForModel`. |
| **Subagents (swarm)** | `enable_subagents`, `disable_subagents` | Per-node override of the run-level swarm toggle. Forces injection of `WorkflowSwarmTool` in `native_engine`. |
| **Model Parameters** | `temperature`, `maxTokens`, `topP` | Standard LLM sampling controls. |
| **Catalog Tools & Skills** | `tools`, `skills` | Attached via `CatalogPicker`. |
| **Knowledge** | `knowledge` | RAG configuration via `KnowledgeSection`; inherits workflow-level KB when node is `KB_MODE_NONE`. |
| **Human-in-the-Loop** | `hitlMode` | Emits `hitl_interrupt` SSE events consumed by [`ChatPanel`](ChatPanel.md). |
| **Triggers** | — | Conditionally rendered when `agentTriggersEnabled` is true and the workflow is saved. Uses `TriggerSection`. |

### Condition Nodes

- Edits `data.cases` through `ConditionBuilder`.
- Cases are evaluated top-to-bottom; the first match wins.
- Unmatched inputs fall through to the implicit `ELSE` branch.
- See `ConditionBuilder`, `ConditionCase`, and `SingleCondition` for the condition DSL.
...
|------|----------------|-------------|
| `while` | A condition expression stays true | `LoopWhileEditor` |
...
The evaluator data is forwarded by `workflowStore.getWorkflowForExecution` and consumed by `loop_evaluator` in the backend.
...
- `workflowStore` — selected node, node list, edges, workflow id, knowledge, active thread.
- `agentsStore` — saved agents catalog for name uniqueness checks.
- `dashboardStore` — saved workflows catalog for name uniqueness checks.

### Child Components

- `ConditionBuilder` / `LoopWhileEditor` — condition DSL editors.
...
- `CatalogPicker` — tools and skills attachment.
- `KnowledgeSection` — RAG / KB attachment.
- `GenerateInstructionsModal` — AI-generated instructions.
- `TriggerSection` — per-node trigger configuration.

### Hooks & Utilities

- `useAvailableModels` — model catalog with loading / error states.
- `useCurrentUser` — department and approval role for KB uploads.
- `validateEntityName` — global name uniqueness validation.
- `stripProviderPrefix` — human-readable model labels.
- `getMaxTokensForModel` — per-model token cap.
- `getUpstreamNodeId` — derives the node wired into a loop.

### Backend Counterparts

- `native_engine` — executes agent, subflow, condition, and loop nodes.
- `loop_evaluator` — optional LLM-as-judge for loops.
...
- The `enable_subagents` / `disable_subagents` flags are intentionally tri-state: explicit ON, explicit OFF, or inherit from the run-level toggle. Do not collapse them into a single boolean without updating `native_engine`.
