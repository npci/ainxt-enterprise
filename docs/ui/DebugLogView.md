...
| workflowStore (implied) | **Data source.** The `runContext` slice (`rows`, `runHistory`, `status`, `executionTrace`, `currentInput`, `finalOutput`) and store actions (`beginRunContext`, `appendRunEvent`, `setRunContextFromComplete`, `setRunStatus`, `stopRunPreservingLog`) drive all data into DebugLogView. |
