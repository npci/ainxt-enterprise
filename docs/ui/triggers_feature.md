...
|---|---|---|
| triggers_feature_editor | Trigger list, draft creation, inline editing, per-trigger execution history, and the modal wrapper used by dashboards. | `TriggerSection.jsx`, `TriggerModal.jsx` |
| triggers_feature_scheduler | Schedule type tabs and inputs: once, hourly, daily, weekdays, weekly, and custom cron. | `TriggerPicker.jsx` |
| triggers_feature_notifications | Global notification bell, transient toasts, execution detail modal, and seen-state management. | `TriggerNotifications.jsx` |
| triggers_feature_utils | Shared helpers: IST formatting, duration labels, and the DOM portal used to escape containing blocks. | `triggerUtils.js`, `triggerPortal.js` |

## External Dependencies

- **triggersStore** — Zustand store that owns all API calls and client-side caching for triggers and executions.
- **workflowStore** — Used by `TriggerNotifications` to suppress duplicate toasts when the user is already viewing the chat for the workflow that just ran.
- **[api_triggers](../api/api_triggers.md)** — REST endpoints: `/triggers`, `/trigger-executions`, `/triggers/{id}/webhook`, `/triggers/config`.
- **[services_trigger_scheduler](../workers/services_trigger_scheduler.md)** — APScheduler-based engine that registers, reschedules, and fires triggers.
- **[core_workflow_repo](../workflows/core_workflow_repo.md)** — Persistence layer for triggers and executions.
