...
For details on how artifacts are created and stored during chat, see the chat_router documentation.
...
|--------|--------------|-------------|
| config | Imports `API_BASE` and `authFetch` | Provides backend base URL and authenticated HTTP helper. |
| chat_router | Backend API consumed | Supplies `GET /chats/{chatId}/artifacts/{artifactId}` and related artifact management endpoints. |
| [Message](../chat/message.md) | Conceptual reuse | Uses the same Mermaid rendering approach as chat messages. |
| [Chat](../chat/chat.md) | Typical parent / orchestrator | Manages the chat session and artifact selection state. |
...
- ai_ui_frontend_config.md — Authentication and API configuration.
- shared_api_routers_chat_router.md — Backend artifact endpoints.
- [message.md](../chat/message.md) — Message rendering including Mermaid diagrams.
- [chat.md](../chat/chat.md) — Main chat orchestration component.
