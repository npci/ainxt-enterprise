...
- **[core_config](../infrastructure/core_config.md)** — `factory_model`, `fill_blank_llm_fields`, `openai_compatible_base_url`, `openai_compatible_api_key` for resolving the judge's LLM endpoint.
- **[core_llm_handler](../models/core_llm_handler.md)** — `Message`, `get_llm_client` for provider-agnostic LLM calls (OpenAI / Ollama / vLLM / LiteLLM).
- **[app_models](../models/app_models.md)** — `LLMConfig` dataclass for constructing the LLM client.

It is consumed by:

- **[engine_native_engine](../reference/engine_native_engine.md)** — `NativeEngine._run_loop` imports the factory helpers and uses `LLMEvaluator` / `LoopController` / `LoopBudget` during `while`-mode loop execution.

It is configured by:

- **[workflow_editor](../workflows/workflow_editor.md)** — the frontend `LoopNode`, `LoopWhileEditor`, and `LoopItemsPicker` components write the loop-node configuration keys that activate this module.
...
A small, standalone token + wall-clock accumulator for loop runs. Token counts are **estimated** using the `len(text) // 4` heuristic (same approach used in [loop_runner](../reference/loop_runner.md)'s `BudgetMeter`). This is sufficient for a "stop runaway loops" guardrail.
...
These functions read raw loop-node configuration dicts and construct the corresponding objects. They are the primary integration point used by [engine_native_engine](../reference/engine_native_engine.md).
...
This module is **distinct** from the governed outer-loop system in [loop_runner](../reference/loop_runner.md) and [loop_models](../models/loop_models.md):
...
- **[loop_runner](../reference/loop_runner.md)** — serves the **governed outer loop** with `LoopRunner`, `ProofEvaluator`, `VerifierAgent`, `BudgetMeter`, `ReflectionWriter`, and `AgentMemory`. That system has richer semantics (goals, triggers, inbox triage, proof checks) and its own `BudgetMeter` with goal/ctx precedence.

The in-graph **evaluation gate** node (`evaluation_gate` type) in `NativeEngine` uses a separate helper (`evaluate_llm_judge` from [loop_runner](../reference/loop_runner.md)), not this module. See [engine_native_engine](../reference/engine_native_engine.md) for details.

### SSE Events

The `NativeEngine._run_loop` method emits the following SSE events that carry data produced by this module. See [engine_native_engine](../reference/engine_native_engine.md) for the full event catalogue:
...
- [engine_native_engine](../reference/engine_native_engine.md) — `NativeEngine._run_loop` integration, SSE event emission, and `_resolve_judge_llm_cfg`
- [loop_runner](../reference/loop_runner.md) — governed outer-loop system with `ProofEvaluator`, `BudgetMeter`, `ReflectionWriter`
- [loop_models](../models/loop_models.md) — loop domain models (`VerificationVerdict`, `VerifySpec`, `ProofCheck`, etc.)
- [core_config](../infrastructure/core_config.md) — `factory_model`, `fill_blank_llm_fields`, `openai_compatible_*` helpers
- [core_llm_handler](../models/core_llm_handler.md) — `Message`, `get_llm_client`, `FallbackLLMClient`
- [app_models](../models/app_models.md) — `LLMConfig` dataclass
- [workflow_editor](../workflows/workflow_editor.md) — `LoopNode`, `LoopWhileEditor`, `LoopItemsPicker` frontend components
- [api_loops](../api/api_loops.md) — `run_loop_stream_route` API endpoint
