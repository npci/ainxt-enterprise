...
- **Downstream**:
  - [`shared_core.md`](../reference/shared_core.md) — the `EvalEngine` in `core/evals.py` produces the metrics and results that evaluation suites consume.
  - [`shared_api_routers.md`](../api/shared_api_routers.md) — the `evals_router` exposes evaluation results through HTTP endpoints such as `list_eval_results`, `eval_summary`, and `eval_trend`.
  - [`ai_ui_frontend.md`](../ui/ai_ui_frontend.md) — the `EvalsDashboard` component visualizes evaluation summaries, trends, and result rows for users.
  - [`workers.md`](../workers/workers.md) — `tenx_eval_worker` runs evaluation jobs asynchronously, using patterns similar to the probe/gate model.
