...
- **[`workflow_system`](workflow_system.md)** — The `WorkflowEngine`, `PlanningEngine`, and related workflow execution infrastructure.
- **[`core_infrastructure`](core_infrastructure.md)** — Job queue, logging, and KV store infrastructure.
- **[`database`](database.md)** — Database models including `WorkflowRunRecord` and `workflow_history`.
- **[`memory_system`](memory_system.md)** — `PostgresMemory` and other memory backends.
...
> **Note:** The `paused` state (used for HITL approval gates) is managed by `WorkflowEngine` directly via the `workflow_runs` table, not by this worker. See [`workflow_system`](workflow_system.md) for details on the approval/pause/resume flow.
