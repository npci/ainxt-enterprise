...
| Module | Relationship |
|---|---|
| [connector_infrastructure.md](connector_infrastructure.md) | `ConnectorEngine`, `ConnectorRegistry`, `OAuth2Handler` — the broader connector framework that DPI consent plugs into. |
| [connectors_router.md](connectors_router.md) | API endpoints `dpi_consent_start` and `dpi_consent_store`, plus request models `DpiConsentStartRequest` and `DpiConsentStoreRequest`. |
| [connector_adapters.md](connector_adapters.md) | `DpiAccountAggregatorAdapter` and `DpiDigilockerAdapter` — the data-fetching adapters that consume the verified consent context. |
