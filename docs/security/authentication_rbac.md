...
| `db.database.SessionLocal` | SQLAlchemy session factory for HOD user-scope queries | [database](../storage/database.md) |
| `db.models.User` | User ORM model (department, id) used for HOD scoping | [database](../storage/database.md) |
| `store.sdlc_governance_approvers.approver_domains_for` | Returns active governance domains a user may approve | [store_layer](../storage/store_layer.md) |
...
|---|---|---|
| **Shared API Routers** | `_require_director`, `_require_admin_or_operator`, `_require_admin`, `require_broadcast_user` | [shared_api_routers](../api/shared_api_routers.md) |
| **ABStudio Backend API** | `_wrapped_gateway_auth`, `require_admin` in `app/api/deps.py` | [api_deps](../api/api_deps.md) |
| **Gateway** | Route-level authorization for agent, workflow, chat endpoints | [gateway](../models/gateway.md) |
| **SDLC Governance Router** | Governance domain approval, suppression management | [sdlc_router](../api/sdlc_router.md) |
| **Budget Router** | Admin/operator gates for budget management | [budget_router](../api/budget_router.md) |
| **Auth Router** | Level overrides, user management | [auth_router](../api/auth_router.md) |
