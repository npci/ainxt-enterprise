...
| **Phase 2** (future) | 🔜 Planned | Replace `_check_keywords()` with an actual `nemoguardrails.LLMRails` call using `guardrails/config.yml`. See [guardrails_tools](../security/guardrails_tools.md) for the Phase 2 runtime. |
...
The HardBlock Engine sits at the **top of the request safety pipeline**. When a message arrives at the Anthropic Messages-compatible endpoint (`/v1/messages`), the compliance gate in `messages_compat_router._compliance_check()` calls `hardblock_engine.check()` on every newly-scanned user and tool-result message *before* the [ComplianceEngine](../security/decision_engines_compliance.md) runs its PCI/PII analysis. If the hardblock gate triggers, the request is refused immediately — the LLM provider is never contacted.
...
4. If not blocked, proceeds to the [ComplianceEngine](../security/decision_engines_compliance.md) for PCI/PII redaction and blocking.
...
> 📖 See [core_infrastructure](../core/core_infrastructure.md) for the full configuration reference.
...
| **ComplianceEngine** | [decision_engines_compliance](../security/decision_engines_compliance.md) | PCI/PII/secret detection, redaction, and blocking | ✅ Yes (block-configured types) |
