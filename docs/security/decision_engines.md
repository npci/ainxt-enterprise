...
- **Agent orchestration**: `DecisionEngine` is consumed by agent runners and ReAct-style loops to pick the next action. See [agent_orchestration](../agents/agent_orchestration.md) and [reaction_engines](../llm/reaction_engines.md).
...
- **Observability**: All engines log through `core.logger`; `HardBlockEngine` also writes a dedicated audit log. See [core_infrastructure](../core/core_infrastructure.md).
