...
| [core_infrastructure](../infrastructure/core_infrastructure.md) | Provides `redis_client` (config) and `logger` used by this module. |
| [database](../storage/database.md) | Provides `SessionLocal` and the `User` / `department_hod_mapping` models. |
