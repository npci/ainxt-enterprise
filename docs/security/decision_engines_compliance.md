...
- **DecisionEngine** ([decision_engines_core](../reference/decision_engines_core.md)) decides *which tools* an agent should invoke.
...
- **HardBlockEngine** ([decision_engines_hardblock](../storage/decision_engines_hardblock.md)) decides *whether a prompt is an AI-safety violation* (jailbreak, harmful intent) using a weighted confidence-score gate.
...
Configuration is managed at runtime through the [admin_router](../api/admin_router.md) endpoints (requires admin flag):
...
The [compliance_router](../api/compliance_router.md) exposes a batch endpoint for bulk PII/PCI regression testing (up to 1000 texts, no LLM call):
...
- **HardBlock integration**: Runs [HardBlockEngine](../storage/decision_engines_hardblock.md) on each message before compliance, returning immediately if a safety block is triggered.
...
| Module | Relationship |
|---|---|
| [decision_engines_core](../reference/decision_engines_core.md) | Sibling — LLM-based tool selection (`DecisionEngine`) |
| [decision_engines_hardblock](../storage/decision_engines_hardblock.md) | Sibling — AI-safety keyword blocking (`HardBlockEngine`) |
| [privacy_service](privacy_service.md) | ML NER service called by the ML layer |
| [admin_router](../api/admin_router.md) | Runtime config management API |
| [compliance_router](../api/compliance_router.md) | Batch compliance check endpoint |
| [guardrails](guardrails.md) | NeMo Guardrails AI-safety layer (complementary) |
| [core_infrastructure](../infrastructure/core_infrastructure.md) | Logger, telemetry, config infrastructure |
| [sdlc_pipeline_agents](../agents/sdlc_pipeline_agents.md) | `ComplianceBlocked` exception consumer |
