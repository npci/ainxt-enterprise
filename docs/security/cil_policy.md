...
- **Pipeline dispatch** ([`pipeline`](../core/pipeline.md)) — uses policy decisions to
  select the response shape (e.g. whether to inject a clarification turn).
- **Model routing** ([`model_routing`](../llm/model_routing.md)) — sensitivity and
...
> [profiles](../llm/profiles.md)), which bundles context, routing, and grounding
...
| [`pipeline`](../core/pipeline.md) | Consumes policy decisions for response shaping and clarification injection. |
| [`model_routing`](../llm/model_routing.md) | Uses sensitivity/risk to influence local-vs-cloud routing (see `classification_from_policy`). |
| [`guardrails`](../security/guardrails.md) | Independent input-safety layer; complementary to `cil_policy` risk decisions. |
| [`profiles`](../llm/profiles.md) | Broader policy bundles; note the separate `DomainProfile` class in `profiles/schema.py`. |
...
- [pipeline](../core/pipeline.md) — Request dispatch and response shaping.
- [model_routing](../llm/model_routing.md) — Model selection, fallback, and privacy-floor classification.
- [guardrails](../security/guardrails.md) — NeMo Guardrails input-safety checks.
- [profiles](../llm/profiles.md) — Enterprise policy profiles (context, routing, grounding).
- [context_engine](../llm/context_engine.md) — Context budget planning and age-tier compaction.
