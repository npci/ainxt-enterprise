...
The [gateway](../models/gateway.md) module calls the ComplianceEngine on every chat/agent request:
...
[Shared API routers](../api/shared_api_routers.md) use `core/security_validation.py` validators on request bodies:
...
The [desktop app](../clients/desktop_app.md) browser agent (`playwrightManager.js`) routes all extracted text and screenshots through the gateway's compliance endpoints:
...
The [AI-UI frontend](../ui/ai_ui_frontend.md) uses `securityValidation.js` for client-side validation before API calls, mirroring the backend validators. This provides immediate user feedback and reduces invalid requests reaching the server.
...
| [core_infrastructure](../infrastructure/core_infrastructure.md) | Logging, telemetry, and configuration infrastructure used by this module |
| [gateway](../models/gateway.md) | Primary consumer — calls ComplianceEngine on every request |
| [shared_api_routers](../api/shared_api_routers.md) | API routers that use security validation on request bodies |
| [desktop_app](../clients/desktop_app.md) | Browser agent that routes extracted content through compliance endpoints |
| [ai_ui_frontend](../ui/ai_ui_frontend.md) | Frontend security validation mirror |
