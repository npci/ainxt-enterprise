...
Desktop and browser automation tools (see [`desktop_router`](../cowork/desktop_router.md)) extract screen text and screenshots from the user's environment. This content can contain sensitive information such as:
...
- **`scan`** — Main orchestration endpoint. Validates the request mode, dispatches text to the `ComplianceEngine`, and handles image payloads with explicit best-effort flags.
...
- **`compliance_engine`** — Shared singleton from `agents/compliance_engine` that performs regex + ML analysis, redaction, and blocking logic.
- **`get_current_user`** — Authentication dependency from `auth.dependencies` that ensures only authenticated users can invoke scanning.
...
- **Cowork / CLI runtime** — read-path content that enters agent context (tool results may use `keep_types` to preserve necessary identifiers; see `agents/compliance_engine`).
...
- **Configuration-driven behavior**: Sensitive-type actions (`redact`, `block`, `off`) are controlled by the shared `ComplianceEngine` configuration, reloadable at runtime via [`admin_router`](../api/admin_router.md).
...
- `agents/compliance_engine` — Core engine performing regex + ML detection, redaction, and configuration management.
- `auth/dependencies` — Authentication dependency used by this router.
- [`desktop_router`](../cowork/desktop_router.md) — Desktop workspace/indexing endpoints that may feed content into this scanner.
- [`connectors_router`](../connectors/connectors_router.md) — Outbound connector actions that enforce separate write-path guardrails.
- `core/logger` — Structured logging used for audit-safe scan logging.
