...
Both workers are designed to be safe (dry-run capable), observable (run records written to the KV trace store), and idempotent (opportunistic throttling via Redis/RustyCluster). They are invoked by the cron scheduler in [`workers/start_workers.py`](worker_orchestration.md) and can also be executed manually or enqueued as one-off jobs.
...
| Opportunistic throttling | `workers/purge_worker.py` | Uses per-purge-type Redis/RustyCluster keys to avoid overlapping runs. |
...
- Deletes bytes via [`core.storage.storage.delete()`](core_infrastructure.md) (supports MinIO and local disk).
...
|------------|------------|---------|
| `core.ckms.load_at_boot` | [core_infrastructure.md](core_infrastructure.md) | Decrypts protected env vars before logger/DB imports |
| `core.logger` | [core_infrastructure.md](core_infrastructure.md) | Structured logging |
| `core.config.RDB_QUEUE`, `RDB_TRACE` | [core_infrastructure.md](core_infrastructure.md) | Logical KV DB numbers |
| `core.kv.get_kv` | [core_infrastructure.md](core_infrastructure.md) | Backend-agnostic KV client |
| `core.storage.storage` | [core_infrastructure.md](core_infrastructure.md) | MinIO/local object deletion |
| `db.database.SessionLocal` | [database.md](database.md) | SQLAlchemy session factory |
| `db.models.GeneratedDocument` | [database.md](database.md) | Generated document audit rows |
| `db.models.GeneratedImage` | [database.md](database.md) | Generated image audit rows |
| `db.models.ChatAttachment` | [database.md](database.md) | Uploaded chat file rows |
| `db.models.Thread`, `ThreadMessage` | [database.md](database.md) | Thread/message rows |

### Upstream Producers

These user-facing modules create the data that purge workers later clean up:

- [`ai_ui_frontend_chat.md`](ai_ui_frontend_chat.md) and [`ai_ui_frontend_kb_chat.md`](ai_ui_frontend_kb_chat.md) — create `ChatAttachment`, `GeneratedDocument`, `GeneratedImage`.
- [`ai_ui_frontend_threads.md`](ai_ui_frontend_threads.md) — creates `Thread` and `ThreadMessage`.
...
- [core_infrastructure.md](core_infrastructure.md) — shared config, logging, KV, and storage layers.
- [database.md](database.md) — PostgreSQL models consumed by purge workers.
- [document_knowledge_workers.md](document_knowledge_workers.md) — workers that produce the generated assets cleaned up here.
- [ai_ui_frontend_chat.md](ai_ui_frontend_chat.md) — chat UI that creates attachments and generated content.
- [ai_ui_frontend_threads.md](ai_ui_frontend_threads.md) — thread UI affected by thread purge policy.
