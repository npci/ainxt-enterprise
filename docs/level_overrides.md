...
This module is part of the broader [ai_ui_frontend](ai_ui_frontend.md) application and is closely related to the [auth](auth.md), [user_management](user_management.md), and [budget_manager](budget_manager.md) modules, all of which rely on AD level for access control and approval workflows.
...
|------------|--------|-------|
| `authFetch` | [config](config.md) | Authenticated HTTP requests to the backend. |
| `useAuth` / `AuthContext` | [auth](auth.md) | Read current user's `ad_level` and `role` for access guard and level constraints. |
| `useConfirm` | [ui_dialog](ui_dialog.md) | Confirmation dialog before revoking an override. |
| `validateDescription`, `validateSecurity` | [ai_ui_frontend_utils](ai_ui_frontend_utils.md) | Client-side input validation for reason, email, and user ID fields. |
| `toISTDate` | [ai_ui_frontend_utils](ai_ui_frontend_utils.md) | Format timestamps in active and history tables. |
| `_require_director` | [auth_router](auth_router.md) | Backend guard ensuring only director-level users or admins can manage overrides. |
| `UserLevelOverride`, `User`, `OrgTree` | [database](database.md) | Persistence of overrides and restoration of original levels. |
...
For more details on how levels are enforced across the platform, see the [auth](auth.md) and [auth_router](auth_router.md) documentation. For the broader user-management surface, see [user_management](user_management.md).
