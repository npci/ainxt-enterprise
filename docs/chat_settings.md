...
- [`kb_chat.md`](kb_chat.md) — `KbChat.jsx` duplicates the same settings logic for knowledge-base conversations.

### Downstream / callees

- [`chat_router.md`](chat_router.md) — backend router exposing `PATCH /chats/{id}/rag-mode` and `PATCH /chats/{id}/scope`.
- [`knowledge_base.md`](knowledge_base.md) — `ScopePicker.jsx` is also used when uploading documents and when starting a KB chat from `KbDrillGraph`.
- [`gateway.md`](gateway.md) — the `/ask` gateway reads `rag_mode` and scope columns from the `Chat` row and injects them into `_user_ctx` for retrieval.
...
- [kb_chat.md](kb_chat.md) — knowledge-base chat surface.
- [chat_router.md](chat_router.md) — backend chat API routes.
- [knowledge_base.md](knowledge_base.md) — KB document management and scope picker usage.
- [gateway.md](gateway.md) — runtime injection of chat settings into retrieval context.
