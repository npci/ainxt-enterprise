...
> See [SDLC Pipeline](shared_core_sdlc_pipeline.md) for how these constants are consumed by the state machine and reaper workers.
...
| [KV Store](kv_store.md) | Per-DB backend selection (`REDIS` vs `RUSTYCLUSTER`) originates from `kv_backend_for()` and `KV_BACKEND_MAP`. |
| [Database](database.md) | `postgres_dsn()`, `postgres_read_dsn()`, `pgvector_dsn()`, and `pgvector_read_dsn()` supply connection strings to SQLAlchemy session factories. |
| [Authentication](authentication.md) | LDAP/AD configuration (`LDAP_*`, `SECURITY_AD_GROUP`, `APPROVER_AD_GROUP`) and `JWT_SECRET` validation are defined here. |
| [SDLC Pipeline](shared_core_sdlc_pipeline.md) | SDLC gate TTLs, state-set constants, and `sdlc_log_context` are consumed by the state machine, reaper, and workers. |
...
| [Worker Orchestration](worker_orchestration.md) | Workers import config for Redis/Kafka/DB connections and use the logger with `sdlc_log_context` for pipeline stages. |
