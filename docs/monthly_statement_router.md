...
|---|---|---|
| `auth.dependencies.get_current_user` | [authentication](authentication.md) | JWT extraction for user endpoints |
| `auth.rbac.require_admin_flag` | [authentication](authentication.md) | Admin-gate for admin endpoints |
| `core.rate_limiter` | [core_infrastructure](core_infrastructure.md) | `SENSITIVE_ADMIN` rate-limit tier |
| `core.logger` | [core_infrastructure](core_infrastructure.md) | Structured logging |
| `db.database.SessionLocal` | [database](database.md) | SQLAlchemy session factory |
| `db.models.User` | [database](database.md) | User ORM model |
| `db.monthly_statement_models` | [database](database.md) | Statement & preference ORM models |
