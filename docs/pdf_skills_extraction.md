...
- **Upstream**: PDFs are uploaded or referenced by [ABStudio backend](abstudio_backend.md) workflows, [agent_factory_pipeline](agent_factory_pipeline.md), or [skill_factory_pipeline](skill_factory_pipeline.md) generated skills.
...
- **Cross-cutting**: The module relies only on standard Python PDF libraries (`pypdf`, `pdfplumber`) and is invoked as a standalone script, making it reusable from local CLI tooling, sandboxed execution, or workflow nodes in [app/engine/native_engine](engine_native_engine.md).
...
- [abstudio_backend](abstudio_backend.md) — backend that orchestrates skill execution
- [engine_native_engine](engine_native_engine.md) — native workflow engine that may invoke these scripts
