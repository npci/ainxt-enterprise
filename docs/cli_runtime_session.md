...
- **[cli_runtime_mcp](cli_runtime_mcp.md)** — the MCP router calls
...
- **[cli_runtime_events](cli_runtime_events.md)** — the `merge()` bridge drains
  tool events from the session bus and interleaves them with CLI text frames.
- **[engine_native_engine](engine_native_engine.md)** — when `ABSTUDIO_CLI_MODE`
...
- **[api_execution](api_execution.md)** / **[api_factories](api_factories.md)**
...
trivial (see `tool_event_to_sse()` in [cli_runtime_events](cli_runtime_events.md)).
...
header (see [cli_runtime_workspace](cli_runtime_workspace.md)). The `on_session`
...
| [cli_runtime_mcp](cli_runtime_mcp.md) | Authenticates inbound JSON-RPC; publishes tool events; enforces tool scope. |
| [cli_runtime_events](cli_runtime_events.md) | Drains the event bus; reads `generated_files` for the final result. |
| [engine_native_engine](engine_native_engine.md) | Delegates agent-node execution to the CLI runtime when CLI mode is on. |
...
- The MCP server dispatches tools through `agent_factory.pipeline.ToolDispatcher`
  (see [agent_factory_pipeline](agent_factory_pipeline.md)), which resolves user
  credentials and executes catalog tools.
- Tool-access policy is enforced by [core_governance](core_governance.md) inside
...
- [cli_runtime_mcp](cli_runtime_mcp.md) — MCP router and server that authenticate against the registry and publish tool events.
- [cli_runtime_events](cli_runtime_events.md) — Event mapper that merges CLI text with tool events from the session bus.
- [cli_runtime_workspace](cli_runtime_workspace.md) — Workspace preparation, MCP config writing, file rescue.
- [engine_native_engine](engine_native_engine.md) — Native orchestration engine with optional CLI execution branch.
- [agent_factory_pipeline](agent_factory_pipeline.md) — `ToolDispatcher` used by the MCP server to execute catalog tools.
- [api_execution](api_execution.md) — Workflow execution SSE endpoint.
- [api_factories](api_factories.md) — Agent-runner chat endpoints.
- [core_governance](core_governance.md) — Tool-access policy enforcement.
