...
2. **Local MCP server registration** — allowing the desktop app to advertise a local [Model Context Protocol (MCP)](mcp_system.md) server, whose tools can be discovered and invoked by the backend orchestrator on behalf of the user.
...
All endpoints require an authenticated user. The router does not implement its own auth logic; it depends on the shared [`auth/dependencies`](auth_dependencies.md) layer.
...
- **Search & RAG**: Files indexed through this router are stored in `document_embeddings` with the `repo` prefix `desktop_<workspace>`. They become available to hybrid retrievers and code-aware agents. See [`models/hybrid_search`](hybrid_search.md) and [`models/kb_graph_expand`](kb_graph_expand.md).
- **Agent Orchestration**: The orchestrator can call `execute_local_mcp_tool` to run tools that only exist on the user's machine (e.g., local file system access, IDE integrations, or custom desktop automations). See [`agents/orchestrator`](orchestrator.md) and [`mcp/registry`](mcp_registry.md).
- **Job Queue**: Batch indexing uses the same RQ/RustyCluster queue infrastructure as repository indexing. See [`core/job_queue`](job_queue.md).
- **KV Store**: MCP registrations are stored in the workflow KV database with an 8-hour TTL, ensuring stale registrations expire automatically. See [`core/kv`](kv_store.md).
- **Authentication**: All routes depend on the shared JWT/API-key/cookie auth layer. See [`auth/dependencies`](auth_dependencies.md).
...
- Department metadata from the user payload is attached to chunks for ACL/RAG filtering. See [`core/rag_acl`](rag_acl.md).

---

## Related Modules

- [`auth/dependencies`](auth_dependencies.md) — Authentication and user enrichment.
- [`workers/index_worker`](index_worker.md) — Chunking, embedding, and bulk upsert logic.
- [`core/job_queue`](job_queue.md) — Background job queue abstraction.
- [`core/kv`](kv_store.md) — Redis/RustyCluster KV store used for MCP registration state.
- [`core/logger`](logger.md) — Structured logging.
- [`models/hybrid_search`](hybrid_search.md) — Retrieval over `document_embeddings`.
- [`mcp/registry`](mcp_registry.md) — MCP server and tool registry.
- [`agents/orchestrator`](orchestrator.md) — Agent orchestration that may invoke local MCP tools.
