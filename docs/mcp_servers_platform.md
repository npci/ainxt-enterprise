...
The module is intentionally thin: it translates MCP `tools/call` requests into authenticated HTTP calls against the platform's REST API (the [gateway](gateway.md)), then returns the results as MCP-compliant text content. It reuses the shared `BaseMCPServer` infrastructure for protocol handling, compliance gating, and audit logging.
...
| [gateway](gateway.md) | Calls HTTP API | All tool handlers invoke endpoints served by the platform gateway. |
| [core_logger](shared_core.md#logging) | Uses | Logs server lifecycle and errors via `core.logger`. |
...
It can also be registered with the platform's MCP registry and exposed through the [mcp_server_router](shared_api_routers.md#mcp_server_router) over SSE or streamable HTTP.
...
- [gateway](gateway.md) — Platform REST API that the platform MCP server calls.
...
- [shared_api_routers.md#mcp_server_router](shared_api_routers.md#mcp_server_router) — HTTP/SSE registration and routing for MCP servers.
