...
[`core_db_pool`](core_db_pool.md). The same lazy binding is repeated inside
...
| [`core_workflow_repo`](core_workflow_repo.md) | **Data access** — provides the psycopg connection pool and trigger CRUD for Path B |
| [`core_db_pool`](core_db_pool.md) | **Infrastructure** — `SHARED_POOL` used for lazy pool binding in worker processes |
| [`database`](database.md) | **Data model** — `ScheduledWorkflow` ORM model for Path A |
| [`core_infrastructure`](core_infrastructure.md) | **Infrastructure** — `core.logger` and `core.job_queue` |
| [`api_triggers`](api_triggers.md) | **Upstream** — trigger CRUD endpoints call `register_trigger` / `reschedule_trigger` to set the initial `next_run_at` that this dispatcher later polls |
...
| `SHARED_POOL` | [`core_db_pool`](core_db_pool.md) | Lazy-bound when `workflow_repo._pool` is None in a worker process |
...
[`core_infrastructure`](core_infrastructure.md)). Key log lines:
