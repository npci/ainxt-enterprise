...
| `get_kv` | `core/kv` | KV store abstraction (Redis or RustyCluster). Used with `RDB_WORKFLOW` (DB 2) for transient OAuth2 flow state. |
...
|---|---|---|
| `connectors_router` | [connectors_router](connectors_router.md) | `oauth_start` → `generate_authorize_url` + `save_state`; `oauth_callback` → `load_state` + `exchange_code`. |
...
| `REDIS_CLIENT_CONFIG` | `REDIS` | KV backend selector (`REDIS` or `RUSTYCLUSTER`). |
...
> See [core_infrastructure](core_infrastructure.md) for the full Redis DB allocation map.
...
| [connectors_router](connectors_router.md) | API endpoints: `oauth_start`, `oauth_callback`, `connector_execute`, `connection_status`. |
| [core_infrastructure](core_infrastructure.md) | Redis DB allocation, KV store abstraction, logging, config. |
