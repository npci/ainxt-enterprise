...
flowchart TD
    ...
    E --> G{"Hard errors?<br/>(JWT_SECRET, PROXY_KEY_TOKEN)"}
...
| `redis_url(db=0)` | Returns a `redis://host:port/db` URL. Callers that need a client object should use `core.kv.get_kv(db)` instead, so the backend stays swappable. |
...
| `kv_backend_for(db)` | Resolves the backend for logical DB `db` and validates the configured value. Per-DB override via `REDIS_CLIENT_CONFIG_DB{db}`. |
...
The KV backend is selectable per logical DB. **Redis** is the backend shipped in this release; the per-DB indirection is retained because it is how callers address a logical database, and it means a second backend can be introduced by configuration rather than by editing every call site.

```mermaid
flowchart TD
    Call["kv_backend_for(db)"] --> P1{"REDIS_CLIENT_CONFIG_DB{db}<br/>set?"}
    P1 -->|"yes"| V1["Validate against<br/>supported backends"]
    P1 -->|"no"| P2{"REDIS_CLIENT_CONFIG<br/>global default set?"}
    P2 -->|"yes"| V2["Validate against<br/>supported backends"]
...
| 10x Award | `ENABLE_TENX_AWARD` | `true` | TenX engineer award module |
...
> See [Security](../core_infrastructure_security.md) for the compliance engine and prompt sanitiser details.
...
- The `"tenx"` parent logger also gets the file handler so all `tenx.*` child loggers appear in `agent.log`.
...
| [Resilience & Storage](../knowledge/core_infrastructure_resilience_storage.md) | `CircuitBreaker`, `DistributedSemaphore`, and `RateLimitConfig` all use `redis_client()` / `kv_backend_for()` from config and `logger` for state transitions. |
| [Security](../core_infrastructure_security.md) | Compliance scan flags (`COMPLIANCE_SCAN_*`, `HARDBLOCK_*`) are defined here; security modules log via `core.logger`. |
| [LLM Tooling](../core_infrastructure_llm_tooling.md) | LLM proxy URLs, model defaults (`DOC_INTENT_MODEL`, `GENERAL_CHAT_MODEL`), and API key env vars are resolved here. |
| [Jobs & Notifications](../core_infrastructure_jobs_notifications.md) | `get_job_connection()` in `core/kv/queue.py` uses `kv_backend_for(RDB_QUEUE)` from config. |
| [KV Store](../storage/kv_store.md) | Per-DB backend selection originates from `kv_backend_for()` and `KV_BACKEND_MAP`. |
...
| [Authentication](../security/authentication.md) | LDAP/AD configuration (`LDAP_*`, `SECURITY_AD_GROUP`, `APPROVER_AD_GROUP`) and `JWT_SECRET` validation are defined here. |
...
| [Gateway](../models/gateway.md) | The gateway is the primary consumer — it loads `.env`, runs CKMS, calls `validate_prod_config()`, and imports the logger at startup. |
...
3. **Per-DB KV backend selection** — Lets a logical DB be repointed by configuration alone, with no code change or redeployment.
...
10. **Feature toggles are removable** — Each major feature (Coach, TenX, Discussions, Skill Loop) is self-contained and can be fully removed by flipping a flag and dropping its tables.
