...
1. **Redis-backed business metrics** — query counts, retrieval scores, latency, and per-repo usage that survive process restarts.
...
| `core.kv.get_kv()` | Factory that returns a Redis client based on configuration. See [kv_store.md](../storage/kv_store.md). |
