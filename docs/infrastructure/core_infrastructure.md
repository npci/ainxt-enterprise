...
flowchart TB
    ...
    CoreInfrastructure --> Redis[(Redis)]
...
| [Resilience & Storage](../knowledge/core_infrastructure_resilience_storage.md) | `storage.py`, `circuit_breaker.py`, `rate_limiter.py`, `distributed_semaphore.py` | Object storage (MinIO/local), circuit breakers, sliding-window rate limiting, and distributed semaphores. |
| [Security & Compliance](../core_infrastructure_security.md) | `prompt_injection.py`, `prompt_sanitizer.py`, `security_validation.py`, `rag_acl.py` | Prompt-injection detection, XSS/input validation, content sanitization, and RAG chunk access control. |
| [LLM Tooling](../core_infrastructure_llm_tooling.md) | `model_registry.py`, `proxy_tool_use.py`, `claude_cache_egress.py`, `prompt_registry.py` | Model registry/tiers, multi-round tool-use proxy, Anthropic cache egress transport, and prompt versioning/A/B testing. |
| [Context & Evaluation](../core_infrastructure_context_eval.md) | `context_compressor.py`, `context_manager.py`, `evals.py`, `confidence_scorer.py` | Token-budget compression, engineer work-context management, LLM-as-judge evaluation, and block-confidence scoring. |
| [Jobs & Notifications](../core_infrastructure_jobs_notifications.md) | `job_queue.py`, `notifications.py`, `generation_registry.py` | RQ-backed job queues, Slack/email/WhatsApp notifications, and active-generation stop registry. |
| [Internationalization](../core_infrastructure_i18n.md) | `translation_wrapper.py`, `lang_detect.py`, `prose_translate.py` | Indic language detection, prose-vs-code segmentation, and translation microservice integration. |
| [Build & Dependencies](../core_infrastructure_build_deps.md) | `build_result_parser.py`, `npci_dependency_resolver.py` | Docker build output parsing and NPCI-internal Maven dependency resolution. |
| [Shared Utilities](../core_infrastructure_shared_utils.md) | `tenant.py`, `coach_events.py`, `discussions_engine_client.py`, `chroma_client.py` | Multi-tenancy, Coach event emission, Discussions engine client, and ChromaDB stub. |
...
The platform can run against Redis for its logical databases. `core/config.py` exposes `kv_backend_for(db)` and `KV_BACKEND_MAP`, and modules such as `circuit_breaker.py`, `rate_limiter.py`, and `job_queue.py` consume the backend-agnostic `core.kv` clients.
...
- `EVAL_ENABLED=false` — disable LLM-as-judge evaluations.
