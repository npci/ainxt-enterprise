...
|---|---|
| Postgres **or** Redis KV unreachable | `unhealthy` |
...
| Queue Health + Recent Jobs | `core/job_queue.py` + `routers/jobs_router.py` | [shared_api_routers](../api/shared_api_routers.md) |
| SDLC Runs by State/Type | `routers/sdlc_router.py::get_stats` | [sdlc_router](../api/sdlc_router.md) |
...
> See [shared_api_routers](../api/shared_api_routers.md) for the full queue back-pressure and DLQ architecture.
...
> See [sdlc_router](../api/sdlc_router.md) for the full SDLC pipeline state machine and HITL gate lifecycle.
