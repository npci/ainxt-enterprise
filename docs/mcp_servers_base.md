...
Both input and output pass through `_compliance_check()`, which delegates to `ComplianceEngine.validate_input()` from the [shared_core](shared_core.md) module's `agent_system` subsystem. The compliance engine uses a combination of regex detectors and an optional ML privacy service to detect PCI/PII patterns (PAN, CVV, Aadhaar, API keys, secrets, etc.).
...
|------------|--------|---------|
| `core.logger` | [shared_core](shared_core.md) → `core_infrastructure` | Structured logging for all server operations. |
| `agents.compliance_engine.ComplianceEngine` | [shared_core](shared_core.md) → `agent_system` → `decision_engines` | PCI/PII detection and blocking for tool I/O. |
| `db.database.SessionLocal` | [shared_core](shared_core.md) → `database` | Postgres session for audit log inserts. |
...
| `mcp_server_router` | [shared_api_routers](shared_api_routers.md) → `mcp_server_router` | Mounts each server's SSE and streamable-HTTP endpoints on FastAPI. Calls `sse_stream()` and `handle_streamable_http()`. |
| `McpSessionManager` | [abstudio_backend](abstudio_backend.md) → `core_mcp_manager` | Spawns MCP server subprocesses (stdio transport) for workflow execution. Uses the MCP client SDK to connect to `BaseMCPServer` instances. |
| `MCPRegistry` | [shared_core](shared_core.md) → `mcp_system` | Dual-registers the same tool functions from MCP servers into the platform's in-memory `ToolRegistry` for direct agent access. |
...
The `MCPRegistry` in [shared_core](shared_core.md) → `mcp_system` provides a parallel, in-memory registry that dual-registers the same tool functions for direct agent access (bypassing the JSON-RPC layer when the agent runtime is in-process). This means tools are accessible both via MCP protocol (for external clients) and via direct Python calls (for internal agent orchestration).
...
2. **Implement tool functions** in `tools/{name}_tools.py` (see [shared_integrations](shared_integrations.md)).
...
4. **Register with the router**: The `mcp_server_router` in [shared_api_routers](shared_api_routers.md) auto-mounts servers at `/mcp/{name}/sse` and `/mcp/{name}/message`.
5. **Dual-register with MCPRegistry**: Add the tool functions to `MCPRegistry._register_tools()` in [shared_core](shared_core.md) → `mcp_system` for in-process agent access.
...
| [shared_core](shared_core.md) | `MCPRegistry`, `MCPBridge`, `ToolRegistry`, `ComplianceEngine`, `core.logger` |
| [shared_api_routers](shared_api_routers.md) | `mcp_server_router` — FastAPI endpoint mounting |
| [shared_integrations](shared_integrations.md) | Tool function implementations (`tools/*_tools.py`) |
| [abstudio_backend](abstudio_backend.md) | `McpSessionManager` — subprocess MCP client for workflow execution |
