...
|------------|--------|---------|
| `mdComponents` | [message](message.md) | Markdown rendering components for assistant messages. |
| `authFetch`, `API_BASE` | [config](config.md) | Authenticated HTTP client and backend base URL. |
| `useFileDrop` | [hooks](hooks.md) | Drag-and-drop file handling. |
| `useToast` | [ui_dialog](ui_dialog.md) | Toast notifications. |
| `AiNxtSpinner` | [spinner](spinner.md) | Loading indicator during streaming. |
| `cacheStore` | [ai_ui_frontend_utils](ai_ui_frontend_utils.md) | Caches uploaded files for preview/download. |
...
| `POST /agents/{name}/run` | [gateway](gateway.md) / [agent_management](gateway.md#agent-management) | Executes the selected agent with streaming output. |
| `POST /chat/upload` | [chat_router](chat_router.md) | Uploads chat attachments with compliance scanning. |

## How It Fits into the System

`AgentsCatalog` sits at the boundary between end users and the agent runtime. It is one of the primary entry points for agent consumption in the `ai_ui_frontend` application, alongside modules such as [chat](chat.md) and [kb_chat](kb_chat.md).
...
- [message](message.md) — message rendering primitives.
- [config](config.md) — API base URL and authenticated fetch helper.
- [hooks](hooks.md) — drag-and-drop hook used by the chat input.
- [ui_dialog](ui_dialog.md) — toast provider.
- [spinner](spinner.md) — loading spinner component.
- [ai_ui_frontend_utils](ai_ui_frontend_utils.md) — preview cache utilities.
- [agents_router](agents_router.md) — backend router for catalog and favorites.
- [chat_router](chat_router.md) — backend router for chat file uploads.
- [gateway](gateway.md) — gateway service that handles `/agents/{name}/run`.
