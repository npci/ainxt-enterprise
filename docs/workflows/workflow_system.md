...
- **Memory & state**: Redis (`memory.redis_memory`, `core.kv`) provides fast transient state; Postgres (`db.models.WorkflowRunRecord`) provides durable run records (see [database](../storage/database.md), [memory_system](../storage/memory_system.md)).
