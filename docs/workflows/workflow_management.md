...
| [shared_core](../core/shared_core.md) → `workflow_system` | Provides `WorkflowEngine`, `Workflow`, `WorkflowStep` |
| [shared_core](../core/shared_core.md) → `database` | Provides `WorkflowRecord`, `WorkflowRunRecord` ORM models |
| [shared_core](../core/shared_core.md) → `mcp_system` | Provides `MCPRegistry` for tool resolution |
| [shared_core](../core/shared_core.md) → `authentication` | Provides `is_admin` for RBAC checks |
| abstudio_backend → `api_workflows` | Modern CRUD API for graph workflows |
| abstudio_backend → `api_execution` | Streaming execution + HITL resume endpoints |
| abstudio_backend → `core_workflow_repo` | Postgres persistence for graph workflows |
| abstudio_backend → `engine_native_engine` | DAG traversal engine with SSE streaming |
| abstudio_backend → `services_trigger_scheduler` | Scheduled workflow trigger dispatch |
| [workers](../workers/workers.md) → `infrastructure_maintenance_workers` | `workflow_scheduler_worker` dispatches due triggers |
| [shared_api_routers](../core/shared_api_routers.md) → `jobs_router` | Durable workflow job submission/resume |
| [shared_api_routers](../core/shared_api_routers.md) → `webhooks_router` | Webhook-triggered workflow execution |
