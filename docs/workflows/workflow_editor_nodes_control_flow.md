...
1. The backend execution engine (see [`engine_native_engine`](../reference/engine_native_engine.md)) or frontend runner updates the workflow store.
...
Together, these node types are registered in the React Flow `nodeTypes` map by [`Canvas`](workflow_editor.md) and persisted as part of the [`Workflow`](../models/app_models.md) model on the backend (see [`app_models`](../models/app_models.md) for `StartNode`, `EndNode`, `Workflow`, `Edge`, etc.).

Execution semantics are implemented by the backend [`engine_native_engine`](../reference/engine_native_engine.md) and triggered through [`api_execution`](../api/api_execution.md). The frontend store layer ([`workflowStore`](../workflowStore.md)) bridges backend events and the visual node states rendered by this module.
...
- [`app_models`](../models/app_models.md) — Backend/frontend shared models for `Workflow`, `StartNode`, `EndNode`, `Edge`.
- [`api_execution`](../api/api_execution.md) — Endpoints that run and resume workflows.
- [`engine_native_engine`](../reference/engine_native_engine.md) — Backend execution engine that drives the node statuses reflected here.
