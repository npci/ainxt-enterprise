...
- **It does not own workflow persistence.** Saving, loading, duplicating, and deleting workflows are handled by the backend via [`api_workflows`](../api/api_workflows.md) and the [`core_workflow_repo`](../reference/core_workflow_repo.md).
- **It does not define the data schema.** The backend [`app_models`](../models/app_models.md) module defines the `Edge` model and the overall `Workflow` schema that the frontend mirrors.
- **It does not execute workflows.** Execution is driven by [`api_execution`](../api/api_execution.md) and the [`engine_native_engine`](../reference/engine_native_engine.md); the edge component only reflects execution progress through the `activeNodeIds` animation.
...
- [`app_models`](../models/app_models.md) — Backend Pydantic models including `Edge` and `Workflow`.
...
- [`engine_native_engine`](../reference/engine_native_engine.md) — Backend execution engine that determines which nodes are active during a run.
- [`core_workflow_repo`](../reference/core_workflow_repo.md) — Backend repository for workflow persistence and template publishing.
