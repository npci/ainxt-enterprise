...
Clicking **Apply Workflow** sends the edited graph to `/workflow-factory/confirm`, then creates the workflow via `POST /workflows`. On success, the chat input is replaced with a trigger-scheduling panel ([`TriggerSection`](../ui/triggers_feature.md)) so the user can immediately schedule the new workflow.

### 6. Post-Save Trigger Scheduling

After persistence, the component surfaces the [`TriggerSection`](../ui/triggers_feature.md) inline, allowing the user to add schedule-based triggers (daily, weekly, etc.) and input messages before finishing and opening the workflow in the canvas editor.
...
The chat message stream, session ID, loading state, and stage are managed by the shared hook [`useFactoryChatStream`](../core/shared_features.md).
...
`WorkflowFactoryChat` reuses the same conversational scaffolding as the [Agent Factory](../agents/agents_feature.md) and [Skill Factory](../skills/skills_feature.md):

- [`FactoryChatShell`](../core/shared_features.md) — modal shell, message list, input, focus trap, and animation.
- [`useFactoryChatStream`](../core/shared_features.md) — SSE client, message/step state, plan-card protocol handling.
- [`FactoryFileChips`](../core/shared_features.md) / [`DownloadNotice`](../core/shared_features.md) — download UX for generated files referenced in replies.
...
- [`TriggerSection`](../ui/triggers_feature.md) is mounted after the workflow is persisted, using `targetKind="workflow"` and the new workflow's `id`.
- [`TriggerScopedCss`](../ui/triggers_feature.md) is injected to style the inline trigger panel consistently with the agent factory.
...
- **Trigger system** — Delegates schedule configuration to [`triggers_feature.md`](../ui/triggers_feature.md).
...
- [shared_features](../core/shared_features.md) — `FactoryChatShell`, `useFactoryChatStream`, `FactoryFileChips`, `DownloadNotice`.
- [common_components](../ui/common_components.md) — `PlanCard`, `InlinePicker`.
- [triggers_feature](../ui/triggers_feature.md) — `TriggerSection`, `TriggerScopedCss`.
...
- [skills_feature](../skills/skills_feature.md) — Parallel factory experience for skills.
