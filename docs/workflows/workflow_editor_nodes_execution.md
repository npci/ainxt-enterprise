...
The backend [`engine_native_engine`](../reference/engine_native_engine.md) is the actual executor. It receives the exported graph from `workflowStore.getWorkflowForExecution()` and dispatches:
...
- [`engine_native_engine`](../reference/engine_native_engine.md) — Backend orchestration engine that executes `agent` and `subflow` nodes.
