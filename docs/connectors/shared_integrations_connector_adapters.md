...
* [`shared_core_tools`](../reference/shared_core_tools.md) — `tools/gitlab_tools.py` and `tools/jira_tools.py` are reused by `GitLabAdapter` and `JiraAdapter` to keep the SDLC and connector code paths identical.
...
* [`shared_core_tools`](../reference/shared_core_tools.md) — canonical tool implementations reused by some adapters.
* [`shared_api_routers`](../api/shared_api_routers.md) — HTTP routers that expose connector actions to clients (e.g., `connectors_router.py`).
