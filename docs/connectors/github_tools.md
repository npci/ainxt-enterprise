...
1. **Circuit Breaker**: All `_get` and `_post` calls are wrapped via `get_breaker("github")` from core.circuit_breaker. When the circuit is open, calls fail fast and return a structured error.
...
- **Agent Builder**: The [agents.agent_builder](../agents/agent_system.md) may register GitHub tools so that agents can perform repository operations autonomously.
- **MCP Tool Registry**: The [mcp.tool_registry](../mcp/mcp_system.md) can expose these functions as Model Context Protocol tools.
- **Workflow Engine**: The [workflows.engine](../workflows/workflow_system.md) can invoke GitHub tools as steps in automated workflows.
- **SDLC Pipeline**: The [shared_core_sdlc_pipeline](../sdlc/shared_core_sdlc_pipeline.md) may use GitHub tools to create PRs, post reviews, and merge code during governance pipelines.
...
- core_circuit_breaker — Resilience wrapper used by HTTP helpers.
- core_logger — Logging infrastructure.
...
- [agent_system](../agents/agent_system.md) — Agent framework that may register these tools.
- [mcp_system](../mcp/mcp_system.md) — MCP registry that may expose these tools.
- [workflow_system](../workflows/workflow_system.md) — Workflow engine that may invoke these tools.
- [shared_core_sdlc_pipeline](../sdlc/shared_core_sdlc_pipeline.md) — SDLC automation that may use these tools.
