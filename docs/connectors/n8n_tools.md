...
| `models.model_router` | `shared_core` / `models/model_router.py` | Routes the workflow-generation prompt to the appropriate LLM (`model_hint="complex"`). See [shared_core.md](../core/shared_core.md) for details on model routing. |
...
- **API routers** such as `n8n_router` (in `shared_api_routers`), which provides HTTP endpoints for listing, creating, triggering, and managing n8n workflows. See [shared_api_routers.md](../core/shared_api_routers.md) and specifically the `n8n_router` sub-section.
