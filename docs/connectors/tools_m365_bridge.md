...
3. **Running the shim in the sandbox subprocess** via [ToolDispatcher](../agents/agent_factory_pipeline.md#tooldispatcher).
...
- Reads environment variables injected by [ToolDispatcher](../agents/agent_factory_pipeline.md#tooldispatcher):
...
| Sandbox isolation | Tool code runs under `python -I` with no `PYTHONPATH` and a 1 MB stdout cap. See [ToolDispatcher](../agents/agent_factory_pipeline.md#tooldispatcher) for details. |
...
[ToolDispatcher](../agents/agent_factory_pipeline.md#tooldispatcher) retries transient errors (timeouts, 5xx, connection issues) up to `TOOL_MAX_ATTEMPTS` with exponential backoff, but does not retry deterministic failures such as `401`, `403`, `404`, or `422`.
...
- Once seeded, [NativeEngine](../agents/engine_native_engine.md) wraps each row as a `_CatalogTool` and dispatches it through [ToolDispatcher](../agents/agent_factory_pipeline.md#tooldispatcher).
