...
|---|---|---|
| `connectors_router` | [connectors_router](../api/connectors_router.md) | `oauth_start` → `generate_authorize_url` + `save_state`; `oauth_callback` → `load_state` + `exchange_code`. |
...
> See [core_infrastructure](../infrastructure/core_infrastructure.md) for the full Redis DB allocation map.
...
| [connectors_router](../api/connectors_router.md) | API endpoints: `oauth_start`, `oauth_callback`, `connector_execute`, `connection_status`. |
| [core_infrastructure](../infrastructure/core_infrastructure.md) | Redis DB allocation, KV store abstraction, logging, config. |
