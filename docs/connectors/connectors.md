...
| Module | Relationship |
|--------|-------------|
| **[shared_integrations.md](../reference/shared_integrations.md)** | Contains the connector infrastructure (`ConnectorEngine`, `ConnectorRegistry`, `OAuth2Handler`, `ConnectorMetrics`) and all adapter implementations (`microsoft365`, `gitlab`, `jira`, `slack`, `gmail`, etc.) |
| **[connectors_router.md](../api/connectors_router.md)** | The API router (`routers/connectors_router.py`) exposing all connector REST endpoints |
...
| **[profile.md](../reference/profile.md)** | The Profile component with the API Token Vault — source of PATs for GitLab/Jira connectors |
| **[config.md](../infrastructure/config.md)** | `authFetch` / `apiFetch` HTTP utilities used by all frontend connector calls |
...
| **[shared_core.md](../reference/shared_core.md)** | `ComplianceEngine` used for PCI/PII scanning of connector responses; `CredentialVault` for token encryption |
| **[database.md](../storage/database.md)** | PostgreSQL schema for `connector_definitions`, `user_oauth_tokens`, `user_connector_permissions` |
