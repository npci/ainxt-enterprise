...
- For the normalized response object returned by connector calls, see [shared_integrations_connector_infrastructure_base](../shared_integrations_connector_infrastructure_base.md) (or the `connectors/base.py` component in the connector infrastructure module).
...
- For the MCP tool layer where connector tools are registered, see [shared_core_mcp_system](../shared_core_mcp_system.md) (specifically `mcp/tool_registry.py`).
