...
The webhook also triggers SDLC pipelines (bug vs. feature) based on issue type, and the BRD→FSD pipeline for epics with a "BRD" label. See [shared_api_routers](../core/shared_api_routers.md) for the full webhook router documentation.

### 4. Jira Indexer

The `index_jira_project()` function in `indexers/jira_indexer.py` fetches Jira issues and indexes them into pgvector for semantic search. It uses `core.platform_credentials.get_atlassian_creds()` directly (not the thread-local path) and has its own HTTP client for pagination. See [indexers](../knowledge/indexers.md) for details.

### 5. SDLC Pipeline

The SDLC pipeline uses `jira_get_issue_full()` during the **TICKET_NORMALIZATION** stage to fetch maximum raw material (comments, attachments, acceptance criteria, labels, epic summary) for the `NormalizationAgent`. See [shared_core_sdlc_pipeline](../sdlc/shared_core_sdlc_pipeline.md) for the SDLC pipeline documentation.
...
- **[shared_integrations_connector_adapters](../skills/shared_integrations_connector_adapters.md)** — `JiraAdapter` and the connector infrastructure
- **[mcp_system](../mcp/mcp_system.md)** — `JiraMCPServer` and MCP tool registration
- **[shared_api_routers](../core/shared_api_routers.md)** — Jira webhook receiver and SDLC trigger routing
- **[indexers](../knowledge/indexers.md)** — Jira project indexing into pgvector
- **[shared_core_sdlc_pipeline](../sdlc/shared_core_sdlc_pipeline.md)** — SDLC pipeline consumption of `jira_get_issue_full()`
- **[core_infrastructure](../core/core_infrastructure.md)** — Circuit breaker, retry, logger, and credential infrastructure
- **[llm_proxy_main](../llm/llm_proxy_main.md)** — The `/atlassian/proxy` relay endpoint on the LLM proxy
