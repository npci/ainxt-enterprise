...
The webhook also triggers SDLC pipelines (bug vs. feature) based on issue type, and the BRD→FSD pipeline for epics with a "BRD" label. See [shared_api_routers](../api/shared_api_routers.md) for the full webhook router documentation.
...
- **[shared_api_routers](../api/shared_api_routers.md)** — Jira webhook receiver and SDLC trigger routing
...
- **[core_infrastructure](../infrastructure/core_infrastructure.md)** — Circuit breaker, retry, logger, and credential infrastructure
- **[llm_proxy_main](../models/llm_proxy_main.md)** — The `/atlassian/proxy` relay endpoint on the LLM proxy
