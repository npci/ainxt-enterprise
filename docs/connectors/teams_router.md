...
This router is a thin FastAPI layer. The heavy lifting—token validation, activity dispatch, command routing, compliance/budget checks, and Adaptive Card rendering—is delegated to the teams_adapter and teams_notifier services, while meeting automation is handled by the meeting_worker.
...
| ---------- | ------------------- | -------------------- |
| `services.teams_adapter` | Token validation, activity dispatch, metrics, conversation mapping. | teams_adapter |
| `services.teams_notifier` | Proactive notification helpers for SDLC lifecycle events. | teams_notifier |
| `integrations.teams_client` | Low-level Bot Framework message and Adaptive Card delivery. | teams_client |
| `auth.jwt_handler` | Decodes platform JWT for the meeting processing endpoint. | auth/jwt_handler |
| `core.job_queue` | Enqueues meeting automation jobs on `Q_CONNECTOR`. | core/job_queue |
| `workers.meeting_worker` | Background worker that fetches transcripts, generates MoM, and distributes email. | meeting_worker |
| `core.logger` | Structured logging. | core/logger |
...
The Teams router is one of several channel-facing entry points in the shared API layer. It is mounted alongside other routers such as [slack_router](slack_router.md), [webhooks_router](webhooks_router.md), and [sdlc_router](../sdlc/sdlc_router.md). While the Slack router handles Slack events and the webhooks router handles GitLab/Jira callbacks, the Teams router is the exclusive endpoint for Microsoft Teams interactions.

The [gateway](../core/gateway.md) may optionally mount the official Teams SDK endpoint via `_mount_teams_sdk`, but the `teams_router` remains the primary integration surface for custom command handling, proactive notifications, and meeting automation.

---

## Related Modules

- teams_adapter — token validation, dispatch, command routing, metrics.
- teams_notifier — proactive notification helpers.
- meeting_worker — post-meeting transcript summarization and distribution.
- [gateway](../core/gateway.md) — top-level gateway that mounts platform routers and the Teams SDK.
...
- [sdlc_router](../sdlc/sdlc_router.md) — SDLC pipeline control that emits HITL approval notifications.
