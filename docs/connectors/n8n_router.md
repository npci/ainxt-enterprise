...
- **`tools.n8n_client`** is the dedicated HTTP client for the n8n REST API. It manages connection pooling, authentication headers, and error normalization. See [shared_integrations.md](../skills/shared_integrations.md) for details.
- **`tools.n8n_autonomous_builder`** generates valid n8n workflow JSON from natural language using the shared model router, validates the structure, and then uses `tools.n8n_client` to create and activate the workflow. See [shared_integrations.md](../skills/shared_integrations.md).
- **`tools.n8n_tool`** is a separate orchestrator-compatible tool that triggers existing workflows; it is not used by the router but shares the same client. See [shared_integrations.md](../skills/shared_integrations.md).
- **`auth.rbac.require_role`** enforces the `operator` minimum role on every route. See [shared_core.md](../core/shared_core.md) for the authentication and authorization subsystem.
- **`models.model_router`** provides LLM generation for the autonomous builder, routing the prompt to an approved model (e.g., Claude Sonnet). See [shared_core.md](../core/shared_core.md) under model routing.
- **`core.logger`** provides structured logging across the router and its dependencies. See [shared_core.md](../core/shared_core.md).
...
The autonomous builder additionally depends on the shared model router configuration (e.g., `LLM_PROXY_URL`, model registry, and circuit breaker settings). See [shared_core.md](../core/shared_core.md) for model routing configuration.
...
The `n8n_router` is part of the `shared_api_routers` layer. It is typically mounted by the platform gateway (see [gateway.md](../core/gateway.md)) alongside other domain routers such as `workflows_router`, `agents_router`, and `triggers_router`.

While the platform's internal workflow engine (see abstudio_backend.md) manages native agentic workflows, the n8n router provides a bridge to the external n8n automation ecosystem. This enables operators to:
...
- [shared_api_routers.md](../core/shared_api_routers.md) — Overview of all shared API routers.
- [gateway.md](../core/gateway.md) — How routers are mounted and served.
- [shared_integrations.md](../skills/shared_integrations.md) — Details on `tools.n8n_client`, `tools.n8n_autonomous_builder`, and `tools.n8n_tool`.
- [shared_core.md](../core/shared_core.md) — RBAC (`auth.rbac`), logging (`core.logger`), and model routing (`models.model_router`).
- abstudio_backend.md — Native workflow and agent management (complementary to n8n integration).
