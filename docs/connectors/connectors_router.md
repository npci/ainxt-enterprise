...
|---|---|---|
| **End-user JWT** | Most endpoints | `Depends(get_current_user)` — validates Bearer JWT or `auth_token` cookie via [auth_router](../auth/auth_router.md) |
...
| **Admin role** | Definition CRUD, metrics | `Depends(require_role("admin"))` via [auth_router](../auth/auth_router.md) RBAC |
...
|---|---|---|
| `ConnectorRegistry` | [shared_integrations](../skills/shared_integrations.md) | Singleton registry for connector definitions, user status, and execution delegation |
| `ConnectorEngine` | [shared_integrations](../skills/shared_integrations.md) | Execution engine with schema validation, caching, rate limiting, compliance, pagination |
| `OAuth2Handler` | [shared_integrations](../skills/shared_integrations.md) | OAuth2 lifecycle: PKCE, authorize URL, code exchange, token refresh, state management |
| `ConsentHandler` | [shared_integrations](../skills/shared_integrations.md) | DPI consent artifact creation, signing (sandbox), and verification |
| `select_probe` | [shared_integrations](../skills/shared_integrations.md) | Capability-based probe selection for connection testing |
| `ConnectorMetrics` | [shared_integrations](../skills/shared_integrations.md) | Redis-backed observability: call stats, top queries, audit log |
| `get_current_user` | [auth_router](../auth/auth_router.md) | JWT/API-key/cookie authentication |
| `require_role` | [auth_router](../auth/auth_router.md) | RBAC role enforcement |
| `encrypt_value` | [shared_core](../core/shared_core.md) | Fernet encryption for stored credentials |
| `compliance_engine` | [shared_core](../core/shared_core.md) | PCI/PII compliance scanning for outgoing messages |
| `get_decrypted_token` | [profile_router](../auth/profile_router.md) | Profile vault token retrieval for PAT connectors |
| `_resolve_doc_attachments` | [shared_integrations](../skills/shared_integrations.md) | Document attachment resolution for M365 write tools |
...
The `ConnectorRegistry` registers connector tools into the MCP `ToolRegistry` with naming convention `{connector}__{tool}` (e.g., `microsoft_365__outlook_search_emails`). This makes connector tools available to LLM tool-use calling. See [mcp_system](../core/shared_core.md) for details on the MCP infrastructure.
