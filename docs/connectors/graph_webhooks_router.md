...
This router is part of the [`shared_api_routers`](../core/shared_api_routers.md) family and is typically mounted by the main gateway / ABStudio backend application.
...
| Subscription secret verification | Constant-time `clientState` comparison against stored subscription | `graph_subscriptions` |
| Resource-path parsing | Extract `organizer_id` and `meeting_id` from Graph resource URLs | `_parse_resource` helper |
| Async work dispatch | Enqueue `workers.meeting_worker.run_post_meeting_job` on `Q_CONNECTOR` | `job_queue` |
...
|---|---|---|
| `services.graph_subscriptions.find_by_subscription_id` | Load the stored subscription record for `clientState` verification | `graph_subscriptions` |
| `core.job_queue.enqueue_job` | Schedule the async worker on `Q_CONNECTOR` | `job_queue` |
| `workers.meeting_worker.run_post_meeting_job` | The worker function being enqueued | `meeting_worker` |
| `core.logger.logger` | Structured logging for security/ops events | `core_logger` |
...
Those are used later by `meeting_worker`.
...
The router itself does not deduplicate notifications. The downstream `meeting_worker` uses a `_claim(meeting_id, organizer_id, ...)` step to ensure the same meeting is not processed twice, even if Graph delivers duplicate notifications.

### Fallback Path

If inbound HTTPS webhooks are blocked (for example, by NPCI network policies), the platform falls back to polling via `meeting_worker`. In that mode, the worker periodically calls `poll_recent_meetings` instead of waiting for a push notification.
...
- **Timeout / Retry**: `enqueue_job` applies its default retry policy (configured in `job_queue`). The router does not override it.
...
- [`shared_api_routers`](../core/shared_api_routers.md) — the router collection this module belongs to
- `graph_subscriptions` — subscription lifecycle and `clientState` storage
- `job_queue` — RQ-based job enqueueing
- `meeting_worker` — the worker that processes enqueued meeting jobs
- `compliance_engine` — PII / sensitive-data redaction used by the worker
- `model_router` — model-agnostic generation used to produce the MoM
