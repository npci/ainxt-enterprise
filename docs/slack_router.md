...
- **`core.slack_bot`** — HMAC-SHA256 signature verification and outbound Slack
  messaging (see [core_infrastructure.md](core_infrastructure.md)).
...
> see [connectors_integrations.md](connectors_integrations.md). This router only
...
> serialise concurrent state transitions. See [store_layer.md](store_layer.md)
...
> concurrent API call (e.g. from [sdlc_router.md](sdlc_router.md)) race to
...
| Module | Relationship |
|---|---|
| [sdlc_router.md](sdlc_router.md) | HTTP API equivalents for HITL approve/reject/resume (`approve_run`, `reject_run`, `resume_run`). |
| [shared_core_sdlc_pipeline.md](shared_core_sdlc_pipeline.md) | SDLC pipeline resume functions and state machine invoked after Slack HITL approvals. |
| [store_layer.md](store_layer.md) | `sdlc_store` persistence layer for run state, events, and audit trail. |
| [core_infrastructure.md](core_infrastructure.md) | `core.slack_bot`, `core.job_queue`, and `core.logger` utilities. |
| [connectors_integrations.md](connectors_integrations.md) | `SlackAdapter` for outbound Slack connector actions (separate from this inbound webhook router). |
| [chat_agent_execution_workers.md](chat_agent_execution_workers.md) | `workers.agent_worker.run_agent_job` — the worker that consumes agent jobs enqueued from `app_mention`. |
