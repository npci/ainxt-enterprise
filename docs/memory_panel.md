...
The component is intentionally simple: it does not create or edit memories directly. Memory creation is driven by the chat pipeline (see [chat](./chat.md) / [kb_chat](./kb_chat.md)) via the backend summarizer.
...
The component uses `authFetch` from [`config.js`](./config.md) (the same authenticated fetch helper used across `ai-ui`).
...
|------------|--------|------|
| `authFetch`, `API_BASE` | [`config.js`](./config.md) | Authenticated HTTP client and backend base URL. |
...
| Dependency | Module | Role |
|------------|--------|------|
| `memory_router.py` | [`shared_api_routers`](./shared_api_routers.md) | Exposes `/memory/user` endpoints. |
...
- [chat.md](./chat.md) — Main chat UI that consumes and produces memories.
- [kb_chat.md](./kb_chat.md) — Knowledge-base chat that also interacts with user memory.
- [config.md](./config.md) — `authFetch` and API configuration used by this component.
- [shared_api_routers.md](./shared_api_routers.md) — Backend router layer including `memory_router.py`.
