...
[workflow store](store.md) for live execution state (active node highlighting, loop
progress) and expose typed **source handles** that the backend
[NativeEngine](engine_native_engine.md) reads during graph traversal to decide which
edge to follow.
...
2. **Data shape contract** — each node's `data` object must match what the
   ...
   [NativeEngine](engine_native_engine.md) reads.
...
([`app/models.py::ConditionCase`](app_models.md)):
...
[`buildCombinedExpressionPreview`](constants.md) from `constants/operators.js` to
...
[workflow store](store.md) to apply the `executing` CSS class. It also reads
...
[`app.loop.runner`](loop_runner.md) — the same helper used by the
...
[loop_runner](loop_runner.md) for the evaluator's rubric and scoring contract.
...
[workflow store](store.md). The backend emits `loop_iteration_start` and
...
|-----------|------|----------------|
| `workflowStore` | State management | [store](store.md) |
| `constants/operators.js` | Expression preview helpers | [constants](constants.md) |
...
| `NativeEngine` | Backend graph traversal | [engine_native_engine](engine_native_engine.md) |
| `app/models.py` | Backend data models | [app_models](app_models.md) |
| `loop_evaluator.py` | LLM judge + controller | [engine_loop_evaluator](engine_loop_evaluator.md) |
| `loop/runner.py` | `evaluate_llm_judge` helper | [loop_runner](loop_runner.md) |
...
The [workflow store](store.md)'s `getDefaultNodeData(type)` function provides
...
See [store](store.md) for the complete `getDefaultNodeData` implementation.
...
| Workflow store (Zustand) | [store](store.md) |
| Backend execution engine | [engine_native_engine](engine_native_engine.md) |
| Backend data models | [app_models](app_models.md) |
| Loop evaluator (LLM judge) | [engine_loop_evaluator](engine_loop_evaluator.md) |
| Loop runner system | [loop_runner](loop_runner.md) |
| Loop models | [loop_models](loop_models.md) |
| Constants (operators) | [constants](constants.md) |
