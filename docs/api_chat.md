...
All endpoints are authenticated via [api_deps](api_deps.md)`::require_access` and call `get_engine()` to obtain the active [engine_native_engine](engine_native_engine.md)`::NativeEngine` instance.
...
| `app.engine.get_engine` | Returns the singleton `NativeEngine` that implements all storage-backed operations. | [engine_native_engine](engine_native_engine.md) |
| `app.models.AuthenticatedUser` | Pydantic model carrying user identity, department, and hierarchy flags used for ACL and audit. | [app_models](app_models.md) |
...
See [checkpoint](checkpoint.md) for the store interface and persistence semantics.
