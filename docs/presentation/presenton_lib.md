...
- **Upstream consumers**: `PPTWizard.jsx`, `PPTChatMessageRenderer.jsx`, and any chat flow that produces a presentation. See `ai_ui_frontend`.
- **Downstream backend**: [`presenton_router`](presenton_router.md) exposes the REST/SSE surface; [`llm_proxy`](../llm/llm_proxy.md) and [`workers/presenton_worker`](../workers/workers.md) perform the actual generation.
...
- `ai_ui_frontend` – the React application that consumes this library.
- [`presenton_router`](presenton_router.md) – backend router that exposes the Presenton REST/SSE API.
- [`llm_proxy`](../llm/llm_proxy.md) – LLM proxy used during slide content generation.
