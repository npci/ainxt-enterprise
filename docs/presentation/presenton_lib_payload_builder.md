...
- **[presenton_lib_layout_registry](presenton_lib_layout_registry.md)** – Stores the canonical slide layout definitions, including `json_schema`, `templateID`, and `templateName`.
- **[presenton_lib_layout_mapping](presenton_lib_layout_mapping.md)** – Provides helpers such as `getPresentonLayoutKey`, `getPresentonLayoutId`, `createPresentonSlide`, and `getLayoutMap` to resolve the right layout for a given theme and slide count.
- **[presenton_lib_api_client](presenton_lib_api_client.md)** – Handles HTTP/SSE communication with Presenton endpoints (`fetchTemplateLayout`, `streamOutlines`, `pollPresentationStatus`).
- **[presenton_lib_stream_reader](presenton_lib_stream_reader.md)** – Parses the server-sent event stream returned by Presenton.

Upstream UI consumers include:

- **[ppt_wizard](ppt_wizard.md)** – The step-by-step presentation creation wizard that collects topic, theme, and outline preferences.
- **[ppt_chat](ppt_chat.md)** – Chat-based presentation generation components that render progress and completion messages.

On the backend, the payloads produced by this module are consumed by:

- **[presenton_router](presenton_router.md)** – FastAPI router exposing `generate_outline`, `generate_presentation`, `download_presentation`, `presentation_status`, `list_themes`, etc.
- **presenton_worker** – Background worker (`generate_ppt_job`) that performs the actual presentation generation job.
...
- [presenton_lib_layout_registry](presenton_lib_layout_registry.md) – Canonical layout definitions and JSON schemas.
- [presenton_lib_layout_mapping](presenton_lib_layout_mapping.md) – Layout key resolution and slide creation helpers.
- [presenton_lib_api_client](presenton_lib_api_client.md) – HTTP/SSE client for Presenton endpoints.
- [presenton_lib_stream_reader](presenton_lib_stream_reader.md) – Server-sent event stream parser.
- [ppt_wizard](ppt_wizard.md) – UI wizard that drives presentation creation.
- [ppt_chat](ppt_chat.md) – Chat-based presentation generation UI.
- [presenton_router](presenton_router.md) – Backend API router for Presenton.
- presenton_worker – Background worker that executes presentation generation jobs.
