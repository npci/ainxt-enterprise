...
The `payload` is what gets passed as the `p` prop to stage renderers. See [sdlc_router](sdlc_router.md) for the full API contract.
...
Returns the `VERIFIED_DIFF` artifact (the real, compiled, test-green diff the human approves at the HITL gate). Used by `DiffApprovalPanel` when rendered inside `StageArtifactDrawer` for the `IMPLEMENT` stage. See [sdlc_router](sdlc_router.md) for details.
...
|-------------------|---------------|----------|
| `PlanningArtifactView` | [sdlc_planning_artifact](sdlc_planning_artifact.md) | PLAN / legacy ANALYZING+DESIGNING stages (self-fetches artifact, falls back to `run.context`) |
| `ManifestValidationPanel` | [manifest](manifest.md) | MANIFEST_VALIDATION stage (structural + OpenAI cross-check results) |
...
| `DiffApprovalPanel` | [diff_approval](diff_approval.md) | IMPLEMENT stage (verified diff with compile/test status, file-level comments) |
| `GovernanceReviewPanel` | [sdlc_governance_review](sdlc_governance_review.md) | GOVERNANCE_REVIEW / AWAITING_GOVERNANCE_APPROVAL stages |
| `PipelineStepper` | [sdlc_pipeline_stepper](sdlc_pipeline_stepper.md) | Backend-manifest-driven stage timeline (triggers drawer via node click) |
| `statusStyle` / `needsAttention` | [sdlc_status_model](sdlc_status_model.md) | State badge colors/labels (single source replacing old STATE_STYLE map) |
| `GateSignalRow` | [sdlc_gate_signal](sdlc_gate_signal.md) | Trust-calibrated signal row at design/solution gates |
...
- [sdlc_pipeline](sdlc_pipeline.md) — Parent module overview (pipeline core, approval actions, governance actions, trigger modal, layout helpers)
- [sdlc_planning_artifact](sdlc_planning_artifact.md) — PlanningArtifactView component (PLAN stage rendering with coverage strip, files, sub-tasks, open questions, reasoning)
- [sdlc_governance_review](sdlc_governance_review.md) — GovernanceReviewPanel component (domain approval gate, finding triage)
- [sdlc_pipeline_stepper](sdlc_pipeline_stepper.md) — PipelineStepper component (backend-manifest-driven stage timeline)
- [sdlc_status_model](sdlc_status_model.md) — statusModel.js (state labels, colors, attention flags)
- [sdlc_gate_signal](sdlc_gate_signal.md) — GateSignalRow component (trust-calibrated review signals)
- [diff_approval](diff_approval.md) — DiffApprovalPanel component (verified diff with file-level comments)
- [manifest](manifest.md) — ManifestValidationPanel component (structural + OpenAI cross-check)
- [sdlc_router](sdlc_router.md) — Backend API router for SDLC endpoints (artifact, verified-diff, governance)
