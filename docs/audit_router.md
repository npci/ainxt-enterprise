...
- **Gateway**: The [gateway](gateway.md) writes one `request_audit_log` row per `/ask`, `/ask_with_image`, `/ide/chat`, and similar request paths. The router only reads these rows.
...
- [gateway](gateway.md) — the service that populates `request_audit_log`.
