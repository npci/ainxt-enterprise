...
| `workers/sdlc_worker.py` | SDLC pipeline worker jobs (bug, feature, PR review, merge, comments) | [sdlc_pipeline_workers](sdlc_pipeline_workers.md) |
| `workers/index_worker.py` | Codebase indexing worker | [document_knowledge_workers](document_knowledge_workers.md) |
| `workers/durable_workflow_worker.py` | Durable workflow execution with checkpoint/resume | [chat_agent_execution_workers](chat_agent_execution_workers.md) |
| `agents/sdlc_pipeline.py` | Core SDLC pipeline orchestration (PR review, bug, feature) | [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) |
| `agents/brd_fsd_pipeline.py` | BRD→FSD document generation pipeline | [sdlc_pipeline_agents](sdlc_pipeline_agents.md) |
| `db/models.py` | `ScheduledWorkflow` ORM model for event-driven workflows | [database](database.md) |
...
- **[sdlc_pipeline_workers](sdlc_pipeline_workers.md)**: The primary consumer of jobs enqueued by this router. All SDLC pipeline jobs (bug, feature, PR review, merge, address comments) are executed by `workers/sdlc_worker.py`.
- **[shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md)**: Contains the core pipeline orchestration logic (`agents/sdlc_pipeline.py`) that workers invoke. The inline fallback thread in `gitlab_webhook` calls `run_pr_review_pipeline` directly from this module.
- **[document_knowledge_workers](document_knowledge_workers.md)**: The `index_repo_job` worker that re-indexes repositories after push/merge events.
- **[chat_agent_execution_workers](chat_agent_execution_workers.md)**: The `execute_durable_workflow` worker that runs event-triggered workflows.
- **[sdlc_pipeline_agents](sdlc_pipeline_agents.md)**: The BRD→FSD pipeline agent invoked when Jira issues carry the `BRD` label.
- **[database](database.md)**: Provides the `ScheduledWorkflow` model queried by `trigger_workflow_by_event` and the `SDLCRun` model managed by `sdlc_store`.
