...
4. Otherwise merges:
   - The platform's cloud model list (from [`core.model_registry`](core_model_registry.md)) so GPT/Claude/Gemini remain visible to clients.
...
See [`auth/api_key_auth.md`](auth_api_key_auth.md) for the shared validation pattern.
...
Compliance is non-bypassable and mirrors the behavior of [`gateway_openai.md`](gateway_openai.md):
...
See [`agents/compliance_engine.md`](agents_compliance_engine.md) for detector details.
...
| [`db/models.md`](db_models.md) | `ManagedEndpoint`, `UserAPIKey`, `User`, `Chat`, `ChatMessage`. |
| [`core/kv.md`](core_kv.md) | Redis access for slug and model list caching. |
| [`core/model_registry.md`](core_model_registry.md) | Platform cloud model list for `/v1/models`. |
| [`agents/compliance_engine.md`](agents_compliance_engine.md) | PCI/PII scanning and redaction. |
| [`auth/api_key_auth.md`](auth_api_key_auth.md) | Shared SHA-256 key validation pattern. |
