...
graph TB
    ...
    subgraph "External Dependencies"
        ...
        kv_store["kv_store<br/>Redis/RustyCluster KV cache"]
...
graph LR
    ...
    subgraph "Infrastructure"
        ...
        kv["core/kv<br/>(get_kv, Redis/RustyCluster)"]
...
|------------|------|---------|
| [`model_routing`](model_routing.md) | **Hard** | `ModelRouter.generate()` — the single LLM call for classification. Cascades local→cloud on outage. |
...
| [`core_config`](core_config.md) | **Hard** | `CIL_INTENT_MODEL` (model tier), `RDB_CACHE` (cache KV namespace). |
| [`kv_store`](kv_store.md) | **Hard** | `get_kv()` for cache read/write via Redis or RustyCluster. |
| [`core_infrastructure`](core_infrastructure.md) | **Hard** | `logger` for structured logging. |
...
| Cache lookup (cache HIT) | <5ms | Redis/RustyCluster GET + JSON deserialize |
