...
`WorkflowFactoryChat` opens as a modal overlay from the [Workflows Dashboard](workflows_feature_dashboard.md). Users type a high-level description such as *"Every morning, summarize support tickets from Jira and email the report to the team."* The component streams the backend's progress via Server-Sent Events (SSE) and renders:
...
When the backend reaches the `confirm` stage, the generated workflow graph is shown in a pipeline summary. Users can expand a per-agent editor overlay to add, remove, or generate tools and skills for each agent node using [`InlinePicker`](common_components.md).

### 5. Apply & Persist

Clicking **Apply Workflow** sends the edited graph to `/workflow-factory/confirm`, then creates the workflow via `POST /workflows`. On success, the chat input is replaced with a trigger-scheduling panel ([`TriggerSection`](triggers_feature.md)) so the user can immediately schedule the new workflow.

### 6. Post-Save Trigger Scheduling

After persistence, the component surfaces the [`TriggerSection`](triggers_feature.md) inline, allowing the user to add schedule-based triggers (daily, weekly, etc.) and input messages before finishing and opening the workflow in the canvas editor.
...
The chat message stream, session ID, loading state, and stage are managed by the shared hook [`useFactoryChatStream`](shared_features.md).
...
`WorkflowFactoryChat` reuses the same conversational scaffolding as the [Agent Factory](agents_feature.md) and [Skill Factory](skills_feature.md):

- [`FactoryChatShell`](shared_features.md) — modal shell, message list, input, focus trap, and animation.
- [`useFactoryChatStream`](shared_features.md) — SSE client, message/step state, plan-card protocol handling.
- [`FactoryFileChips`](shared_features.md) / [`DownloadNotice`](shared_features.md) — download UX for generated files referenced in replies.

### Common UI Components

- [`PlanCard`](common_components.md) — renders structured questionnaires and collects answers.
- [`InlinePicker`](common_components.md) — searchable dropdown for attaching catalog tools/skills; supports on-demand generation of new entries.

### Triggers Integration

- [`TriggerSection`](triggers_feature.md) is mounted after the workflow is persisted, using `targetKind="workflow"` and the new workflow's `id`.
- [`TriggerScopedCss`](triggers_feature.md) is injected to style the inline trigger panel consistently with the agent factory.

### Backend Pipeline

The backend counterpart lives in [`api_factories.md`](api_factories.md) (`workflow_factory_chat` and `workflow_factory_confirm`) and [`workflow_factory_pipeline.md`](workflow_factory_pipeline.md). The pipeline handles:
...
`WorkflowFactoryChat` is part of the **ABStudio frontend workflow builder** ([`workflows_feature`](workflows_feature.md)). It connects:

- **Dashboard entry** — Launched from [`WorkflowsDashboard`](workflows_feature_dashboard.md) when the user clicks **Create with AI**.
- **Editor exit** — On finish, the persisted workflow is passed to `App.jsx`'s `handleOpenWorkflow`, which seeds the [workflow editor](workflow_editor.md) store and opens the canvas in preview mode.
- **Backend factory APIs** — Relies on [`api_factories.md`](api_factories.md) for conversational generation and confirmation.
- **Trigger system** — Delegates schedule configuration to [`triggers_feature.md`](triggers_feature.md).
...
- [workflows_feature_dashboard](workflows_feature_dashboard.md) — Dashboard that hosts the factory entry point.
- [workflow_editor](workflow_editor.md) — Canvas editor where the finished workflow is opened.
- [shared_features](shared_features.md) — `FactoryChatShell`, `useFactoryChatStream`, `FactoryFileChips`, `DownloadNotice`.
- [common_components](common_components.md) — `PlanCard`, `InlinePicker`.
- [triggers_feature](triggers_feature.md) — `TriggerSection`, `TriggerScopedCss`.
- [api_factories](api_factories.md) — Backend `/workflow-factory/chat` and `/workflow-factory/confirm` endpoints.
- [workflow_factory_pipeline](workflow_factory_pipeline.md) — Backend workflow generation pipeline.
- [agents_feature](agents_feature.md) — Parallel factory experience for agents.
- [skills_feature](skills_feature.md) — Parallel factory experience for skills.
