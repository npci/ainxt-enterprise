...
[`engine_native_engine`](engine_native_engine.md) calls `get_llm_client` to obtain a client for the configured model, then drives the agent loop by streaming messages and tool calls. The engine relies on `finish_reason` and `usage` fields to detect truncation and meter token budgets.

### With the Swarm Orchestrator

[`swarm`](swarm.md) uses `complete_with_finish_reason` to distinguish genuine `stop` completions from `length`-truncated responses, avoiding wasted retries on phantom truncations. It also uses the `notice.model_fallback` signal to inform users when the fallback model has taken over.

### With Factory Pipelines

[`skill_factory_pipeline`](skill_factory_pipeline.md), [`workflow_factory_pipeline`](workflow_factory_pipeline.md), and [`agent_factory_pipeline`](agent_factory_pipeline.md) call `complete_nonstream` to generate JSON blobs and plans. Non-streaming avoids broken streaming behavior on some gateways for large generations.

### With Generation Endpoints

[`api_generation`](api_generation.md) uses the handler for instruction generation and model listing. [`core_factory_utils`](core_factory_utils.md) provides `call_factory_llm_with_finish_reason`, which delegates to the handler's `complete_with_finish_reason`.
...
- [`engine_native_engine`](engine_native_engine.md) — consumes the handler to run agent/tool loops.
- [`llm_proxy_main`](llm_proxy_main.md) — the proxy service that handles cloud model streams.
- [`swarm`](swarm.md) — uses `complete_with_finish_reason` and fallback notices.
- [`api_generation`](api_generation.md) — generation endpoints that call through this handler.
- [`core_factory_utils`](core_factory_utils.md) — factory helper that wraps `complete_with_finish_reason`.
- [`agent_factory_pipeline`](agent_factory_pipeline.md), [`skill_factory_pipeline`](skill_factory_pipeline.md), [`workflow_factory_pipeline`](workflow_factory_pipeline.md) — factory pipelines that use non-streaming completions.
- [`app_models`](app_models.md) — defines `LLMConfig` and `LLMProvider` consumed by the factory.
- [`core_config`](core_config.md) — resolves `LLM_PROXY_URL`, OpenAI-compatible base URL, and API keys.
