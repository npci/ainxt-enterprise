...
It does **not** handle vector embedding, semantic similarity, approval workflows, or document upload lifecycle. Those concerns live in companion modules such as [`docs_store`](docs_store.md), [`hybrid_search`](hybrid_search.md), and [`kb_router`](kb_router.md).
...
The module is stateless. All configuration comes from environment variables, and all data is read from the filesystem at call time. The MCP server (`KBSearchMCPServer`) wraps the three functions as MCP tools and adds JSON-RPC dispatch, compliance gating, and audit logging via [`BaseMCPServer`](mcp_servers.md).
...
- [`KBSearchMCPServer`](mcp_servers.md) imports `list_namespaces`, `search`, and `get_document` and exposes them as MCP tools.
- [`BaseMCPServer`](mcp_servers.md) provides JSON-RPC dispatch, input/output compliance gating, and optional PCI audit logging.
...
- [`kb_router`](kb_router.md) exposes the canonical entity registry.
- [`docs_router`](docs_router.md) exposes namespace listing over HTTP.
- [`connectors/mcp_bridge`](connectors.md) contains a legacy `_kb_search` helper that routes through the agent state retrieval tool.
...
- Input/output compliance scanning is performed by [`BaseMCPServer`](mcp_servers.md) before and after each tool invocation.
- For fine-grained RAG ACL filtering, see [`rag_acl`](core_infrastructure.md) and [`docs_store`](docs_store.md).
...
- [`mcp_servers`](mcp_servers.md) – MCP server base class and `KBSearchMCPServer`.
- [`docs_store`](docs_store.md) – Document upload, approval, chunking, and embedding.
- [`docs_router`](docs_router.md) – HTTP routes for document and namespace management.
- [`kb_router`](kb_router.md) – Canonical entity registry routes.
- [`hybrid_search`](hybrid_search.md) – Semantic/vector retrieval.
- [`local_model`](local_model.md) – Local LLM and embedding retriever.
- [`connectors`](connectors.md) – Connector infrastructure including the MCP bridge.
