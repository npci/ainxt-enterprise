...
The modern ABStudio backend extends workflow management with a visual graph editor and a more powerful execution engine. See [agent_factory_pipeline](agent_factory_pipeline.md) for the agent assembly pipeline and [api_execution](api_execution.md) for the streaming execution API.
...
| Module | Relationship |
|---|---|
| [agent_management](agent_management.md) | Shares the gateway auth layer; agents can be invoked as workflow steps |
| [chat_and_messaging](chat_and_messaging.md) | Workflow run results feed into chat history |
| [shared_core](shared_core.md) → `workflow_system` | Provides `WorkflowEngine`, `Workflow`, `WorkflowStep` |
| [shared_core](shared_core.md) → `database` | Provides `WorkflowRecord`, `WorkflowRunRecord` ORM models |
| [shared_core](shared_core.md) → `mcp_system` | Provides `MCPRegistry` for tool resolution |
| [shared_core](shared_core.md) → `authentication` | Provides `is_admin` for RBAC checks |
| [abstudio_backend](abstudio_backend.md) → `api_workflows` | Modern CRUD API for graph workflows |
| [abstudio_backend](abstudio_backend.md) → `api_execution` | Streaming execution + HITL resume endpoints |
| [abstudio_backend](abstudio_backend.md) → `core_workflow_repo` | Postgres persistence for graph workflows |
| [abstudio_backend](abstudio_backend.md) → `engine_native_engine` | DAG traversal engine with SSE streaming |
| [abstudio_backend](abstudio_backend.md) → `services_trigger_scheduler` | Scheduled workflow trigger dispatch |
| [workers](workers.md) → `infrastructure_maintenance_workers` | `workflow_scheduler_worker` dispatches due triggers |
| [shared_api_routers](shared_api_routers.md) → `jobs_router` | Durable workflow job submission/resume |
| [shared_api_routers](shared_api_routers.md) → `webhooks_router` | Webhook-triggered workflow execution |
...
See [api_triggers](api_triggers.md) for the trigger CRUD API and [services_trigger_scheduler](services_trigger_scheduler.md) for the scheduler internals.
...
See [api_governance](api_governance.md) and [core_governance](core_governance.md) for the governance submission and approval lifecycle.
...
5. **Swarm delegation** — The NativeEngine integrates with the [swarm](swarm.md) module via the `WorkflowSwarmTool`, allowing agent nodes to dynamically decompose tasks into parallel sub-agent swarms with live status streaming.
