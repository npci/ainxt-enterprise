...
- **[pptx_add_slide](pptx_add_slide.md)**: Creates or duplicates slides in an unpacked presentation; the resulting directory is typically passed to `pack()` to produce the final `.pptx`.
- **[pptx_clean](pptx_clean.md)**: Removes unused files from an unpacked presentation before packing.
- **[pptx_thumbnail](pptx_thumbnail.md)**: Generates thumbnail images from packed presentations.
