...
In the broader ABStudio architecture, tools are the executable capabilities that agents and workflows invoke at runtime. The backend owns tool storage, generation, dispatch, and governance (see [api_catalog](api_catalog.md), [core_workflow_repo](core_workflow_repo.md), and [engine_native_engine](engine_native_engine.md)). The `tools_feature` module consumes that backend catalog and presents it to users without duplicating tool-management logic.
...
The backend endpoint `list_tools_catalog` (in [api_catalog](api_catalog.md)):
...
| Module | Relationship |
|--------|--------------|
| [api_catalog](api_catalog.md) | Exposes `/tools-catalog` and filters tools by visibility rules (generated, platform, M365 connection). |
| [core_workflow_repo](core_workflow_repo.md) | Reads tool rows from the database and maps them to the catalog shape. |
| [engine_native_engine](engine_native_engine.md) | Wraps catalog rows as `_CatalogTool` instances at runtime and dispatches calls via `ToolDispatcher`. |
| [agent_factory_pipeline](agent_factory_pipeline.md) | Provides `ToolDispatcher`, which executes tool code in a sandboxed subprocess. |
...
- **Authoring** happens in [skills_feature](skills_feature.md) and [agent_factory_pipeline](agent_factory_pipeline.md), where new tools can be generated, packaged, and registered.
- **Governance** is handled by [api_governance](api_governance.md) and [core_governance](core_governance.md), which approve or deny tool submissions before they appear in the catalog.
- **Execution** is performed by [engine_native_engine](engine_native_engine.md) and [agent_factory_pipeline](agent_factory_pipeline.md) when an agent or workflow invokes a tool.
...
The Tools Catalog is typically reachable from the main navigation rendered by [app_core](app_core.md) and embedded inside [build_studio](build_studio.md).
...
The backend already filters M365 tools when the user is not connected. The frontend simply renders whatever the API returns, keeping visibility logic centralized in [api_catalog](api_catalog.md).

### Read-Only Surface

`ToolsDashboard` does not create, edit, delete, or execute tools. Those operations are delegated to:

- [api_catalog](api_catalog.md) for CRUD and generation.
- [skills_feature](skills_feature.md) for skill-centric tool packaging.
- [engine_native_engine](engine_native_engine.md) for runtime invocation.
