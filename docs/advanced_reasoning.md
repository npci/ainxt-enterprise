...
3. **Unified LLM Proxy** — All LLM invocations route through the same
   ...
   [model_routing](model_routing.md)). No direct Anthropic/OpenAI SDK calls.
...
The `ReactOrchestrator` (see [agent_orchestration](agent_orchestration.md)) is
...
|---|---|---|
| `ModelRouter` | [model_routing](model_routing.md) | Routes every `_llm_call` to the appropriate LLM gateway (Claude/OpenAI/Gemini/Local) with fallback chains and circuit breakers. The `model_hint` parameter (`"simple"` or `"complex"`) influences tier selection. |
...
| `logger` | [core_infrastructure](core_infrastructure.md) | Structured logging for all strategy lifecycle events (start, depth progression, scoring, verification results, errors). |
| `ReactOrchestrator` | [agent_orchestration](agent_orchestration.md) | Primary consumer — invokes strategies based on risk classification and post-verification confidence. |
...
- **[agent_orchestration](agent_orchestration.md)** — The `ReactOrchestrator` is
...
- **[core_agent_framework](core_agent_framework.md)** — The `AgentBuilder` and
...
- **[model_routing](model_routing.md)** — The `ModelRouter` handles all LLM
  ...
  the router selects.
- **[core_infrastructure](core_infrastructure.md)** — Provides the `logger`
