...
auth_dependencies). Job submission is further restricted
...
core_job_queue, workflows_planner,
and memory_postgres_memory.
...
|---|---|---|
| `auth.dependencies.get_current_user` | Enforces authentication on every endpoint | auth_dependencies |
| `core.job_queue` | RQ/Redis-backed enqueue, cancel, status, list, stats | core_job_queue |
| `memory.postgres_memory.PostgresMemory` | Loads workflow run history for plan analysis | memory_postgres_memory |
| `workflows.engine.WorkflowStep` | Lightweight step object reconstructed for analysis | workflows_engine |
| `workflows.planner.PlanningEngine` | CPM analysis, risk scoring, checkpoint/resume | workflows_planner |
...
|---|---|
| `workers.sdlc_worker.run_feature_pipeline_job` | workers_sdlc_worker |
| `workers.sdlc_worker.run_bug_pipeline_job` | workers_sdlc_worker |
| `workers.sdlc_worker.run_mr_review_job` | workers_sdlc_worker |
| `workers.sdlc_worker.run_mr_merge_job` | workers_sdlc_worker |
| `workers.sdlc_worker.run_reindex_job` | workers_sdlc_worker |
| `workers.durable_workflow_worker.execute_durable_workflow` | workers_durable_workflow_worker |
| `workers.chat_worker.run_chat_job` | workers_chat_worker |
| `workers.graph_worker.run_graph_index_job` | Knowledge graph indexing worker |
| `workers.memory_maintenance_worker.run_memory_maintenance` | workers_memory_maintenance_worker |
| `workers.feedback_loop_worker.run_feedback_loop` | workers_feedback_loop_worker |
| `workers.workflow_scheduler_worker.dispatch_scheduled_workflows` | workers_workflow_scheduler_worker |
...
> See workflows_planner for checkpoint lifecycle details.
...
> a default `500` response. See core_job_queue for
...
core_job_queue, memory_postgres_memory,
and workflows_planner.
...
higher-level routers like [sdlc_router](../sdlc/sdlc_router.md) and
[chat_router](../chat/chat_router.md) that enqueue jobs internally through
