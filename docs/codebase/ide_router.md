...
|-------------------|---------------------|
| [auth_router](../auth/auth_router.md) / auth.dependencies | `get_current_user` validates JWT or API-key tokens. |
| [agents_router](../agents/agents_router.md) / agents.agent_builder | `AgentBuilder` and `AgentRunner` execute named agents. |
| [chat_router](../chat/chat_router.md) | `ide_chat` mirrors `POST /ask` but returns JSON instead of streaming. |
| workflows / workflows.engine | `WorkflowEngine` executes DAG workflows. |
| mcp_registry | `MCPRegistry.execute_tool()` binds workflow tool steps. |
| model_router | `ModelRouter.async_generate()` routes chat prompts to the correct LLM gateway. |
| compliance_engine | `ComplianceEngine.check()` blocks PCI/PII in IDE chat input. |
| [budget_router](../llm/budget_router.md) / `store.budget_store` | Budget gate and usage/cost increment. |
| [threads_router](../chat/threads_router.md) / `store.threads_store` | Thread listing. |
| [index_router](../knowledge/index_router.md) | `repo_index_status` table is queried for repo resolution. |
| [projects_router](../products/projects_router.md) / `store.projects_store` | Project-to-repo mapping for RAG context. |
| evals / `core.evals` | Fire-and-forget answer quality evaluation. |
...
- [auth_router](../auth/auth_router.md) — authentication and API-key management.
- [agents_router](../agents/agents_router.md) — agent CRUD and run endpoints.
- [chat_router](../chat/chat_router.md) — streaming chat endpoints.
- workflows / workflows_engine — workflow execution engine.
- mcp_registry — tool and skill registry.
- model_router — LLM routing and gateway selection.
- [budget_router](../llm/budget_router.md) — budget enforcement and usage tracking.
- [threads_router](../chat/threads_router.md) — conversation thread management.
- [index_router](../knowledge/index_router.md) — repository indexing status.
