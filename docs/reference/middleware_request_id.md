...
middleware stack (registered in [`gateway.md`](../models/gateway.md) before
...
* **Gateway / API entry point** — registered in [`gateway.md`](../models/gateway.md) as
  the first middleware so every request is tagged before routing.
* **Logging & tracing** — relies on [`core/logger.md`](../core/logger.md) context
...
* **Budget & rate limiting** — [`middleware_budget.md`](../middleware_budget.md)
  and [`middleware_rate_limit.md`](../middleware_rate_limit.md) run after this
  middleware, so their audit and denial logs include the request ID.
* **Client source tracking** — [`middleware_client_source.md`](../clients/middleware_client_source.md)
...
* [`core/logger.md`](../core/logger.md) — structlog context helpers
  (`set_request_id`, `set_correlation_id`, `clear_bound_context`).
* [`gateway.md`](../models/gateway.md) — FastAPI/Starlette application where the
  middleware is registered.
* [`middleware_budget.md`](../middleware_budget.md) — budget enforcement
  middleware that runs after request ID assignment.
* [`middleware_rate_limit.md`](../middleware_rate_limit.md) — rate limiting
  middleware that runs after request ID assignment.
* [`middleware_client_source.md`](../clients/middleware_client_source.md) — client source
