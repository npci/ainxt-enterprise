...
All API calls use the shared `apiFetch` wrapper from [`config.js`](../infrastructure/config.md), which handles authentication headers and error normalisation.
...
| `config.js` | [config](../infrastructure/config.md) | `API_BASE` and `apiFetch` |
...
|---|---|
| `routers/sdlc_router.py` | [sdlc_router](../api/shared_api_routers.md) — all SDLC run, approval, resume, and governance endpoints |
| `routers/compliance_router.py` | [compliance_router](../api/shared_api_routers.md) — audit chain verification and report export |
| `workers/sdlc_worker.py` | [sdlc_pipeline_workers](../workers/sdlc_pipeline_workers.md) — background job execution |
