...
The component consumes the shared `apiFetch` utility from [`config`](../infrastructure/config.md) and uses icons from `lucide-react`.
...
This endpoint is exposed by the [`sdlc_router`](../api/sdlc_router.md) in the shared API routers layer and is handled by the backend SDLC pipeline state machine (see [`shared_core_sdlc_pipeline`](../sdlc/shared_core_sdlc_pipeline.md)).
...
- **API client**: `apiFetch` and `API_BASE` from [`config`](../infrastructure/config.md).
- **Backend endpoint**: `POST /sdlc/runs/{runId}/answer-questions` exposed by [`sdlc_router`](../api/sdlc_router.md).
...
- [`sdlc_router`](../api/sdlc_router.md) — Backend router exposing `answer-questions`.
- [`shared_core_sdlc_pipeline`](../sdlc/shared_core_sdlc_pipeline.md) — Backend SDLC state machine and normalization logic.
- [`config`](../infrastructure/config.md) — Shared frontend API configuration and `apiFetch` helper.
