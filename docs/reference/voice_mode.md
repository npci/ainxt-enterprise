...
- [chat](../chat/chat.md) — `Chat.jsx` provides `sendMessageForVoice` and `handleMicToggle` for the main chat surface.
- [kb_chat](../knowledge/kb_chat.md) — `KbChat.jsx` provides an identical `sendMessageForVoice` implementation for knowledge-base chat.
- [gateway](../models/gateway.md) — Exposes `/voice/tts` and `/voice/stt`; the TTS path is consumed by the `ttsApi` wrapper.
- [llm_proxy](../models/llm_proxy.md) — Hosts `/llm/tts`, the production TTS proxy that reaches OpenAI from the web tier.
