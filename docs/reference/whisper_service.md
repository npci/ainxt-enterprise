...
- **[gateway](../models/gateway.md)** — routes `POST /voice/stt` to this service when `WHISPER_SVC_URL` is configured. See [gateway.md](../models/gateway.md) for the broader routing and health/monitoring picture.
- **[ai_ui_frontend](../ui/ai_ui_frontend.md)** — the Voice Mode UI in `ai-ui/src/components/VoiceMode.jsx` initiates the audio capture that ultimately reaches this service.
...
| `WHISPER_DEVICE` | `cpu` | Compute device. Kept CPU-only by default to mirror [embedding_service](../knowledge/embedding_service.md) and [privacy_service](../security/privacy_service.md). |
| `WHISPER_COMPUTE_TYPE` | `int8` | Quantization type; `int8` keeps CPU memory footprint small. |
| `WHISPER_SVC_URL` | — | URL the [gateway](../models/gateway.md) uses to proxy STT requests here. |
...
For platform-level health aggregation, see [gateway.md](../models/gateway.md) under health and monitoring.
...
| Service | Role |
|---------|------|
| [gateway](../models/gateway.md) | Proxies voice STT requests and exposes `/voice/stt` to clients. |
| [llm_proxy](../models/llm_proxy.md) | Handles LLM, TTS, and image generation; Whisper Service is the STT counterpart. |
| [embedding_service](../knowledge/embedding_service.md) | Follows the same "no lazy ML load in gateway" pattern. |
| [privacy_service](../security/privacy_service.md) | Also follows the same eager-load, separate-process pattern for ONNX models. |
| [ai_ui_frontend](../ui/ai_ui_frontend.md) | Provides the Voice Mode UI that captures audio and sends it through the gateway. |

---

## See Also

- [gateway.md](../models/gateway.md)
- [llm_proxy.md](../models/llm_proxy.md)
- [embedding_service.md](../knowledge/embedding_service.md)
- [privacy_service.md](../security/privacy_service.md)
- [ai_ui_frontend.md](../ui/ai_ui_frontend.md)
