...
|------------|------|----------------|
| [pipeline_degradation](../pipeline_degradation.md) | `pipeline/degradation.py` | Health-to-serving-level policy with a fail-safe floor. |
| [pipeline_dispatch](../pipeline_dispatch.md) | `pipeline/dispatch.py` | Explicit lane and fork decisions for the request lifecycle. |
| [pipeline_stream_events](../pipeline_stream_events.md) | `pipeline/stream_events.py` | Pure builders for structured SSE events. |
...
- **Related modules**: See [shared_core](shared_core.md) for the broader subsystem, and [gateway](../models/gateway.md) for the request lifecycle that consumes these decisions.
