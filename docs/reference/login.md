...
This module is intentionally thin: it owns only the UI surface and client-side validation. All credential verification, session minting, token storage, and RBAC enrichment happen in the backend [auth_router](../api/auth_router.md) and [authentication](../security/authentication.md) layers.
...
| `apiFetch` | [config](../infrastructure/config.md) | Unauthenticated fetch wrapper that sends `credentials: include` and a correlation id. |
| `authFetch` | [config](../infrastructure/config.md) | Authenticated fetch wrapper with a single retry for idempotent GETs. |
| `App` / `handleAuth` | [ai_ui_frontend_app_core](../ui/ai_ui_frontend_app_core.md) | Receives the normalized user object and promotes it to global auth state. |
| `AuthProvider` / `useAuth` | [auth](../security/auth.md) | React context that exposes session state to descendants (currently not persisted). |

### Backend Dependencies

| Dependency | Module | Role |
| ---------- | ------ | ---- |
| `POST /auth/login` | [auth_router](../api/auth_router.md) | Validates credentials, issues JWT, sets httpOnly `auth_token` cookie, registers session. |
| `GET /auth/me` | [auth_router](../api/auth_router.md) | Returns the authenticated user's profile from the database. |
| `encode_token` | [authentication](../security/authentication.md) | Creates a signed JWT with minimal claims (no PII). |
| `get_current_user` | [authentication](../security/authentication.md) | Extracts and validates the JWT or API key from the request. |
| `session_manager` | [authentication](../security/authentication.md) | Registers and enforces concurrent session limits. |
...
- [ai_ui_frontend_app_core](../ui/ai_ui_frontend_app_core.md) — global auth state and routing.
- [chat](../chat/chat.md) / [kb_chat](../knowledge/kb_chat.md) — personalized chat history and scopes.
- [budget_manager](../models/budget_manager.md) / [level_overrides](../clients/level_overrides.md) — RBAC/ABAC-gated features.
- [inbox](../chat/inbox.md) — unread approvals and notifications.

The backend counterpart lives in the shared [auth_router](../api/auth_router.md) and [authentication](../security/authentication.md) modules, which are also used by the desktop app, IDE integrations, and API-key clients.

---

## References

- [ai_ui_frontend_app_core](../ui/ai_ui_frontend_app_core.md) — application shell, `handleAuth`, `handleLogout`, and route guards.
- [auth](../security/auth.md) — React auth context (`AuthProvider`, `useAuth`).
- [config](../infrastructure/config.md) — `apiFetch` and `authFetch` wrappers.
- [brand_mark](brand_mark.md) — AiNxt logo component.
- [auth_router](../api/auth_router.md) — backend login, logout, refresh, and `/auth/me` endpoints.
- [authentication](../security/authentication.md) — JWT handling, session management, and dependency injection.
