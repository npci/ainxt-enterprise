...
The Projects module sits alongside the main [Chat](../chat/chat.md), [Knowledge Base](../knowledge/knowledge_base.md), [Code](code.md), and [Agents Catalog](../agents/agents_catalog.md) experiences in the `ai_ui_frontend` portal. It reuses shared infrastructure:

- **Authentication** — Uses `authFetch` and the httpOnly-cookie session managed by [AuthContext](../security/auth.md).
- **Budget governance** — Every ask checks the user's budget via [budget_store](../models/budget.md) and displays usage metadata via [MessageMeta](../chat/message_meta.md).
...
The frontend consumes the following endpoints. Full backend contract details live in [projects_router](../api/shared_api_routers.md#projects_router).
...
- [auth](../security/auth.md) — Authentication and `authFetch`.
- [product_manager](product_manager.md) — Product catalog used by project forms.
- [shared_api_routers](../api/shared_api_routers.md#projects_router) — Backend `projects_router` API details.
