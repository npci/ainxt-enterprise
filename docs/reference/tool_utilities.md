...
- Calls `hybrid_retrieve_context` from [model_routing.md](../models/model_routing.md) with the user's repo filter, user context, and query complexity.
...
`local_llm_tool` is a simple state toggle that sets `state.use_local_llm = True`. When this flag is set, `generate_answer_tool` routes the final generation through the in-house [local_llm_gateway.md](../models/local_llm_gateway.md) instead of the cloud model router.
...
- **Model routing:** Uses [model_routing.md](../models/model_routing.md) by default, with complexity upgrades when context is present. Honors `state.model_hint` and `state.use_local_llm`.
...
- **Fire-and-forget evaluation:** Spawns background threads to evaluate retrieval quality (Point C) and answer quality (Point D) via [core_infrastructure.md](../infrastructure/core_infrastructure.md)'s `eval_engine`.
...
- **[model_routing.md](../models/model_routing.md)** provides the hybrid retriever and model router consumed by `retrieve_tool` and `generate_answer_tool`.
- **[compliance_engine.md](../compliance_engine.md)** provides the security scanning used by `compliance_tool`.
- **[local_llm_gateway.md](../models/local_llm_gateway.md)** is the optional generation backend selected by `local_llm_tool`.
- **[core_infrastructure.md](../infrastructure/core_infrastructure.md)** provides logging, telemetry, and the evaluation engine used for fire-and-forget quality scoring.
...
- [model_routing.md](../models/model_routing.md) – Hybrid retrieval and model routing.
- [local_llm_gateway.md](../models/local_llm_gateway.md) – In-house local LLM backend.
- [core_infrastructure.md](../infrastructure/core_infrastructure.md) – Logging, telemetry, and evaluation engine.
