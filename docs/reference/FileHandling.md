...
This module lives inside [`ChatPanel.jsx`](../chat/ChatPanel.md) and is tightly coupled to the workflow execution stream, the markdown renderer, and the generated-file download infrastructure.
...
- **[ChatPanel](../chat/ChatPanel.md)** — The parent component that owns chat state, SSE streaming, and the assistant-message renderer. FileHandling is invoked from the `assistant` message branch.
- **[DownloadNotice](../DownloadNotice.md)** — Displays the `notice` object produced by `useGeneratedDownload`.
- **[workflowStore](../workflowStore.md)** — Provides `runContext`, execution logs, and generated-file metadata consumed by the message renderer.
- **[AgentEditor](../AgentEditor.md)** — Contains a sibling `FileDownloadCard` used in the agent-builder chat; both share the same backend download contract.
...
- **[engine_native_engine](engine_native_engine.md)** — Produces the `generated_files` metadata that FileHandling renders.
...
The helper indexes files by `filename`, `disk_name`, and the URL tail so that references like `[Report.pptx](/generated-files/...)` or `` `Report.pptx` `` are both resolved.
...
- **Agent chat** ([AgentEditor](../AgentEditor.md)) — Uses the same `FileDownloadCard` pattern for agent-runner outputs.
- **Knowledge upload** ([KnowledgeUploadInline](../KnowledgeUploadInline.md), [api_kb](../api/api_kb.md)) — Handles ingestion of documents into the knowledge base; not to be confused with generated-file downloads.
- **Document generation** ([doc_download_router](../api/doc_download_router.md), [doc_generator](../documents/doc_generator.md)) — Produces DOCX/PPTX/XLSX files on the backend; the resulting download URLs are consumed here.
...
- [ChatPanel](../chat/ChatPanel.md) — Parent component and SSE event handling.
- [AgentEditor](../AgentEditor.md) — Sibling file-download usage in agent builder.
- [DownloadNotice](../DownloadNotice.md) — Toast / banner renderer.
...
- [engine_native_engine](engine_native_engine.md) — Engine that produces `generated_files`.
