...
The module lives at `ABStudio/backend/app/engine/native_engine.py` and is the central execution surface invoked by the [API Execution](../api/api_execution.md) endpoints (`run_workflow_stream`, `resume_workflow_stream_endpoint`).
...
Wraps tools stored in the `tools_catalog` database table. Dispatches execution to `ToolDispatcher` (from [agent_factory_pipeline](../agents/agent_factory_pipeline.md)), which runs the tool's Python source in a subprocess sandbox with a 300s timeout and 1MB stdout cap.
...
When `useLlmEvaluator` is enabled, an `LLMEvaluator` (from [engine_loop_evaluator](../evaluation/engine_loop_evaluator.md)) scores each iteration against a rubric. A `LoopController` applies a multi-signal termination policy:
...
| Module | Purpose |
|---|---|
| [engine_loop_evaluator](../evaluation/engine_loop_evaluator.md) | `LoopController` (hybrid stop policy) and `LLMEvaluator` (rubric-based LLM judge) |
...
| [loop_models](../models/loop_models.md) | `ReflectionKind` enum, `ActionSpec`, `VerifySpec` and other loop data models |
| [agent_factory_pipeline](../agents/agent_factory_pipeline.md) | `ToolDispatcher` (sandbox execution), `AgentRunner` (subflow agent execution) |
| [swarm](../agents/swarm.md) | `SwarmRuntime`, `SwarmContext`, `SwarmOrchestrator`, `SwarmAggregator` for subagent delegation |
| [core_llm_handler](../models/core_llm_handler.md) | `FallbackLLMClient`, `OpenAIClient`, `Message` for LLM communication |
...
| [core_governance](../sdlc/core_governance.md) | `check_tool_access()`, `audit_event()` for tool policy enforcement |
| [checkpoint](checkpoint.md) | `FileCheckpointStore`, `PostgresCheckpointStore` for state persistence |
| [app_models](../models/app_models.md) | `Workflow`, `RunRequest`, `ResumeRequest` data models |
| [api_execution](../api/api_execution.md) | HTTP endpoints that invoke the engine |
| [api_chat](../api/api_chat.md) | Chat thread history and interrupt management endpoints |
