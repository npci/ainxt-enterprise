...
This document focuses on the frontend management surface. For the backend management API, see [`endpoint_mgmt_router.md`](../api/endpoint_mgmt_router.md). For the runtime proxy that serves the actual chat completions and model listing calls, see [`endpoint_proxy_router.md`](../api/endpoint_proxy_router.md).
...
|------------|---------------|---------|
| `authFetch` | [`config.js`](../infrastructure/config.md) | Authenticated fetch wrapper that includes credentials and a correlation header. |
...
|------------|--------|---------|
| Management API | [`endpoint_mgmt_router.md`](../api/endpoint_mgmt_router.md) | CRUD, toggle, key regeneration, and model preview for managed endpoints. |
| Proxy API | [`endpoint_proxy_router.md`](../api/endpoint_proxy_router.md) | Serves `/{slug}/v1/chat/completions` and `/{slug}/v1/models` at runtime. |
| Local model catalog | [`gateway_local_llm.md`](../gateway_local_llm.md) | Validates selected models against the LiteLLM model list. |
| API key store | [`api_keys_router.md`](../api/api_keys_router.md) / `UserAPIKey` | Platform keys are stored and revoked using the same mechanism as CLI keys. |
...
- **API key management**: Endpoint keys live in the same `UserAPIKey` table as CLI keys and share revocation, hashing, and prefix-display semantics. See [`api_keys_router.md`](../api/api_keys_router.md).
