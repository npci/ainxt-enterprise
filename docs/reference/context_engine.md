...
The **Context Engine** is a pure, deterministic planner that decides how a conversation transcript should be packed into a model's context window. It implements the age-tiered history-assembly strategy described in [`docs/architecture/06-context-engineering.md`](../architecture/06-context-engineering.md) §6.2–6.5.
...
- Compress tool output, RAG chunks, or code (see [core_infrastructure](../infrastructure/core_infrastructure.md) for that).
- Manage durable cross-session memory (see [memory_system](memory_system.md)).
...
See [memory_system](memory_system.md) for details on how `chat_summarizer` maintains per-chat rolling summaries and decides what deserves long-term memory.

### Cross-session context

While the Context Engine handles the current conversation transcript, cross-session engineer context is managed separately by `core/context_manager`. That module injects a preface at session start and rebuilds a rolling work summary in the background. See [core_infrastructure](../infrastructure/core_infrastructure.md).

### Compression of non-conversation data

Tool outputs, build logs, RAG chunks, and IDE file reads are not handled by the Context Engine. They are compressed by `core/context_compressor` before they reach the context window. See [core_infrastructure](../infrastructure/core_infrastructure.md).

### Degradation ladder

The platform's degradation policy treats the Context Engine as a first-class subsystem. If the engine is unhealthy, `pipeline/degradation` sets `rich_context = False` and the runtime falls back to a flat-summary path. See [pipeline](pipeline.md).
...
- [memory_system](memory_system.md) — rolling chat summaries, `distill_turn`, and long-term memory decisions.
- [core_infrastructure](../infrastructure/core_infrastructure.md) — `core/context_manager` for cross-session engineer context and `core/context_compressor` for tool/RAG compression.
- [pipeline](pipeline.md) — `pipeline/degradation` serving-level policy including the Context Engine health flag.
- [`docs/architecture/06-context-engineering.md`](../architecture/06-context-engineering.md) — full context-engineering strategy and slot model.
