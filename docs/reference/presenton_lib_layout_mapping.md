...
|---|---|---|
| [`presenton_lib_api_client`](../api/presenton_lib_api_client.md) | HTTP client for Presenton endpoints | Uses resolved layout IDs when preparing API requests. |
...
| [`presenton_lib_payload_builder`](../ui/presenton_lib_payload_builder.md) | Builds per-slide content objects | Uses layout keys to decide which content schema to generate. |
...
| [`presenton_router`](../api/presenton_router.md) | Backend router for Presenton | Receives payloads containing `layout_group` and `layout` fields. |
...
3. The returned object is passed to [`presenton_lib_payload_builder`](../ui/presenton_lib_payload_builder.md), which fills in the content schema (title, statement, points, website, etc.).
4. [`presenton_lib_api_client`](../api/presenton_lib_api_client.md) assembles the final payload and sends it to the Presenton backend.
...
- **Backend counterpart** is [`presenton_router`](../api/presenton_router.md), which exposes endpoints such as `generate_outline`, `generate_presentation`, and `download_presentation`.
...
5. Coordinate with [`presenton_lib_payload_builder`](../ui/presenton_lib_payload_builder.md) to add the matching content schema in `buildSlideContent`.
...
- [`presenton_lib_api_client`](../api/presenton_lib_api_client.md) — HTTP client for Presenton
- [`presenton_lib_layout_registry`](presenton_lib_layout_registry.md) — layout metadata registry
- [`presenton_lib_payload_builder`](../ui/presenton_lib_payload_builder.md) — slide content builder
- [`presenton_lib_stream_reader`](presenton_lib_stream_reader.md) — streaming response reader
- [`presenton_router`](../api/presenton_router.md) — backend Presenton API router
