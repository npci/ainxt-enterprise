...
- Uses `authFetch` from the shared [config](../infrastructure/config.md) module for authenticated, retry-aware HTTP requests.
...
| Module | Relationship |
|--------|--------------|
| [config](../infrastructure/config.md) | Provides `authFetch` and `API_BASE` for authenticated API calls. |
| [audit_router](../api/audit_router.md) | Backend router exposing the `/audit` endpoint and `list_audit_logs` logic. |
| [gateway](../models/gateway.md) | Hosts the `/audit` route and forwards requests to the audit router. |
...
- [config](../infrastructure/config.md) — Authentication and API base configuration.
- [audit_router](../api/audit_router.md) — Backend audit log retrieval.
- [gateway](../models/gateway.md) — API gateway routing and middleware.
