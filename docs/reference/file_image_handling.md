...
| `authFetch`, `API_BASE` | [config](../infrastructure/config.md) | Authenticated HTTP client and base URL. |
| `chat_router` backend | [chat_router](../api/chat_router.md) | `POST /chat/upload` and `GET /chat/attachments/{id}/raw`. |
...
- Backend upload/preview API: [chat_router](../api/chat_router.md)
...
- Authenticated fetch config: [config](../infrastructure/config.md)
