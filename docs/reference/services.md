...
|------------|---------|-----------|
| [services_budget_digest](../models/services_budget_digest.md) | HOD monthly allocation caps, budget-increase request emails, and HOD/Manager monthly usage digests. | `budget_request_email.py`, `hod_budget_governor.py`, `hod_statement_service.py`, `manager_statement_service.py`, `digest_service.py` |
...
- [routers](../api/shared_api_routers.md) — HTTP API layer that consumes these services.
- [workers](../workers/workers.md) — Background job workers that invoke these services.
- [llm_spend](../models/llm_spend.md) — Sibling module for LLM spend fetching and reporting.
