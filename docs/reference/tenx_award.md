...
| `workers` → `tenx_evaluation_workers` | RQ worker entrypoints and dedicated worker process | [worker_orchestration](../workers/worker_orchestration.md) |
| `shared_core` → `authentication` | JWT auth, admin guards (`get_current_user`, `require_admin`) | [authentication](../security/authentication.md) |
| `shared_core` → `database` | SQLAlchemy models (`TenXSubmission`, `TenXScore`, etc.) | [database](../storage/database.md) |
| `shared_core` → `core_infrastructure` | Job queue (`enqueue_job`, `Q_TENX`), config flags, storage | [core_infrastructure](../infrastructure/core_infrastructure.md) |
| `shared_core` → `agent_system` | `react_orchestrator` for agentic evaluation runs | [agent_system](../agents/agent_system.md) |
| `shared_core` → `model_routing` | `model_router` for LLM calls (verdict synthesis, chat comparison) | [model_routing](../models/model_routing.md) |
...
| `ai_ui_frontend` → `config` | `authFetch`, `API_BASE` for authenticated API calls | [config](../infrastructure/config.md) |
| `ai_ui_frontend` → `ui_dialog` | `useConfirm`, `useToast` for user interactions | [ui_dialog](../ui/ui_dialog.md) |
...
- [worker_orchestration](../workers/worker_orchestration.md) — How RQ workers are started and managed
- [authentication](../security/authentication.md) — JWT auth, admin guards, RBAC
- [database](../storage/database.md) — SQLAlchemy models and database access patterns
- [core_infrastructure](../infrastructure/core_infrastructure.md) — Job queue, config, storage, telemetry
- [agent_system](../agents/agent_system.md) — `react_orchestrator` and agent framework
- [model_routing](../models/model_routing.md) — Model router for LLM calls
- [shared_integrations](shared_integrations.md) — GitLab tools and connector infrastructure
- [config](../infrastructure/config.md) — Frontend API configuration (`authFetch`, `API_BASE`)
