...
The `reaction_engines` module provides the iterative reasoning and session-recovery primitives used by the platform's autonomous agents. It lives inside the broader [`agent_system`](../agents/agent_system.md) and is consumed by features such as Threads @AiNxt and the SDLC pipeline.
...
- `RecoveryEngine` delegates persistence to the shared key/value layer ([`core_infrastructure`](../infrastructure/core_infrastructure.md)) and delegates inverse operations to the existing tool implementations in [`shared_integrations`](shared_integrations.md)).
...
- **Model routing**: `ReactEngine` calls [`models/model_router`](../models/model_routing.md)`::model_router.generate()` for analysis, critique, and synthesis. It uses the `complex` hint for mid-loop calls (cost-controlled Sonnet) and the `solution` hint for the final answer (Opus when enabled).
...
- **Key/value store**: `RecoveryEngine` uses [`core/kv`](../infrastructure/core_infrastructure.md) for undo stacks and ReACT checkpoints. Keys are namespaced by `user_id` to prevent cross-user access (SEC-05).
...
- **Privacy floor**: `ReactEngine` inherits the platform-wide privacy floor through [`model_routing`](../models/model_routing.md); sensitive data is never routed to cloud providers.
...
For higher-level orchestration of multiple agents, see [`agent_orchestration`](../agents/agent_orchestration.md). For the full agent builder/runner, see [`core_agent_framework`](../agents/core_agent_framework.md).
