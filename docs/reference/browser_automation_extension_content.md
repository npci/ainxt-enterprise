...
| [`browser_automation_extension_llm`](../models/browser_automation_extension_llm.md) | `lib/llm.js` | LLM communication — planning, narration, root-cause analysis |
...
- **Snapshot → LLM**: The runner calls `snapshotPageReady()` which sends a `snapshot` message to the content script, then passes the result to `planSteps()` in the LLM client. See [`browser_automation_extension_llm`](../models/browser_automation_extension_llm.md).
