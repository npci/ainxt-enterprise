...
- **Triggered by**: the `ai_ui_frontend` build process (typically via a `postbuild` npm script or CI step). See [ai_ui_frontend.md](../ui/ai_ui_frontend.md) for the broader frontend architecture.
...
- [ai_ui_frontend.md](../ui/ai_ui_frontend.md) — the parent frontend application that owns the build pipeline and service worker.
- [abstudio_frontend.md](../ui/abstudio_frontend.md) — the sibling frontend application (ABStudio) which uses a different build flow and does not share this stamper.
