...
- **Upstream consumers**: `PPTWizard.jsx`, `PPTChatMessageRenderer.jsx`, and any chat flow that produces a presentation. See [`ai_ui_frontend`](../ui/ai_ui_frontend.md).
- **Downstream backend**: [`presenton_router`](../api/presenton_router.md) exposes the REST/SSE surface; [`llm_proxy`](../models/llm_proxy.md) and [`workers/presenton_worker`](../workers/workers.md) perform the actual generation.
...
|------------|---------|----------------|---------------|
| API Client | `presenton-api.js` | Retry-aware HTTP wrapper for all Presenton endpoints | [`presenton_lib_api_client.md`](../api/presenton_lib_api_client.md) |
...
| Payload Builder | `presenton-payload.js` | Builds create, prepare, update, and export payloads | [`presenton_lib_payload_builder.md`](../ui/presenton_lib_payload_builder.md) |
...
- [`ai_ui_frontend`](../ui/ai_ui_frontend.md) – the React application that consumes this library.
- [`presenton_router`](../api/presenton_router.md) – backend router that exposes the Presenton REST/SSE API.
- [`llm_proxy`](../models/llm_proxy.md) – LLM proxy used during slide content generation.
- [`workers`](../workers/workers.md) – includes `presenton_worker` for asynchronous PPT generation jobs.
