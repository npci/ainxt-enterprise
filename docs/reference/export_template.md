...
| **Backend dependency** | [`chat_router`](../api/chat_router.md) (`/prompt-templates` endpoints) for template persistence. |
| **Related modules** | [`chat`](../chat/chat.md), [`kb_chat`](../knowledge/kb_chat.md), [`auth`](../security/auth.md) |
...
| `authFetch` | Authenticated HTTP client; see [`auth`](../security/auth.md). |
...
| `/prompt-templates` endpoints | Backend CRUD for prompt templates; owned by [`chat_router`](../api/chat_router.md). |
...
- The authentication and HTTP layer from [`auth`](../security/auth.md).
- The prompt-template storage and listing APIs from [`chat_router`](../api/chat_router.md).
