...
|---|---|---|
| **app_core** | Root `App` component, top-level navigation, dashboard/editor view switch, autosave orchestration, reload-restore, template preview/clone, and error boundary. | [app_core.md](app_core.md) |
...
| **workflows_feature** | Workflow dashboard, workflow factory chat, `RunningWorkflowToast`, and orchestration of the visual editor. | [workflows_feature.md](workflows_feature.md) |
| **workflow_editor** | React Flow canvas (`Canvas`), node configuration (`ConfigPanel`), run chat (`ChatPanel`), debug log (`DebugLogView`), sidebar palette, condition builders, node/edge renderers, and run settings. | [workflow_editor.md](workflow_editor.md) |
| **agents_feature** | Agents dashboard, `AgentEditor`, and `AgentFactoryChat` for building and previewing agents. | [agents_feature.md](agents_feature.md) |
| **skills_feature** | Skills catalog dashboard and `SkillFactoryChat` for AI-assisted skill authoring. | [skills_feature.md](skills_feature.md) |
| **tools_feature** | Read-only tools catalog with search, grouping, and detail modal. | [tools_feature.md](tools_feature.md) |
| **templates_feature** | Feature-flagged template admin UI: create, edit, reset, save-to-seed, and delete templates. | [templates_feature.md](templates_feature.md) |
...
| **governance_feature** | `StatusBadge` and `SubmitApprovalButton` for workflow/agent/skill approval lifecycle. | [governance_feature.md](governance_feature.md) |
...
| **shared_features** | Cross-cutting factory chat shell, file chips, OCR text preview, and download notices. | [shared_features.md](shared_features.md) |
| **store** | Zustand stores: `workflowStore` (canvas + execution state) and `triggersStore` (trigger CRUD + notifications). | [store.md](store.md) |
...
| **constants** | Canonical field/operator catalogs and expression-preview helpers for the visual condition builder. | [constants.md](constants.md) |
...
- [api_workflows.md](api_workflows.md) — workflow CRUD
- [api_execution.md](api_execution.md) — synchronous and streaming workflow execution
- [api_factories.md](api_factories.md) — workflow, agent, and skill factory chat/confirm
- [api_agents.md](api_agents.md) — agent CRUD
- [api_agent_chat.md](api_agent_chat.md) — agent chat thread metadata
- [api_catalog.md](api_catalog.md) — skills/tools catalog
- [api_templates.md](api_templates.md) — template listing and usage
- [api_template_admin.md](api_template_admin.md) — template admin CRUD + seed persistence
- [api_triggers.md](api_triggers.md) — trigger CRUD and execution history
- [api_governance.md](api_governance.md) — governance status and approval submission
- [api_documents.md](api_documents.md) — file/image upload and OCR extraction
- [core_workflow_repo.md](core_workflow_repo.md) — backend persistence layer
