...
- **[engine_native_engine](../reference/engine_native_engine.md)** — `NativeEngine._run_agent()` injects `WorkflowSwarmTool` (wrapped by `_DedupingSwarmTool`) when subagents are enabled for the node.
...
- **[tools_canonical_seed](../reference/tools_canonical_seed.md)** — Platform utilities such as `code_executor` and `read_skill_file` are often co-injected with `spawn_swarm`; the ordering gate in callers ensures `spawn_swarm` and purpose-built tools are tried before `code_executor`.
- **[tools_m365_bridge](../reference/tools_m365_bridge.md)** / catalog tools — The swarm orchestrator scopes worker tool manifests from the live catalog; this module does not know tool semantics.
...
- [engine_native_engine](../reference/engine_native_engine.md) — workflow-engine agent-node execution.
- [swarm](swarm.md) — swarm runtime, orchestrator, aggregator, worker spec, and registry.
- [tools_canonical_seed](../reference/tools_canonical_seed.md) — platform utility tools co-injected with `spawn_swarm`.
- [app_models](../models/app_models.md) — request/response models for agent and workflow execution.
