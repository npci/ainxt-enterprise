...
| `authFetch`, `API_BASE` | [config](../infrastructure/config.md) | Authenticated HTTP client and backend base URL. |
| `useFileDrop` | [hooks](../reference/hooks.md) | Drag-and-drop file handling. |
| `useToast` | [ui_dialog](../ui/ui_dialog.md) | Toast notifications. |
| `AiNxtSpinner` | [spinner](../reference/spinner.md) | Loading indicator during streaming. |
...
|----------|----------------|---------|
| `GET /agents/catalog` | [agents_router](../api/agents_router.md) | Returns public, department, and favorite agents. |
| `POST/DELETE /agents/{name}/favorite` | [agents_router](../api/agents_router.md) | Adds or removes an agent from favorites. |
| `POST /agents/{name}/run` | [gateway](../models/gateway.md) / [agent_management](../models/gateway.md#agent-management) | Executes the selected agent with streaming output. |
| `POST /chat/upload` | [chat_router](../api/chat_router.md) | Uploads chat attachments with compliance scanning. |
...
- [config](../infrastructure/config.md) — API base URL and authenticated fetch helper.
- [hooks](../reference/hooks.md) — drag-and-drop hook used by the chat input.
...
- [agents_router](../api/agents_router.md) — backend router for catalog and favorites.
- [chat_router](../api/chat_router.md) — backend router for chat file uploads.
- [gateway](../models/gateway.md) — gateway service that handles `/agents/{name}/run`.
