...
- **Governance integration**: Shows the agent's approval status via [StatusBadge](../sdlc/governance_feature.md) and exposes a [SubmitApprovalButton](../sdlc/governance_feature.md) for submitting the agent for governance review.
...
|--------|------------------------|
| [common_components](../ui/common_components.md) `ConfirmModal` | Destructive-action confirmation dialog. |
| [common_components](../ui/common_components.md) `HoverTooltip` | Tooltip shown on hover for long agent descriptions. |
| [hooks](../reference/hooks.md) `useHoverTooltip` | Manages tooltip visibility, anchor bindings, and unique ID. |
| [utils](../reference/utils.md) `formatDate` | Formats `updated_at` for display. |
| [utils](../reference/utils.md) `stripProviderPrefix` | Cleans the model name label (e.g., removes provider prefix). |
| [utils](../reference/utils.md) `stripTemplateTag` | Removes template markers from the description text. |
| [governance_feature](../sdlc/governance_feature.md) `StatusBadge` | Displays governance approval state. |
| [governance_feature](../sdlc/governance_feature.md) `SubmitApprovalButton` | Allows submitting the agent for governance approval. |
...
- [api_agents](../api/api_agents.md) — list, duplicate, delete agents.
- [api_agent_chat](../api/api_agent_chat.md) — initiate a chat thread with the agent.
- [api_agent_templates](../api/api_agent_templates.md) — list and use preset/template agents.
- [api_governance](../api/api_governance.md) — submit/withdraw governance approval.
...
- [api_agents](../api/api_agents.md) — backend CRUD endpoints for agents.
- [api_agent_chat](../api/api_agent_chat.md) — backend chat-thread endpoints for talking to an agent.
- [api_agent_templates](../api/api_agent_templates.md) — backend preset/template agent endpoints.
- [governance_feature](../sdlc/governance_feature.md) — frontend governance components (`StatusBadge`, `SubmitApprovalButton`).
- [api_governance](../api/api_governance.md) — backend governance submission endpoints.
- [common_components](../ui/common_components.md) — shared UI primitives (`ConfirmModal`, `HoverTooltip`).
- [hooks](../reference/hooks.md) — shared React hooks (`useHoverTooltip`).
- [utils](../reference/utils.md) — formatting and text utilities used by the card.
