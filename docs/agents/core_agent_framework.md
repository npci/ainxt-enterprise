...
> **Governance lifecycle**: New user-created agents start as `DRAFT`. Only agents with `status == "PRODUCTION"` are loaded and executable. The governance approval flow (see [governance_router](../api/shared_api_routers.md)) promotes agents through `DRAFT → PENDING_APPROVAL → APPROVED → PRODUCTION`.
...
Compliance is enforced at **three checkpoints** during every agent run, using the [ComplianceEngine](../reference/decision_engines.md) (from the `decision_engines` module):
...
- **Kafka** (`ainxt.agent_events`) emits a `conversation_turn` event with token estimates and cost, consumed by the [kafka_event_consumer](../reference/kafka_event_consumer.md) worker which writes to Postgres for analytics.

---

## Model Routing

The `AgentRunner._generate()` method routes to the LLM via the [ModelRouter](../models/model_routing.md):
...
- The [agents_router](../api/shared_api_routers.md) exposes agent CRUD via the builder.
...
- [decision_engines](../reference/decision_engines.md) — ComplianceEngine, DecisionEngine, HardBlockEngine
...
- [model_routing](../models/model_routing.md) — ModelRouter, classifier, gateway fallback chains
- [memory_system](../reference/memory_system.md) — PostgresMemory, RedisMemory, MemoryService
- [database](../storage/database.md) — AgentRecord, SkillRecord, SessionLocal
- [core_infrastructure](../infrastructure/core_infrastructure.md) — logger, config, kafka_producer, model_registry
- [store_layer](../storage/store_layer.md) — inbox_store, skill_loop_store
- [shared_api_routers](../api/shared_api_routers.md) — agents_router, governance_router
- [claude_gateway](../models/claude_gateway.md) — ClaudeGateway tool-use API
