...
| [core_workflow_repo](../workflows/core_workflow_repo.md) | `create_agent` / `update_agent` database operations. |
...
The `TriggerSection` component (see [triggers_feature](../ui/triggers_feature.md)) is embedded in Edit Mode, allowing users to configure webhook and scheduled triggers for the agent. It is disabled until the agent has been saved (`savedId` is set).
...
The preview chat intentionally mirrors the Workflow editor's `ChatPanel` (see [ChatPanel](../ui/ChatPanel.md)) for:
...
| [shared_features](../core/shared_features.md) | Shared feature utilities (downloadGeneratedFile, ExtractedTextPreview, etc.). |
...
| [triggers_feature](../ui/triggers_feature.md) | Trigger configuration and notification components. |
| [governance_feature](../sdlc/governance_feature.md) | Governance status badge and approval submission. |
| [ChatPanel](../ui/ChatPanel.md) | Workflow editor chat panel (mirrored patterns). |
| [core_workflow_repo](../workflows/core_workflow_repo.md) | Backend agent persistence layer. |
