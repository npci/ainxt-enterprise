...
| `triggers_feature` ([triggers_feature.md](../reference/triggers_feature.md)) | Trigger creation and execution history | Dashboard opens `TriggerModal` for a selected agent. |
| `common_components` ([common_components.md](../ui/common_components.md)) | Shared UI primitives | Uses `TemplatesEmptyState`. |
| `api_agents` ([api_agents.md](../api/api_agents.md)) | Backend CRUD for agents | Store calls these endpoints. |
| `api_agent_templates` ([api_agent_templates.md](../api/api_agent_templates.md)) | Template listing / instantiation | Store calls these endpoints. |
| `api_factories` ([api_factories.md](../api/api_factories.md)) | Agent factory chat and confirm | Factory chat uses these endpoints. |
| `api_governance` ([api_governance.md](../api/api_governance.md)) | Governance status and withdrawal | Dashboard polls and cancels pending approvals. |
| `api_triggers` ([api_triggers.md](../api/api_triggers.md)) | Trigger scheduling | Trigger modal persists schedules here. |
...
- **Agent management** ([api_agents.md](../api/api_agents.md)): Provides the CRUD backend for the user's agent library.
- **Template marketplace** ([api_agent_templates.md](../api/api_agent_templates.md)): Supplies reusable presets and handles template instantiation.
- **Factory pipeline** ([api_factories.md](../api/api_factories.md) / [agent_factory_pipeline.md](agent_factory_pipeline.md)): Powers the AI-assisted builder that turns natural language into a deployable agent.
- **Governance** ([api_governance.md](../api/api_governance.md) / [governance_feature.md](../sdlc/governance_feature.md)): Enforces approval workflows before an agent can be promoted to production.
- **Triggers** ([api_triggers.md](../api/api_triggers.md) / [triggers_feature.md](../reference/triggers_feature.md)): Allows agents to run on schedules or webhooks.
