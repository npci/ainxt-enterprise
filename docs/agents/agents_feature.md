...
- **Shared Features**: `FactoryChatShell`, `useFactoryChatStream`, `DownloadNotice`, and `ExtractedTextPreview` are shared by the agent, workflow, and skill factory chats. See [shared_features](../core/shared_features.md).
- **Governance Feature**: `StatusBadge` and `SubmitApprovalButton` are imported from [governance_feature](../sdlc/governance_feature.md).
- **Triggers Feature**: `TriggerSection`, `TriggerModal`, and `TriggerNotifications` are imported from [triggers_feature](../ui/triggers_feature.md).
