...
- **Common Components**: `CatalogPicker`, `ConfirmModal`, `HoverTooltip`, `KnowledgeSection`, `InlinePicker`, `PlanCard`, and `TemplatesEmptyState` are reused across agents, workflows, and skills. See [common_components](../ui/common_components.md).
- **Shared Features**: `FactoryChatShell`, `useFactoryChatStream`, `DownloadNotice`, and `ExtractedTextPreview` are shared by the agent, workflow, and skill factory chats. See [shared_features](../reference/shared_features.md).
- **Governance Feature**: `StatusBadge` and `SubmitApprovalButton` are imported from [governance_feature](../sdlc/governance_feature.md).
- **Triggers Feature**: `TriggerSection`, `TriggerModal`, and `TriggerNotifications` are imported from [triggers_feature](../reference/triggers_feature.md).
- **Workflow Editor**: The editor intentionally hides sub-asset attachment UI; sub-flow/sub-agent linking is configured from the workflow editor via `SubflowPicker`. See [workflow_editor](../workflows/workflow_editor.md).
