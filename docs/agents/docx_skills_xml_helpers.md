...
`docx_skills_xml_helpers` is one of the smallest but most specialized modules in the [shared_skills](shared_skills.md) layer. It supports the DOCX generation skills under [abstudio_backend](../ui/abstudio_backend.md) and the legacy `ainxt_docskills` tree. The cleaned XML it produces is ultimately consumed by:
...
Because the helpers are pure XML processors, they can also be reused by any other skill or worker that needs to post-process an extracted DOCX directory, such as the document-generation workers in [workers](../workers/workers.md).
