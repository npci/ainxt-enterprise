...
> See [core_config](../core/core_config.md) for the full resolution chain of `factory_base_url` and `factory_api_key`.
...
> See [core_llm_handler](../llm/core_llm_handler.md) for the `OpenAIClient`, `FallbackLLMClient`, and `Message` classes that execute the actual HTTP call.
...
- [core_config](../core/core_config.md) — `factory_base_url()` and `factory_api_key()` resolution chain, `LLM_PROXY_URL` routing.
- [core_llm_handler](../llm/core_llm_handler.md) — `OpenAIClient`, `FallbackLLMClient`, `Message`, `get_llm_client()`, and the `complete` / `complete_nonstream` / `complete_with_finish_reason` methods that execute the actual HTTP call.
...
- [skill_factory_pipeline](../skills/skill_factory_pipeline.md) — `SkillQualityLoop`, `_SkillCreatorGuidance`, `_CatalogCache` (caches the `build_service_index` result on the catalog TTL).
...
- [app_models](../core/app_models.md) — `LLMConfig` and `LLMProvider` data models.
