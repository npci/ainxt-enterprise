...
This router is intentionally narrow: it does **not** create, update, or delete agent definitions (see [`api_agents`](../api/api_agents.md) / [`agent_factory_pipeline`](../agents/agent_factory_pipeline.md)) and does **not** run agents (see [`api_execution`](../api/api_execution.md) / [`gateway`](../core/gateway.md)). Instead, it sits in front of the agent registry and knowledge document store to serve the agent catalog UI and related self-service operations.
...
- The actual embedding/activation into pgvector is **not** performed here; it is triggered when the agent is approved through [`governance_router`](../sdlc/governance_router.md).
...
- `undo_last_action` and `get_undo_stack` delegate to `agents.recovery_engine`.
...
- All endpoints require a valid user via [`get_current_user`](../auth/auth_router.md).
...
|------------|--------|---------|
| `get_current_user` | [`auth_router`](../auth/auth_router.md) / `auth.dependencies` | Authenticate requests. |
| `is_admin` | [`auth_router`](../auth/auth_router.md) / `auth.rbac` | Admin check for catalog visibility. |
| `AgentRecord`, `UserFavorite`, `AgentKbDoc`, `KnowledgeDocument` | `db/models` | ORM models. |
| `SessionLocal`, `VectorSessionLocal` | `db/database` | Relational and vector database sessions. |
| `logger` | `core/logger` | Structured logging. |
| `recovery_engine` | `agents/recovery_engine` | Undo stack operations. |

---

## Related Modules

- [`api_agents`](../api/api_agents.md) — CRUD operations for agent definitions in ABStudio.
- [`agent_factory_pipeline`](../agents/agent_factory_pipeline.md) — LLM-driven agent creation pipeline.
- [`governance_router`](../sdlc/governance_router.md) — Approves agents and triggers KB doc activation.
- [`docs_router`](../documents/docs_router.md) / [`kb_router`](../knowledge/kb_router.md) — Upload and manage knowledge documents.
- [`api_execution`](../api/api_execution.md) / [`gateway`](../core/gateway.md) — Run agents and workflows.
- `recovery_engine` — Reversible action tracking used by the undo endpoints.
