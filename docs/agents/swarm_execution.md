...
- **swarm_aggregation** — `SwarmAggregator.reduce()` transforms the `SharedBlackboard` into a parent-facing envelope. The `SharedBlackboard` provides per-run shared workspace with channel-based append-only writes and char-budgeted digests.
...
- **[core_config](../core/core_config.md)** — Provides `openai_compatible_base_url()` and `factory_model()` for the pre-flight setup snapshot in the JSON dump.
