...
- **Surfaces candidate skills**: Lists auto-detected skill proposals produced by the background [skill_loop_worker](../skill_loop_worker.md).
...
| --- | --- |
| Load and display proposals | `GET /skills/proposals` via [skills_router](../api/skills_router.md) |
...
| Approve / promote / reject | `POST /governance/skills/{skill_name}/{approve|promote|reject}` via [governance_router](../api/governance_router.md) |
...
The [skill_loop_worker](../skill_loop_worker.md) periodically scans for repeated successful run signatures. When a signature crosses the configured threshold, it:

1. Checks for duplicate open proposals and existing skills.
2. Synthesizes skill code via the shared [skill_synthesis](../skill_synthesis.md) service.
3. Runs a compliance hard-gate check through the [compliance_engine](../compliance_engine.md).
...
- The backend [skills_router](../api/skills_router.md) scopes the list:
...
- The destructive actions (`approve`, `promote`, `reject`) are enforced by the [governance_router](../api/governance_router.md), which validates scoped approver rights.
...
- **[skills_router](../api/skills_router.md)** — backend endpoint that lists proposals and enriches them with live skill status.
- **[governance_router](../api/governance_router.md)** — backend endpoint for `approve`, `promote`, and `reject` actions.
- **[skill_loop_worker](../skill_loop_worker.md)** — background worker that detects repeated runs and synthesizes candidate skills.
- **[skill_proposal_store](../skill_proposal_store.md)** — persistence layer for proposal records.
- **[skill_loop_store](../skill_loop_store.md)** — persistence layer for run-signature aggregation.
- **[skill_synthesis](../skill_synthesis.md)** — service that generates skill code from a description.
- **[compliance_engine](../compliance_engine.md)** — hard-gate validator for synthesized skill code.
- `usePermission` hook — frontend permission helper.
- `API_BASE` in [config](../infrastructure/config.md) — backend base URL.
