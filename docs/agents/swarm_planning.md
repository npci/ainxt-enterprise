...
- **[core_workflow_repo](../reference/core_workflow_repo.md)** — provides `list_tools()` and `list_skills()` used to build the manifest.
- **[core_llm_handler](../models/core_llm_handler.md)** / **[core_factory_utils](../reference/core_factory_utils.md)** — the orchestrator calls `call_factory_llm_with_finish_reason` and uses `extract_json_block` for robust JSON extraction.
- **[tools/spawn_swarm_tool](../reference/tools.md)** — the parent-facing entry point that invokes `SwarmRuntime.execute`.
- **[app_models](../models/app_models.md)** — workflow/agent node models may carry `parent_attached_tools`, `strict_scope`, and `allowed_extra_domains` that influence planning.
