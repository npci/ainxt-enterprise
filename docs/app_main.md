...
The `_seed_platform_skills()` coroutine is a critical startup task that loads Anthropic's official skill folders from `ABStudio/skills/ainxt-skills/` into the `skills_catalog` and `skill_files` PostgreSQL tables.

```mermaid
flowchart TD
    START([_seed_platform_skills]) --> CHECK_DIR{"skills/ainxt-skills/<br/>exists?"}
...
| `development` | `claude-api`, `frontend-design`, `mcp-builder`, `skill-creator`, `web-artifacts-builder`, `webapp-testing` |
...
> See [cli_runtime](cli_runtime.md) for details on CLI workspace and session management.
...
|---|---|---|
| `execution` | [api_execution](api_execution.md) | Workflow run/stream/resume endpoints |
| `chat` | [api_chat](api_chat.md) | Workflow chat thread management |
| `generation` | [api_generation](api_generation.md) | LLM-powered workflow/instruction generation |
| `documents` | [api_documents](api_documents.md) | Document extraction & agent runner attachments |
| `workflows` | [api_workflows](api_workflows.md) | Workflow CRUD |
| `templates` | [api_templates](api_templates.md) | Template listing & usage |
| `agents` | [api_agents](api_agents.md) | Agent CRUD |
| `agent_templates` | [api_agent_templates](api_agent_templates.md) | Agent template listing & usage |
| `mcp` | [api_mcp](api_mcp.md) | MCP server testing |
| `catalog` | [api_catalog](api_catalog.md) | Skills/tools catalog management |
| `triggers` | [api_triggers](api_triggers.md) | Trigger CRUD & execution history |
| `factories` | [api_factories](api_factories.md) | Agent/skill/workflow/agent-runner factory chat |
| `agent_chat` | [api_agent_chat](api_agent_chat.md) | Agent chat thread management |
| `kb` | [api_kb](api_kb.md) | Build-Studio KB upload proxy |
| `template_admin` | [api_template_admin](api_template_admin.md) | Feature-flagged template editor |
| `loops` | [api_loops](api_loops.md) | Loop/goal CRUD & governance |
| `governance` | [api_governance](api_governance.md) | Governance/approval bridge |
| `cli_mcp_router` | [cli_runtime](cli_runtime.md) | MCP tool plane for spawned CLI processes |
...
graph TD
    ...
    MAIN -->|Anthropic skills| SKILLS["ABStudio/skills/ainxt-skills/<br/>See: shared_skills"]
