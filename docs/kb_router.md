...
handled here; they live in [`docs_router.md`](docs_router.md).
...
`kb_router` sits on top of the same database tables that [`docs_router.md`](docs_router.md)
...
flowchart LR
    ...
    D --> H["KV store (Redis/RustyCluster)"]
    E --> G
...
[`models/kb_version_resolver.md`](models_kb_version_resolver.md) and follows
this precedence:
...
[`store/kb_doc_cache.md`](store_kb_doc_cache.md) to warm the full markdown
...
[`docs_router.md`](docs_router.md) during deprecation and deletion flows.
...
[`docs_router.md`](docs_router.md). For knowledge-graph consumption of these
...
- All endpoints require a valid user via `get_current_user`
  ([`auth/dependencies.md`](auth_dependencies.md)).
...
- **Diff** requires warming two full document payloads from
  [`store/kb_doc_cache.md`](store_kb_doc_cache.md). First access may be slower
...
- [`docs_router.md`](docs_router.md) — document upload, approval, deprecation
...
- [`models_kb_version_resolver.md`](models_kb_version_resolver.md) — the
  resolution cascade, lineage walk and section-level diff implementation.
- [`store_kb_doc_cache.md`](store_kb_doc_cache.md) — warmed document cache used
  by `diff_versions` and invalidated by `docs_router`.
- [`auth_dependencies.md`](auth_dependencies.md) — authentication dependency
