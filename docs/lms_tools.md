...
- Agents built with the [agent_factory_pipeline](agent_factory_pipeline.md).
- Workflows executed by the [engine_native_engine](engine_native_engine.md).
- Chat and agent-runner flows exposed by [api_factories](api_factories.md) and [api_agent_chat](api_agent_chat.md).

For details on how MCP servers are bootstrapped and discovered, see [mcp_system](mcp_system.md).
...
| Module | Relationship |
| --- | --- |
| [mcp_system](mcp_system.md) | Provides `BaseMCPServer`, `MCPTool`, and the registry that exposes `lms_tools` to agents. |
| [agent_factory_pipeline](agent_factory_pipeline.md) | May compose `lms_tools` into agent blueprints for training or tutoring use cases. |
| [engine_native_engine](engine_native_engine.md) | Executes workflows that invoke MCP tools such as `list_modules` and `save_learning_plan`. |
| [api_factories](api_factories.md) | Serves factory chat endpoints where agents are assembled and may bind to LMS tools. |
...
- [mcp_system](mcp_system.md) — MCP server framework and registration.
- [agent_factory_pipeline](agent_factory_pipeline.md) — Agent assembly and tool binding.
- [engine_native_engine](engine_native_engine.md) — Workflow execution engine that invokes MCP tools.
- [api_factories](api_factories.md) — Factory chat API where LMS tools are consumed.
