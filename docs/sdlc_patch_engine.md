...
Validation delegates to `SelfHealingEngine._execute()` (compile-only, no healing). For the full sandbox infrastructure see [sandbox](sandbox.md).
...
| `models.model_router.model_router` | LLM generation for patch prompts and Tier-2 region location. See [model_routing](model_routing.md). |
| `sandbox.self_healing_engine.SelfHealingEngine` | Compile-only sandbox execution (`_execute`). See [sandbox](sandbox.md). |
