...
4. **CI Content Indexing**  
   Accept file content from CI pipelines, chunk and embed it via the [embedding service](embedding_service.md), and persist vectors to `pgvector` for RAG retrieval.
...
Both routers rely on [auth.dependencies](auth.md) for authentication and authorization. Admin endpoints additionally require the `admin` role.
...
|------------|---------------------|----------------------|
| `auth.dependencies.get_current_user` | Authenticates all public endpoints via JWT, API key, or cookie. | [auth.md](auth.md) |
| `auth.dependencies.require_admin` | Enforces admin role on `/admin/templates/*` endpoints. | [auth.md](auth.md) |
| `core.config.EMBED_SVC_URL` | URL of the embedding microservice used by `_embed_chunks`. | [core_config.md](core_config.md) |
| `core.logger.logger` | Structured logging for operation initialization and indexing events. | [core_logger.md](core_logger.md) |
| `db.database.VectorSessionLocal` | SQLAlchemy session factory for the pgvector database. | [database.md](database.md) |
| `db.models.DocumentEmbedding` | ORM model for vector chunks written by `_write_pgvector`. | [db_models.md](db_models.md) |
| `embedding_service` | External microservice that produces text embeddings. | [embedding_service.md](embedding_service.md) |
...
The embedding service URL is read from `EMBED_SVC_URL` in [core/config.py](core_config.md) (default: `http://localhost:8001`).
...
- [gateway.md](gateway.md) — mounts this router under `/ainxt/v1/api`.
- [auth.md](auth.md) — authentication and authorization dependencies.
- [embedding_service.md](embedding_service.md) — microservice used for vector generation.
- [database.md](database.md) and [db_models.md](db_models.md) — pgvector session and `DocumentEmbedding` schema.
