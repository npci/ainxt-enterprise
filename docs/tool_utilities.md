...
These tools are intentionally low-level and state-mutating: each tool receives an `AgentState`, updates it, and returns it. They are invoked directly by the [agent_orchestration.md](agent_orchestration.md) loop and are also available to the broader [agent_system.md](agent_system.md) runtime.
...
- Calls `hybrid_retrieve_context` from [model_routing.md](model_routing.md) with the user's repo filter, user context, and query complexity.
...
- Calls `compliance_engine.analyze()` from [compliance_engine.md](compliance_engine.md).
...
`local_llm_tool` is a simple state toggle that sets `state.use_local_llm = True`. When this flag is set, `generate_answer_tool` routes the final generation through the in-house [local_llm_gateway.md](local_llm_gateway.md) instead of the cloud model router.
...
- **Model routing:** Uses [model_routing.md](model_routing.md) by default, with complexity upgrades when context is present. Honors `state.model_hint` and `state.use_local_llm`.
...
- **Fire-and-forget evaluation:** Spawns background threads to evaluate retrieval quality (Point C) and answer quality (Point D) via [core_infrastructure.md](core_infrastructure.md)'s `eval_engine`.
...
This is used by reasoning loops (e.g., [reaction_engines.md](reaction_engines.md)) to decide whether to retrieve more context or synthesize an answer.
...
- **[agent_orchestration.md](agent_orchestration.md)** (`OrchestratorAgent`) is the primary caller. It builds an execution plan, then dispatches `retrieve_tool`, `compliance_tool`, `local_llm_tool`, and `generate_answer_tool` in sequence.
- **[core_agent_framework.md](core_agent_framework.md)** (`AgentRunner`) uses the same underlying concepts but executes tools through the [mcp_system.md](mcp_system.md) registry rather than calling this module directly.
- **[reaction_engines.md](reaction_engines.md)** (`ReactEngine`, `ReactOrchestrator`) use `evaluate_context_tool` and similar retrieval patterns inside their iterative reasoning loops.
- **[model_routing.md](model_routing.md)** provides the hybrid retriever and model router consumed by `retrieve_tool` and `generate_answer_tool`.
- **[compliance_engine.md](compliance_engine.md)** provides the security scanning used by `compliance_tool`.
- **[local_llm_gateway.md](local_llm_gateway.md)** is the optional generation backend selected by `local_llm_tool`.
- **[core_infrastructure.md](core_infrastructure.md)** provides logging, telemetry, and the evaluation engine used for fire-and-forget quality scoring.
- **[n8n_tools.md](n8n_tools.md)** provides workflow-automation tools that are imported but defined outside this module.
...
- [agent_orchestration.md](agent_orchestration.md) – OrchestratorAgent that schedules these tools.
- [core_agent_framework.md](core_agent_framework.md) – AgentBuilder / AgentRunner runtime.
- [reaction_engines.md](reaction_engines.md) – ReAct loops that use context evaluation.
- [compliance_engine.md](compliance_engine.md) – Security and PCI compliance scanning.
- [model_routing.md](model_routing.md) – Hybrid retrieval and model routing.
- [local_llm_gateway.md](local_llm_gateway.md) – In-house local LLM backend.
- [core_infrastructure.md](core_infrastructure.md) – Logging, telemetry, and evaluation engine.
- [n8n_tools.md](n8n_tools.md) – n8n workflow automation tools.
- [mcp_system.md](mcp_system.md) – MCP registry used by AgentRunner for tool dispatch.
