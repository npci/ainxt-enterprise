...
`dependency_utilities` grouping. It relies on the [store_layer](store_layer.md)
for run-repo persistence and on [gitlab_tools](gitlab_tools.md) for all GitLab
API interactions.
...
|---|---|---|
| `store.sdlc_store.list_run_repos` | [store_layer](store_layer.md) | Retrieves all `sdlc_run_repos` rows for a given run, ordered by `build_order` then `repo` |
| `tools.gitlab_tools.gitlab_read_file` | [gitlab_tools](gitlab_tools.md) | Reads `.sdlc.yml` content from a GitLab repo at a specific branch |
| `tools.gitlab_tools.gitlab_create_branch` | [gitlab_tools](gitlab_tools.md) | Creates a feature branch for the manifest update (idempotent) |
| `tools.gitlab_tools.gitlab_create_or_update_file` | [gitlab_tools](gitlab_tools.md) | Commits the updated `.sdlc.yml` to the new branch |
| `tools.gitlab_tools.gitlab_create_mr` | [gitlab_tools](gitlab_tools.md) | Opens a Merge Request from the feature branch to the primary branch |
| `tools.gitlab_tools._find_existing_mr` | [gitlab_tools](gitlab_tools.md) | Checks for an existing open MR on the branch (idempotency) |
...
post-run reconciliation in the [sdlc_pipeline_workers](sdlc_pipeline_workers.md)
or [sdlc_state_machine](sdlc_state_machine.md) phases.
...
| [sdlc_pipeline_core](sdlc_pipeline_core.md) | Orchestrates the overall SDLC pipeline phases |
| [sdlc_state_machine](sdlc_state_machine.md) | Drives run state transitions; may invoke manifest update during reconciliation |
| [sdlc_pipeline_workers](sdlc_pipeline_workers.md) | Background workers that execute SDLC jobs and post-run tasks |
| [store_layer](store_layer.md) | Provides `sdlc_store.list_run_repos` for run-repo persistence |
| [gitlab_tools](gitlab_tools.md) | All GitLab API operations (file read, branch, commit, MR) |
| [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) | Broader SDLC pipeline agent and tooling context |
