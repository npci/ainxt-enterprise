...
Located in [`mcp/servers/document_tools_server.py`](mcp_servers.md), this class extends [`BaseMCPServer`](mcp_servers.md) and registers four tools from `document_tools_tools.py`:
...
It handles JSON-RPC dispatch, input/output compliance scanning, and audit logging through the shared MCP base class. See the [MCP server documentation](mcp_servers.md) for transport details (stdio, SSE, streamable HTTP).
...
| `mcp/servers/base.py` | MCP server base class, transport, compliance, audit | [mcp_servers.md](mcp_servers.md) |
| `mcp/servers/document_tools_server.py` | MCP server registration for these tools | [mcp_servers.md](mcp_servers.md) |
...
- **MCP registration:** `DocumentToolsMCPServer` registers the four tools during server initialisation. The server is typically exposed through the shared [MCP registry](mcp_system.md).
- **API documents layer:** The backend API for document uploads and agent-runner attachments uses the same [`core.document_parser`](core_document_parser.md) pipeline, ensuring consistency between interactive uploads and agent reads. See [api_documents.md](api_documents.md).
...
For document *generation* (DOCX, PPTX, XLSX), use [`doc_generator`](doc_generator.md). For knowledge-base upload and approval workflows, use the [documents API](api_documents.md) and [docs router](docs_router.md).
