...
|------------|--------|---------|
| `FactoryChatShell` | [shared_features](shared_features.md) | Reusable modal chat shell with message list, input, suggestions, and overlay slots |
| `useFactoryChatStream` | [shared_features](shared_features.md) | SSE streaming hook managing messages, stage, session, and thinking steps |
| `PlanCard` | [common_components](common_components.md) | Structured multi-question confirmation card with chip selection and free-text |
| `InlinePicker` | [common_components](common_components.md) | Tool/skill picker with catalog loading, search, and on-the-fly generation |
| `TriggerSection` | [triggers_feature](triggers_feature.md) | Post-deploy trigger configuration UI |
| `FactoryFileChips` | [shared_features](shared_features.md) | Download chips for generated files in messages |
| `DownloadNotice` | [shared_features](shared_features.md) | Transient error/success banner for file downloads |

### Backend Dependencies

| Dependency | Module | Purpose |
|------------|--------|---------|
| `agent_factory_chat` | [api_factories](api_factories.md) | SSE endpoint orchestrating the multi-turn agent creation pipeline |
| `agent_factory_confirm` | [api_factories](api_factories.md) | Persists the assembled agent to the database |
| `use_agent_template_route` | [api_agent_templates](api_agent_templates.md) | Adopts an existing agent template when user selects a match |
...
| `workflow_repo.create_agent` | [core_workflow_repo](core_workflow_repo.md) | Database persistence for the deployed agent |
