...
- Both preview components are typically rendered inside message bubbles by `Chat` ([chat](chat.md)) or `KbChat` ([kb_chat](kb_chat.md)).
- `DocPreviewCard` uses the same collapsible pattern as `AnswerCards` from [common_components](common_components.md).

### With Backend APIs

- The structured preview payloads consumed by `DocLivePreview` are produced by backend document-generation workers and exposed through the [doc_download_router](doc_download_router.md).
- For presentation-specific generation, the [presenton_lib](presenton_lib.md) and [ppt_wizard](ppt_wizard.md) modules may provide analogous streaming previews.
...
| [documents_chat](documents_chat.md) | Provides `DocPickerCard` for document selection in chat. |
| [documents_guide](documents_guide.md) | Provides `DocsPanel` for document guidance and management. |
| [doc_download_router](doc_download_router.md) | Backend router exposing document generation, streaming, and download endpoints. |
| [presenton_lib](presenton_lib.md) | Library for presentation-specific generation and layout handling. |
| [ppt_wizard](ppt_wizard.md) | Presentation wizard UI that may use live preview patterns. |
| [chat](chat.md) | Primary chat UI that hosts inline document previews. |
| [kb_chat](kb_chat.md) | Knowledge-base chat UI that may host document previews. |
