...
- **Model routing** ([`model_routing`](model_routing.md)) — sensitivity and
  risk can influence local-vs-cloud routing decisions.
- **Guardrails** ([`guardrails`](guardrails.md)) — operate independently on
...
| [`model_routing`](model_routing.md) | Uses sensitivity/risk to influence local-vs-cloud routing (see `classification_from_policy`). |
| [`guardrails`](guardrails.md) | Independent input-safety layer; complementary to `cil_policy` risk decisions. |
...
- [model_routing](model_routing.md) — Model selection, fallback, and privacy-floor classification.
- [guardrails](guardrails.md) — NeMo Guardrails input-safety checks.
