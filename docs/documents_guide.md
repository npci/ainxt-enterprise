...
|-------------|-------------------|---------------------------|
| **Chat** | [chat](chat.md) | RAG chat over indexed codebases. |
| **Agents** | [agents_catalog](agents_catalog.md) | Building and running autonomous AI agents. |
| **Agent Chains** | [workflows_feature](workflows_feature.md) | DAG-based multi-step automation pipelines. |
| **Skills** | [skills_feature](skills_feature.md) | Reusable Python tool functions for agents. |
| **Codebase** | [shared_core_knowledge_base](shared_core_knowledge_base.md) | Repository indexing and semantic retrieval. |
| **Projects** | [projects](projects.md) | Scoped AI workspaces for teams. |
| **Threads** | [threads](threads.md) | Team discussions with `@AiNxt` mentions. |
| **Tool Marketplace** | [marketplace_router](marketplace_router.md), [mcp_system](mcp_system.md) | MCP tool registry and external tool registration. |
| **Budget** | [budget_router](budget_router.md), [budget_manager](budget_manager.md) | Per-user LLM cost tracking and budget enforcement. |
| **SDLC Pipeline** | [sdlc_pipeline](sdlc_pipeline.md), [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) | Autonomous ticket-to-PR software development lifecycle. |
| **Monitoring** | [monitoring](monitoring.md) | Platform health, circuit breakers, job queues, Prometheus metrics. |
| **Analytics** | [agent_analytics](agent_analytics.md) | Per-agent performance and cost analytics. |
| **AiNxt Coach** | [coach](coach.md), [coach_evaluator](coach_evaluator.md) | AI-usage coaching and prompt-practice scoring. |
...
`DocsPanel` is part of the `ai_ui_frontend/documents` subtree, specifically the `documents_guide` leaf. It is rendered inside the main `ai-ui` application shell (see [ai_ui_frontend_app_core](ai_ui_frontend_app_core.md)) and is reachable through the application sidebar navigation.
...
- [ai_ui_frontend_app_core](ai_ui_frontend_app_core.md) — main application shell that hosts `DocsPanel`.
- [chat](chat.md) — RAG chat capability.
- [agents_catalog](agents_catalog.md) — agent catalog and chat.
- [workflows_feature](workflows_feature.md) — workflow/agent-chain builder.
- [skills_feature](skills_feature.md) — skills dashboard and factory chat.
- [shared_core_knowledge_base](shared_core_knowledge_base.md) — codebase indexing and retrieval.
- [projects](projects.md) — scoped project workspaces.
- [threads](threads.md) — threaded team discussions.
- [marketplace_router](marketplace_router.md) — tool marketplace API.
- [mcp_system](mcp_system.md) — MCP tool registry.
- [budget_router](budget_router.md) — budget management API.
- [budget_manager](budget_manager.md) — budget UI.
- [sdlc_pipeline](sdlc_pipeline.md) — SDLC pipeline UI.
- [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md) — SDLC pipeline backend.
- [monitoring](monitoring.md) — platform monitoring dashboard.
- [agent_analytics](agent_analytics.md) — agent analytics dashboard.
- [coach](coach.md) — AI-usage coaching UI.
- [coach_evaluator](coach_evaluator.md) — coaching rule evaluation engine.
