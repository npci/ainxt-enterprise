...
| [codebase_manager](../codebase/codebase_manager.md) | Manages repository access and indexing; the `repo` identifiers in the payload originate from the codebase/product configuration. |
| [shared_core agent_system workspace_utilities](../core/shared_core.md) | Backend `agents/multi_repo_workspace.py` builds the multi-repo workspace and produces the `repos` payload. |
...
* The backend contract for `repos` is defined by the multi-repo workspace builder in [shared_core agent_system workspace_utilities](../core/shared_core.md). Any change to the `RepoEntry` shape must be kept in sync with both the backend producer and this frontend consumer.
...
* [codebase_manager](../codebase/codebase_manager.md) — repository and codebase management.
* [shared_core](../core/shared_core.md) — backend multi-repo workspace builder (`agents/multi_repo_workspace.py`).
