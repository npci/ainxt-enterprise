...
1. **Governance Engine** (`agents/sdlc_governance/engine.py`)
   ...
   See sdlc_governance_engine for detailed engine behavior.
...
3. **SDLC Agent Loop** (`agents/sdlc_agent_loop.py`)
   ...
   See [sdlc_agent_loop](sdlc_agent_loop.md) for the agent loop implementation.
...
4. **Model Registry** (`core/model_registry.py`)
   ...
   See core_model_registry for model resolution logic.
...
- sdlc_governance_engine — Governance engine that consumes this configuration.
- [sdlc_pipeline](sdlc_pipeline.md) — SDLC pipeline orchestration and fix-loop integration.
- [sdlc_agent_loop](sdlc_agent_loop.md) — Shared environment-variable helper functions.
- [sdlc_cli_engine](sdlc_cli_engine.md) — CLI session execution engine.
- core_model_registry — Model constants and resolution helpers.
