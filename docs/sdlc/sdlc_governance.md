...
- **Upstream callers**: [sdlc_pipeline](shared_core_sdlc_pipeline.md), [sdlc_state_machine](shared_core_sdlc_pipeline.md), [sdlc_agent_loop](shared_core_sdlc_pipeline.md), [sdlc_router](../api/shared_api_routers.md), and the front-end [SDLCPipeline](../ai_ui_frontend_sdlc_pipeline.md) / [GovernanceReviewPanel](../ai_ui_frontend_sdlc_governance_review.md).
- **Downstream runtime**: [sdlc_cli_engine](shared_core_sdlc_pipeline.md) executes the actual LLM sessions; [sdlc_worker](../workers/sdlc_pipeline_workers.md) runs the jobs asynchronously.
...
- **Cost accounting**: every review and scan session records token/cost usage via `sdlc_cli_budget` so governance consumes HOD budget like any other SDLC CLI call.
...
- [sdlc_governance_engine](../sdlc_governance_engine.md) – skill resolution, staging, review/scan sessions, suppressions, fixer prompts, and reporting.
...
- [sdlc_pipeline_workers](../workers/sdlc_pipeline_workers.md) – asynchronous workers that execute governance pipeline jobs.
- [ai_ui_frontend_sdlc_pipeline](../ai_ui_frontend_sdlc_pipeline.md) – front-end SDLC pipeline view that triggers and displays governance runs.
- [ai_ui_frontend_sdlc_governance_review](../ai_ui_frontend_sdlc_governance_review.md) – front-end governance review panel for findings and approvals.
- [shared_api_routers](../api/shared_api_routers.md) – API surface including `sdlc_router` and `governance_router`.
