...
|--------|---------|-----------|
| `ai-ui/src/config.js` | Provides `API_BASE` and `apiFetch` for authenticated backend calls. | config |
...
The PLAN artifact is generated by the backend SDLC pipeline workers (see [sdlc_pipeline_workers](sdlc_pipeline_workers.md) and [shared_core_sdlc_pipeline](shared_core_sdlc_pipeline.md)). The frontend retrieves it via the SDLC router (see [sdlc_router](sdlc_router.md)).
...
- [sdlc_router](sdlc_router.md) — Backend router serving the artifact endpoint.
- [sdlc_pipeline_workers](sdlc_pipeline_workers.md) — Workers that generate PLAN artifacts.
