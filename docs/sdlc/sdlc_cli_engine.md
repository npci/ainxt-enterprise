...
- **[sdlc_agent_loop](sdlc_agent_loop.md)** — The bounded Anthropic tool-use loop (`AgentLoop`) used for recovery-on-red scenarios. Provides the env-reader helpers (`_env_str`, `_env_int`, `_service_api_key`, `_looks_truncated_json`) reused by this module.
