...
Redis is **best-effort**: if the KV client is unavailable, the router degrades gracefully to Postgres and in-memory sources. See core_kv for the KV client abstraction.
...
See db_models for model definitions.

### Audit Signing

Every `_pg_record_event` call computes an HMAC-SHA256 signature over the canonical event dict via `core.audit_signer.sign_event` and stores it in `governance_events.signature`. This makes the audit log tamper-evident — the same scheme used for `sdlc_run_events`. See core_audit_signer.
...
- **`_resolve_hod_user_ids(department)`** — query `DepartmentHodMapping` → active `User` IDs for the department's HOD(s). Returns `[]` immediately, before consulting the mapping table at all, when `core/config.py::HOD_APPROVAL_ENABLED` is `False` (the default — flat/admin-only mode, for deployments with no department/HOD hierarchy). This is the module's only change for that flag; every other line here, and the whole approval/notification pipeline below, is unchanged — it simply always takes the "no HOD mapped" branch.
...
- **`_require_scoped_approver(entity_type, name, current_user)`** — scope-aware guard for approve/reject/promote/deprecate:
  ...
  - **Flat/admin-only mode** (`HOD_APPROVAL_ENABLED=false`, the default): `_resolve_hod_user_ids` always returns `[]` (above), so every private item's "HOD mapped?" check fails and the "else admin only" branch fires unconditionally — private workflow/skill/agent approval, rejection, promotion, and deprecation become admin-only for every department, with this guard's own pre-written *"No HOD is mapped for department '&lt;dept&gt;'; this item requires admin approval"* 403 shown to anyone else. Public items were already admin-only regardless of this flag. Existing HOD-hierarchy deployments must set `HOD_APPROVAL_ENABLED=true` to keep the HOD branch reachable.
...
| Module | Usage |
|---|---|
| auth_dependencies | `get_current_user` — JWT/API-key/cookie authentication for all endpoints |
| auth_rbac | `is_admin`, `is_hod`, `get_hod_departments` — HOD-scoped authorization |
| core_kv | `get_kv`, `KVError` — Redis KV client for entity cache and marketplace sync |
| core_logger | `logger` — structured logging |
| [core_config](../core/core_config.md) | `REDIS_HOST`, `REDIS_PORT`, `RDB_WORKFLOW`, `RDB_REGISTRY` — KV configuration |
| core_notifications | `notify` — Slack/Teams/email notifications |
| core_audit_signer | `sign_event` — HMAC-SHA256 audit event signing |
| db_models | `GovernanceEvent`, `AgentRecord`, `SkillRecord`, `MCPServer`, `WorkflowRecord`, `User`, `DepartmentHodMapping`, `InboxItem` |
| store_inbox_store | `publish_inbox_item` — inbox notification delivery |
| agents_agent_builder | `agent_builder` singleton — in-memory agent cache and hot-reload |
| mcp_registry | `mcp_registry` singleton — in-memory MCP tool/skill registry |
| app_core_workflow_repo | `publish_workflow_as_template`, `publish_agent_as_template`, `get_workflow_by_name`, `get_agent_by_name`, `get_skill`, `delete_agent`, trigger cleanup |
| app_services_trigger_scheduler | `deregister_trigger` — trigger cleanup on published-agent deletion |
| app_api_agent_chat | Agent chat store — thread cleanup on published-agent deletion |
...
The governance router is the **approval gateway** for artifacts created in ABStudio's Build Studio. When a user clicks "Deploy" on a workflow or agent, the frontend calls `POST /governance/{entity_type}/{name}/submit`. Upon approval, the artifact is published as a shared template via `workflow_repo`. See [api_governance](../api/api_governance.md) for the ABStudio-side governance API wrappers.
