...
- `sdlc_governance_engine` — `run_review()` and `run_scan_session()` read `review_turns()`, `scan_turns()`, `scan_profile()`, `review_model()`, and `fix_model()`.
- `sdlc_governance_bundle` — `resolve_bundle()` and `discover_skills()` read `source()`, `git_url()`, `git_ref()`, `bundle_path()`, and `pin_version()`.
- `sdlc_governance_schema` — `is_blocking()` uses `block_severity()` as the default threshold.
...
- [`sdlc_agent_loop`](sdlc_agent_loop.md) — shares the `_env_str` / `_env_int` idiom for call-time env reading.
...
Reads `SDLC_GOVERNANCE_PIN` (default `true`). When true, the bundle resolver records the exact resolved commit SHA or path state, ensuring reproducible governance runs. See `sdlc_governance_bundle` for how the pinned reference is stored and used.

### `require_binaries() -> bool`

Reads `SDLC_GOVERNANCE_REQUIRE_BINARIES` (default `true`). When true, a governance skill whose `SKILL.md` references a binary that is absent on the worker host causes the scan session to **suspend** (fail-closed) rather than silently pass. This is enforced in `sdlc_governance_engine` during scan session setup.
...
- **Call-time env reading**: Every public function reads `os.environ` when invoked. This mirrors the pattern in [`sdlc_agent_loop`](sdlc_agent_loop.md) and [`sdlc_cli_engine`](sdlc_cli_engine.md), allowing operators to flip governance behavior at deploy time without restarting workers.
...
- `sdlc_governance_engine` — uses config to drive review and scan sessions.
- `sdlc_governance_bundle` — uses config to resolve and discover governance skill bundles.
- `sdlc_governance_schema` — finding schema and blocking logic.
- [`sdlc_pipeline`](sdlc_pipeline.md) — orchestrates governance within the broader SDLC pipeline.
- [`sdlc_agent_loop`](sdlc_agent_loop.md) — shared call-time env-reading helpers.
- `core/model_registry` — model constants and tier resolution.
