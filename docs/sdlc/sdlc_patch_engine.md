...
The engine is a singleton (`patch_engine = PatchEngine()`) and is consumed by the SDLC state machine, the agentic coding loop, and the post-gate verified-diff applier.  For the broader pipeline context see [sdlc_state_machine](sdlc_state_machine.md) and [sdlc_agent_loop](../agents/sdlc_agent_loop.md).
...
Validation delegates to `SelfHealingEngine._execute()` (compile-only, no healing). For the full sandbox infrastructure see [sandbox](../storage/sandbox.md).
...
| `models.model_router.model_router` | LLM generation for patch prompts and Tier-2 region location. See [model_routing](../models/model_routing.md). |
| `sandbox.self_healing_engine.SelfHealingEngine` | Compile-only sandbox execution (`_execute`). See [sandbox](../storage/sandbox.md). |
...
| `AgentLoop` (code profile) | The agentic loop's `propose_edit` tool uses the same SEARCH/REPLACE format; the loop's `validate_fn` is often backed by `patch_engine._validate()`. See [sdlc_agent_loop](../agents/sdlc_agent_loop.md). |
