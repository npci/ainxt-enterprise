...
| **Authenticate callers** | Every endpoint requires a valid JWT session, API key, or `auth_token` cookie via auth.dependencies. |
...
- **Router stays thin**: request parsing, auth, and dispatch only. All security logic lives in the worker so it can be reused by other entry points (e.g., background jobs, the [compliance_scan_router](../security/compliance_scan_router.md)).
...
- **Shared job queue**: the `/status/{job_id}` endpoint reuses the platform-wide job_queue so deep-tier scans can be polled consistently with other long-running tasks.
...
Polls the status of an enqueued scan job. It calls core.job_queue.get_job_status and returns RQ metadata such as `status`, `result`, `error`, and timestamps.
...
| ---------- | ------------------- |
| auth.dependencies | Validates JWT / API key / cookie and enriches the user context. |
| core.logger | Structured logging for scan activity. |
| core.job_queue | Provides `get_job_status` for the async status endpoint. |
| workers.secure_code_gate_worker | Performs the actual SAST scan and optional LLM fix loop. |
...
- **Related modules**:
  - secure_code_gate_worker — the scanning/fixing engine.
  - [compliance_scan_router](../security/compliance_scan_router.md) — broader compliance/image scanning endpoints.
  - security_scan_worker — general security scan worker used elsewhere in the platform.
  - job_queue — shared RQ-based job status infrastructure.
...
- **Audit trail**: every scan is logged with file count, finding count, blocked count, and fix count via core.logger.
