...
- **[core_infrastructure.md](../core/core_infrastructure.md)** — `core.model_registry.sdlc_stage_hint` for model tier resolution; `core.logger` for structured logging.
- **[messages_compat_router.md](../chat/messages_compat_router.md)** — The frozen `/v1/messages` endpoint (`messages_endpoint`, `_compliance_check`, `_translate_in_msgs`) that enforces auth, budget, compliance, and multi-provider routing.
