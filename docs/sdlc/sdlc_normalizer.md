...
The normalizer does not call an LLM directly. It uses the shared `models.model_router` so that:
...
The routing hint is resolved via `core.model_registry.sdlc_stage_hint()` with stage `"normalize"`. The default hint is `haiku` for speed and cost efficiency.

### Logging and Observability

All normalization events are emitted through `core.logger`. Log lines include the `run_id` prefix `[NORM {run_id}]` and capture:
...
For the full model routing configuration, see core_model_registry.md.
...
|-----------|--------------|
| `models.model_router` | Routes the extraction prompt to the appropriate LLM gateway. |
| `core.model_registry` | Resolves the `normalize` stage model hint. |
| `core.logger` | Structured logging for normalization events. |
...
| `sdlc_store` | Persists run state and artifacts (referenced by the broader pipeline). |
