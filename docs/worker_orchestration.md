...
| [kv_store](kv_store.md) | Provides `core/kv/queue.py` — the worker/connection factory that abstracts Redis vs. RustyCluster |
| [core_infrastructure](core_infrastructure.md) | Provides `core/config.py`, `core/logger.py`, `core/ckms/`, `core/job_queue.py` — foundational imports |
| [gateway](gateway.md) | The API gateway that enqueues jobs consumed by workers spawned here; also prepends `ABStudio/backend` to `sys.path` so worker imports resolve |
