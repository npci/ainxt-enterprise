...
- **[tools_canonical_seed](tools_canonical_seed.md)** — Platform utilities such as `code_executor` and `read_skill_file` are often co-injected with `spawn_swarm`; the ordering gate in callers ensures `spawn_swarm` and purpose-built tools are tried before `code_executor`.
- **[tools_m365_bridge](tools_m365_bridge.md)** / catalog tools — The swarm orchestrator scopes worker tool manifests from the live catalog; this module does not know tool semantics.
...
- [tools_canonical_seed](tools_canonical_seed.md) — platform utility tools co-injected with `spawn_swarm`.
- [app_models](app_models.md) — request/response models for agent and workflow execution.
