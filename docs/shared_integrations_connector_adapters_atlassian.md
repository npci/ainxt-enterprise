...
- **JiraAdapter** is a thin dispatch layer that injects per-user credentials and delegates all HTTP work to [`tools/jira_tools.py`](shared_integrations_jira_tools.md). This ensures the connector path and the SDLC pipeline share identical code for proxy relay, circuit breaking, retry, and tracing.
...
A thin dispatch layer over [`tools/jira_tools.py`](shared_integrations_jira_tools.md). It does **not** make HTTP calls directly. Instead, it:
...
| [`tools/jira_tools.py`](shared_integrations_jira_tools.md) — `set_credentials`, `clear_credentials`, 13 tool functions | Shared Jira HTTP client with proxy, circuit breaker, retry |
...
| [LLM Proxy `atlassian_proxy`](llm_proxy_main.md) | Production egress relay for Jira calls (reachable only from `web02`) |
...
- **Connectors Router** (`routers/connectors_router.py`) — manages connector definitions, OAuth/PAT connections, permissions, and execution. See [shared_api_routers_connectors_router](shared_api_routers_connectors_router.md).
- **MCP Servers** — `JiraMCPServer` and `ConfluenceMCPServer` expose Jira/Confluence tools to LLM agents via the MCP protocol. See [mcp_servers](mcp_servers.md).
...
- [shared_integrations_jira_tools](shared_integrations_jira_tools.md) — shared Jira HTTP client used by both the adapter and SDLC pipeline
- [shared_integrations_confluence_tools](shared_integrations_confluence_tools.md) — standalone Confluence tool functions (SDLC path)
- [llm_proxy_main](llm_proxy_main.md) — LLM proxy service including the `atlassian_proxy` endpoint
- [shared_api_routers_connectors_router](shared_api_routers_connectors_router.md) — REST API for connector management
- [mcp_servers](mcp_servers.md) — MCP server implementations for Jira and Confluence
