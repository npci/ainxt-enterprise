...
|----------------|-------------------------------|
| **[chat_and_messaging](chat_and_messaging.md)** | `ask_submit` / `ask_stream` invoke `hybrid_retrieve_context` for RAG-augmented chat. The `rag_mode` (`off`/`auto`/`on`) controls whether retrieval runs. |
| **[agent_management](agent_management.md)** | Agent runs use `hybrid_retrieve_context` for codebase context injection. Agent KB docs are indexed under `agent_kb:{agent_id}` namespace. |
| **[worker_orchestration](worker_orchestration.md)** | `start_workers.py` launches the RQ worker process that consumes `Q_INDEX` jobs. |
| **[kafka_event_consumer](kafka_event_consumer.md)** | `_handle_embeddings` consumer processes indexing telemetry events from `TOPIC_EMBEDDINGS`. |
| **[sandbox](sandbox.md)** | `SandboxImageBuilder` reads `repo_build_metadata` (populated by `BuildMetadataExtractor`) to determine build tool, language version, and registry configuration for SDLC sandbox images. |
| **[sdlc_pipeline_agents](sdlc_pipeline_agents.md)** | SDLC agents use `sdlc:conventions:{repo}` (cached by `_derive_and_cache_conventions`) for codebase-aware code generation. Graph traversal supports impact analysis. |
