...
The module lives inside the `KbChat` React component and depends on state managed by that parent. It is a leaf concern of the larger [kb_chat_core_chat](kb_chat_core_chat.md) module.
...
- **KB Chat Context**: The module is a sub-feature of [kb_chat_core_chat](kb_chat_core_chat.md). It reuses the active chat state (`activeChatId`, `activeChat.messages`, `messages`) and the `authFetch` utility from [config](config.md).
- **Quality Loop**: Feedback is the human signal that closes the [feedback_processor](feedback_processor.md) learning loop. Thumbs-up data is mined for technology preferences and stored in memory; thumbs-down data is correlated with RAG access logs to penalize low-quality chunks.
...
|------------|--------|---------|
| `authFetch`, `API_BASE` | [config](config.md) | Authenticated HTTP requests to the backend. |
| `useToast` | [DialogProvider](ui_dialog.md) | Success / error toast notifications. |
| `activeChat`, `activeChatId`, `messages` | [kb_chat_core_chat](kb_chat_core_chat.md) | Chat state consumed by the handlers. |
| `feedbackModal`, `feedbackIssue`, `feedbackSubIssue`, `feedbackComment`, `feedbackMap` | [kb_chat_core_chat](kb_chat_core_chat.md) | Local state for the feedback modal and ratings. |
...
| `FeedbackProcessor` | [feedback_processor](feedback_processor.md) | Extracts preferences and computes chunk penalties. |
| `run_feedback_loop` | [feedback_loop_worker](feedback_loop_worker.md) | Hourly cron that invokes the feedback processor. |
...
- [kb_chat_core_chat](kb_chat_core_chat.md) — parent chat container and state management.
...
- [feedback_processor](feedback_processor.md) — learning loop that consumes feedback.
- [feedback_loop_worker](feedback_loop_worker.md) — cron worker that schedules feedback processing.
- [config](config.md) — `authFetch` and API base configuration.
