...
| Approve / promote / reject | `POST /governance/skills/{skill_name}/{approve|promote|reject}` via [governance_router](governance_router.md) |
...
- The destructive actions (`approve`, `promote`, `reject`) are enforced by the [governance_router](governance_router.md), which validates scoped approver rights.
...
- **[governance_router](governance_router.md)** — backend endpoint for `approve`, `promote`, and `reject` actions.
...
- `API_BASE` in [config](config.md) — backend base URL.
