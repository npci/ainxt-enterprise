...
| [`Message.jsx`](message.md) | Shared markdown | Supplies `mdComponents` so fallback Markdown rendering matches the chat experience. |
| [`config.js`](config.md) | Shared utility | Provides `authFetch` (cookie-authenticated fetch) and `API_BASE`. |
...
For details on the backend implementation of these endpoints, see [`docs_router`](docs_router.md) and [`doc_download_router`](doc_download_router.md). The workers that execute the jobs are documented in [`doc_worker`](doc_worker.md) and [`doc_worker_agent`](doc_worker_agent.md).
...
- [`message`](message.md) — shared Markdown rendering components.
- [`config`](config.md) — `authFetch` and API base configuration.
- [`docs_router`](docs_router.md) — backend version history and preview endpoints.
- [`doc_download_router`](doc_download_router.md) — backend revision, job status, cancel, and download endpoints.
