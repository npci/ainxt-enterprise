...
- [`shared_api_routers.governance_router`](shared_api_routers_governance_router.md) — the canonical governance REST API and state transitions.
- [`core_governance_client`](core_governance_client.md) — the client library used by `api_governance` to interact with the platform router.
- [`core_governance`](core_governance.md) — lower-level governance utilities and policy enforcement (e.g., tool policy denial).
...
- [`governance_feature.StatusBadge`](governance_feature.md) — polls and displays the governance status.
- [`governance_feature.SubmitApprovalButton`](governance_feature.md) — triggers submit/withdraw actions.

### Relationship to Execution

Once an artifact is approved, the execution paths can rely on the governance mirror record. For example, [`core_governance_client.is_usable`](core_governance_client.md) is used elsewhere to check whether a governed artifact may be run. A missing governance record is treated as **not usable** (fail-closed), ensuring unapproved artifacts cannot be executed.
...
|--------|---------------|---------|
| `AuthenticatedUser` | [`app_models`](app_models.md) | Typed user context from the JWT. |
| `require_access` | [`api_deps`](api_deps.md) | Authentication/authorization dependency. |
| `workflow_repo` | [`core_workflow_repo`](core_workflow_repo.md) | Reads workflows, agents, and skills from Build Studio tables. |
| `governance_client` | [`core_governance_client`](core_governance_client.md) | Submits, withdraws, and queries governance status via the platform router. |
| `logger` | `core.logger` | Structured logging. |

### Upstream Dependencies

- [`shared_api_routers.governance_router`](shared_api_routers_governance_router.md) — provides `_get_entity_status`, `_transition`, `_governance_notify`, and the mirror-record upsert logic.
...
- [`app_models`](app_models.md) — shared Pydantic models including `AuthenticatedUser`.
- [`core_workflow_repo`](core_workflow_repo.md) — Build Studio artifact repository.
- [`core_governance_client`](core_governance_client.md) — governance client library.
- [`core_governance`](core_governance.md) — governance utilities and policy enforcement.
- [`shared_api_routers.governance_router`](shared_api_routers_governance_router.md) — platform governance router.
- [`governance_feature`](governance_feature.md) — Build Studio frontend governance UI.
