...
These servers inherit from [`BaseMCPServer`](mcp_servers_base.md) and are registered through the shared [`MCPRegistry`](shared_core.md#mcp_system), allowing them to be consumed by the agent runtime, workflow engine, and CLI runtime.
...
Wraps [`tools/data_tools_tools.py`](shared_integrations.md#data_tools) to expose tabular-file operations as MCP tools. It is intended for deterministic analysis of CSV/XLSX files stored under a configured data root.
...
Wraps [`tools/kb_search_tools.py`](shared_integrations.md#kb_search_tools) to let agents retrieve grounded knowledge for reply drafting, HR policy Q&A, and RFP content lookup.
...
- `DataToolsMCPServer` and `KBSearchMCPServer` delegate to the shared tool layer documented in [`shared_integrations`](shared_integrations.md).
- `DatabaseMCPServer` uses [`db.database.SessionLocal`](shared_core.md#database) directly, bypassing the tool layer because it needs fine-grained control over SQL parsing, parameter binding, and masking.
...
1. **Agent runtime** — [`AgentBuilder`](shared_core.md#agent_system) / [`MultiAgentRunner`](shared_core.md#agent_system) resolves tool names through [`ToolRegistry`](shared_core.md#mcp_system) and calls the appropriate MCP server.
2. **Workflow engine** — [`WorkflowEngine`](shared_core.md#workflow_system) and [`NativeEngine`](abstudio_backend.md#engine_native_engine) can include MCP nodes that invoke `data_tools` or `kb_search` tools.
3. **CLI runtime** — [`AbstudioMcpServer`](abstudio_backend.md#cli_runtime) and the desktop client use stdio/streamable HTTP to reach these servers.
4. **Gateway** — [`mcp_server_router`](shared_api_routers.md#mcp_server_router) exposes SSE and streamable HTTP endpoints that route to the running MCP server instances.
...
- [`shared_integrations.md`](shared_integrations.md) — underlying tool implementations (`data_tools_tools.py`, `kb_search_tools.py`).
- [`shared_core.md`](shared_core.md) — database layer, MCP registry, and audit infrastructure.
- [`abstudio_backend.md`](abstudio_backend.md) — workflow engine, CLI runtime, and API routes that consume these servers.
- [`shared_api_routers.md`](shared_api_routers.md) — `mcp_server_router` exposing SSE/streamable HTTP endpoints.
