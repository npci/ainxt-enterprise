...
[`gateway.py`](gateway.md). They keep API keys server-side, enforce
...
(the "no-lazy-load" rule), either in the dedicated [`llm_proxy`](llm_proxy.md)
...
1. If `LLM_PROXY_URL` is configured, the request is forwarded to
   [`llm_proxy /llm/tts`](llm_proxy.md#llm_tts). This is the production path
...
|---------|-----------|---------|---------|
| `llm_proxy` | [`llm_proxy.md`](llm_proxy.md) | `voice_tts` | Production TTS proxy with outbound internet access. |
...
* [`gateway.md`](gateway.md) — the host module providing routing, auth, and
  common exception handling.
* [`auth.md`](auth.md) / [`shared_core.md`](shared_core.md) — identity and RBAC
  via `_require_auth`.
...
* The [`ai_ui_frontend`](ai_ui_frontend.md) chat components use browser-native
...
* The [`gateway`](gateway.md) module hosts these endpoints alongside chat,
  agent, workflow, and audit routes, reusing the same auth and logging
  infrastructure.
* The [`llm_proxy`](llm_proxy.md) service handles the actual OpenAI TTS call in
