...
`ToolCard` and `ToolGroup` live inside the monolithic `Chat.jsx` component. They are imported/rendered as part of the message content pipeline, alongside other chat sub-features such as message actions, file/image handling, enhancement features, and voice input. The [chat](chat.md) module owns the message state, streaming, and overall layout; the tool integration module owns only the visual representation of tool events.
...
- **Frontend chat experience** ([chat](chat.md)) – receives and displays streaming messages.
- **Backend tool execution** ([shared_integrations](shared_integrations.md), [agent_system](agent_system.md)) – produces the tool events being rendered.
- **Message rendering** ([message](message.md)) – the broader message component ecosystem that includes markdown, code blocks, images, and downloadable artifacts.

A similar pattern may be reused in knowledge-base chat ([kb_chat](kb_chat.md)), which also supports tool calls and attachments.
...
- [chat](chat.md) – Parent chat module that owns message state, streaming, and the overall chat UI.
- [message](message.md) – General message rendering including markdown, code blocks, and artifacts.
- [kb_chat](kb_chat.md) – Knowledge-base chat interface with similar tool-call and attachment handling.
- [agent_system](agent_system.md) – Backend agents that invoke tools and emit tool events.
