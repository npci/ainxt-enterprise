...
sequenceDiagram
    ...
    participant BE as Backend
    participant KV as KV Store (Redis/RustyCluster)
...
|---|---|---|
| `useDesktop.js` | [ai_ui_frontend_hooks](ai_ui_frontend_hooks.md) | Electron bridge: `isDesktop`, `pickFolder`, `listFolder`, `readFile`, `watchFolder`, `unwatchFolder`, `getWatchedFolders`, `onWorkspaceChange`/`offWorkspaceChange`, `onClipboardChange`/`offClipboardChange`, `onShortcutContext`/`offShortcutContext`, `getMcpPort`, `onMcpServerReady` |
| `config.js` | [config](config.md) | `authFetch` — authenticated HTTP wrapper that attaches JWT cookie credentials and correlation IDs |

### Backend Dependencies

| Endpoint | Method | Backend Module | Description |
|---|---|---|---|
| `/desktop/index/file` | POST | [desktop_router](desktop_router.md) | Inline single-file indexing (incremental) |
| `/desktop/index/batch` | POST | [desktop_router](desktop_router.md) | Batch indexing via RQ background job |
| `/desktop/index/{workspace}` | DELETE | [desktop_router](desktop_router.md) | Remove all vectors for a workspace |
| `/desktop/index/{workspace}/status` | GET | [desktop_router](desktop_router.md) | Chunk count and last-indexed timestamp |
| `/desktop/register-mcp` | POST | [desktop_router](desktop_router.md) | Register local MCP server with backend |
...
> For full documentation of the desktop bridge, see [ai_ui_frontend_hooks](ai_ui_frontend_hooks.md).
...
| Module | Relationship |
|---|---|
| [ai_ui_frontend_hooks](ai_ui_frontend_hooks.md) | Provides the `useDesktop.js` Electron bridge used for all desktop interactions |
| [config](config.md) | Provides `authFetch` for authenticated API calls |
| [desktop_router](desktop_router.md) | Backend router handling all `/desktop/*` endpoints |
| [chat](chat.md) | Parent component that receives `onAskWithContext` prompts from clipboard and workspace actions |
