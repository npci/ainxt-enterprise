...
- **API routers** such as `n8n_router` (in `shared_api_routers`), which provides HTTP endpoints for listing, creating, triggering, and managing n8n workflows. See [shared_api_routers.md](shared_api_routers.md) and specifically the `n8n_router` sub-section.
