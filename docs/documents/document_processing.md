...
2. **Legacy per-format extraction** (`core/document_parser.py`) — a dispatcher that routes files to dedicated parsers such as `pymupdf4llm`, `python-docx`, `pandas`, `BeautifulSoup`, and Gemini Vision.
...
flowchart TB
    ...
    subgraph "External / Optional"
        ...
        LEG[pymupdf4llm / python-docx / pandas / etc.]
    end
...
| Legacy Document Parser | `core/document_parser.py` | Per-format dispatcher and fallback parsers for PDF, DOCX, PPTX, Excel, CSV, HTML, images, etc. | [document_processing_legacy_parser](document_processing_legacy_parser.md) |
