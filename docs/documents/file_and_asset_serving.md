...
Caching is governed by `NoCacheMiddleware` (part of the [Security & Governance](../security/security_and_governance.md) module), which wraps every response:
...
|---|---|---|
| `NoCacheMiddleware` | [Security & Governance](../security/security_and_governance.md) | Sets `Cache-Control` headers on all responses |
| `FastAPI` / `FileResponse` / `StaticFiles` | FastAPI / Starlette | HTTP response and static file serving primitives |
| ABStudio routers | [ABStudio Backend](../core/app_main.md) | API routers mounted under `/abs` prefix that create the generated files |
...
- [Security & Governance](../security/security_and_governance.md) — `NoCacheMiddleware`, rate limiting, and request authentication
- [Health & Monitoring](../observability/health_and_monitoring.md) — Gateway health endpoints including ABStudio health check
- [app_main](../core/app_main.md) — ABStudio standalone FastAPI app with the original `download_generated_file` route and background sweeper
- [api_execution](../api/api_execution.md) — ABStudio workflow execution routes that generate downloadable artifacts
