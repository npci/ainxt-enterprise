...
> **Sibling modules:** document_processing_legacy_parser, [document_processing_paddle_ocr](document_processing_paddle_ocr.md)
...
`core/document_parser.py` because the
legacy parsers (python-docx, BeautifulSoup) only recover document
...
|---|---|
| [`core/logger.py`](../core/core_infrastructure.md) | Provides the `logger` used throughout for structured logging. |
| `core/document_parser.py` | `_try_docling` is the caller that invokes `docling_parser.parse()` / `is_active()` / `supports()`. `parse_file` routes Docling-supported formats to `_try_docling` first. |
...
- document_processing_legacy_parser — `core/document_parser.py`: per-format parsers (python-docx, BeautifulSoup), `parse_file`, `_try_docling`, and the fallback chain.
...
- [core_infrastructure](../core/core_infrastructure.md) — `core/logger.py`, `core/config.py`: logging and configuration infrastructure.
