...
|------------|--------|------|
| `useFileDrop` | useFileDrop | Provides drag-and-drop registration and file filtering. |
| `previewCache` | previewCache | Browser Cache API wrapper (`cacheStore`, `cachedGet`, `cachedGetOrFetch`). |
| `useDesktop` / `readFileSpreadsheet` | useDesktop | Pre-parses Excel files on the Electron desktop. |
| `DocumentPreviewModal` | [document_preview](../documents/document_preview.md) | Full-screen preview modal for documents and images. |
| `authFetch`, `API_BASE` | [config](../core/config.md) | Authenticated HTTP client and base URL. |
| `chat_router` backend | [chat_router](../chat/chat_router.md) | `POST /chat/upload` and `GET /chat/attachments/{id}/raw`. |
...
- Parent chat module: [chat](../chat/chat.md)
- Related KB chat file handling: [kb_chat_file_image_handling](../knowledge/kb_chat_file_image_handling.md)
- Backend upload/preview API: [chat_router](../chat/chat_router.md)
- Browser cache utility: previewCache
- Drag-and-drop hook: useFileDrop
- Document preview modal: [document_preview](../documents/document_preview.md)
- Desktop helpers: useDesktop
- Authenticated fetch config: [config](../core/config.md)
