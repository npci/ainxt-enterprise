...
Uses `cachedGetOrFetch` from previewCache to load the `Response` object from the browser Cache API, falling back to a server fetch on the first request. The hook converts the response body to the requested type.
...
| Module | File | Purpose |
|--------|------|---------|
| previewCache | `ai-ui/src/utils/previewCache.js` | Browser Cache API wrapper (`cachedGet`, `cachedGetOrFetch`) |
| xlsxParser | `ai-ui/src/utils/xlsxParser.js` | Pure-JS XLSX parser used by `XlsxRenderer` |
...
- [chat](../chat/chat.md) — message attachments and file chips
- [kb_chat](../knowledge/kb_chat.md) — knowledge-base chat file uploads
...
The cache TTL and eviction policy are managed by previewCache. If a file is missing from the cache, the module shows an expiration message because the attachment only exists in browser storage.
