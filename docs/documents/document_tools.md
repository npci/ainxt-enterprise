...
The module is intentionally thin: the MCP server wraps plain Python functions, and those functions delegate complex parsing to the shared `core.document_parser`. This avoids duplicating parsing logic across the codebase and ensures that agent reads produce the same text as the knowledge-base ingestion pipeline.
...
|--------|-------------------|----------|
| PDF, `.doc`, `.ppt`, `.pptx`, `.rtf` | `core.document_parser.parse_file` | `pypdf.PdfReader` for PDF only |
...
|------------|------|---------------|
| `core.document_parser` | High-quality parsing for PDF, Office, RTF, HTML, images | core_document_parser.md |
...
- **API documents layer:** The backend API for document uploads and agent-runner attachments uses the same `core.document_parser` pipeline, ensuring consistency between interactive uploads and agent reads. See [api_documents.md](../api/api_documents.md).
...
For document *generation* (DOCX, PPTX, XLSX), use [`doc_generator`](doc_generator.md). For knowledge-base upload and approval workflows, use the [documents API](../api/api_documents.md) and [docs router](docs_router.md).
