...
- **ABStudio backend** uses document skills when agents or workflows need to create, modify, or convert Office files. See [`abstudio_backend`](../ui/abstudio_backend.md) for the orchestration layer.
- **AI UI frontend** exposes PPT wizards, document previews, and download flows that ultimately rely on these skills. See [`ai_ui_frontend`](../ui/ai_ui_frontend.md).
- **Shared skills** such as [`pptx_add_slide`](../clients/pptx_add_slide.md), [`pptx_thumbnail`](../reference/pptx_thumbnail.md), [`pptx_clean`](../reference/pptx_clean.md), [`pptx_office_pack`](pptx_office_pack.md), and [`pptx_office_unpack`](pptx_office_unpack.md) perform the actual OpenXML work and may delegate to LibreOffice through this module.
...
- [`pptx_add_slide`](../clients/pptx_add_slide.md) — creates new slides from layouts.
- [`pptx_thumbnail`](../reference/pptx_thumbnail.md) — generates thumbnail grids from `.pptx` files.
- [`pptx_clean`](../reference/pptx_clean.md) — removes unused files from `.pptx` packages.
...
- [`abstudio_backend`](../ui/abstudio_backend.md) — backend that orchestrates agent and workflow execution.
- [`ai_ui_frontend`](../ui/ai_ui_frontend.md) — frontend exposing document-generation UIs.
