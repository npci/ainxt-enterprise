...
| `config.js` | [config](../core/config.md) | `authFetch` — authenticated HTTP wrapper that attaches JWT cookie credentials and correlation IDs |

### Backend Dependencies

| Endpoint | Method | Backend Module | Description |
|---|---|---|---|
| `/desktop/index/file` | POST | [desktop_router](../cowork/desktop_router.md) | Inline single-file indexing (incremental) |
| `/desktop/index/batch` | POST | [desktop_router](../cowork/desktop_router.md) | Batch indexing via RQ background job |
| `/desktop/index/{workspace}` | DELETE | [desktop_router](../cowork/desktop_router.md) | Remove all vectors for a workspace |
| `/desktop/index/{workspace}/status` | GET | [desktop_router](../cowork/desktop_router.md) | Chunk count and last-indexed timestamp |
| `/desktop/register-mcp` | POST | [desktop_router](../cowork/desktop_router.md) | Register local MCP server with backend |
...
| [config](../core/config.md) | Provides `authFetch` for authenticated API calls |
| [desktop_router](../cowork/desktop_router.md) | Backend router handling all `/desktop/*` endpoints |
| [chat](../chat/chat.md) | Parent component that receives `onAskWithContext` prompts from clipboard and workspace actions |
| [sidebar](../ui/Sidebar.md) | Navigation entry point that renders the LocalFiles panel |
