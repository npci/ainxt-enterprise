...
| doc_worker / doc_worker_agent | RQ workers that orchestrate LLM-based content planning and call `generate()`. |
| DocGeneratorMCPServer | MCP server exposing `write_markdown`, `markdown_to_docx`, and `slides_to_pptx`. |
| generate_docs | Helper utilities in `ABStudio/generate_docs.py` for building DOCX tables, paragraphs, code blocks, and bullets. |

### Downstream consumers

| Module | Relationship |
|--------|--------------|
| ai-ui DocWorkflowCard / DocLivePreview / DocPreviewCard | Frontend components that display doc generation status, previews, and download actions. |
| DocGenSpinner | Loading indicator used while documents are being generated. |
...
1. **User intent** is captured in the frontend (ai-ui or [ABStudio](../ui/abstudio_frontend.md)).
2. The request is validated and enqueued by [doc_download_router](doc_download_router.md).
3. A background worker (doc_worker or doc_worker_agent) plans, structures, and sometimes rewrites content using LLMs.
...
In addition, agents can invoke document-generation capabilities directly through the DocGeneratorMCPServer, making the module a reusable building block across chat, workflow, and agent-runtime contexts.
...
- doc_worker / doc_worker_agent — Background workers that drive `doc_generator`.
- DocGeneratorMCPServer — MCP tool registration for agent access.
- generate_docs — Low-level DOCX helper utilities.
- DocWorkflowCard, DocLivePreview, DocPreviewCard, DocGenSpinner — Frontend document UX.
