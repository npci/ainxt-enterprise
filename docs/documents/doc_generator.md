...
| Module | Relationship |
|--------|--------------|
| [doc_download_router](../api/doc_download_router.md) | API endpoint that enqueues document jobs and derives filename hints using `smart_filename` and `FORMAT_EXTENSIONS`. |
...
1. **User intent** is captured in the frontend ([ai-ui](../ai-ui.md) or [ABStudio](../ui/abstudio_frontend.md)).
2. The request is validated and enqueued by [doc_download_router](../api/doc_download_router.md).
...
- [doc_download_router](../api/doc_download_router.md) — API surface for document generation requests.
