...
the [`workflowStore`](store.md) (Zustand + zundo temporal middleware).  The relevant
...
|---|---|---|---|
| `/run-stream` | POST (SSE) | [`api_execution`](api_execution.md) | Start a new workflow execution; streams SSE events |
| `/resume-stream` | POST (SSE) | [`api_execution`](api_execution.md) | Resume a paused/failed run with a HITL decision or retry |
| `/chat-threads/{workflow_id}` | GET | [`api_chat`](api_chat.md) | List conversation threads for a workflow |
| `/chat-history/{thread_id}` | GET | [`api_chat`](api_chat.md) | Load persisted messages for a thread |
| `/chat-threads/{thread_id}` | DELETE | [`api_chat`](api_chat.md) | Delete a conversation thread |
| `/chat-pending/{thread_id}` | GET | [`api_chat`](api_chat.md) | Hydrate HITL / failure snapshot on thread open |
| `/chat-pending/{thread_id}` | DELETE | [`api_chat`](api_chat.md) | Abort / discard a paused-run checkpoint |
| `/agent-runner/attachment` | POST (multipart) | [`api_documents`](api_documents.md) | Upload a document for OCR / text extraction |
| `/agent-runner/image-asset` | POST (multipart) | [`api_documents`](api_documents.md) | Upload an image as a sandbox asset |
| `/tools-catalog` | GET | [`api_catalog`](api_catalog.md) | Fetch tool names + descriptions for smart-parse |
| `/health` | GET | [`app_main`](app_main.md) | Backend + DB health polling |
...
reload.  See [`threadHelpers`](utils.md) for the shared sanitisation logic.
...
- **Active thread per workflow** is persisted via `saveActiveThread()` / `loadActiveThread()`
  in [`editorPersistence`](utils.md), so a browser reload reopens the same conversation.
...
The Debug Log view itself is rendered by [`DebugLogView`](workflows_feature_editor_debug_log.md),
...
integrates with [`triggersStore`](store.md):
...
[`TriggerNotifications`](triggers_feature.md) to skip the toast pop-up for executions
...
All downloads route through [`useGeneratedDownload`](shared_features.md), which:
...
|---|---|---|
| `workflowStore` | [`store`](store.md) | Chat state, execution state, run context, node graph |
| `dashboardStore` | [`store`](store.md) | `updateWorkflow` for Save & approve persistence |
| `triggersStore` | [`store`](store.md) | Recent trigger executions, mark seen |
| `subagentSelectors` | this module | Pure selectors for sub-agent timeline |
| `RunSettingsStrip` | [`workflows_feature_editor_sidebar_pickers`](workflows_feature_editor_sidebar_pickers.md) | Run-level subagent toggle |
| `DebugLogView` | [`workflows_feature_editor_debug_log`](workflows_feature_editor_debug_log.md) | Debug Log full-swap view |
| `ExtractedTextPreview` | [`shared_features`](shared_features.md) | Attachment text preview modal |
| `DownloadNotice` | [`shared_features`](shared_features.md) | Download expiry / error notice |
| `useGeneratedDownload` | [`shared_features`](shared_features.md) | Auth'd download orchestration |
| `sniffGeneratedFiles` | [`shared_features`](shared_features.md) | Detect generated filenames in prose |
| `threadHelpers` | [`utils`](utils.md) | Thread grouping, title, preview, history mapping |
| `editorPersistence` | [`utils`](utils.md) | Active thread + composer draft persistence |
...
| Dependency | Module | Purpose |
|---|---|---|
| `/run-stream`, `/resume-stream` | [`api_execution`](api_execution.md) | SSE workflow execution |
| `/chat-threads`, `/chat-history`, `/chat-pending` | [`api_chat`](api_chat.md) | Thread management & HITL hydration |
| `/agent-runner/attachment`, `/agent-runner/image-asset` | [`api_documents`](api_documents.md) | Document OCR & image asset upload |
| `/tools-catalog` | [`api_catalog`](api_catalog.md) | Tool catalog for smart-parse |
| `/health` | [`app_main`](app_main.md) | Backend health polling |
| `NativeEngine` | [`engine_native_engine`](engine_native_engine.md) | SSE event source (consumed indirectly) |
| `LoopRunner` | [`loop_runner`](loop_runner.md) | Loop evaluation events (consumed indirectly) |
| `SwarmOrchestrator` | [`swarm`](swarm.md) | Sub-agent delegation events (consumed indirectly) |
...
- [Workflow Store (`store`)](store.md) — Zustand store with chat, execution, and run-context slices
- [Debug Log View (`workflows_feature_editor_debug_log`)](workflows_feature_editor_debug_log.md) — Per-run timeline rendering
- [Run Settings Strip (`workflows_feature_editor_sidebar_pickers`)](workflows_feature_editor_sidebar_pickers.md) — Run-level configuration popover
- [Canvas (`workflows_feature_editor_canvas`)](workflows_feature_editor_canvas.md) — React Flow canvas with node activation
- [Config Panel (`workflows_feature_editor_config_panel`)](workflows_feature_editor_config_panel.md) — Node configuration (consumes `activeThreadId`)
- [Execution API (`api_execution`)](api_execution.md) — `/run-stream` and `/resume-stream` backend endpoints
- [Chat API (`api_chat`)](api_chat.md) — Thread history and pending-interrupt endpoints
- [Documents API (`api_documents`)](api_documents.md) — Attachment upload and OCR pipeline
- [Native Engine (`engine_native_engine`)](engine_native_engine.md) — SSE event source
- [Loop Runner (`loop_runner`)](loop_runner.md) — Loop evaluation events
- [Swarm (`swarm`)](swarm.md) — Sub-agent orchestration
- [Shared Features (`shared_features`)](shared_features.md) — `ExtractedTextPreview`, `DownloadNotice`, `useGeneratedDownload`
- [Utils (`utils`)](utils.md) — `threadHelpers`, `editorPersistence`
- [Triggers Feature (`triggers_feature`)](triggers_feature.md) — `TriggerNotifications` bell integration
