...
- **[sdlc_state_machine.md](sdlc_state_machine.md)** — `CodingStateMachine` that consumes `AgentLoop` for recovery-on-red; manages the full pipeline state machine (IDLE → CODING → REVIEWING → TESTING → COMMITTING).
- **[sdlc_loop_tools.md](sdlc_loop_tools.md)** — Wiring layer: `make_explore_tool_context`, `make_code_tool_context`, `make_test_tool_context`, `run_explore_pull`, `book_loop_usage`, `evaluate_test_loop_outcome`, `grep_workspace`.
- **[sdlc_patch_engine.md](sdlc_patch_engine.md)** — `PatchEngine` with `_apply_search_replace` (two-tier matching) and `restore_missing_imports`; the deterministic applier the `propose_edit` tool delegates to.
- **[sdlc_coder_tools.md](sdlc_coder_tools.md)** — `execute_tool` dispatcher and `_outline_with_lines` used by `_focus_large_file`.
- **[sdlc_pipeline.md](sdlc_pipeline.md)** — Top-level SDLC pipeline orchestration (`_env_flag`, `_parse_json`, review/governance phases).
- **[sdlc_governance.md](sdlc_governance.md)** — Governance review engine (`run_review`) that runs as a separate end-gate after commit.
- **[sdlc_baseline_gate.md](sdlc_baseline_gate.md)** — `baseline_failure_class` for build-failure telemetry classification.
- **[sdlc_metrics.md](sdlc_metrics.md)** — `log_exploration_metrics` / `compute_exploration_metrics_for_run` for run-level metrics.
- **[sdlc_normalizer.md](sdlc_normalizer.md)** — `NormalizationAgent` that converts raw Jira issues into locked WorkItems.
- **[core_infrastructure.md](core_infrastructure.md)** — `core.model_registry.sdlc_stage_hint` for model tier resolution; `core.logger` for structured logging.
- **[messages_compat_router.md](messages_compat_router.md)** — The frozen `/v1/messages` endpoint (`messages_endpoint`, `_compliance_check`, `_translate_in_msgs`) that enforces auth, budget, compliance, and multi-provider routing.
