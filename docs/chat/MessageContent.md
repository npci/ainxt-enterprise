...
`MessageContent` is a peer of [`ChatPanelCore`](ChatPanel.md), [`ChatActions`](ChatPanel.md), and [`FileHandling`](../reference/FileHandling.md). It does not own message state; it receives props from the message-mapping logic in [`ChatPanel`](ChatPanel.md) and renders the appropriate visual representation.
...
For details on the surrounding chat panel, see [ChatPanel](ChatPanel.md). For file download and attachment rendering, see [FileHandling](../reference/FileHandling.md). For the workflow editor canvas and node configuration, see [workflow_editor](../workflows/workflow_editor.md).
...
- [FileHandling](../reference/FileHandling.md) — sibling module for file download cards and attachment chips.
