...
Every chat surface in the platform — the main [Chat](chat.md) interface, [KbChat](../knowledge/kb_chat.md), [Threads](threads.md), [CoworkDesktop](../cowork/cowork_desktop.md), and [Projects](../products/projects.md) — relies on the exports from this module to render assistant responses consistently.
...
**Canvas integration:** When the backend returns an `artifact_id`, an "Edit in Canvas" button opens [CoworkCanvas](../cowork/cowork_desktop.md) — a full-screen modal with version history and AI-powered iterative editing (see [CoworkCanvas](../cowork/cowork_desktop.md) documentation for details).
...
A simpler download component for presentations generated via the Presenton service. Uses `buildExportPayload(presentationId, title)` from [presenton-payload](../presentation/presenton_lib.md) and `presentonApi.exportPresentation()` from [presenton-api](../presentation/presenton_lib.md) to fetch the file as a blob, then triggers a browser download with a sanitized filename.
...
A comprehensive map of custom React renderers for every Markdown element type, exported for reuse by [Chat](chat.md), [Threads](threads.md), [CoworkDesktop](../cowork/cowork_desktop.md), and [Projects](../products/projects.md). Provides consistent indigo-themed styling across all surfaces:
...
| `[DOC_PICKER_BEGIN]{json}[DOC_PICKER_END]` | `/\DOC_PICKER_BEGIN\\[DOC_PICKER_END\]/g` | `{ type: "docpicker", data: JSON.parse(...) }` |
...
| `CoworkCanvas` | [cowork_canvas](../cowork/cowork_canvas.md) | Full-screen document editing modal with version history and AI revision |
...
| `buildExportPayload` | [presenton-payload](../presentation/presenton_lib.md) | Builds PPT export request payload |
| `presentonApi` | [presenton-api](../presentation/presenton_lib.md) | Presenton API client (export, outline, status) |
...
| [CoworkDesktop](../cowork/cowork_desktop.md) | `mdComponents` | Desktop cowork — uses shared markdown renderers |
| [Projects](../products/projects.md) | `mdComponents` | Projects — uses shared markdown renderers |
