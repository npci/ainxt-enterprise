...
The module lives in the `ai-ui` frontend application and is consumed by the broader chat components (see [ai_ui_frontend](../ui/ai_ui_frontend.md)).
...
- **Presentation generation backend**: The actual generation is performed by the Presenton service and the document-generation workers. The frontend module only receives the result id/title/format and triggers the download. See [presenton_lib](../reference/presenton_lib.md) for the client library that streams outlines and polls generation status, and [workers](../workers/workers.md) for the backend job that builds the PPTX file.
- **PPT wizard**: A separate, dedicated PPT creation UI is provided by [ppt_wizard](../reference/ppt_wizard.md). The chat-based PPT flow (`ppt_chat`) is the lightweight alternative that keeps the user inside the conversation.
