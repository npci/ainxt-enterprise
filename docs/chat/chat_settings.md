...
- [`gateway.md`](../core/gateway.md) — the `/ask` gateway reads `rag_mode` and scope columns from the `Chat` row and injects them into `_user_ctx` for retrieval.
...
- [gateway.md](../core/gateway.md) — runtime injection of chat settings into retrieval context.
