...
The module is intentionally framework-agnostic: every export is a pure JavaScript function with no React, state, or side effects. It is consumed by the main chat surface ([chat](chat.md)), the knowledge-base-embedded chat ([kb_chat](../knowledge/kb_chat.md)), and the message renderer ([message](message.md)).
