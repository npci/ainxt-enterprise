...
`core.notifications` dispatch engine.
...
When `channels` is omitted, `core.notifications` falls
...
|------------|--------|------|
| `notify` | `core.notifications` | Dispatches notifications to configured channels |
| `require_operator` | `auth.rbac` | Intended RBAC guard (see note below) |
...
See `core.notifications` for the exact default values
...
`auth.rbac`, but the dependency is **not currently wired** via
...
* **Channel delivery failures** are logged by `core.notifications`;
...
* [`broadcast_router.md`](../connectors/broadcast_router.md) – targeted email/Teams broadcasts.
* [`teams_router.md`](../connectors/teams_router.md) – Teams-specific notifications and meeting processing.
* `core.notifications` – shared dispatch engine used by
...
* `core_notifications.md` – notification dispatch implementation
* `auth_rbac.md` – role-based access control
* [`inbox_router.md`](inbox_router.md) – in-app notification/approval inbox
* [`broadcast_router.md`](../connectors/broadcast_router.md) – broadcast messaging
* [`teams_router.md`](../connectors/teams_router.md) – Microsoft Teams integration
