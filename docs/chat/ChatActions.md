...
| `loadComposerDraft` / `saveComposerDraft` | [utils/editorPersistence.js](../reference/utils.md) | Persists unsent composer text per (workflow, thread) — `handleRegenerate` repopulates the composer via `setMessage`, which triggers the draft-persistence effect. |
| `mapHistoryToUiMessages` | [utils/threadHelpers.js](../reference/utils.md) | Maps backend chat history to UI message objects — relevant because `handleRegenerate` operates on the `messages` array that may have been hydrated from history. |
...
| [FileHandling.md](../reference/FileHandling.md) | Sibling sub-module — covers `FileDownloadCard`, `fallbackDownload`. |
...
| [utils.md](../reference/utils.md) | Utility modules including `threadHelpers.js` (message mapping) and `editorPersistence.js` (composer draft persistence). |
