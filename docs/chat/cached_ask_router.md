...
|------------|--------|---------|
| `get_current_user` | auth.dependencies | JWT authentication and user identity extraction. |
| `logger` | core.logger | Structured logging for budget blocks, compliance blocks, model errors, and telemetry. |
| `check_budget` | store.budget_store | Pre-flight budget gate; returns `429` if the user has exhausted their allocation. |
| `compliance_engine.validate_input` | agents.compliance_engine | Scans assembled input for restricted content (PCI/PII); can redact text before it reaches the model. |
| `model_router.generate_structured` | models.model_router | Routes the content-blocks payload to a Claude model and returns the generated text. |
| `PostgresMemory.create_model_usage` | memory.postgres_memory | Persists token usage, cost, latency, and cache telemetry for billing and analytics. |
...
- auth.dependencies — JWT authentication.
- store.budget_store — Budget enforcement.
- agents.compliance_engine — Input compliance scanning and redaction.
- models.model_router — Model selection and generation.
- memory.postgres_memory — Usage persistence.
