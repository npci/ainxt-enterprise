...
For full router details, see [shared_api_routers](../core/shared_api_routers.md).
...
Models listed in `BLOCKED_MODELS` (e.g., retired or kill-switched models) are excluded from governance assignment. See model_router and core_model_registry for routing details.
...
|----------------|--------------|
| [shared_api_routers](../core/shared_api_routers.md) | Hosts the `model_governance_router.py` REST API. |
| model_router | Enforces governance at runtime via `is_model_allowed_for_dept` and `filter_allowed_models`. |
| core_model_registry | Defines the model IDs and feature flags that populate the governance catalog. |
| [auth](../auth/auth.md) | All governance mutations require admin privileges (`_require_admin`). |
| [dept_metrics](../observability/dept_metrics.md) | The UI fetches the department list from `/dept-metrics/departments`. |
| [chat](../chat/chat.md) / [kb_chat](../knowledge/kb_chat.md) / [ide_router](../codebase/ide_router.md) | Consumers of `get_my_models` that hide disallowed models from model pickers. |
...
- **Privacy floor**: `ModelRouter` pins `CONFIDENTIAL`/`RESTRICTED` data to local models regardless of governance rules. See model_router.
...
- Related docs: [shared_api_routers](../core/shared_api_routers.md), model_router, [auth](../auth/auth.md), [dept_metrics](../observability/dept_metrics.md)
