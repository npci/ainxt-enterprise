...
2. **Model Router:** The [model_router](../core/shared_core.md#model_routing) selects providers based on policy, cost, availability, and compliance. It does not select `ollama_gateway`; it selects `local_llm_gateway` when a local model is appropriate.
