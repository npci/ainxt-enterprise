...
|---|---|---|
| `core.circuit_breaker.get_breaker` | [shared_core](../core/shared_core.md) | Per-provider circuit breaker with Redis-backed state (CLOSED → OPEN → HALF_OPEN) |
| `core.retry.retry_llm` | [shared_core](../core/shared_core.md) | Exponential backoff retry on transient errors (rate-limit, timeout, connection) |
| `core.logger` | [shared_core](../core/shared_core.md) | Structured logging with request ID / chat ID context binding |
| `core.model_registry` | [shared_core](../core/shared_core.md) | Centralized model IDs and pricing (`GEMINI_VISION_MODEL`, `GEMINI_IMAGE_MODEL`, `VEO_MODEL`, `MODEL_COST_PER_1M`) |
