...
See [shared_core](../core/shared_core.md) for ComplianceEngine details.
...
| Dependency | Role |
|---|---|
| **`models.model_router.ModelRouter`** | Routes prompts to the correct LLM gateway based on model hints, complexity classification, vision detection, and privacy floors. See [model_routing](model_routing.md). |
| **`agents.compliance_engine.ComplianceEngine`** | Scans input for PCI/PII/secrets. Redacts configured types and blocks configured block-types. See [shared_core](../core/shared_core.md). |
| **`store.budget_store`** | Enforces per-user cost budgets. `check_budget()` gates cloud model access; `increment_usage()` records spend. See [budget_manager](budget_manager.md). |
...
| **`services/llm_proxy`** | Centralized LLM proxy service that holds all cloud API keys and forwards requests to OpenAI/Anthropic/Google. See [llm_proxy_main](llm_proxy_main.md). |
...
- [model_routing](model_routing.md) — ModelRouter, tier classification, gateway dispatch, and fallback chains
- [shared_core](../core/shared_core.md) — ComplianceEngine, context compression, and core infrastructure
- [budget_manager](budget_manager.md) — Budget enforcement, usage tracking, and allocation management
- [llm_proxy_main](llm_proxy_main.md) — Centralized LLM proxy service holding all cloud API keys
- [messages_compat_router](../chat/messages_compat_router.md) — Anthropic Messages API compatibility endpoint
- [endpoint_proxy_router](../products/endpoint_proxy_router.md) — Managed endpoint proxy for per-team OpenAI-compatible endpoints
