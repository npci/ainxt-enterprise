...
| `agents.compliance_engine` | `compliance_engine.validate_input()` runs regex + ML PII/PCI detection; returns `{blocked, findings, redacted_text, ...}`. Block-configured types reject the request; redact-configured types are masked. | [shared_core](../core/shared_core.md) → `agent_system` |
| `core.circuit_breaker` | `get_breaker("gemini")` returns a per-provider `CircuitBreaker` with Redis-persisted state (CLOSED → OPEN → HALF_OPEN). Wraps every SDK call via `breaker.call(retry_llm, _call)`. | [shared_core](../core/shared_core.md) → `core_infrastructure` |
| `core.retry` | `retry_llm` provides exponential-backoff retry for transient API failures, composed inside the circuit breaker call. | [shared_core](../core/shared_core.md) → `core_infrastructure` |
| `core.prompt_sanitizer` | `sanitize()` strips prompt-injection vectors and normalizes content before it reaches the model. | [shared_core](../core/shared_core.md) → `core_infrastructure` |
| `core.proxy_tool_use` | `run_tool_use_via_proxy()` drives multi-round tool-use through the proxy's `/llm/chat`. `_execute_with_web_search_governance()` applies governance, pricing, and budget gating for web-search tools. `flush_web_search_billing()` writes accumulated billing at loop end. | [shared_core](../core/shared_core.md) → `core_infrastructure` |
| `core.model_registry` | Source of truth for model IDs (`GEMINI_VISION_MODEL`, `GEMINI_IMAGE_MODEL`, `VEO_MODEL`) and per-model pricing (`MODEL_COST_PER_1M`). Env-overridable without code changes. | [shared_core](../core/shared_core.md) → `core_infrastructure` |
| `pipeline.stream_events` | `ReasoningMarker` — a streaming event type that surfaces Gemini thinking-model "thought" parts as first-class reasoning deltas to the UI. | [shared_core](../core/shared_core.md) → `pipeline` |
| `core.logger` | Structured logging with request-ID correlation via `get_request_id()`. | [shared_core](../core/shared_core.md) → `core_infrastructure` |
...
| [gateway](../core/gateway.md) | The main API gateway (`gateway.py`) that calls `GeminiGateway` for `ask`, `ask_with_image`, and image generation endpoints. |
| [shared_core](../core/shared_core.md) | Contains `compliance_engine`, `circuit_breaker`, `retry`, `prompt_sanitizer`, `proxy_tool_use`, `model_registry`, `logger`, and `pipeline.stream_events` — all cross-cutting dependencies of this module. |
