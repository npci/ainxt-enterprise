...
The **Budget** module is a React-based frontend subsystem in `ai-ui` that surfaces LLM usage budgets, team spending, and utilization breakdowns to end users. It provides read-only reporting views for managers and self-service budget analytics for individual users, consuming the backend `budget_router` API.
...
> **HOD-approval-hierarchy mode**: `TeamBudgetPanel` is rendered for reporting managers regardless of `HOD_APPROVAL_ENABLED`; it never itself reads the flag. Manager-scoped views work identically in flat/admin-only mode (the default) and HOD-hierarchy mode — see [`budget_manager`](budget_manager.md) for the flag's effect on HOD-specific tabs (Delegation, HOD Caps, Team → Delegated) that live in that module instead.
...
The module is intentionally lightweight: it contains only presentation components and small helper utilities. All budget logic, allocation rules, approval workflows, and data persistence live in the backend `budget_router` and the shared [`budget_manager`](budget_manager.md) component.

## Sub-modules

| Sub-module | File | Responsibility |
|------------|------|----------------|
| [Team Budget Panel](../analytics/budget_team_panel.md) | `TeamBudgetPanel.jsx` | Reporting-manager view: roster table of direct/indirect reports with base/extra/total budget and utilization bars. |
| [Utilization View](../analytics/budget_utilization_view.md) | `UtilizationView.jsx` | Reusable donut chart, legend, and MTD history views for channel/model/date dimensions. |
...
|------------|---------------|-------|
| `authFetch`, `API_BASE` | `config.js` | Authenticated HTTP requests. |
...
| Backend `budget_router` | `shared_api_routers_budget_router.md` | Data source for team and utilization endpoints. |

## Related Documentation

- shared_api_routers_budget_router â€” Backend REST API for budgets, allocations, and utilization.
- [budget_manager](budget_manager.md) â€” User-facing budget management, increase requests, and HOD/admin workflows.
- ai_ui_frontend_config â€” authFetch and API_BASE configuration.
- [ai_ui_frontend_app_core](../ui/ai_ui_frontend_app_core.md) â€” Top-level application shell that hosts budget views.
