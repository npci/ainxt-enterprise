...
- **`ABStudio/backend/app/core/llm_handler.py`**: The backend's LLM client uses the same logging conventions when calling the proxy.
- **[`gateway.py`](../core/gateway.md)**: The main backend gateway orchestrates agent/workflow/chat runs and relies on correlated proxy logs for debugging.
