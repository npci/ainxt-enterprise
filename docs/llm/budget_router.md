...
- `GET`/`PUT /budget/admin/org-cap` (admin-only) — new endpoints, unrelated to `hod_allocation_caps`, that manage a single org-wide monthly spend cap (`ainxt.org_wide_budget_cap`, see [`services/org_budget_governor`](../reference/services.md)) used to gate SDLC-run and managed-endpoint cloud spend in flat mode. Budget-increase requests are **not** gated by this cap.
...
The `BudgetMiddleware` (see [middleware](../reference/middleware.md) documentation) intercepts every LLM-generating request and calls `check_budget(user_id)` from `budget_store` before allowing the request through. If the user's cumulative spend has reached their `max_cost_usd_total`, the middleware returns a `429 BUDGET_EXCEEDED` response and publishes an inbox notification.
...
| Module | Usage |
|--------|-------|
| [`store/budget_store`](../storage/store_layer.md) | Redis + Postgres budget/usage CRUD, increase request lifecycle |
| [`services/hod_budget_governor`](../reference/services.md) | HOD monthly cap enforcement and status |
| [`services/budget_audit_service`](../reference/services.md) | Monthly snapshot, reset, and pre-reset warnings |
| [`services/hierarchy_service`](../reference/services.md) | Org-tree subtree resolution for reporting managers |
| [`store/inbox_store`](../storage/store_layer.md) | In-app notification publishing |
| [`services/smtp_service`](../reference/services.md) | HTML email delivery for HOD notifications |
| [`auth/rbac`](../security/authentication.md) | Role checks (`is_admin`, `is_hod`), scope filters |
| [`auth/dependencies`](../security/authentication.md) | `get_current_user` dependency |
| [`core/rate_limiter`](../infrastructure/core_infrastructure.md) | `enforce_rate_limit_with_behaviour`, `BUDGET_REQUEST`, `BUDGET_ADMIN` |
| [`core/security_validation`](../infrastructure/core_infrastructure.md) | `validate_budget_request` input sanitization |
| [`core/logger`](../infrastructure/core_infrastructure.md) | Structured logging |
| [`core/config`](../infrastructure/core_infrastructure.md) | `postgres_dsn()` |
| [`db/database`](../storage/database.md) | `SessionLocal` for SQLAlchemy sessions |
| [`db/models`](../storage/database.md) | `User`, `BudgetConfig`, `ModelRateTable` ORM models |
| [`middleware/budget_middleware`](../reference/middleware.md) | Per-request budget enforcement (consumes `check_budget`) |
