...
- `RecoveryEngine` delegates persistence to the shared key/value layer ([`core_infrastructure`](../core/core_infrastructure.md)) and delegates inverse operations to the existing tool implementations in [`shared_integrations`](../skills/shared_integrations.md)).
...
- **Key/value store**: `RecoveryEngine` uses [`core/kv`](../core/core_infrastructure.md) for undo stacks and ReACT checkpoints. Keys are namespaced by `user_id` to prevent cross-user access (SEC-05).
- **Tool layer**: Inverse operations for tracked tool calls are implemented by reusing existing tools such as [`tools/gitlab_tools`](../skills/shared_integrations.md) and [`tools/jira_tools`](../skills/shared_integrations.md).
