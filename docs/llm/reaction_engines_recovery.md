...
| KV store | `core/kv/redis_impl.py` | Redis client used for lists and expiring strings. See `core_kv`. |
...
For more details on the tool layer, see [`shared_integrations`](../skills/shared_integrations.md).
...
- **KV store**: `core_kv` provides the Redis client used for lists and expiring strings.
- **Tool layer**: [`shared_integrations`](../skills/shared_integrations.md) contains `gitlab_tools` and `jira_tools`, which supply both the forward and inverse operations.
- **Logging**: `core_logger` is used for debug/info/warning messages; failures in recovery are intentionally non-fatal.
