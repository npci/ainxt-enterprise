...
This is consumed by orchestration health checks and by [gateway.md](../core/gateway.md) (`get_compression_metrics`).
...
- Platform-level compression metrics are exposed through [gateway.md](../core/gateway.md) (`get_compression_metrics`).
...
- **Observability**: [gateway.md](../core/gateway.md) surfaces compression health/metrics via `get_compression_metrics`.

The service is intentionally isolated from the main gateway and from other model services such as [llm_proxy.md](llm_proxy.md) and [embedding_service.md](../knowledge/embedding_service.md). It only depends on the shared `core.kv` and `core.logger` utilities.
