...
- **AI UI** administrators use `ModelGovernance.jsx` to configure access.
- **Chat and IDE clients** call `GET /my-models` to populate the model picker with only permitted models.
- **`model_router.py`**, the central request-time routing component, calls `is_model_allowed_for_dept` before dispatching to a gateway. See `model_router.md` for routing details.
...
- `model_router.md` — Request-time LLM routing, tier selection, and gateway dispatch.
- `ai_ui_frontend_model_governance.md` — Frontend governance configuration page.
- [`auth_router.md`](../auth/auth_router.md) — Authentication and session management.
- [`admin_router.md`](../api/admin_router.md) — Other administrative endpoints.
