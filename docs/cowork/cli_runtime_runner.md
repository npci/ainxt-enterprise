...
When `ABSTUDIO_CLI_MODE` is enabled, the [NativeEngine](../agents/engine_native_engine.md) routes individual agent nodes through the CLI runtime instead of its internal ReAct loop. The runner's responsibilities are:
...
The CLI emits a small NDJSON event stream (`text`, `thought`, `end`, `error`). The runner parses these into `CliEvent` dataclasses. The bridge then merges them with tool events from the `RunSession` event queue via event_mapper.merge, producing the same SSE event names (`agent_token`, `tool_call_start`, `tool_call_result`, `agent_usage`) that the native engine uses.
...
[NativeEngine._run_agent_via_cli](../agents/engine_native_engine.md) decides whether to route an agent node through the CLI. It builds an `AgentTurnSpec` and calls cli_runtime.bridge.run_agent_turn_via_cli. The bridge converts that into a `CliTurnRequest` and invokes `run_cli_turn`.

### 7.2 Tool Plane

The CLI discovers tools through the per-run MCP server configured in the workspace. HTTP MCP requests hit mcp_router.abstudio_mcp, which authenticates the bearer token against [SessionRegistry](cli_runtime_session.md) and dispatches to AbstudioMcpServer. The server calls [agent_factory.pipeline.ToolDispatcher](../agents/agent_factory_pipeline.md), the same dispatcher used by the native engine.

### 7.3 Workspace and File Handling

workspace.prepare_workspace creates the private directory. The runner may also:

- Clone a git repo into `<workspace>/repo` via ensure_repo.
- Stage uploaded documents into `<workspace>/inputs/` via stage_documents.
- Write large prompts to a file via write_prompt_file.
- Rescue model-written files into the generated-files store via rescue_workspace_files.

### 7.4 Event Streaming

cli_runtime.bridge and event_mapper translate CLI events and tool events into the SSE vocabulary the frontend already understands. The runner itself only exposes raw `CliEvent` objects; normalisation into `agent_token`, `tool_call_start`, `tool_call_result`, etc. happens upstream.

### 7.5 Configuration

All knobs are read from environment variables by cli_runtime.config and snapshotted into a `CliRuntimeConfig` at turn start. Key settings include `ABSTUDIO_CLI_PATH`, `ABSTUDIO_CLI_API_KEY`, `ABSTUDIO_CLI_MAX_CONCURRENCY`, `ABSTUDIO_CLI_RUN_TIMEOUT_S`, and `ABSTUDIO_CLI_PROMPT_FILE_THRESHOLD`.
...
- cli_runtime_workspace.md — workspace preparation, MCP config, git clone, document staging.
- cli_runtime_mcp.md — MCP router and server (`abstudio_mcp`, `AbstudioMcpServer`).
- cli_runtime_events.md — event mapping and SSE conversion.
- cli_runtime_bridge.md — `AgentTurnSpec` and the bridge from `NativeEngine` to the runner.
- cli_runtime_config.md — environment-based configuration.
- [engine_native_engine.md](../agents/engine_native_engine.md) — the in-process engine that delegates to this module.
