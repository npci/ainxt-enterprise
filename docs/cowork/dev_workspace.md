...
- shared_core_model_routing for `classify_query_complexity` and `rewrite_query`.
- shared_core_agent_system for the `agents.orchestrator` runtime.
- shared_core_core_infrastructure for logging and request-context management.
...
- shared_core_agent_system — agent orchestration and multi-agent execution.
- shared_core_model_routing — query classification, rewriting, and model routing.
- shared_core_core_infrastructure — logging, telemetry, and request-context utilities.
- abstudio_backend — production backend API and workflow engine.
- [gateway](../core/gateway.md) — public gateway exposing agent, workflow, and chat endpoints.
