...
| `desktop_app_preload_bridge` | Secure context-bridge exposing only the approved API surface to the renderer. | `desktop/src/preload.js` | desktop_app_preload_bridge.md |
...
- **ai_ui_frontend**: The desktop renderer loads the same `ai-ui` SPA. The preload bridge exposes `window.ainxtDesktop` so the web components can detect the desktop surface and call local APIs. See ai_ui_frontend.md.
- **gateway**: The desktop depends on the gateway for authentication (`/auth/me`, `/auth/sso/desktop/*`), compliance redaction (`/compliance/scan`, `/compliance/scan-image`), audit logging (`/cowork/computer-use/audit`), and MCP registration (`/desktop/register-mcp`). See [gateway.md](../core/gateway.md).
- **shared_core / shared_integrations**: Local tools such as browser automation and computer-use are conceptually extensions of the agent tool surface documented in [shared_integrations.md](../skills/shared_integrations.md) and [shared_core.md](../core/shared_core.md), but they execute on the client machine rather than the server.
