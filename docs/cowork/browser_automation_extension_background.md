...
The side panel (browser_automation_extension_sidepanel) and the runner (browser_automation_extension_runner) communicate with the background exclusively through `chrome.runtime.sendMessage` calls, each carrying a `type` discriminator. The background worker handles each message type asynchronously and returns a structured `{ ok, ... }` response.
...
|---|---|---|
| **Side Panel** | browser_automation_extension_sidepanel | `chrome.runtime.sendMessage` for notifications, screenshot capture, tab listing, and group management |
| **Runner** | browser_automation_extension_runner | `chrome.runtime.sendMessage` for screenshot/zoom capture, header rules, tab navigation, window resize, web-request capture, spawned-tab tracking, content-script injection, and tab-group operations |
...
| Module | Relationship |
|---|---|
| browser_automation_extension_sidepanel | Primary consumer: sends `notify`, `captureVisibleTab`, `listTabs`, and group-management messages |
| browser_automation_extension_runner | Primary consumer: sends screenshot, zoom, header-rule, navigation, resize, web-request, spawned-tab, and group messages |
...
| [browser_automation_extension_llm](browser_automation_extension_llm.md) | Indirect dependency: the runner uses LLM calls that may trigger screenshot captures routed through the background |
| browser_automation_extension_support | No direct dependency; persistence and parsing utilities are used by the side panel and runner, not the background |
