...
- **Live generation preview**: While a revision is running, streams the worker's incremental snapshot section-by-section through `DocLivePreview`.
...
|---|---|---|
| `DocLivePreview` | Child render | Shows the incremental section-by-section preview while an edit is building. |
...
| [`config.js`](../core/config.md) | Shared utility | Provides `authFetch` (cookie-authenticated fetch) and `API_BASE`. |
| [`docs_router`](../documents/docs_router.md) | Backend API | Serves version history and page preview images. |
| [`doc_download_router`](../documents/doc_download_router.md) | Backend API | Accepts revision instructions, streams job status, cancels jobs, and serves downloads. |
| `doc_worker` / `doc_worker_agent` | Backend worker | Executes the actual document generation and revision jobs. |
...
For details on the backend implementation of these endpoints, see [`docs_router`](../documents/docs_router.md) and [`doc_download_router`](../documents/doc_download_router.md). The workers that execute the jobs are documented in `doc_worker` and `doc_worker_agent`.
...
- `doc_live_preview` — live section-by-section preview during generation.
- [`message`](../chat/message.md) — shared Markdown rendering components.
- [`config`](../core/config.md) — `authFetch` and API base configuration.
- [`docs_router`](../documents/docs_router.md) — backend version history and preview endpoints.
- [`doc_download_router`](../documents/doc_download_router.md) — backend revision, job status, cancel, and download endpoints.
- `doc_worker` — backend document conversion worker.
- `doc_worker_agent` — backend agentic document generation worker.
