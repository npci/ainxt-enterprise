...
**Token format:** The profile stores GitLab tokens as `"username:PAT"`. The module splits on the first `:` to extract just the PAT, mirroring `tools/gitlab_tools.py:set_token()` in the [shared_integrations](../skills/shared_integrations.md) module.
...
- **desktop_app_preload_bridge** — The preload script that exposes IPC bridges between the renderer and main process.
...
- **[gateway](../core/gateway.md)** — The backend gateway API that validates tokens (`/auth/me`) and serves agent/workflow endpoints.
- **abstudio_backend** — The ABStudio backend with agent factory, workflow engine, and CLI runtime.
- **[shared_integrations](../skills/shared_integrations.md)** — Contains `gitlab_tools.py` whose token-splitting logic is mirrored in `clone.js`.
- **ai_ui_frontend** — The web UI frontend; the `useDesktop.js` hook provides desktop-specific cowork APIs (`coworkListSessions`, `coworkSessionHistory`, etc.) that consume session data managed by this engine.
