...
The Cowork dev agent runs the **full ainxt agent loop locally** via the CLI binary. It can edit local files, run commands, and conduct multi-turn chat: the desktop app drives a local agent process. The `SessionManager` (from [desktop_app_cowork_engine](desktop_app_cowork_engine.md)) manages CLI subprocess lifecycle, and events are forwarded to the renderer via `_coworkEmit()`.
...
| desktop_app_preload_bridge | The `preload.js` context bridge that exposes a curated set of IPC channels to the renderer under `contextIsolation: true`. |
| [gateway](../core/gateway.md) | The main process loads the gateway-served SPA, injects `x-ainxt-surface: desktop` on all requests, validates tokens via `/auth/me`, exchanges SSO codes via `/auth/sso/desktop/exchange` and `/auth/sso/desktop/refresh`, and long-polls dispatch tasks via `/cowork/dispatch/pending`. |
| ai_ui_frontend | The React SPA loaded inside the `BrowserWindow`. The main process injects `window.__AINXT_DESKTOP__` and `window.__AINXT_API__` globals, and communicates via IPC channels exposed through the preload bridge. |
| [auth_router](../auth/auth_router.md) | Server-side SSO endpoints (`sso_desktop_exchange`, `sso_desktop_refresh`, `sso_authorize`) called by the main process for authentication. |
