...
> **File:** `connectors/browser-automation-agent/content.js`
...
| `browser_automation_extension_runner` | `lib/runner.js` | Orchestrates the agent run loop — planning, step execution, human gates, tab following |
| `browser_automation_extension_sidepanel` | `sidepanel.js` | UI — chat, plan approval, settings, history, debug monitor |
...
| `browser_automation_extension_support` | `lib/*.js` | Parser, markdown, persistence utilities |
...
- **Runner → Content Script**: The runner's `executeStepAndFollowTabs()` function sends `execAction` messages via the background's `sendToContent()` helper. See `browser_automation_extension_runner`.
...
*See also: [`browser_automation_extension_background`](browser_automation_extension_background.md) · `browser_automation_extension_runner` · `browser_automation_extension_sidepanel` · [`browser_automation_extension_llm`](browser_automation_extension_llm.md) · `browser_automation_extension_support`*
