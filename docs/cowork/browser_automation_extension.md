...
| Runner | `lib/runner.js` | Run orchestration, iterative agent loop, approval gates, tab following, selector healing, document reading | browser_automation_extension_runner |
| Sidepanel UI | `sidepanel.js` | Chat UI, settings, plan approval, history, debug monitor, GIF export, shortcuts | browser_automation_extension_sidepanel |
| Support Libraries | `lib/markdown.js`, `lib/parser.js`, `lib/persist.js` | Safe markdown rendering, test-file parsing, settings backup/sync | browser_automation_extension_support |
...
The Browser Automation Extension is a standalone Chrome extension under `connectors/browser-automation-agent/`. It does not import backend Python modules; it communicates directly with any OpenAI-compatible LLM endpoint configured by the user. It complements the broader platform by providing a browser-native agent connector that can be used alongside:

- [gateway](../core/gateway.md) — for platform-level agent/workflow orchestration.
- abstudio_backend — for no-code workflow and agent authoring.
- [shared_integrations](../skills/shared_integrations.md) — for connector adapters that the browser agent may navigate to and interact with through the web UI.
