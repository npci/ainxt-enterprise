...
All endpoints in this router are **admin-only**; every route depends on `require_admin` from `auth_rbac.md`.
...
- `auth.dependencies.md` — extracts the current user from the request.
- `auth.rbac.md` — enforces the admin requirement on every route.
- `core.logger.md` — structured logging for policy changes.
- `db.database.md` — SQLAlchemy engine for raw SQL access.
...
- [`connectors_router.md`](../connectors/connectors_router.md) — defines connectors, permissions, and OAuth flows. The `connector` / `connector_name` values used in policy rules must match connector definitions managed there.
- `auth_rbac.md` — provides the `require_admin` dependency used by every endpoint in this router.
...
- [`connectors_router.md`](../connectors/connectors_router.md)
- `auth_rbac.md`
