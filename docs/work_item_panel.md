...
The component consumes the shared `apiFetch` utility from [`config`](config.md) and uses icons from `lucide-react`.
...
`WorkItemPanel` is typically rendered by the [`SDLCPipeline`](sdlc_pipeline.md) component (or an equivalent SDLC run detail view) when the run is in the `AWAITING_USER_INPUT` state and `gate_kind === "normalization"`. It receives the following props:
...
This endpoint is exposed by the [`sdlc_router`](sdlc_router.md) in the shared API routers layer and is handled by the backend SDLC pipeline state machine (see [`shared_core_sdlc_pipeline`](shared_core_sdlc_pipeline.md)).

### Related Frontend Components

- [`OpenQuestionsForm`](open_questions.md) — Implements **GATE 2** (classify-stage questions). `WorkItemPanel` explicitly does not delegate to it; GATE 1 questions are answered inline so they can ride the same payload as the work item edits.
- [`SDLCPipeline`](sdlc_pipeline.md) — Orchestrates the overall SDLC run UI and decides which gate panel to render.
...
2. **GATE 2 — Classify/Plan** ([`OpenQuestionsForm`](open_questions.md)): Answer deeper planning questions after the work item is locked.
...
- **API client**: `apiFetch` and `API_BASE` from [`config`](config.md).
- **Backend endpoint**: `POST /sdlc/runs/{runId}/answer-questions` exposed by [`sdlc_router`](sdlc_router.md).
- **Pipeline orchestration**: [`SDLCPipeline`](sdlc_pipeline.md) and the backend [`shared_core_sdlc_pipeline`](shared_core_sdlc_pipeline.md).

---

## References

- [`SDLCPipeline`](sdlc_pipeline.md) — Parent SDLC run UI orchestrator.
- [`OpenQuestionsForm`](open_questions.md) — GATE 2 question form for classify/plan stage.
- [`sdlc_router`](sdlc_router.md) — Backend router exposing `answer-questions`.
- [`shared_core_sdlc_pipeline`](shared_core_sdlc_pipeline.md) — Backend SDLC state machine and normalization logic.
- [`config`](config.md) — Shared frontend API configuration and `apiFetch` helper.
