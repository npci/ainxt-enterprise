...
- The helper relies on [auth/jwt_handler.py::decode_token](auth.md), which verifies the HS256 signature, required claims, revocation blacklist, and active session state.
...
The function calls `reload_rails()` from [guardrails/runtime_guardrails.py](shared_integrations.md), which clears the in-memory LLMRails cache. The next `/ask` request re-initialises the rails from the updated files.

> **Note:** This module only exposes the reload hook. The actual blocking logic (keyword-only or LLM-backed) lives in [guardrails/runtime_guardrails.py::check_input](shared_integrations.md).
...
This schema is the first validation gate for the chat pipeline and is referenced by the [chat_and_messaging](gateway.md) gateway endpoints.
...
|------------------|-----|----------|
| `auth/jwt_handler.py::decode_token` | Validating JWTs to extract a user-scoped rate-limit key. | [auth.md](auth.md) |
| `core/rate_limiter.py::enforce_rate_limit` | Enforcing sliding-window limits backed by Redis. | [shared_core.md](shared_core.md) |
| `middleware/rate_limit_middleware.py::RateLimitMiddleware` | The middleware that orchestrates `_rate_key` and limit enforcement. | [gateway.md](gateway.md) |
| `guardrails/runtime_guardrails.py::reload_rails` | Clearing the NeMo Guardrails cache. | [shared_integrations.md](shared_integrations.md) |
| `core/security_validation.py` | Additional request validation used downstream. | [shared_core.md](shared_core.md) |
...
2. **No wildcard CORS.** The gateway reads `CORS_ALLOWED_ORIGINS` from the environment and never uses `*` when credentials are enabled. See the [gateway](gateway.md) documentation for CORS setup.
...
- If Redis is unavailable, the rate limiter falls back to an in-process counter unless configured otherwise. See [core/rate_limiter.py](shared_core.md) for `block_on_redis_failure` behavior.

---

## Related Modules

- [gateway.md](gateway.md) — Parent gateway module that registers the middleware and admin hooks.
- [shared_core.md](shared_core.md) — Contains `core/rate_limiter`, `core/security_validation`, and other shared security primitives.
- [shared_integrations.md](shared_integrations.md) — Contains the `guardrails/runtime_guardrails` engine.
- [auth.md](auth.md) — Authentication and JWT handling.
