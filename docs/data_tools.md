...
- **`DataToolsMCPServer`** — Wraps the six functions as MCP tools with JSON schemas and exposes them over SSE. See [mcp_servers](mcp_servers.md) for server mechanics.
- **`MCPRegistry` / `ToolRegistry`** — Bootstrap the tools at platform startup so agents and workflows can invoke them by name. See [mcp_system](mcp_system.md) and [tool_registry](tool_registry.md).
...
|----------------|--------------|
| [mcp_system](mcp_system.md) | `MCPRegistry._register_tools()` imports and registers all six `data_tools` functions into the platform-wide `ToolRegistry`. |
| [mcp_servers](mcp_servers.md) | `DataToolsMCPServer` exposes the same six functions as MCP-over-SSE tools. |
...
Agents typically combine `data_tools` with [mcp_system](mcp_system.md) orchestration, [tool_registry](mcp_system.md) execution, and [shared_integrations](shared_integrations.md) sibling tools such as `document_tools` (for extracting source documents) and `doc_generator` (for writing analysis reports).
