...
For the full database model definitions, see db_models.md.
...
- **auth/dependencies.py** — Provides `get_current_user`, which resolves the caller from JWT or API key credentials.
- **db/database.py** — Provides `SessionLocal`, the SQLAlchemy session factory.
- **db/models.py** — Defines the `UserAPIKey` persistence model.

### Related Modules

- **[auth_router.md](../auth/auth_router.md)** — Handles login, token refresh, SSO, and session revocation. API keys are an alternative credential type produced by this router but consumed by the auth layer.
- **[profile_router.md](../auth/profile_router.md)** — Manages user profile settings and custom tokens. The API keys router is mounted under the `/profile` namespace conceptually.
...
- **[desktop_router.md](../cowork/desktop_router.md)** — May trigger key minting for desktop clients through the shared `mint_api_key_for_user` helper.
...
- IDE extensions via the [ide_router.md](../codebase/ide_router.md).
- Desktop CLI via the [desktop_router.md](../cowork/desktop_router.md).
...
- auth_dependencies.md — User resolution and credential validation.
- [auth_router.md](../auth/auth_router.md) — Login, SSO, session, and token management.
- [profile_router.md](../auth/profile_router.md) — User profile and custom token management.
- [endpoint_mgmt_router.md](endpoint_mgmt_router.md) — Managed endpoint credentials (`endpoint:*` keys).
- [desktop_router.md](../cowork/desktop_router.md) — Desktop CLI integration and workspace indexing.
- [ide_router.md](../codebase/ide_router.md) — IDE extension endpoints that consume API keys.
- db_database.md — Database session management.
- db_models.md — Full persistence model definitions, including `UserAPIKey`.
