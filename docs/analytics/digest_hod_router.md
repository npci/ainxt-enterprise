...
`digest_hod_router` is a small, admin-only FastAPI router that exposes HTTP endpoints for triggering **Head-of-Department (HOD) monthly usage digest emails**. It acts as a thin HTTP adapter over the HOD-specific statement pipeline in `services/hod_statement_service.py`, which in turn delegates the shared rendering, LLM inference, SMTP dispatch, and archival logic to `services/digest_service.py`.
...
### HOD-approval-hierarchy mode (`HOD_APPROVAL_ENABLED`)

The entire HOD digest — both endpoints, plus the monthly cron's HOD branch — is suppressed when `core/config.py::HOD_APPROVAL_ENABLED` is `False` (the default — flat/admin-only mode, for deployments with no department/HOD hierarchy):

- **Bulk trigger** (`POST /admin/send-hod-statements`) and the cron's HOD branch both call `services/hod_statement_service.py::generate_and_send_hod_bulk`, which now guards at the top: when the flag is off, it returns immediately (`{"ok": True, "total_departments": 0, "sent": 0, "skipped": 0, "failed": 0}`, logged as a no-op) without touching `department_hod_mapping` at all.
- **Single-department trigger** (`POST /admin/send-hod-statement/{corrected_department_name}`) additionally guards directly in this router, after rate-limiting and period validation: it raises `422` with *"HOD usage digests are disabled: this deployment has no department/HOD hierarchy (HOD_APPROVAL_ENABLED is False)."* rather than silently no-op'ing, since a manual single-department call with an explicit department name is more likely to be a mistake worth surfacing than the bulk/cron paths are.

There is no admin-rollup equivalent this suppression redirects to — flat-mode deployments simply receive no HOD usage-digest emails. Existing HOD-hierarchy deployments must set `HOD_APPROVAL_ENABLED=true` to keep receiving them.
...
| Department roster resolution & HOD payload construction | `services/hod_statement_service.py` |
| Shared LLM inference, rendering, SMTP dispatch, archival | `services/digest_service.py` |
| Monthly cron orchestration (HOD + Manager bulk) | `services/digest_service.py::_job_team_digest` |
| Admin authorization | `auth/rbac.py::require_admin_flag` |
| Rate limiting | `core/rate_limiter.py` (`SENSITIVE_ADMIN`) |
...
| `require_admin_flag` | `auth/rbac.py` | Admin-only access control |
| `logger` | `core/logger.py` | Structured audit logging |
| `enforce_rate_limit_with_behaviour` | `core/rate_limiter.py` | Rate-limit enforcement |
| `SENSITIVE_ADMIN` | `core/rate_limiter.py` | Pre-defined sensitive-admin rate-limit config |
| `generate_and_send_hod` | `services/hod_statement_service.py` | Single-department pipeline |
| `generate_and_send_hod_bulk` | `services/hod_statement_service.py` | Bulk fan-out pipeline |
...
- `services/digest_service.py` — shared digest rendering, LLM inference, SMTP dispatch, and archival logic.
- `services/hod_statement_service.py` — HOD-specific roster resolution and payload building.
...
- **HOD-specific orchestration** lives in `services/hod_statement_service.py`.
- **Cross-cutting digest logic** (LLM inference, email rendering, SMTP, archive) lives in `services/digest_service.py`.
- **Scheduled/cron execution** is handled by `services/digest_service.py::_job_team_digest`, which runs both HOD and Manager bulk sends for the current IST billing month.
