...
2. **Rate limit** — `enforce_rate_limit_with_behaviour(request, SENSITIVE_ADMIN)` applies a 50 req/min per IP+user sliding-window limit (see [core_infrastructure](../core/core_infrastructure.md) / `core/rate_limiter.py`).
...
RBAC), see [core_infrastructure](../core/core_infrastructure.md). For the HOD
