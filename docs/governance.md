...
|------------|-------------|
| [auth](auth.md) | RBAC permission checks (`is_admin`, `can_manage_suppression`, `require_admin`, `get_current_user`) |
| [inbox](inbox.md) | Governance approval notifications delivered via inbox store |
...
| [core_infrastructure](core_infrastructure.md) | Redis KV, Postgres, logger, audit signer |
| [database](database.md) | `GovernanceEvent`, `GovernanceSuppression` models |
| [agent_system](agent_system.md) | Agent builder integration for listing and hot-reloading agents |
| [mcp_system](mcp_system.md) | MCP tool registry and marketplace status sync |
| [core_workflow_repo](core_workflow_repo.md) | Artifact resolution (agents, skills, workflows) for approval previews |
