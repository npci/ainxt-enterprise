...
The `scim_router` module exposes a **SCIM 2.0** (System for Cross-domain Identity Management) provisioning surface at `/scim/v2`. It allows enterprise Identity Providers (IdPs) such as Okta, Azure AD, Ping, or OneLogin to automatically provision, update, and de-provision users in the AiNxt Cowork platform.
...
- **db/models.py** (referenced as `db.models.User`): Defines the `User` table schema that SCIM maps to and from.
- **db/database.py**: Provides `SessionLocal` used for transactional database access.
- **core/logger.py**: Structured logging used for provisioning and de-provisioning audit events.
- **sso.py** (in `auth/`): Implements the actual SSO provider integration for SCIM-provisioned users.
