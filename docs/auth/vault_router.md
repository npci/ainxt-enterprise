...
- **auth.dependencies** — JWT-based authentication and admin role enforcement. `vault_router` uses `get_current_user` for read-only endpoints and `require_admin` for all mutating endpoints and value retrieval.
- **store.credential_vault** — The underlying encrypted credential store. It handles Fernet encryption, conflict detection, atomic rotation, and metadata retrieval.
- **core.logger** — Structured logging used to emit an audit record whenever an admin retrieves a decrypted credential value.
...
`vault_router` is one of many shared API routers in the platform. It is typically mounted alongside routers such as [auth_router](auth_router.md), [connectors_router](../connectors/connectors_router.md), and [api_keys_router](../products/api_keys_router.md). While `api_keys_router` manages user-level API keys, `vault_router` manages shared organizational secrets used by connectors, agents, and backend services.

Credentials stored through this router are consumed by the connector engine and other backend components that need to retrieve secrets without embedding them in code or configuration. See connectors/engine and store/credential_vault for how stored credentials are resolved at runtime.
