...
This module is part of the broader ai_ui_frontend application and consumes the [auth_router](auth_router.md) backend endpoints for user lifecycle operations.
...
|------------|--------|---------|
| `usePermission` | [ai_ui_frontend_hooks](../ui/ai_ui_frontend_hooks.md) | Role-based access check (`isAdmin`, `isSecurity`) |
| `authFetch` | ai_ui_frontend_config | Authenticated HTTP requests |
| `toISTDate` | [ai_ui_frontend_utils](../ui/ai_ui_frontend_utils.md) | Date formatting to IST timezone |
...
- **RBAC:** The roles managed here (`viewer`, `developer`, `operator`, `security`, `admin`) align with the platform-wide role-based access control implemented in auth_rbac.

For related administrative interfaces, see:

- [Budget Manager](../llm/budget_manager.md) — budget allocation and approvals
- [Model Governance](../llm/model_governance.md) — model access permissions
- [Level Overrides](../llm/level_overrides.md) — temporary permission elevation
- [Endpoint Manager](../products/endpoint_manager.md) — API key and endpoint management
