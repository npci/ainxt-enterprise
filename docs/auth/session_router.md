...
The router is mounted under the `/auth` prefix (see [gateway.md](../core/gateway.md) for the application route table) and provides three operations:
...
|------------|---------------------|
| `auth.dependencies.get_current_user` | Extracts and validates the JWT or API key, enriches the identity payload, and returns the user's `sub` and current `sid`. See auth_dependencies.md. |
| `auth.session_manager` | Provides `get_active_sessions`, `revoke_all_sessions`, and `revoke_session`. This is the authoritative session store interface. See session_manager.md. |
| `auth.jwt_handler.decode_token` | Validates the JWT on every request and checks that the `sid` claim is still active. See jwt_handler.md. |
...
- session_manager.md — Redis/Postgres session registry implementation.
- jwt_handler.md — token encoding, decoding, and `sid` validation.
- auth_dependencies.md — identity extraction and enrichment.
- [gateway.md](../core/gateway.md) — top-level FastAPI application and router mounting.
