...
- **[KbChat.jsx](kb_chat.md)** — mounts `DocPickerCard` and handles `onConfirm` by re-sending the scoped query.

### Backend Collaborators

- **[gateway.py](gateway.md) `POST /ask`** — emits the `__clarify__` SSE frame and honors `kb_doc_ids` on re-query.
...
- **[DocsPanel.jsx](documents_guide.md)** — platform documentation browser.
- **[DocPreviewCard.jsx](documents_preview.md)** — expandable document summary card.
- **[DocWorkflowCard.jsx](documents_generation.md)** — PPTX theme picker for generated presentations.
...
- [kb_chat.md](kb_chat.md) — the parent KB chat component that hosts the picker.
- [gateway.md](gateway.md) — the `/ask` endpoint and disambiguation gate.
...
- [documents.md](documents.md) — overview of the `documents` feature family.
