...
Both routers share the same local release directory (`CLI_RELEASES_DIR`) and the same authentication/authorization primitives from [`auth_router`](auth_router.md) and [`auth/dependencies`](shared_core.md#authentication).
...
| `core.logger.logger` | Structured logging for update and fleet events. | [`shared_core.md#core_infrastructure`](shared_core.md#core_infrastructure) |
| `db.database.SessionLocal` | SQLAlchemy session management. | [`shared_core.md#database`](shared_core.md#database) |
| `db.models.CliVersionRecord` | Persistence model for CLI installation telemetry. | [`shared_core.md#database`](shared_core.md#database) |
...
- Fleet analytics data is stored in the same PostgreSQL database used by the rest of the platform, modeled by `CliVersionRecord` in [`shared_core.md#database`](shared_core.md#database).
- Admin dashboards that display version adoption or drive update campaigns consume the `/cli/versions/*` endpoints. These dashboards are typically part of the [`ai_ui_frontend`](ai_ui_frontend.md) admin surfaces.
...
- **Monitoring**: The router emits structured log lines prefixed with `[cli-updates]` and `[cli-fleet]`; these can be scraped by the platform's observability stack (see [`shared_core.md#observability`](shared_core.md#observability)).
