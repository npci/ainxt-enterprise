...
The [gateway](gateway.md) exposes trace data through its `audit_and_tracing` sub-module:
...
- **[middleware](middleware.md)** (`request_id_middleware.py`) — assigns / propagates the ID on inbound HTTP.
...
- **[llm_proxy](llm_proxy.md)** (`core/logger.py`) — `set_request_id()` / `set_span_id()` bind the ID into structured logs.
...
The [agent_system](agent_system.md) (`agents/react_engine.py::ReactEngine`) is a primary consumer of per-turn tracing. Its iterative retrieve → analyze → critique → synthesize loop maps naturally onto spans:
...
| **Consumers** | [evals_evolution](evals_evolution.md) harness, [gateway](gateway.md) trace endpoints, debug replay, evolution experiments. |
