...
|---|---|---|
| `hooks` (`useFileDrop`) | Drag-and-drop file handling with ref-callback pattern for lazy-mounted drop zones | [hooks](hooks.md) |
| `config` | `authFetch` (cookie-auth + retry on GET), `API_BASE` | [config](config.md) |
| `ui_dialog` | `useToast` for notifications, `useConfirm` for destructive-action dialogs | [ui_dialog](ui_dialog.md) |
| `scope_picker` | Shared Domain/Product/Version/SourceType selector used by both upload and chat scope | [scope_picker](scope_picker.md) |
...
|---|---|---|
| `api_kb` | ABStudio backend KB API routes (`upload_build_studio_doc`, `_parse_with_retry`, etc.) | [api_kb](api_kb.md) |
...
| `document_knowledge_workers` | `kb_worker.run_activate_doc`, `kb_cleanup_worker`, `kb_entity_worker`, `knowledge_graph_worker` | [document_knowledge_workers](document_knowledge_workers.md) |
