...
> See [governance_feature.md](governance_feature.md) for `StatusBadge` and `SubmitApprovalButton`.
...
|---|---|---|
| `useWorkflowStore` | [store.md](store.md) | Canvas graph (`nodes`, `edges`), execution state (`isExecuting`, `workflowId`), knowledge config, name/id setters |
| `useDashboardStore` | [store.md](store.md) | Workflow list, `updateWorkflow`, `loadWorkflows`, `useTemplate` (clone) |
| `useAgentsStore` | [store.md](store.md) | Agent list, `loadAgents` |
| `useTemplateAdminStore` | [store.md](store.md) | `updateTemplate` (optional template editor; safe to remove) |
...
| `KnowledgeSection` (`KB_MODE_NONE`) | [common_components.md](common_components.md) | Canonical default knowledge config constant. |
...
> See [workflow_editor.md](workflow_editor.md) for `ChatPanel`'s SSE handling and abort controller.
...
> See [workflow_editor.md](workflow_editor.md) for `Sidebar`, `Canvas`, `ConfigPanel`, and `ChatPanel` internals.
...
> See [governance_feature.md](governance_feature.md) for details. The backend governance client lives in `ABStudio/backend/app/core/governance_client.py`.
...
| Module | Relationship |
|---|---|
| [workflows_feature.md](workflows_feature.md) | `WorkflowsDashboard` and `RunningWorkflowToast` are rendered in the dashboard view. `App` passes `onOpenWorkflow`, `onOpenTemplate`, `onPreviewTemplate` callbacks. |
| [agents_feature.md](agents_feature.md) | `AgentsDashboard` is rendered in the agents section. `AgentEditor` takes over the full screen when `editingAgent` is set. |
| [skills_feature.md](skills_feature.md) | `SkillsDashboard` is rendered in the skills section (no callbacks needed — self-contained). |
| [tools_feature.md](tools_feature.md) | `ToolsDashboard` is rendered in the tools section (self-contained). |
| [workflow_editor.md](workflow_editor.md) | `Sidebar`, `Canvas`, `ConfigPanel`, `ChatPanel` compose the editor surface inside `EditorShell`. |
...
| [governance_feature.md](governance_feature.md) | `StatusBadge` and `SubmitApprovalButton` render in the editor header. |
| [store.md](store.md) | All four Zustand stores consumed by `App`. |
| [utils.md](utils.md) | `editorPersistence` (localStorage view-state) and `validateName` (name validation). |
| [common_components.md](common_components.md) | `KnowledgeSection` provides `KB_MODE_NONE`, the canonical default knowledge config. |
| [abstudio_backend.md](abstudio_backend.md) | Backend API endpoints consumed via stores: `/workflows`, `/workflows/{id}`, `/templates/{id}`, `/templates/{id}/use`, `/agents`, `/auth/me`. |
