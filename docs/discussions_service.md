...
- **macOS compatibility**: Uses `rq.SimpleWorker` on Darwin to avoid the fork-crash documented in `workers/tenx_worker_main.py` and `workers/start_workers.py`.
- **Compliance**: Unlike the legacy Threads @AiNxt flow, this module routes through `AgentRunner`, which applies compliance checks on input, prompt, and output stages. See [shared_core.md](shared_core.md) for details on `AgentRunner`.
...
|------------|--------|------|
| `core.discussions_engine_client` | [shared_core.md](shared_core.md) | Headless read/write client for the Apache Answer engine. |
| `agents.agent_builder.AgentRunner` | [shared_core.md](shared_core.md) | Shared agent runtime and compliance wrapper. |
| `db.models` (Discussions tables) | [shared_core.md](shared_core.md) | Local mirror and event-log schema. |
| `routers/discussions_router.py` | [shared_api_routers.md](shared_api_routers.md) | API surface that enqueues the bot job. |
| `workers/skill_loop_worker.py` | [workers.md](workers.md) | Future consumer of `discussions_events` for self-improvement. |
| `core.job_queue` | [shared_core.md](shared_core.md) | RQ queue factory and `Q_DISCUSSIONS` constant. |
