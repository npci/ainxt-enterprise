...
- Uses `authFetch` from the shared [config](config.md) module for authenticated, retry-aware HTTP requests.
...
| Module | Relationship |
|--------|--------------|
| [config](config.md) | Provides `authFetch` and `API_BASE` for authenticated API calls. |
| [audit_router](audit_router.md) | Backend router exposing the `/audit` endpoint and `list_audit_logs` logic. |
| [gateway](gateway.md) | Hosts the `/audit` route and forwards requests to the audit router. |
...
- [config](config.md) — Authentication and API base configuration.
- [audit_router](audit_router.md) — Backend audit log retrieval.
