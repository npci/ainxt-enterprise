...
Read methods are used by dashboards, health probes, and the connectors_router `get_metrics` endpoint. They fetch from the same KV keys and sorted sets written during recording.
...
- **connectors_router** — exposes metrics through the `get_metrics` endpoint.
...
- shared_api_routers_connectors_router — API surface that exposes connector metrics.
- shared_core_core_infrastructure — `core.config` and `core.kv` configuration details.
