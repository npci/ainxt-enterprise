...
- **MCP system**: The registry bootstraps connector tools into the shared `mcp_system` `ToolRegistry` so agents can call them via `tool_use`.
- **Guardrails**: The engine calls the shared [`guardrails`](../security/guardrails.md) / compliance engine for input/output scanning.
- **Authentication**: User tokens are read from `ainxt.user_oauth_tokens`; PAT auto-connect reads from the profile vault managed by `shared_api_routers_profile_router`.
...
- **Document workers**: Document generation and skill-driven builds are enqueued to RQ workers documented under `workers_document_knowledge_workers`.
- **Memory**: Cowork durable memory is persisted through the memory system documented under `shared_core_memory_system`.
