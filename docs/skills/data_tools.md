...
- **`MCPRegistry` / `ToolRegistry`** — Bootstrap the tools at platform startup so agents and workflows can invoke them by name. See [mcp_system](../mcp/mcp_system.md) and tool_registry.
...
| tool_registry | Stores `ToolDefinition` records, executes tools, and captures timing/errors. |
