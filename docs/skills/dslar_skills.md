...
|------------|---------|---------------|
| `dslar_pdf_extraction` | Extract text, tables, and image metadata from DL-SAR PDFs. | dslar_pdf_extraction.md |
| `dslar_image_enrichment` | Describe embedded images with vision models and clean raw agent JSON output. | dslar_image_enrichment.md |
| `dslar_clause_chunking` | Split large reports into chunks and aggregate parallel clause validation results. | dslar_clause_chunking.md |
| `dslar_report_rendering` | Render the final validation report PDF from enriched.json. | dslar_report_rendering.md |
