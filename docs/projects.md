...
This module is intentionally modeled after the main [Chat](chat.md) experience but adds repository-aware retrieval: every question is automatically prefixed with project and codebase context so the orchestrator never classifies a project question as "general" and skips RAG retrieval.
...
5. **Message history** — Conversation history is fetched from the server (`workspace_messages_store`) and is the single source of truth. History survives reloads and navigation because message/loading state is hoisted in [App.jsx](ai_ui_frontend_app_core.md).
...
The Projects module sits alongside the main [Chat](chat.md), [Knowledge Base](knowledge_base.md), [Code](code.md), and [Agents Catalog](agents_catalog.md) experiences in the `ai_ui_frontend` portal. It reuses shared infrastructure:

- **Authentication** — Uses `authFetch` and the httpOnly-cookie session managed by [AuthContext](auth.md).
- **Budget governance** — Every ask checks the user's budget via [budget_store](budget.md) and displays usage metadata via [MessageMeta](message_meta.md).
- **File upload & compliance** — Document uploads go through the shared `/chat/upload` endpoint; blocked files render the same compliance-block card used in [Chat](chat.md).
...
Each `Message` object follows the shared chat message schema used by [Chat](chat.md) and includes optional metadata fields: `modelLabel`, `costUsd`, `latency`, `inTok`, `outTok`, `tokensToday`, `maxTokensToday`, `requestsToday`, `maxRequestsToday`.

---

## API Surface

The frontend consumes the following endpoints. Full backend contract details live in [projects_router](shared_api_routers.md#projects_router).
...
- [ai_ui_frontend_app_core](ai_ui_frontend_app_core.md) — Hoisted project state and routing.
- [chat](chat.md) — Shared chat patterns, upload, and stop behavior.
- [message](message.md) / [message_meta](message_meta.md) — Message rendering and usage metadata.
- [auth](auth.md) — Authentication and `authFetch`.
- [product_manager](product_manager.md) — Product catalog used by project forms.
- [shared_api_routers](shared_api_routers.md#projects_router) — Backend `projects_router` API details.
- [shared_core](shared_core.md) — `OrchestratorAgent`, `model_router`, and store layer.
