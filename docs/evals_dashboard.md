...
The dashboard is read-only analytics; it does not trigger evaluations itself. Evaluation execution is handled by [`core/evals.py`](core_evals.md) and persisted in the `eval_results` table.
...
Evaluations are stored in the `eval_results` table (see [`db/models.py`](db_models.md)):
...
All requests are made through [`authFetch`](ai_ui_frontend_config.md), which attaches credentials and a correlation header.
...
|------------|--------|---------|
| `authFetch` | [`ai-ui/src/config.js`](ai_ui_frontend_config.md) | Authenticated HTTP client with credentials and retry logic. |
| `API_BASE` | [`ai-ui/src/config.js`](ai_ui_frontend_config.md) | Base URL for API calls. |
...
| `EvalEngine` | [`core/evals.py`](core_evals.md) | Runs LLM-as-judge evaluations and writes to the database. |
| `EvalResult` model | [`db/models.py`](db_models.md) | Persistence schema for evaluation results. |
...
- **[`AgentAnalytics`](agent_analytics.md)** — focuses on per-agent usage and cost, whereas Evals Dashboard focuses on output quality.
- **[`Monitoring`](monitoring.md)** — tracks queue health and circuit breakers, not LLM judgment scores.
- **[`Coach`](coach.md)** — provides user-facing coaching recommendations derived from similar quality signals.
- **[`SDLCPipeline`](sdlc_pipeline.md)** — produces `sdlc_classification` and `sdlc_solution` evaluations that appear in the dashboard.
- **[`Chat`](chat.md)** / **[`KbChat`](kb_chat.md)** — generate `retrieval_quality`, `groundedness`, `relevance`, and `code_quality` evaluations in the background.

The dashboard is a pure consumer of the evaluation pipeline. It does not write evaluations, trigger retries, or modify SDLC state. For blocking SDLC quality gates, see [`EvalEngine.eval_sdlc_classification`](core_evals.md) and [`EvalEngine.eval_sdlc_solution`](core_evals.md).
...
- [`core_evals.md`](core_evals.md) — LLM-as-judge evaluation engine.
- [`db_models.md`](db_models.md) — Database models including `EvalResult`.
- [`ai_ui_frontend_config.md`](ai_ui_frontend_config.md) — `authFetch` and `API_BASE` configuration.
- [`agent_analytics.md`](agent_analytics.md) — Related agent usage analytics.
- [`sdlc_pipeline.md`](sdlc_pipeline.md) — SDLC pipeline that produces SDLC evals.
- [`chat.md`](chat.md) / [`kb_chat.md`](kb_chat.md) — Chat surfaces that produce chat evals.
