...
- [ai_ui_frontend.md](ai_ui_frontend.md) — Chat, KbChat, IDE, and Cowork surfaces that emit events.
- [shared_core.md](shared_core.md) — `core/coach_events.py` and the coach ingestor for normalization and redaction.

### Downstream: Storage & UI

Evaluated hits and scores are consumed by:

- [shared_api_routers.md](shared_api_routers.md) — `coach_router.py` and `coach_admin_router.py` expose dashboard, rules, and admin APIs.
- [ai_ui_frontend.md](ai_ui_frontend.md) — `Coach.jsx` and `CoachAdmin.jsx` render scores, recommendations, and admin controls.
- [shared_core.md](shared_core.md) — `store/inbox_store.py` receives nudges published by `publish_coach_inbox`.

### Async Processing

The Kafka-based consumer decouples event ingestion from evaluation:

- [workers.md](workers.md) — `workers/coach_consumer.py` polls the coach topic and calls the evaluator.
- [workers.md](workers.md) — `workers/broadcast_worker.py` can deliver weekly coaching digests.
...
Defined in [shared_core.md](shared_core.md) (`db/models.py`):
...
- [ai_ui_frontend.md](ai_ui_frontend.md) — Coach user and admin interfaces.
- [shared_api_routers.md](shared_api_routers.md) — Coach REST API endpoints.
- [shared_core.md](shared_core.md) — Coach event ingestion, data models, and inbox store.
- [workers.md](workers.md) — Coach consumer and broadcast workers.
