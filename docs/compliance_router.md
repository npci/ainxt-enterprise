...
The `ComplianceEngine.validate_input()` method (see [agent_system](agent_system.md) for full details) executes a multi-layer pipeline:
...
> **Note:** The ML privacy filter is skipped when (a) `PRIVACY_SVC_URL` is not set, (b) the text is pure code, or (c) a regex detector already found a block-configured type (the request will be rejected regardless). See [privacy_service](privacy_service.md) for ML filter details.
...
|---|---|---|
| `ComplianceEngine` | [agent_system](agent_system.md) | Multi-layer PII/PCI detection (regex + ML) and redaction/blocking |
...
| `get_current_user` | [authentication](authentication.md) | JWT-based authentication for batch endpoint |
| `logger` | [core_infrastructure](core_infrastructure.md) | Structured logging |
...
| [privacy_service](privacy_service.md) | The ML-based NER privacy filter service called by `ComplianceEngine` when `PRIVACY_SVC_URL` is set |
