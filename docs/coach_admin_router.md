...
The router interacts with the following models from [db/models.py](database.md):
...
|---|---|---|
| `auth/rbac.py` | [Authentication](authentication.md) | `require_admin_flag` dependency for all routes. |
| `core/config.py` | [Core Infrastructure](core_infrastructure.md) | Feature flags and weekly mail schedule config. |
| `db/models.py` | [Database](database.md) | All Coach-related ORM models. |
...
| `workers/coach_weekly_mail_worker.py` | [Workers](worker_orchestration.md) | HTML template rendering for coaching messages. |
| `services/smtp_service.py` | [Services](services.md) | HTML email delivery for manual coaching notes. |
| `store/inbox_store.py` | [Store Layer](store_layer.md) | Inbox item publishing for coaching note delivery. |
