...
The component is part of the [workflow editor](#workflow-editor-context) sub-module tree and is rendered inside the [ConfigPanel](ConfigPanel.md) when a Loop node is selected and its mode is set to `for_each`.
...
The `itemsExpression` value flows through the [workflow store](store.md) (`workflowStore.js`):
...
Defined in `ABStudio/backend/app/api/chat.py::get_node_last_output`, this endpoint delegates to `NativeEngine.get_node_last_output(thread_id, node_id)`, which loads the persisted record from the [CheckpointStore](checkpoint.md) via `load_node_output(thread_id, node_id)`.
...
The [LoopNode](workflow_editor_nodes.md) component reads `data.itemsExpression` to render a human-friendly label on the canvas (e.g. "For each doc" instead of the raw path). This label is purely cosmetic — the dotted path remains the source of truth in `node.data`.
...
| [LoopNode](workflow_editor_nodes.md) | Canvas node component that displays the loop mode and reads `itemsExpression` for its label. |
| [workflowStore](store.md) | Zustand store holding nodes, edges, and the `getWorkflowForExecution` serializer that forwards `itemsExpression` to the backend. |
| [api_execution](api_execution.md) | Backend execution endpoints (`/run-stream`) that drive the NativeEngine, which persists per-node outputs consumed by the picker. |
| [api_chat](api_chat.md) | Backend chat endpoints including `GET /node-last-output` that the picker calls to fetch upstream output. |
| [engine_native_engine](engine_native_engine.md) | The `NativeEngine` class that executes workflows, runs `_run_loop`, and provides `get_node_last_output()`. |
| [checkpoint](checkpoint.md) | Checkpoint store layer that persists per-node outputs (`load_node_output`) queried by the picker. |
| [LoopWhileEditor](workflow_editor_conditions.md) | Sibling component rendered in the ConfigPanel for the Loop node's `while` mode (condition-driven iteration). |
