...
Additionally, `core/telemetry.py` provides a **separate** `_TelemetryMetrics` object (`telemetry_metrics`) that tracks request/agent/workflow counters, latency percentiles, and per-model cost. The gateway merges both in its `get_metrics` JSON endpoint. See [core_infrastructure.md](core_infrastructure.md) for details on the telemetry layer.
...
| `local_llm_queries` | `int` | Queries escalated to a local LLM (`local_llm` / `neuron` scope). |
...
flowchart LR
    ...
    C --> D{execution_scope?}
    D -->|"local_llm / neuron"| E["local_llm_queries += 1"]
...
| `core.kv.get_kv()` | Factory that returns a Redis or RustyCluster client based on configuration. See [kv_store.md](kv_store.md). |
| `core.logger.logger` | Structured logging; all errors are caught and logged, never raised. See [core_infrastructure.md](core_infrastructure.md). |
...
Background workers (see [worker_orchestration.md](worker_orchestration.md)) call the Prometheus recording methods to instrument task lifecycle events:
...
The module sits within the `shared_core > observability` namespace alongside [observability_tracing.md](observability_tracing.md). While tracing captures per-request span trees, this module captures aggregate counters and time-series. The `core/telemetry.py` layer (see [core_infrastructure.md](core_infrastructure.md)) provides a third, overlapping telemetry surface focused on request/agent/workflow counts and per-model cost — the gateway merges all three for its JSON and Prometheus endpoints.
