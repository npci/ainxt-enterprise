...
- Links the canvas node to a saved agent or workflow via [`SubflowPicker`](SubflowPicker.md).
...
| `for_each` | Iterate over a list | [`LoopItemsPicker`](LoopItemsPicker.md) |
...
- [`SubflowPicker`](SubflowPicker.md) — saved agent / workflow / template linker.
- [`LoopItemsPicker`](LoopItemsPicker.md) — upstream list detector for `for_each` loops.
...
- [`workflow_factory`](workflow_factory_pipeline.md) / [`agent_factory`](agent_factory_pipeline.md) — generate instructions and templates.
