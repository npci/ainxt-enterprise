...
This module is part of the `ai_ui_frontend` application and is closely related to the broader [budget](../llm/budget.md), [agent_analytics](../agents/agent_analytics.md), [evals_dashboard](evals_dashboard.md), and [model_governance](../llm/model_governance.md) modules.
...
| Module | Relationship |
|--------|--------------|
| [budget](../llm/budget.md) | Also consumes `model_usages` and `users` for cost and utilization tracking. |
...
| [model_governance](../llm/model_governance.md) | Defines which models are allowed per department; department metrics show actual model consumption. |
| [auth](../auth/auth.md) | Supplies user identity, roles, and department membership used by the queries. |
...
- **Cost governance:** It surfaces which departments are driving token spend and which models they use, complementing the [budget](../llm/budget.md) and [budget_manager](../llm/budget_manager.md) modules.
- **Model governance:** By showing per-department model breakdowns, it helps administrators verify compliance with department-level model allow-lists managed by [model_governance](../llm/model_governance.md).
