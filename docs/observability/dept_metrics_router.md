...
This router is part of the [shared_api_routers](../core/shared_api_routers.md) layer and is consumed by dashboards such as the AI-UI dept_metrics component to render cross-department usage summaries, per-department drill-downs, and model-level breakdowns.
...
- **`_get_db`** — A local dependency factory that imports `SessionLocal` from db.database and yields a SQLAlchemy session, ensuring it is closed after each request.
...
|---|---|---|
| `db.database.SessionLocal` | SQLAlchemy session factory | shared_core_database |
| `users` table | Department membership and user identity | db/models |
| `model_usages` table | Per-request LLM usage records | telemetry, [budget_router](../llm/budget_router.md) |
| `eval_scores` table | Evaluation quality scores | [evals_router](evals_router.md) |

The router does **not** currently apply its own authorization decorator; it is expected to be mounted behind an admin-scoped dependency at the application level or protected by upstream gateway rules. For role-based access patterns, see auth/rbac and [admin_router](../api/admin_router.md).

---

## Integration with the Wider System

- **AI-UI Dashboard** — The DeptMetrics React component consumes `/dept-metrics/summary`, `/{dept}`, and `/{dept}/models` to render `StatCard` visualizations.
- **Budget & Cost Governance** — Cost and token totals overlap conceptually with [budget_router](../llm/budget_router.md), which manages per-user and per-department budgets, HOD caps, and allocation ledgers. The metrics router is read-only; budget enforcement happens elsewhere.
- **Model Governance** — Per-department model breakdowns complement [model_governance_router](../llm/model_governance_router.md), which controls which models a department or user is allowed to use.
...
- **Telemetry** — `model_usages` is populated by the telemetry subsystem described in shared_core_core_infrastructure.
...
- [shared_api_routers](../core/shared_api_routers.md) — Parent module containing all shared routers.
- [budget_router](../llm/budget_router.md) — Department and user budget management.
- [evals_router](evals_router.md) — Evaluation result listing and summaries.
- [model_governance_router](../llm/model_governance_router.md) — Department-level model permissions.
- [admin_router](../api/admin_router.md) — Admin configuration and circuit-breaker controls.
- ai_ui_frontend_dept_metrics — Frontend dashboard that consumes these endpoints.
- shared_core_database — Database models and session management.
