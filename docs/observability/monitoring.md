...
| `../config` (`API_BASE`, `authFetch`) | Authenticated fetch wrapper (JWT via httpOnly cookie). See [config](../core/config.md). |
...
| Circuit Breakers | `core/circuit_breaker.py` | [core_infrastructure](../core/core_infrastructure.md) |
| Key Metrics (requests, errors, latency, cache, compliance, agents) | `core/telemetry.py::_TelemetryMetrics` | [core_infrastructure](../core/core_infrastructure.md) |
| Key Metrics (legacy summary) | `metrics.py::Metrics` | [observability](observability.md) |
| Queue Health + Recent Jobs | `core/job_queue.py` + `routers/jobs_router.py` | [shared_api_routers](../core/shared_api_routers.md) |
| SDLC Runs by State/Type | `routers/sdlc_router.py::get_stats` | [sdlc_router](../sdlc/sdlc_router.md) |
| Context Compression | `core/compress_metrics.py` | [core_infrastructure](../core/core_infrastructure.md) |
| Model Usage (calls/tokens/cost) | `core/telemetry.py::record_model_usage` | [core_infrastructure](../core/core_infrastructure.md) |
...
> The Prometheus endpoint (`GET /metrics/prometheus`) augments in-memory counters with DB-backed values on each scrape so the dashboard always shows real numbers even after a gateway restart. See [core_infrastructure](../core/core_infrastructure.md) for the dual-mode (in-memory + DB-seeded) counter design.
...
> See [core_infrastructure](../core/core_infrastructure.md) for the Redis-backed state machine (CLOSED → OPEN → HALF_OPEN → CLOSED) and the fail-open-on-KV-unavailable safety policy.
...
> See [shared_api_routers](../core/shared_api_routers.md) for the full queue back-pressure and DLQ architecture.
...
> See [sdlc_router](../sdlc/sdlc_router.md) for the full SDLC pipeline state machine and HITL gate lifecycle.
...
> See [core_infrastructure](../core/core_infrastructure.md) for the Redis-backed counter design (8-day TTL, no hot-path DB writes).
