...
| **KV backend map** | Logs the per-database KV backend resolution  for incident postmortems. |
...
| **Skill seeding** | Seeds bundled first-party skills and NPCI domain skills/agent templates (idempotent upserts). |
...
7. **KV clients** — Closes all sync and async KV clients (Redis connection pools).
...
| **KV Store** | Non-critical (reported per DB) | Per-DB backend ping via `kv_health_status()` |
...
> **See also:** [shared_core](../core/shared_core.md) — `core/circuit_breaker.py` for the `CircuitBreaker` class implementation and state machine details.
...
> **See also:** [shared_core](../core/shared_core.md) — `core/telemetry.py` for `_TelemetryMetrics` and `metrics.py` for the legacy `Metrics` class.
...
> **See also:** [shared_core](../core/shared_core.md) — `core/compress_metrics.py::get_stats` for the implementation.
...
> **See also:** [llm_proxy](../llm/llm_proxy.md) — `core/logger.py` for `bind_context` / `clear_bound_context` implementation.
...
|------------|--------|---------|
| `_TelemetryMetrics` | [shared_core](../core/shared_core.md) (`core/telemetry.py`) | In-memory + Prometheus counters, latency histograms, model usage tracking |
| `Tracer` | [shared_core](../core/shared_core.md) (`core/telemetry.py`) | OTLP/in-memory distributed tracing |
| `Metrics` | [shared_core](../core/shared_core.md) (`metrics.py`) | Legacy Redis-persisted query metrics |
| `CircuitBreaker`, `all_breaker_states` | [shared_core](../core/shared_core.md) (`core/circuit_breaker.py`) | Per-provider circuit breaker state machine with Redis persistence |
| `kv_health_status`, `kv_backend_map`, `close_all_kv`, `async_close_all_kv` | [shared_core](../core/shared_core.md) (`core/kv/`) | KV store health, backend resolution, and connection cleanup |
| `get_stats` | [shared_core](../core/shared_core.md) (`core/compress_metrics.py`) | Context compression telemetry from Redis |
| `bind_context`, `clear_bound_context` | [llm_proxy](../llm/llm_proxy.md) (`core/logger.py`) | Structured logging context for Loki probe |
| `validate_prod_config` | [shared_core](../core/shared_core.md) (`core/config.py`) | Production configuration validation |
| `docker_executor`, `LANGUAGE_CONFIG` | [shared_core](../core/shared_core.md) (`sandbox/docker_executor.py`) | Docker sandbox availability and image verification |
| `seed_platform_skills` | [shared_api_routers](../core/shared_api_routers.md) (`routers/skills_router.py`) | Skill seeding on startup |
| `gateway_bootstrap` | [shared_core](../core/shared_core.md) (`services/llm_spend/`) | LLM spend tracking scheduler lifecycle |
| `digest_service` | [shared_core](../core/shared_core.md) (`services/digest_service.py`) | HOD/Manager digest cron lifecycle |
