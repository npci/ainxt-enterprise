...
The Canvas is mounted inside the `EditorShell` layout (see [app_core](app_core.md)) alongside the [Sidebar](workflows_feature_editor_sidebar_pickers.md), [ConfigPanel](workflows_feature_editor_config_panel.md), and [ChatPanel](workflows_feature_editor_chat_panel.md). It reads and writes all graph state through the shared Zustand-based `workflowStore`, which also powers undo/redo, execution tracking, and the serialization path that sends the workflow definition to the backend `NativeEngine` for execution.
...
For details on the execution engine, see [engine_native_engine](engine_native_engine.md).
...
| Module | Relationship |
|--------|-------------|
| [workflowStore](store.md) (`store/workflowStore.js`) | Central state store — nodes, edges, selection, execution state, undo/redo |
...
| [EditorShell](app_core.md) (`App.jsx`) | Parent layout — wraps Canvas in `ReactFlowProvider`, manages edit/preview mode |
...
| [NativeEngine](engine_native_engine.md) | Backend graph traversal engine that consumes the Canvas's workflow definition |
...
- [EditorShell & App Core](app_core.md) — Parent layout, mode switching, workflow name editing
...
- [Workflow Store](store.md) — Zustand store with undo/redo, execution state, graph validation
- [NativeEngine](engine_native_engine.md) — Backend graph traversal and agent execution engine
- [Workflows Dashboard](workflows_feature_dashboard.md) — Workflow list, creation, and card management
