...
- [gateway](gateway.md) — enqueues jobs via `core.job_queue.enqueue_job`.
- [shared_core](shared_core.md) — provides `agents`, `models.model_router`, `workflows.engine`, `sandbox.docker_executor`, `tools.security_scan_tools`, and compliance engines.
- [shared_api_routers](shared_api_routers.md) — exposes endpoints that trigger these workers (chat, agents, workflows, secure code gate, etc.).
...
1. A user or system request reaches the [gateway](gateway.md) or a [shared_api_routers](shared_api_routers.md) endpoint.
...
All producers use `core.job_queue.enqueue_job` (see [shared_core](shared_core.md)). The function:
...
Every worker that handles user input or generated code invokes the shared compliance engine (`agents.compliance_engine`) and/or security scanners (`tools.security_scan_tools`). See [shared_core](shared_core.md) for details.
