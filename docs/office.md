...
Unlike the [Code](code.md) module, which runs a local coding agent inside the desktop app, Buddy is designed to run **server-side** through the gateway orchestrator. This allows it to work identically in both the browser and the desktop application without requiring a local runtime. When the desktop app is available, Office can optionally delegate to the full local agent via [CoworkDesktop](cowork_desktop.md).
...
The main server-side Buddy UI. It mirrors the chat shell from [Chat.jsx](chat.md) but adds office-specific behavior:
...
These helpers mirror the patterns used in [Chat.jsx](chat.md) and the backend [chat_worker](workers.md#chat_agent_execution_workers).
...
Office reuses the shared marker pipeline from [Message.jsx](message.md):
...
| Module | Purpose |
|--------|---------|
| [Message.jsx](message.md) | `mdComponents`, `parseDocMarkers`, `DocDownloadButton` |
| [CoworkDesktop.jsx](cowork_desktop.md) | Full local-agent desktop experience |
| [useDesktop.js](ai_ui_frontend_hooks.md) | `isCoworkOfficeAvailable` detection |
| [config.js](config.md) | `API_BASE`, `authFetch` |

### Backend Modules

| Module | Purpose |
|--------|---------|
| [gateway.py](gateway.md) | `POST /ask`, `POST /docs/generate`, `POST /connectors/action`, `POST /chat/upload` |
| [routers/chat_router.py](chat_router.md) | Chat upload and messaging endpoints |
| [routers/doc_download_router.py](doc_download_router.md) | Document generation, status polling, download |
| [routers/connectors_router.py](connectors_router.md) | Connector action execution |
| [workers/doc_worker_agent.py](workers.md#document_knowledge_workers) | Document generation worker |
| [workers/chat_worker.py](workers.md#chat_agent_execution_workers) | Office-mode chat execution |
| [connectors/engine.py](shared_integrations.md#connector_infrastructure) | Connector runtime |
...
- **Document retention**: Generated files have a TTL and are cleaned by [purge_worker](workers.md#infrastructure_maintenance_workers). The `DocDownloadButton` handles `expired` and `410 Gone` states gracefully.
- **Safe markdown rendering**: Uses the same `mdComponents` and URL transform as [Chat.jsx](chat.md) to prevent unsafe content injection.
...
- [Chat](chat.md) — general chat UI that Office mirrors
- [Message](message.md) — shared message rendering and document markers
- [CoworkDesktop](cowork_desktop.md) — local-agent desktop mode
- [Gateway](gateway.md) — main API gateway
- [Connectors Router](connectors_router.md) — connector action endpoints
- [Document Download Router](doc_download_router.md) — document generation endpoints
- [Workers](workers.md) — background job workers
- [Shared Integrations](shared_integrations.md) — connector adapters and engine
