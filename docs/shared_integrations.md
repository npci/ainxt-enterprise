...
All tools in this module are registered into the platform's [MCP system](mcp_system.md) and/or exposed through the connector engine, making them discoverable and invocable by agents without each agent implementing its own integration logic.
...
| **Guardrails** | NeMo Guardrails-based input safety with keyword-only and full LLM evaluation modes. | [guardrails.md](guardrails.md) |
| **ATS Tools** | Applicant tracking pipeline listing, resume-to-JD scoring, and stage-change proposals. | [ats_tools.md](ats_tools.md) |
| **Calendar Tools** | Read-only `.ics` calendar introspection, free-slot computation, and draft event creation. | [calendar_tools.md](calendar_tools.md) |
| **Confluence Tools** | Thin wrapper around Confluence Cloud REST API for page read/write/search. | [confluence_tools.md](confluence_tools.md) |
...
| **Doc Generator** | Multi-format document and presentation generation (DOCX, PPTX, PDF, XLSX, TXT, MD, CSV). | [doc_generator.md](doc_generator.md) |
| **Docker Execution Tool** | Sandboxed execution of arbitrary Python code inside ephemeral Docker containers. | [docker_execution_tool.md](docker_execution_tool.md) |
| **Document Tools** | Read-only text extraction and search across PDF, Office, HTML, and plain-text files. | [document_tools.md](document_tools.md) |
| **Email Tools** | Read-only `.eml` triage and draft-reply creation to an outbox. | [email_tools.md](email_tools.md) |
| **GitHub Tools** | GitHub REST API wrapper for PRs, issues, branches, files, reviews, and merges. | [github_tools.md](github_tools.md) |
| **GitLab Tools** | Shared GitLab REST API v4 client used by SDLC, MCP, and connector paths. | [gitlab_tools.md](gitlab_tools.md) |
| **Jira Tools** | Shared Jira Cloud REST API v3 client for issues, search, transitions, comments, and audit logging. | [jira_tools.md](jira_tools.md) |
| **KB Search Tools** | Namespaced keyword retrieval over local document corpora. | [kb_search_tools.md](kb_search_tools.md) |
...
| **Security Scan Tools** | Unified SAST scanner orchestration (SonarQube, Checkmarx, PMD, Semgrep, Bandit, secrets) with CVSS normalization. | [security_scan_tools.md](security_scan_tools.md) |
| **Task Tracker Tools** | Lightweight task CRUD with JSON-backed persistence and MCP exposure. | [task_tracker_tools.md](task_tracker_tools.md) |
...
- [mcp_system](mcp_system.md) — MCP registry, bridge, and server infrastructure that exposes `shared_integrations` tools to agents.
...
- [shared_api_routers](shared_api_routers.md) — HTTP routers that surface connector actions, document generation, and SDLC operations to clients.
- [abstudio_backend](abstudio_backend.md) — Backend APIs for catalog, agents, workflows, and templates that may bind integration tools.