...
MCP System sits between the higher-level agent/workflow engines and the concrete integrations. It is consumed by [agent_system](../agents/agent_system.md), [workflow_system](../workflows/workflow_system.md), [shared_api_routers](../api/shared_api_routers.md) (e.g. `mcp_server_router`, `mcp_governance_router`, `marketplace_router`), and the [gateway](../models/gateway.md).
...
| [shared_api_routers](../api/shared_api_routers.md) | `mcp_server_router` mounts SSE endpoints for internal servers; `mcp_governance_router` manages tool approvals; `marketplace_router` hot-registers tools. |
| [gateway](../models/gateway.md) | Exposes agent/workflow/chat endpoints that ultimately route through `MCPBridge` or `MCPRegistry`. |
| [mcp_servers](mcp_servers.md) | Internal `BaseMCPServer` subclasses are instantiated by `MCPBridge.bootstrap()`. |
| [shared_integrations](../reference/shared_integrations.md) | Concrete Jira/GitLab/Confluence/etc. adapters back the registered MCP tools. |
