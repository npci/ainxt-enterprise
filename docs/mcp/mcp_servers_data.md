...
These servers inherit from [`BaseMCPServer`](mcp_servers_base.md) and are registered through the shared [`MCPRegistry`](../core/shared_core.md#mcp_system), allowing them to be consumed by the agent runtime, workflow engine, and CLI runtime.
...
Wraps [`tools/data_tools_tools.py`](../skills/shared_integrations.md#data_tools) to expose tabular-file operations as MCP tools. It is intended for deterministic analysis of CSV/XLSX files stored under a configured data root.
...
Wraps [`tools/kb_search_tools.py`](../skills/shared_integrations.md#kb_search_tools) to let agents retrieve grounded knowledge for reply drafting, HR policy Q&A, and RFP content lookup.
...
- `DatabaseMCPServer` uses [`db.database.SessionLocal`](../core/shared_core.md#database) directly, bypassing the tool layer because it needs fine-grained control over SQL parsing, parameter binding, and masking.
...
1. **Agent runtime** — [`AgentBuilder`](../core/shared_core.md#agent_system) / [`MultiAgentRunner`](../core/shared_core.md#agent_system) resolves tool names through [`ToolRegistry`](../core/shared_core.md#mcp_system) and calls the appropriate MCP server.
2. **Workflow engine** — [`WorkflowEngine`](../core/shared_core.md#workflow_system) and `NativeEngine` can include MCP nodes that invoke `data_tools` or `kb_search` tools.
3. **CLI runtime** — `AbstudioMcpServer` and the desktop client use stdio/streamable HTTP to reach these servers.
4. **Gateway** — [`mcp_server_router`](../core/shared_api_routers.md#mcp_server_router) exposes SSE and streamable HTTP endpoints that route to the running MCP server instances.
