...
| `operator` | `string` | One of the supported operators from [`constants/operators.js`](constants.md). |
...
| [`workflowStore`](store.md) | Persists the `cases` array inside the selected node's `data`. |

### 7.2 Backend Counterparts

| Module | Relationship |
|--------|--------------|
| [`app_models`](app_models.md) | Defines `ConditionNode`, `ConditionCase`, and `SingleCondition` Pydantic models that mirror the frontend shape. |
| [`engine_native_engine`](engine_native_engine.md) | Evaluates the condition cases at workflow runtime. |
| [`services_services`](services_services.md) | Provides graph utilities such as `get_linear_order` used when validating condition routing. |
...
| [`constants/operators.js`](constants.md) | Defines field presets, operator lists, type coercion helpers, and the plain-English preview builder. |
...
- Backend models: [`app_models`](app_models.md)
- Runtime engine: [`engine_native_engine`](engine_native_engine.md)
- Shared operators/constants: [`constants`](constants.md)
- Workflow persistence: [`workflowStore`](store.md)
