...
Both endpoints are called through `authFetch` from [`config.js`](config.md), which attaches credentials and the auth token.
...
- [`config.js`](config.md) — `API_BASE` and `authFetch`.
- [`DialogProvider`](ui_dialog.md) — `useConfirm` and `useToast` hooks.
