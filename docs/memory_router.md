...
- **[auth system](auth.md)** — Provides `get_current_user` for JWT/API-key authentication and user enrichment. The user-memory endpoints depend on it for scoping.
- **[memory system](memory.md)** — Implements `PostgresMemory`, which handles cross-chat user memory persistence, smart merging, semantic de-duplication, and pruning.
...
- **[embedding_service](embedding_service.md)** — Used by `PostgresMemory` for semantic similarity checks when merging memory entries.
- **[model_router](model_router.md)** — Used by `PostgresMemory` to generate merged memory summaries.
- **[Agent Analytics UI](agent_analytics.md)** — Frontend component that consumes `/agents/{name}/analytics` and `/analytics/platform`.
