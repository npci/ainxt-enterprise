...
- **Auth**: All requests use `authFetch` from [`config.md`](config.md), which attaches correlation IDs and credentials.
...
| Dependency | Purpose |
|------------|---------|
| [`config.md`](config.md) | `authFetch`, `API_BASE` |
...
The Agent Analytics module is part of the [`ai_ui_frontend`](ai_ui_frontend.md) analytics surface. It consumes data produced by:

- The gateway's analytics and metrics endpoints (see [`gateway.md`](gateway.md)).
- The agent management surface (see [`agents_catalog.md`](agents_catalog.md)).
- The LLM spend reporting subsystem (see [`llm_spend_report_router.md`](llm_spend_report_router.md)).
...
- [`ai_ui_frontend.md`](ai_ui_frontend.md) — parent frontend module
- [`config.md`](config.md) — `authFetch` and API base configuration
- [`time.md`](time.md) — IST timestamp formatting
- [`gateway.md`](gateway.md) — backend gateway providing `/metrics` and analytics
- [`agents_catalog.md`](agents_catalog.md) — agent catalog and related UI
- [`llm_spend_report_router.md`](llm_spend_report_router.md) — cloud spend backfill API
