...
| Dependency | Role |
| --- | --- |
| [auth.dependencies](../auth_dependencies.md) | `get_current_user` extracts and enriches the authenticated user. |
| [auth.rbac](../auth_rbac.md) | `can_approve` and `is_admin` enforce role-based access. |
| [core.security_validation](../core_security_validation.md) | Validates and sanitizes product, repo, and department inputs. |
| [core.logger](../core_logger.md) | Logs warnings on notification failures. |
| [store.inbox_store](../store_inbox_store.md) | Publishes approval and result notifications. |
| [db.database](../db_database.md) | Provides SQLAlchemy sessions. |
| [db.models](../db_models.md) | Defines `Product`, `DeptProductMapping`, `ProductRepo`, `IndexRequest`, `OrgTree`, and `User`. |
...
- **[sdlc_router](sdlc_router.md)**: SDLC runs and governance are scoped to product repositories.
- **[index_router](index_router.md)**: Repositories are indexed per product; `get_product` merges linked and indexed repos.
- **[governance_router](governance_router.md)**: Governance entities can be promoted from product artifacts.
- **[budget_router](budget_router.md)**: Budget chargebacks can be assigned at the product level.
- **[ai-ui ProductManager](../ai_ui_frontend_product_manager.md)**: Frontend UI for product creation and approval.
