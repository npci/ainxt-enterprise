...
| **Non-goal** | This module does not manage long-lived document lifecycle (list, delete, approve, reject). Those operations remain in the platform router and are documented in [docs_router.md](../documents/docs_router.md). |
...
| `validate_upload` | `core/file_validator.py` | Extension, magic-byte, size, and HTML-script validation. See core_file_validator.md. |
...
| `compliance_engine` | `agents/compliance_engine.py` | PII/PCI detection and redaction. See agents_compliance_engine.md. |
| `upload_doc` | `store/docs_store.py` | Shared persistence, chunking, embedding, and activation helper. See store_docs_store.md. |
...
Approver capability is checked via `auth.rbac.can_approve`. See auth_rbac.md.
...
| [app_models.md](../core/app_models.md) | Uses `AuthenticatedUser` for identity, role, and department. |
...
| core_document_parser.md | Falls back to `parse_file_structured` for direct parsing (legacy path). |
| core_file_validator.md | Uses `validate_upload` for extension, magic-byte, size, and HTML-script checks. |
| agents_compliance_engine.md | Uses `compliance_engine.validate_input` and `BLOCKING_TYPES` for PII/PCI redaction and blocking. |
| core_parser_errors.md | Uses `is_parser_error` to detect parser sentinel strings. |
| store_docs_store.md | Uses `upload_doc(..., auto_approve=True)` for persistence, chunking, embedding, and inline activation. |
| auth_rbac.md | Uses `can_approve` to decide whether a caller may scope `PRIVATE` uploads to arbitrary departments. |
...
| [docs_router.md](../documents/docs_router.md) | Platform sidebar KB route that this module mirrors but with `auto_approve=False` and admin approval workflow. |
