...
- **[presenton_lib_layout_registry](../reference/presenton_lib_layout_registry.md)** – Resolves slide layout definitions by ID and provides JSON Schema helpers.
- **[presenton_lib_layout_mapping](../reference/presenton_lib_layout_mapping.md)** – Maps template groups and slide types to concrete layout keys and creates slide objects.
- **[presenton_lib_payload_builder](../ui/presenton_lib_payload_builder.md)** – Builds per-slide content payloads and full update payloads consumed by this API client.
- **[presenton_lib_stream_reader](../reference/presenton_lib_stream_reader.md)** – Lower-level Presenton stream reader with logging; conceptually similar to `streamPresentationRSC`.
- **[config](../infrastructure/config.md)** – Provides `presentonFetch`, `PRESENTON_POLL_INTERVAL`, `PRESENTON_MAX_RETRIES`, and `PRESENTON_TIMEOUT`.
