...
- **Lifecycle coupling**: `startup()` and `shutdown()` are invoked from [`app_main`](../core/app_main.md) lifespan hooks.
...
All endpoints depend on `require_access` from [`api_deps`](api_deps.md), which resolves the caller to an [`AuthenticatedUser`](../core/app_models.md) and enforces that the user has access to the `agent-chain` framework.
...
|------------|--------|------|
| `AuthenticatedUser` | [`app_models`](../core/app_models.md) | User identity model. |
| `require_access` | [`api_deps`](api_deps.md) | Authentication / framework-access dependency. |
| `postgres_enabled` | [`core_config`](../core/core_config.md) | Feature flag for PostgreSQL backend. |
| `AgentChatStore` | [`checkpoint`](../agents/checkpoint.md) | Abstract store interface. |
| `PostgresAgentChatStore` | [`checkpoint`](../agents/checkpoint.md) | Production PostgreSQL implementation. |
| `FileAgentChatStore` | [`checkpoint`](../agents/checkpoint.md) | Local JSON file implementation. |

### 5.2 Runtime dependencies

| Dependency | Module | Role |
|------------|--------|------|
| `app.main._lifespan` | [`app_main`](../core/app_main.md) | Calls `startup()` / `shutdown()`. |
...
- [`checkpoint`](../agents/checkpoint.md) — `AgentChatStore` implementations.
- [`api_deps`](api_deps.md) — authentication dependencies.
- [`app_models`](../core/app_models.md) — `AuthenticatedUser` and request models.
- [`app_main`](../core/app_main.md) — application lifespan and startup hooks.
