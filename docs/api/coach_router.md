...
- [auth_dependencies](../auth_dependencies.md) — `get_current_user` JWT extraction.
- [core_config](../infrastructure/core_config.md) — `ENABLE_COACH` feature flag.
- [db_database](../db_database.md) and [db_models](../db_models.md) — `SessionLocal`, `CoachEvent`, `CoachRuleHit`, `CoachRulePack`, `CoachScoreSnapshot`.
- [coach_evaluator](../coach_evaluator.md) — rule definitions, `compute_scores`, `evaluate_dry_run`, `rule_catalog`.
- [coach_ingestor](../coach_ingestor.md) — `crypto.decrypt` for redacted prompts.
- [model_router](../model_router.md) — per-prompt model recommendation.
- [model_registry](../model_registry.md) — `MODEL_COST_PER_1M` and `OPENAI_SIMPLE_MODEL`.
- [compliance_engine](../compliance_engine.md) — prompt redaction for the dry-run tester.
- [gateway_openai](../gateway_openai.md) — `OpenAIGateway.generate` for LLM rewrites.
- [ai_ui_frontend_coach](../ai_ui_frontend_coach.md) — `Coach.jsx` and `CoachAdmin.jsx` front-end consumers.
