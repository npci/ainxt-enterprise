...
`review_router` exposes a REST API for automated code review. It is a thin FastAPI router layer that delegates all review logic to [`agents/review_engine.py`](../agents_review_engine.md). The module provides four endpoints covering three review phases: multi-model LLM consensus, static analysis (Bandit/Semgrep), and knowledge-base-driven architecture review. A composite `/review/full` endpoint orchestrates all three phases and returns a single weighted overall score.

All endpoints require an authenticated user via [`auth.dependencies.get_current_user`](../auth_dependencies.md).
...
- All endpoints depend on [`get_current_user`](../auth_dependencies.md); unauthenticated requests are rejected before reaching review logic.
...
- [`agents/review_engine.py`](../agents_review_engine.md) for all review logic.
- [`auth.dependencies.get_current_user`](../auth_dependencies.md) for authentication.
- [`core.logger`](../core_logger.md) for structured logging.
- [`models/model_router.py`](../models_model_router.md) and [`models/hybrid_retriever.py`](../models_hybrid_retriever.md), which are used indirectly by `ReviewEngine` for LLM generation and KB retrieval.
...
- [`agents/review_engine.md`](../agents_review_engine.md) — underlying review engine that implements consensus, static analysis, and architecture review.
- [`auth_dependencies.md`](../auth_dependencies.md) — authentication dependency used by all endpoints.
- [`core_logger.md`](../core_logger.md) — logging utilities.
- [`models_model_router.md`](../models_model_router.md) — model routing used for LLM generation.
- [`models_hybrid_retriever.md`](../models_hybrid_retriever.md) — hybrid retrieval used for architecture patterns.
