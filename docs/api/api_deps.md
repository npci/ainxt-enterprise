...
|---|---|---|
| **Authentication** | `_wrapped_gateway_auth`, `require_access` | Resolve the current user from the platform gateway's JWT/API-key auth and expose it as an [`AuthenticatedUser`](../core/app_models.md) model. |
| **Authorization** | `require_admin` | Reuse `require_access` and additionally enforce `role == "admin"`. |
| **Logging context** | `bind_log_context`, `clear_log_context` | Stamp each request with `request_id`, `chat_id` (thread id), `user_id`, `span_id`, and `client_source` using the shared `core/logger` thread-local setters. |
| **Execution context** | `build_execution_context` | Build an `ExecutionContext` from the authenticated user and request metadata so workflow/agent runs inherit department, admin flags, hierarchy fields, and loop-engineering parameters. |
| **Model conversion** | `to_chain` | Convert a Pydantic [`Workflow`](../core/app_models.md) model into the engine-agnostic `ChainDefinition` used by the execution engine. |
...
A FastAPI dependency that bridges the platform-wide `auth/dependencies.py::get_current_user` with AB Studio's [`AuthenticatedUser`](../core/app_models.md) model.
...
- **When running standalone** (e.g., local development without the gateway), the import fails and `require_access` falls back to [`require_framework_access("agent-chain")`](../core/app_models.md), which returns a dev-stub admin user.
...
Populates the shared `core/logger` thread-local context so every structured log line written during a request carries the same identifiers:
...
These fields allow the engine to apply the same `PUBLIC` + user-department `PRIVATE` pgvector ACL used by the chat Knowledge toggle, and to pass loop-control parameters to [`LoopRunner`](../agents/loop_runner.md).

### `to_chain`

Converts a [`Workflow`](../core/app_models.md) Pydantic model into the engine's `ChainDefinition`:
...
- **Authentication** is delegated to the shared `auth/dependencies.py` so AB Studio reuses the same JWT/API-key/cookie logic as the main gateway, IDE integrations, and CLI.
- **Logging** is aligned with `core/logger.py`, ensuring `agent.log` entries from AB Studio use the same `request_id`, `chat_id`, `user_id`, `span_id`, and `client_source` fields as the platform, CLI, and IDE surfaces.
- **User model** is imported from [`app/models.py`](../core/app_models.md), which defines `AuthenticatedUser` and the fallback `require_framework_access` stub.
- **Engine integration** passes `ExecutionContext` and `ChainDefinition` into the AB Studio execution engine (see [engine_native_engine.md](../agents/engine_native_engine.md) and [loop_runner.md](../agents/loop_runner.md)).
...
- **Thread safety**: `bind_log_context` relies on `core/logger.py` thread-local variables. Under uvicorn's worker threads and within a single async handler (including its SSE generator), both run on the same event-loop thread, so the context is safe. Always call `clear_log_context()` in `finally`.
