...
This module is intentionally thin: it validates authentication, delegates persistence to [core_workflow_repo](../workflows/core_workflow_repo.md), emits audit events through [core_governance](../sdlc/core_governance.md), and handles domain-specific cleanup such as deregistering scheduled triggers and removing associated chat history when a workflow is deleted.
...
- **[core_workflow_repo](../workflows/core_workflow_repo.md)**: Provides all persistence operations, name validation, optimistic concurrency, and trigger cleanup helpers.
...
- **[engine_native_engine](../agents/engine_native_engine.md)**: The execution engine that runs workflow graphs and provides checkpoint-store cleanup on deletion.
