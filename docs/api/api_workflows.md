...
This module is intentionally thin: it validates authentication, delegates persistence to [core_workflow_repo](../reference/core_workflow_repo.md), emits audit events through [core_governance](../sdlc/core_governance.md), and handles domain-specific cleanup such as deregistering scheduled triggers and removing associated chat history when a workflow is deleted.
...
- **[core_workflow_repo](../reference/core_workflow_repo.md)**: Provides all persistence operations, name validation, optimistic concurrency, and trigger cleanup helpers.
...
- **[core_governance](../sdlc/core_governance.md)**: Receives audit events for workflow CRUD operations.
- **[engine_native_engine](../reference/engine_native_engine.md)**: The execution engine that runs workflow graphs and provides checkpoint-store cleanup on deletion.
