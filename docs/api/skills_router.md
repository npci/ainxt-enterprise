...
The `generate_skill` endpoint allows non-technical users to describe a desired capability in plain English. The router delegates code synthesis to [services/skill_synthesis](../reference/shared_core.md#services), which uses the [model_router](../reference/shared_core.md#model-routing) to generate Python code. Generated skills can be returned as drafts or saved directly as `PRODUCTION`.
