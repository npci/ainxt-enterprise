...
|---|---|---|
| `ConnectorRegistry` | [shared_integrations](../reference/shared_integrations.md) | Singleton registry for connector definitions, user status, and execution delegation |
| `ConnectorEngine` | [shared_integrations](../reference/shared_integrations.md) | Execution engine with schema validation, caching, rate limiting, compliance, pagination |
| `OAuth2Handler` | [shared_integrations](../reference/shared_integrations.md) | OAuth2 lifecycle: PKCE, authorize URL, code exchange, token refresh, state management |
| `ConsentHandler` | [shared_integrations](../reference/shared_integrations.md) | DPI consent artifact creation, signing (sandbox), and verification |
| `select_probe` | [shared_integrations](../reference/shared_integrations.md) | Capability-based probe selection for connection testing |
| `ConnectorMetrics` | [shared_integrations](../reference/shared_integrations.md) | Redis-backed observability: call stats, top queries, audit log |
...
| `encrypt_value` | [shared_core](../reference/shared_core.md) | Fernet encryption for stored credentials |
| `compliance_engine` | [shared_core](../reference/shared_core.md) | PCI/PII compliance scanning for outgoing messages |
| `get_decrypted_token` | [profile_router](profile_router.md) | Profile vault token retrieval for PAT connectors |
| `_resolve_doc_attachments` | [shared_integrations](../reference/shared_integrations.md) | Document attachment resolution for M365 write tools |
...
The `ConnectorRegistry` registers connector tools into the MCP `ToolRegistry` with naming convention `{connector}__{tool}` (e.g., `microsoft_365__outlook_search_emails`). This makes connector tools available to LLM tool-use calling. See [mcp_system](../reference/shared_core.md) for details on the MCP infrastructure.
