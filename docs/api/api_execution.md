...
- Converts the [`RunRequest.workflow`](../core/app_models.md) into a [`ChainDefinition`](../agents/engine_native_engine.md) via [`to_chain`](api_deps.md).
- Builds an [`ExecutionContext`](api_deps.md) and a [`RunUsageTracker`](../sdlc/core_governance.md).
- Streams events from [`NativeEngine.execute`](../agents/engine_native_engine.md), collecting the final `output` and `thread_id`.
- Returns a [`RunResponse`](../core/app_models.md) with `status`, `output`, and `thread_id`.
...
- [`engine_native_engine`](../agents/engine_native_engine.md) — pure-Python workflow engine that executes the chain.
- [`core_governance`](../sdlc/core_governance.md) — budget checks, audit events, and usage tracking.
- [`app_models`](../core/app_models.md) — `RunRequest`, `ResumeRequest`, `RunResponse`, `Workflow`.
