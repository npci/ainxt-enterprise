...
This module is part of the shared API router layer. It depends on the [store layer](../storage/store_layer.md) for persistence and pub/sub, the [authentication module](../security/authentication.md) for identity and authorization, and the [database models](../storage/database.md) for the entity records it aggregates.
...
These endpoints delegate directly to [`store.inbox_store`](../storage/store_layer.md). The store handles SQLAlchemy sessions, row-to-dict mapping, and optimistic error handling.
...
1. **Authorizes the caller** using the JWT payload from [`get_current_user`](../security/authentication.md). Only the following roles may receive results:
...
- All endpoints except the SSE stream accept a `user` query parameter for scoping, but the **authoritative identity** for approvals comes from the JWT via [`get_current_user`](../security/authentication.md).
...
|----------------|--------------|
| [store_layer](../storage/store_layer.md) | Provides `InboxItem` persistence and the SSE pub/sub registry. |
| [authentication](../security/authentication.md) | Supplies `get_current_user` for JWT/API-key/cookie validation and claims extraction. |
| [database](../storage/database.md) | Defines `InboxItem`, `User`, `DepartmentHodMapping`, `AgentRecord`, `SkillRecord`, `WorkflowRecord`, `Product`, `DeptProductMapping`, `KnowledgeDocument`, and `index_requests`. |
...
| [ai_ui_frontend](../ui/ai_ui_frontend.md) | The `Inbox.jsx` component consumes these endpoints and renders approval previews. |
