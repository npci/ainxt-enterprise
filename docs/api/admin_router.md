...
Every endpoint in the admin router is protected by the `require_admin_flag` dependency from the [authentication](../security/authentication.md) module, which validates that the caller's JWT contains `role = "admin"`. This is the **first authentication factor** for all admin endpoints.
...
> See [local_llm_gateway](../models/local_llm_gateway.md) for full details on model discovery, tier classification, and the streaming generation interface.
...
> See [core_infrastructure](../infrastructure/core_infrastructure.md) for the full `CircuitBreaker` implementation, state machine, and per-provider tuned defaults.
...
- [authentication](../security/authentication.md) — RBAC, `require_admin_flag`, SSO, and session management
- [core_infrastructure](../infrastructure/core_infrastructure.md) — `CircuitBreaker`, rate limiter, file validator, logger
- [agent_system](../agents/agent_system.md) — `ComplianceEngine`, PII detection, redaction, and blocking logic
- [local_llm_gateway](../models/local_llm_gateway.md) — `LocalLLMGateway`, `_ModelCatalog`, tier-based model selection
- [database](../storage/database.md) — `OrgTree` and `User` models, `SessionLocal`, PostgreSQL connection pooling
