...
> **Note:** Module references in brackets (e.g. `[core_infrastructure]`, `[database]`) point to sibling documentation files. See [core_infrastructure](../infrastructure/core_infrastructure.md), [database](../storage/database.md), [agent_system](../agents/agent_system.md), [store_layer](../storage/store_layer.md), [mcp_system](../mcp/mcp_system.md), [document_knowledge_workers](../workers/document_knowledge_workers.md), and [shared_integrations](../reference/shared_integrations.md) for details on those subsystems.
...
- **Authentication**: All endpoints require `get_current_user` (JWT, API key, or httpOnly cookie). See [authentication](../security/authentication.md).
