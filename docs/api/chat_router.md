...
> The `/ask` gateway reads these Chat columns on every request and injects them into `_user_ctx['scope_filter']` + `_user_ctx['kb_doc_id']`, so hybrid search's product/domain/version WHERE clauses fire deterministically. See [gateway](../models/gateway.md) for the `/ask` path details.
...
| Dependency | Purpose |
|---|---|
| **`ObjectStorage`** | Unified file storage backend (MinIO with local disk fallback). Uploads are sharded by `subdir/user_id/chat_id`. Opaque path strings returned by `save()` are passed back to `load()`/`delete()`/`presigned_url()`. See [shared_core](../reference/shared_core.md). |
...
| **`GeminiGateway`** | Direct Gemini SDK client for image (Imagen) and video (Veo 3.1) generation. Routes through LLM Proxy when `LLM_PROXY_URL` is set. Implements gemini→openai fallback for images. See [gemini_gateway](../models/gemini_gateway.md). |
| **`budget_store`** | Redis + Postgres budget ledger. `increment_usage` debits cost; `check_budget` / `get_budget` / `get_usage_total` used for Veo projection checks. See [shared_core](../reference/shared_core.md). |
...
See [llm_proxy](../models/llm_proxy.md) for proxy internals.
...
See [shared_core](../reference/shared_core.md) and [budget_router](budget_router.md) for budget management details.
