...
The **Prompt Management Router** exposes a secure, admin-only REST API for managing versioned LLM prompts. It sits in front of the `core.prompt_registry` and provides operators with explicit control over prompt registration, activation, rollback, A/B testing, and evaluation-score inspection.
...
All endpoints require the caller to have the `admin` role. Authentication is delegated to `auth.dependencies.get_current_user`.
...
A FastAPI dependency that wraps `get_current_user` and enforces role-based access control.
...
The router relies on the `PromptVersion` table:
...
For the full model definition and surrounding tables, see `db/models.md`.
...
A/B routing is implemented inside `PromptRegistry.get()`, not in the router itself. The router only configures the experiment. At runtime:
...
| `auth.dependencies.get_current_user` | Authentication | `auth_dependencies.md` |
| `core.logger.logger` | Structured logging | `core_logger.md` |
| `core.prompt_registry.PromptRegistry` | Business logic & caching | `core_prompt_registry.md` |
| `db.database.SessionLocal` | Database sessions | `db_database.md` |
| `db.models.PromptVersion` | ORM model | `db_models.md` |
...
The Prompt Management Router is part of the [`shared_api_routers`](../core/shared_api_routers.md) layer. It is consumed by:
...
At runtime, other modules retrieve the active prompt through `PromptRegistry.get()`, which transparently handles caching and A/B routing. See also:

- `core_prompt_registry.md` for the full registry implementation.
- `db_models.md` for the `PromptVersion` schema.
- `auth_dependencies.md` for authentication details.
