...
|------------|--------|------|
| `get_current_user` | [authentication](../security/authentication.md) | JWT/API-key extraction and user context enrichment |
| `require_admin` | [authentication](../security/authentication.md) | Admin-role enforcement for marketplace endpoints |
| `can_approve` | [authentication](../security/authentication.md) | Non-raising check for `ad_level ≤ 3` or admin |
| `CoworkMemory` | [memory_system](../reference/memory_system.md) | Postgres-backed per-user preference and note store |
| `cowork_roles` service | [services](../reference/services.md) | Role specialist CRUD, context rendering, marketplace |
| `_compliance_check` | [mcp_system](../mcp/mcp_system.md) | PII/secret/PAN screening for memory notes |
| `publish_inbox_item` | [store_layer](../storage/store_layer.md) | Inbox notifications for governance events |
| `SessionLocal` / `User` | [database](../storage/database.md) | Approver lookup for role submission notifications |
...
- [authentication](../security/authentication.md) — JWT validation, RBAC, and SSO
- [memory_system](../reference/memory_system.md) — `CoworkMemory` and the broader memory service
- [services](../reference/services.md) — `cowork_roles` service layer with role CRUD and context rendering
- [store_layer](../storage/store_layer.md) — `inbox_store` for governance notifications
- [mcp_system](../mcp/mcp_system.md) — MCP server base with compliance checking
- [database](../storage/database.md) — Database models and session management
