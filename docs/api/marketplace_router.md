...
- Validation and sanitization happen first via `validate_tool_register_request` in [core/security_validation](../reference/shared_core.md#core-infrastructure).
...
- **[auth.dependencies](../security/auth.md)** — `get_current_user` for actor resolution.
- **[core.config](../reference/shared_core.md#core-infrastructure)** — `RDB_REGISTRY` constant selecting the KV database index.
- **[core.kv](../reference/shared_core.md#kv-store)** — `get_kv` and `KVError` for marketplace metadata storage.
- **[core.logger](../reference/shared_core.md#core-infrastructure)** — Structured logging.
- **[core.security_validation](../reference/shared_core.md#core-infrastructure)** — `validate_tool_register_request` for input sanitization.
...
- **Agent Builder / ABStudio**: The ABStudio frontend lists and selects tools/skills from the marketplace when configuring agents. See [abstudio_backend](../ui/abstudio_backend.md) and [abstudio_frontend](../ui/abstudio_frontend.md).
