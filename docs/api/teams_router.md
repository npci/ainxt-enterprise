...
The [gateway](../models/gateway.md) may optionally mount the official Teams SDK endpoint via `_mount_teams_sdk`, but the `teams_router` remains the primary integration surface for custom command handling, proactive notifications, and meeting automation.
...
- [gateway](../models/gateway.md) — top-level gateway that mounts platform routers and the Teams SDK.
