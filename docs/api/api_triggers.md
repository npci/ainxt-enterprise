...
Triggers are persisted in Postgres, dispatched by a dedicated scheduler worker, and executed by the same engine paths used for interactive workflow/agent runs. This module is intentionally thin: it validates requests, delegates persistence to [`core_workflow_repo`](../workflows/core_workflow_repo.md), and delegates firing to [`services_trigger_scheduler`](../workers/services_trigger_scheduler.md).
...
The module does **not** run workflows itself. Execution is delegated to [`services_trigger_scheduler`](../workers/services_trigger_scheduler.md), which in turn calls the workflow engine ([`engine_native_engine`](../agents/engine_native_engine.md)) or agent runner.
...
`api_triggers` is one of several FastAPI routers under `ABStudio/backend/app/api/`. It is mounted alongside [`api_workflows`](api_workflows.md), [`api_agents`](api_agents.md), [`api_execution`](api_execution.md), and [`api_chat`](api_chat.md). It reuses the same authentication dependency ([`api_deps`](api_deps.md)) and the same workflow/agent repository ([`core_workflow_repo`](../workflows/core_workflow_repo.md)) as the rest of AB Studio.
...
Trigger shapes are defined in [`app_models`](../core/app_models.md).
...
| [`app_models`](../core/app_models.md) | Pydantic models for triggers and executions. |
| [`core_workflow_repo`](../workflows/core_workflow_repo.md) | Postgres persistence for triggers and executions. |
| [`services_trigger_scheduler`](../workers/services_trigger_scheduler.md) | Computes next run times and fires triggers. |
| [`engine_native_engine`](../agents/engine_native_engine.md) | Executes workflow runs. |
