...
Triggers are persisted in Postgres, dispatched by a dedicated scheduler worker, and executed by the same engine paths used for interactive workflow/agent runs. This module is intentionally thin: it validates requests, delegates persistence to [`core_workflow_repo`](../reference/core_workflow_repo.md), and delegates firing to [`services_trigger_scheduler`](../workers/services_trigger_scheduler.md).
...
The module does **not** run workflows itself. Execution is delegated to [`services_trigger_scheduler`](../workers/services_trigger_scheduler.md), which in turn calls the workflow engine ([`engine_native_engine`](../reference/engine_native_engine.md)) or agent runner.
...
`api_triggers` is one of several FastAPI routers under `ABStudio/backend/app/api/`. It is mounted alongside [`api_workflows`](api_workflows.md), [`api_agents`](api_agents.md), [`api_execution`](api_execution.md), and [`api_chat`](api_chat.md). It reuses the same authentication dependency ([`api_deps`](api_deps.md)) and the same workflow/agent repository ([`core_workflow_repo`](../reference/core_workflow_repo.md)) as the rest of AB Studio.
...
Trigger shapes are defined in [`app_models`](../models/app_models.md).
...
Before a trigger actually runs, [`services_trigger_scheduler._fire_trigger`](../workers/services_trigger_scheduler.md) applies:
...
See [`core_governance`](../sdlc/core_governance.md) and [`services_trigger_scheduler`](../workers/services_trigger_scheduler.md) for details.
...
1. On create/update, it asks [`trigger_scheduler.register_trigger`](../workers/services_trigger_scheduler.md) to compute the next fire time.
2. It persists `next_run_at` in the `triggers` table via `workflow_repo.update_trigger_run_metadata`.
3. A separate [`workers/workflow_scheduler_worker`](../workers/workers.md) polls the table every 60 seconds and enqueues due triggers to Redis/RQ.
...
The frontend trigger UI lives in [`abstudio_frontend/triggers_feature`](../ui/abstudio_frontend.md):
...
| [`app_models`](../models/app_models.md) | Pydantic models for triggers and executions. |
| [`core_workflow_repo`](../reference/core_workflow_repo.md) | Postgres persistence for triggers and executions. |
| [`services_trigger_scheduler`](../workers/services_trigger_scheduler.md) | Computes next run times and fires triggers. |
| [`engine_native_engine`](../reference/engine_native_engine.md) | Executes workflow runs. |
...
| [`abstudio_frontend/triggers_feature`](../ui/abstudio_frontend.md) | Frontend UI for triggers. |
| [`workers/workflow_scheduler_worker`](../workers/workers.md) | Polls `triggers` table and dispatches due runs. |
...
No other runtime configuration is required by this module; scheduler cadence and worker behavior are controlled in [`services_trigger_scheduler`](../workers/services_trigger_scheduler.md) and [`workers/workflow_scheduler_worker`](../workers/workers.md).
