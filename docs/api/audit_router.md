...
| Dependency | Role |
|------------|------|
| `auth.dependencies.get_current_user` | Resolves and validates the caller from JWT, API key, or cookie; enriches the payload with server-side profile data. See [auth_router](auth_router.md). |
| `db.database.get_db` / `DB_SCHEMA` | Provides a SQLAlchemy session and schema name for queries. See [db_models](../db_models.md). |
| `db.models.RequestAuditLog` | ORM model for the standard request audit table. See [db_models](../db_models.md). |
| `core.logger` | Structured logging. |
| `core.graph_audit` | Boundary-log writer and `verify_stream` helper. See [core_graph_audit](../core_graph_audit.md). |
...
Raw transcripts, summaries, and prompts are never stored. See [core_graph_audit](../core_graph_audit.md) for how rows are written.
...
- **Gateway**: The [gateway](../models/gateway.md) writes one `request_audit_log` row per `/ask`, `/ask_with_image`, `/ide/chat`, and similar request paths. The router only reads these rows.
...
- [auth_router](auth_router.md) — authentication, JWT/API-key resolution, and user enrichment.
- [db_models](../db_models.md) — `RequestAuditLog` and `graph_audit_log` schema definitions.
- [core_graph_audit](../core_graph_audit.md) — writing and verifying tamper-evident boundary logs.
- [gateway](../models/gateway.md) — the service that populates `request_audit_log`.
