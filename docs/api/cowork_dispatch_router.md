...
|------------|------|------|
| `auth.dependencies.get_current_user` | JWT/API-key authentication and user enrichment. | [auth_dependencies](../auth_dependencies.md) |
| `core.config` | Redis host/port/password and other environment settings. | [core_config](../infrastructure/core_config.md) |
| `core.logger` | Structured logging for dispatch events. | [core_logger](../core_logger.md) |
| `db.database.engine` | SQLAlchemy engine for the `cowork_dispatch` table. | [db_database](../db_database.md) |
...
| [cowork_task_worker](../cowork_task_worker.md) | Executes scheduled Cowork tasks in server-side office mode. |
| [desktop_app](../clients/desktop_app.md) | The user's desktop runtime that claims and executes dispatches. |
...
For details on the desktop side of the dispatch flow, see [desktop_app](../clients/desktop_app.md). For scheduled/headless Cowork execution, see [cowork_tasks_router](cowork_tasks_router.md) and [cowork_task_worker](../cowork_task_worker.md).
