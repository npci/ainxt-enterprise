...
| `validate_profile_update_request` | [core/security_validation](../reference/shared_core.md) | Validates `name` and `gitlab_username` for injection / XSS / special characters. |
| `validate_token_upsert_request` | [core/security_validation](../reference/shared_core.md) | Validates token value and label before persistence. |
| `logger` | [core/logger](../reference/shared_core.md) | Used for audit logging of token reveals and upserts. |
| `db.database.engine` | [db/database](../storage/database.md) | SQLAlchemy engine for `users` and `user_tokens` tables. |
...
- **Model routing**: `get_decrypted_token` is used by the model-routing layer (see [model_router](../models/model_routing.md)) when a NEURON-tier credential is required.
- **Desktop / Git workflows**: `reveal_token` for `gitlab` enables the desktop client to perform `git clone` with the user's stored PAT.
- **Security**: input validation is delegated to [core/security_validation](../reference/shared_core.md); audit logging uses [core/logger](../reference/shared_core.md).
