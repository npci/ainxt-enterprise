...
The auth router is a **thin orchestration layer** — it delegates all cryptographic, persistence, and session-tracking concerns to the [authentication](../security/authentication.md) module in `shared_core`. The router itself contains no token-signing logic, no Redis client management, and no LDAP protocol handling; it wires these dependencies together per-request.
...
|---|---|---|
| `auth.dependencies` | [authentication](../security/authentication.md) | `get_current_user`, `enrich_user_context` |
| `auth.jwt_handler` | [authentication](../security/authentication.md) | `encode_token`, `revoke_token`, `_SECRET_KEY`, `ALGORITHM`, `EXPIRE_HOURS` |
| `auth.rbac` | [authentication](../security/authentication.md) | `require_role` dependency factory |
| `auth.session_manager` | [authentication](../security/authentication.md) | `register_session`, `get_active_sessions`, `revoke_session`, `revoke_all_sessions` |
| `auth.ldap_handler` | [authentication](../security/authentication.md) | `authenticate_user` (LDAP bind-validate) |
| `auth.sso` | [authentication](../security/authentication.md) | `get_sso_provider`, `get_sso_login_url`, `exchange_sso_code` |
| `core.rate_limiter` | [core_infrastructure](../infrastructure/core_infrastructure.md) | `enforce_rate_limit`, rate-limit configs |
| `core.logger` | [core_infrastructure](../infrastructure/core_infrastructure.md) | Structured logging |
| `core.security_validation` | [core_infrastructure](../infrastructure/core_infrastructure.md) | `validate_level_override_request` |
| `core.config` | [core_infrastructure](../infrastructure/core_infrastructure.md) | `LDAP_ENABLED`, `redis_client`, `RDB_QUEUE` |
...
| `services.hierarchy_service` | [services](../reference/services.md) | `has_direct_reports` |
