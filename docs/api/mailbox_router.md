...
- **[`auth.dependencies.get_current_user`](../auth_dependencies.md)** — Extracts and validates the caller identity from a JWT Bearer token, API key, or `auth_token` cookie, and enriches the payload with server-side profile data.
- **[`core.config.redis_client`](../infrastructure/core_config.md)** — Returns a configured Redis client for the requested database number. The router uses database `2` for transient agent coordination.
...
- **[`memory_router`](memory_router.md)** — Stores long-term agent and platform memory. Use memory for durable recall, not transient coordination.
- **[`agents_router`](agents_router.md)** — Manages agent CRUD, KB attachments, and favorites. Agents are the typical senders/receivers of mailbox messages.
