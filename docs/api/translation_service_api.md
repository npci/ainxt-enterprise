...
This module is the API layer only: it validates requests, orchestrates caching, delegates blocking model work to a thread pool, and returns structured responses. The actual model inference lives in [translation_service_engine.md](../reference/translation_service_engine.md), and the Redis cache wrapper is documented in [translation_service_cache.md](../reference/translation_service_cache.md).
...
- Prevents blocking the main [gateway](../models/gateway.md) asyncio loop.
...
1. Connects the [translation_service_cache](../reference/translation_service_cache.md) to Redis.
...
| --- | --- | --- |
| `services/translate_svc/translator.py` | Delegates all blocking model inference. | [translation_service_engine.md](../reference/translation_service_engine.md) |
| `services/translate_svc/cache.py` | Provides async Redis get/set/get_many/set_many. | [translation_service_cache.md](../reference/translation_service_cache.md) |
...
| `core/logger.py` | Optional structured logger; falls back to stdlib `logging`. | [shared_core.md](../reference/shared_core.md) |
...
- [embedding_service.md](../knowledge/embedding_service.md) — text embedding and reranking.
- [privacy_service.md](../security/privacy_service.md) — PII/screening inference.
- [whisper_service.md](../reference/whisper_service.md) — speech-to-text.
- [compression_service.md](../reference/compression_service.md) — text compression.
- [llm_proxy.md](../models/llm_proxy.md) — general LLM routing.

Callers such as the [gateway](../models/gateway.md), [shared_core](../reference/shared_core.md) (`core/translation_wrapper.py`), and background [workers](../workers/workers.md) send HTTP requests to `POST /translate` or `POST /translate_batch` when they need deterministic, cached English ↔ Indic translations. The service is not invoked directly by end-user browsers; it is consumed server-side by other platform components.
