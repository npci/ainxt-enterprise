...
|---|---|---|
| `auth/dependencies.py` | [authentication](../security/authentication.md) | JWT/API-key auth, `get_current_user`, `require_admin` |
| `core/config.py` | [core_infrastructure](../infrastructure/core_infrastructure.md) | `ENABLE_TENX_AWARD` feature flag |
| `core/job_queue.py` | [core_infrastructure](../infrastructure/core_infrastructure.md) | `enqueue_job`, `cancel_job`, `Q_TENX` queue |
| `db/models.py` | [database](../storage/database.md) | `TenXSubmission`, `TenXMember`, `TenXEvidence`, `TenXScore`, `TenXAuditLog`, `TenXMetric` |
| `db/database.py` | [database](../storage/database.md) | `SessionLocal` SQLAlchemy session factory |
| `tenx/config.py` | [tenx_system](../reference/tenx_system.md) | Dimensions, weights, tracks, `SubmissionStatus`, `previous_month_period` |
| `tenx/ingestion.py` | [tenx_system](../reference/tenx_system.md) | `audit_and_redact`, `classify_artifact`, `valid_url`, `repo_index_key` |
| `tenx/storage.py` | [tenx_system](../reference/tenx_system.md) | `save_document`, `resolve`, `kind_for`, `is_video` |
| `tenx/orchestrator.py` | [tenx_system](../reference/tenx_system.md) | `run()` — end-to-end evaluation pipeline |
| `tenx/cli_runner.py` | [tenx_system](../reference/tenx_system.md) | `clone_repo`, `clone_state`, `run_eval_dimension` |
| `tenx/chat.py` | [tenx_system](../reference/tenx_system.md) | `chat()` — committee codebase Q&A |
| `tenx/scoring.py` | [tenx_system](../reference/tenx_system.md) | `cohort_normalize` — percentile + z-score |
| `workers/tenx_eval_worker.py` | [tenx_evaluation_workers](../workers/tenx_evaluation_workers.md) | `evaluate_job`, `clone_job`, `_resolve_gitlab_token`, `_validate_gitlab_token` |
| `store/inbox_store.py` | [store_layer](../storage/store_layer.md) | `publish_inbox_item` — team notifications |
| `ai-ui/src/components/TenXAward.jsx` | [tenx_award](../reference/tenx_award.md) | Frontend UI (5 tabs: Submit, My, Others, Leaderboard, Committee) |
