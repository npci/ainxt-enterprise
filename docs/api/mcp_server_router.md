...
The `mcp_server_router` is one of many [shared API routers](shared_api_routers.md) mounted into the main FastAPI application. It exposes the platform's MCP capabilities to authenticated clients while keeping the protocol implementation inside the dedicated [MCP system](../mcp/mcp_system.md).
...
- **[Authentication](../security/authentication.md)**: All endpoints require a valid JWT via `get_current_user`. External server mutation endpoints additionally require the `admin` role via `require_role("admin")`.
- **[Core Infrastructure](../infrastructure/core_infrastructure.md)**: Uses the shared `logger` for observability and `validate_external_server_request` for sanitizing external server registration payloads.
...
- [Authentication](../security/authentication.md) — JWT dependency and role-based access control.
- [Core Infrastructure](../infrastructure/core_infrastructure.md) — Shared logging and security validation utilities.
