...
This module is the **transport and session-routing layer**. It does not implement tool logic itself; it delegates all dispatch and per-user tool logic to the [`connectors/mcp_bridge`](../reference/shared_integrations.md) module. The router's primary responsibilities are:
...
The router delegates all JSON-RPC dispatch and tool logic to the bridge. See [shared_integrations](../reference/shared_integrations.md) for the connector infrastructure details. Key bridge functions:
...
|---|---|---|
| `auth.dependencies.get_current_user` | [authentication](../reference/shared_core.md) (auth) | JWT-based user resolution for every endpoint. |
| `core.config` | [core_infrastructure](../reference/shared_core.md) | Redis connection params, compliance config flags. |
| `core.logger` | [core_infrastructure](../reference/shared_core.md) | Structured logging. |
| `connectors.mcp_bridge` | [shared_integrations](../reference/shared_integrations.md) | All dispatch + per-user tool logic. |
| `connectors.registry` | [shared_integrations](../reference/shared_integrations.md) | Per-user connected connector tool listing + execution. |
| `connectors.engine` | [shared_integrations](../reference/shared_integrations.md) | PAT auto-connect for GitLab/Jira. |
| `agents.compliance_engine` | [agent_system](../reference/shared_core.md) | Input blocking + output redaction (PAN/PII/secret). |
| `mcp.servers.base._compliance_check` | [mcp_servers](../mcp/mcp_servers.md) | Input compliance check for tool arguments. |
| `core.job_queue` | [core_infrastructure](../reference/shared_core.md) | Enqueue doc-generation and sandbox-exec jobs. |
...
| `memory.cowork_memory` | [memory_system](../reference/shared_core.md) | Durable per-user Cowork memory (`remember` tool). |
| `models.model_router` | [model_routing](../reference/shared_core.md) | LLM generation for `deep_research` and `revise_artifact`. |
| `db.database` / `db.models` | [database](../reference/shared_core.md) | `GeneratedDocument` / `ChatAttachment` lookups. |
| `core.storage` | [core_infrastructure](../reference/shared_core.md) | Object storage for chat-uploaded attachments. |
| `core.otel` | [core_infrastructure](../reference/shared_core.md) | OTLP span wrapping for tool-call telemetry. |
| `services.cowork_policy` | [services](../reference/shared_core.md) | Org/dept connector policy enforcement. |
...
| Topic | Document |
|---|---|
| Connector infrastructure (registry, engine, adapters) | [shared_integrations](../reference/shared_integrations.md) |
| Compliance engine (PAN/PII detection, redaction) | [shared_core](../reference/shared_core.md) (agent_system) |
...
| Memory system (Cowork memory) | [shared_core](../reference/shared_core.md) (memory_system) |
| Model routing (LLM generation) | [shared_core](../reference/shared_core.md) (model_routing) |
| Database models (GeneratedDocument, ChatAttachment) | [shared_core](../reference/shared_core.md) (database) |
| Core infrastructure (config, logger, storage, OTel) | [shared_core](../reference/shared_core.md) (core_infrastructure) |
| Authentication (JWT, RBAC) | [shared_core](../reference/shared_core.md) (authentication) |
