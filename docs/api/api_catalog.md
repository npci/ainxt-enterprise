...
The module is a thin FastAPI router layer: it validates requests, enforces authentication, delegates persistence to [`core_workflow_repo`](../workflows/core_workflow_repo.md), delegates AI generation to [`agent_factory_pipeline`](../agents/agent_factory_pipeline.md) and [`skill_factory_pipeline`](../skills/skill_factory_pipeline.md), and routes governance submissions through [`core_governance_client`](../sdlc/core_governance_client.md). It owns no business logic of its own beyond request shaping, security guards (zip-bomb / path-traversal protection on uploads), and M365-connection-aware filtering.
...
- **See:** [`skill_factory_pipeline`](../skills/skill_factory_pipeline.md) for the generation pipeline and [`agent_factory_pipeline`](../agents/agent_factory_pipeline.md) for `DynamicSkillGenerator`.
...
- **`workflow_repo`** ([`core_workflow_repo`](../workflows/core_workflow_repo.md)): All Postgres persistence. Uses a shared connection pool (`get_pool_or_raise`) and runs blocking psycopg calls via `asyncio.to_thread`.
...
- **`skill_factory.pipeline`** ([`skill_factory_pipeline`](../skills/skill_factory_pipeline.md)): Provides SKILL.md validation, frontmatter parsing, safe-path normalization, and the `catalog_cache` (invalidated after any skill mutation).
...
3. **Skill Factory** ([`skill_factory_pipeline`](../skills/skill_factory_pipeline.md)) and **Agent Factory Chat** ([`api_factories`](api_factories.md)) create skills/agents that subsequently appear in the catalog listings.
4. **Native Engine** ([`engine_native_engine`](../agents/engine_native_engine.md)) wraps catalog tools as `_CatalogTool` instances at execution time, reading the `code` and `input_schema` persisted by this module.
...
- [`core_workflow_repo`](../workflows/core_workflow_repo.md) — Postgres persistence for tools, skills, workflows, agents
...
- [`skill_factory_pipeline`](../skills/skill_factory_pipeline.md) — SKILL.md validation, frontmatter parsing, `catalog_cache`, SkillFactory generation pipeline
...
- [`engine_native_engine`](../agents/engine_native_engine.md) — `_CatalogTool` wraps catalog tools for execution
