...
| `ocr_pipeline.extract` | `app/core/ocr_pipeline.py` | Hybrid extraction pipeline (structured parse, table extraction, image OCR, scanned-PDF fallback, cache). See [core_ocr.md](core_ocr.md). |
...
| [app_models.md](app_models.md) | Uses `AuthenticatedUser` for identity, role, and department. |
| [core_ocr.md](core_ocr.md) | Uses `ocr_pipeline.extract`, `ExtractionOptions`, and `supported_extensions` for hybrid document/image extraction. |
