...
| Module | Relationship |
|---|---|
| [knowledge_base](knowledge_base.md) | Primary consumer — renders `ScopePicker` in the Upload tab with `includeUploadFields` |
| [kb_chat](kb_chat.md) | Hydrates scope fields from the Chat row; uses scope for RAG retrieval via `/ask` |
| [chat](chat.md) | Same scope hydration pattern as `kb_chat`; supports embedded KB mode |
| [config](config.md) | Provides `API_BASE` and `authFetch` used by all API calls |
| [kb_graph](kb_graph.md) | Alternative visual graph-based scope picker (`KbScopeGraph`) used by `KbChatPanel` for chat-side scope selection |
