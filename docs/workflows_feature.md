...
- [`workflowStore`](store.md) – Zustand store holding the current workflow graph, execution state, run telemetry, and chat thread state.
- [`dashboardStore`](store.md) – loads, creates, duplicates, and deletes workflows/templates.
- [`useFactoryChatStream`](shared_features.md) – reusable Server-Sent Events (SSE) hook that powers the factory chat experience.
- [`FactoryChatShell`](shared_features.md) – generic modal chat UI used by both agent and workflow factories.
- [`triggersStore`](store.md) / [`TriggerModal`](triggers_feature.md) / [`TriggerSection`](triggers_feature.md) – scheduling and webhook triggers attached to workflows.
- [`StatusBadge`](governance_feature.md) / [`SubmitApprovalButton`](governance_feature.md) – governance status and approval submission.
...
- [`workflowStore`](store.md) owns the in-editor state:
...
- [`dashboardStore`](store.md) owns the list views:
...
|------------|--------|------|
| `workflowStore` | [store](store.md) | Current workflow graph and execution telemetry |
| `dashboardStore` | [store](store.md) | Workflow/template library |
| `useFactoryChatStream`, `FactoryChatShell` | [shared_features](shared_features.md) | Reusable factory chat engine and UI shell |
| `FactoryFileChips`, `DownloadNotice`, `sniffGeneratedFiles` | [shared_features](shared_features.md) | Generated file download UX |
| `InlinePicker`, `PlanCard`, `ConfirmModal`, `HoverTooltip`, `TemplatesEmptyState` | [common_components](common_components.md) | Generic UI primitives |
| `StatusBadge`, `SubmitApprovalButton` | [governance_feature](governance_feature.md) | Governance status and approval |
| `TriggerModal`, `TriggerSection` | [triggers_feature](triggers_feature.md) | Scheduling/webhook triggers |
| `TemplateCardMenu`, `TemplateCreateModal` | [templates_feature](templates_feature.md) | Template admin UI |
| `useTriggerPortalContainer` | [triggers_feature](triggers_feature.md) | Shared portal container for toasts/modals |
| `formatDate`, `stripTemplateTag` | [utils](utils.md) | Formatting helpers |
| `API_BASE`, `buildAuthHeaders` | [config](config.md) | Backend API configuration |
...
- [`api_execution`](api_execution.md) – synchronous and streaming workflow execution.
- [`api_factories`](api_factories.md) – workflow factory chat/confirm endpoints.
- [`api_templates`](api_templates.md) – template listing and usage.
- [`api_triggers`](api_triggers.md) – trigger CRUD and execution history.
- [`api_governance`](api_governance.md) – governance status and approval submission.
