...
The module is intentionally thin: it contains **no scoring logic**. All evaluation orchestration is delegated to the [`tenx_system`](tenx_system.md) module (`tenx.orchestrator.run`), while repository cloning is delegated to [`tenx_system`](tenx_system.md) (`tenx.cli_runner.clone_repo`). The worker’s only responsibilities are job dispatch, GitLab token resolution, lightweight validation, audit logging, and status management.
...
3. The orchestrator handles eligibility, dimension scoring, consistency checks, and persistence (see [`tenx_system`](tenx_system.md) documentation).
...
The worker consumes `tenx_queue` (`Q_TENX`), defined in [`core/job_queue.py`](core_infrastructure.md). The queue has a depth limit of 200 and is listed in `ALL_QUEUES` with lower priority than interactive queues, reflecting the long-running nature of 10x evaluations.

### Triggering Jobs

- **Clone jobs** are enqueued by the submission flow in [`routers/tenx_router.py`](tenx_router.md) (not shown in the provided snippets but implied by the `CLONING` status).
- **Evaluate jobs** are enqueued by the admin-only `POST /tenx/{id}/evaluate` endpoint in [`routers/tenx_router.py::evaluate`](tenx_router.md).

### Database Entities

The worker reads and writes the following tables (defined in [`db/models.py`](database.md)):
...
Statuses are defined in [`tenx/config.py::SubmissionStatus`](tenx_system.md).
...
- [`tenx_system`](tenx_system.md) — evaluation orchestration, scoring, eligibility, and CLI runner.
- [`core_infrastructure`](core_infrastructure.md) — job queue, logging, CKMS, and configuration.
- [`database`](database.md) — `TenXSubmission`, `TenXEvidence`, `TenXAuditLog`, and related models.
- [`routers/tenx_router`](tenx_router.md) — HTTP endpoints that enqueue clone and evaluate jobs.
