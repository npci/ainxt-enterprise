...
- **[auth.md](auth.md)** — `auth.jwt_handler.decode_token` and `auth.api_key_auth` for identity resolution from Bearer tokens and platform API keys.
- **[store_layer.md](store_layer.md)** — `store.budget_store` and `store.inbox_store` for budget checks and budget-alert notifications.
