...
- **[chat.md](chat.md)** — The main chat surface where a selected history item would be loaded.
- **[kb_chat.md](kb_chat.md)** — Knowledge-base chat surface that also maintains conversation history.
- **[threads.md](threads.md)** — Thread management UI; the history panel will likely reuse or mirror thread-listing behavior.
- **[message.md](message.md)** — Message rendering; history entries may preview message content.
- **[app_core.md](app_core.md)** — The top-level `App.jsx` shell that hosts layout regions such as the history panel.
