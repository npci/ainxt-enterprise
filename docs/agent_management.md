...
The `AgentRunner` (in [shared_core](shared_core.md)) drives the full execution pipeline when `run_agent` or `talk_to_agent` is called:
...
The `AgentBuilder` (in [shared_core](shared_core.md)) manages the agent definition lifecycle:
...
See [governance](governance.md) for the full governance workflow documentation.
...
| Module | Relationship |
|---|---|
| [shared_core](shared_core.md) | Provides `AgentBuilder`, `AgentRunner`, `AgentDefinition`, compliance, model routing, and MCP tool registry |
| [ABStudio backend](abstudio_backend.md) | Provides a parallel agent CRUD surface (`api/agents.py`) with audit events, trigger cleanup, and the AI-powered `agent_factory/pipeline.py` |
| [shared_api_routers](shared_api_routers.md) | Provides `agents_router.py` for agent catalog browsing, favorites, and KB document attachment |
| [governance](governance.md) | Manages the agent approval lifecycle (DRAFT → PRODUCTION) |
| [chat_and_messaging](chat_and_messaging.md) | Shares the `_save_chat_messages` persistence pattern and Kafka event production |
