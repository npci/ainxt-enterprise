...
| Module | Role |
|--------|------|
| [`app_core`](app_core.md) | Mounts `KbChatPanel`, provides `onHandoff`, owns routing and global chat state. |
...
| Module | Role |
|--------|------|
| [`chat_router`](chat_router.md) | `POST /chats`, `PATCH /chats/{id}/scope`, `PATCH /chats/{id}/rag-mode`. |
| [`gateway`](gateway.md) chat_and_messaging | `POST /ask` and streaming endpoints used by [`kb_chat`](kb_chat.md). |
...
- [`app_core`](app_core.md) — Application shell, routing, and global chat state.
- [`chat_router`](chat_router.md) — Backend router for chat CRUD and scope/rag-mode updates.
- [`gateway`](gateway.md) chat_and_messaging — Backend streaming and messaging services.
