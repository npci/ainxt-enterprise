...
The `CatalogPicker` component (see [common_components](common_components.md)) fetches the tools or skills catalog from `/{kind}-catalog` and provides:
...
The `KnowledgeSection` component (see [common_components](common_components.md)) supports three modes:
...
| Backend Module | Role |
|---|---|
| [api_agents](api_agents.md) | Agent CRUD routes (`create_agent_route`, `update_agent_route`). |
| [api_factories](api_factories.md) | Agent runner chat streaming (`agent_runner_chat_stream`). |
| [api_agent_chat](api_agent_chat.md) | Thread listing, history loading, deletion. |
| [api_documents](api_documents.md) | File attachment upload and image asset handling. |
| [api_catalog](api_catalog.md) | Tools/skills catalog listing and generation. |
| [core_workflow_repo](core_workflow_repo.md) | `create_agent` / `update_agent` database operations. |
...
| [api_governance](api_governance.md) | Governance submission/status (via `SubmitApprovalButton` / `StatusBadge`). |
| [api_triggers](api_triggers.md) | Trigger configuration (via `TriggerSection`). |
...
For governance workflow details, see the [api_governance](api_governance.md) and [governance_feature](governance_feature.md) documentation.

---

## 9. Trigger Integration

The `TriggerSection` component (see [triggers_feature](triggers_feature.md)) is embedded in Edit Mode, allowing users to configure webhook and scheduled triggers for the agent. It is disabled until the agent has been saved (`savedId` is set).
...
The preview chat intentionally mirrors the Workflow editor's `ChatPanel` (see [ChatPanel](ChatPanel.md)) for:
...
| [common_components](common_components.md) | Shared UI components (CatalogPicker, KnowledgeSection, PlanCard, etc.). |
| [shared_features](shared_features.md) | Shared feature utilities (downloadGeneratedFile, ExtractedTextPreview, etc.). |
| [agent_factory_pipeline](agent_factory_pipeline.md) | Backend AgentRunner — runtime execution, system prompt, tool dispatch, swarm. |
| [api_agents](api_agents.md) | Backend agent CRUD API routes. |
| [api_factories](api_factories.md) | Backend agent runner chat streaming endpoint. |
| [api_agent_chat](api_agent_chat.md) | Backend agent chat thread management. |
| [api_documents](api_documents.md) | Backend file attachment and image asset endpoints. |
| [api_governance](api_governance.md) | Backend governance submission and status. |
| [triggers_feature](triggers_feature.md) | Trigger configuration and notification components. |
| [governance_feature](governance_feature.md) | Governance status badge and approval submission. |
| [ChatPanel](ChatPanel.md) | Workflow editor chat panel (mirrored patterns). |
| [core_workflow_repo](core_workflow_repo.md) | Backend agent persistence layer. |
