...
The `export_template` module provides lightweight, user-facing utilities for persisting chat content from the AI-UI frontend. It exposes two complementary capabilities: **exporting a chat thread as a Markdown file** and **saving the current composer text as a reusable prompt template**. These helpers live inside the main chat components and are reused by both the general-purpose [`chat`](chat.md) experience and the knowledge-base-focused [`kb_chat`](kb_chat.md) experience.
...
| **Backend dependency** | [`chat_router`](chat_router.md) (`/prompt-templates` endpoints) for template persistence. |
| **Related modules** | [`chat`](chat.md), [`kb_chat`](kb_chat.md), [`auth`](auth.md) |
...
| `authFetch` | Authenticated HTTP client; see [`auth`](auth.md). |
...
| `/prompt-templates` endpoints | Backend CRUD for prompt templates; owned by [`chat_router`](chat_router.md). |
...
- The chat state model defined in [`chat`](chat.md) and [`kb_chat`](kb_chat.md).
- The authentication and HTTP layer from [`auth`](auth.md).
- The prompt-template storage and listing APIs from [`chat_router`](chat_router.md).
