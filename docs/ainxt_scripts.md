...
- **Backend ingestion endpoint**: `index_ainxt.py` posts to `/ainxt/v1/api/admin/templates/index`, which is implemented by the `templates_router` in the [`shared_api_routers`](shared_api_routers.md) module. See [`templates_router`](templates_router.md) for details on how payloads are chunked, stored, and versioned.
