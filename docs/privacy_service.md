...
- **Result caching**: Caches per-text entity results in a Redis/RustyCluster-backed KV store (database index `RDB_PRIVACY`) to avoid repeated inference on identical inputs.
...
For details on how detected PII is redacted or governed, see [compliance_engine.md](compliance_engine.md) and [guardrails.md](guardrails.md). For the KV cache implementation used for result caching, see [kv_store.md](kv_store.md).
