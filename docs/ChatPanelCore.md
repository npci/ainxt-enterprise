...
| Dependency | Purpose |
|---|---|
| [workflowStore](workflowStore.md) | Primary state store (chat slice, execution state, run context, node graph) |
...
- [workflowStore](workflowStore.md) — Global Zustand store for workflow editor state
- [DebugLogView](DebugLogView.md) — Debug Log timeline view component
- [RunSettingsStrip](RunSettingsStrip.md) — Run-settings popover for workflow-wide execution options
- [api_execution](api_execution.md) — Backend execution API (`/run-stream`, `/resume-stream`)
- [api_chat](api_chat.md) — Backend chat API (`/chat-history`, `/chat-threads`, `/chat-pending`)
- [api_documents](api_documents.md) — Backend documents API (`/agent-runner/attachment`, `/agent-runner/image-asset`)
- [engine_native_engine](engine_native_engine.md) — Backend native workflow engine (SSE event source)
