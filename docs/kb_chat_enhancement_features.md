...
| Dependency | Role |
|------------|------|
| `authFetch` / `API_BASE` ([`config.js`](ai_ui_frontend_utils.md)) | Authenticated HTTP client and backend base URL. |
| `useToast` ([`DialogProvider.jsx`](ui_dialog.md)) | Toast notifications for errors. |
...
| Dependency | Role |
|------------|------|
| `gateway.py::enhance_prompt` ([`gateway`](gateway.md)) | FastAPI route that accepts `{prompt}` and returns `{enhanced, followups}`. |
| `gateway.py::_enhance_core` | Shared core logic: compliance check, model call, JSON parsing, follow-up generation. |
| `agents.compliance_engine` ([`compliance_engine`](agent_system.md)) | Input validation / redaction before the LLM call. |
| `models.model_router` ([`model_router`](model_routing.md)) | Routes the enhancement request to the configured model (default `mini`). |
...
- [`chat_enhancement_features`](chat_enhancement_features.md) — Equivalent enhancement feature in general chat.
- [`gateway`](gateway.md) — Backend `/enhance` route and `_enhance_core` implementation.
- [`model_routing`](model_routing.md) — Model router used by the enhancement backend.
- [`agent_system`](agent_system.md) — Compliance engine that pre-screens prompts.
