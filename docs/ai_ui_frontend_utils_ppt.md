...
These utilities are intentionally lightweight and regex-based so that intent classification and parameter extraction can happen synchronously on the client without calling an LLM. They feed the [PPT Wizard](ppt_wizard.md) and [PPT Chat](ppt_chat.md) experiences and are consumed by the broader [KB Chat](kb_chat.md) flow.
...
| Module | Role |
|--------|------|
| [presenton_router](presenton_router.md) | Receives `GenerateRequest` / `OutlineRequest` and orchestrates outline generation and presentation download |
| [presenton_lib](presenton_lib.md) | Client-side library that streams outlines, polls status, and builds slide payloads |
| [workers/presenton_worker](workers.md) | Background worker that executes the actual PPT generation job |
...
- **Downstream:** [ppt_wizard](ppt_wizard.md) consumes extracted parameters to render the step-by-step presentation builder.
- **Downstream:** [presenton_lib](presenton_lib.md) and [presenton_router](presenton_router.md) perform the actual outline streaming, status polling, and file generation.
- **Downstream:** [workers/presenton_worker](workers.md) executes the generation job asynchronously.
...
- [kb_chat](kb_chat.md) – primary consumer of PPT intent detection
- [ppt_wizard](ppt_wizard.md) – UI that consumes extracted parameters
- [ppt_chat](ppt_chat.md) – chat-based PPT generation UI
- [presenton_lib](presenton_lib.md) – client-side PPT generation library
- [presenton_router](presenton_router.md) – backend API for PPT generation
- [workers](workers.md) – background worker ecosystem, including `presenton_worker`
