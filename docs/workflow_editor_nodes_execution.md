...
- Surface **name conflicts** by turning the node red and showing a warning badge. The conflict check compares the node's name against:
  ...
  - Saved agents from [`agentsStore`](store.md).
  - Saved workflows from [`dashboardStore`](store.md).
...
The backend [`engine_native_engine`](engine_native_engine.md) is the actual executor. It receives the exported graph from `workflowStore.getWorkflowForExecution()` and dispatches:
...
Defined in [`workflowStore.js`](store.md) `getDefaultNodeData('agent')`:
...
- [`store`](store.md) — `workflowStore`, `agentsStore`, `dashboardStore` state management.
- [`engine_native_engine`](engine_native_engine.md) — Backend orchestration engine that executes `agent` and `subflow` nodes.
