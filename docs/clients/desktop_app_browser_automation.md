...
[`desktop_app_cowork_engine`](../buddy/desktop_app_cowork_engine.md) and
...
| [`desktop_app_cowork_engine`](../buddy/desktop_app_cowork_engine.md) | Sibling | The cowork CLI manager (`SessionManager`) and local MCP server route `browser_*` tool calls to `executeTool`. Browser tools are exposed alongside `computer_*` and CLI tools. |
...
| [`desktop_app_preload_bridge`](../desktop_app_preload_bridge.md) | Sibling | The preload bridge exposes IPC to the renderer; browser automation runs entirely in the main process and does not cross the preload boundary. |
