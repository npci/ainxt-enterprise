...
`pptx_add_slide` is one step in a typical unpack-modify-pack workflow. It expects an unpacked PPTX directory produced by [pptx_office_unpack](../documents/pptx_office_unpack.md) and is usually followed by validation ([pptx_office_validate](../documents/pptx_office_validate.md)) and repacking ([pptx_office_pack](../documents/pptx_office_pack.md)).
...
- [doc_generator](../documents/doc_generator.md) — generates simple `.pptx` files from slide dictionaries using `python-pptx`.
- [presenton_router](../api/presenton_router.md) / [presenton_worker](../presenton_worker.md) — orchestrates AI-driven presentation generation via the Presenton service.
- [doc_download_router](../api/doc_download_router.md) — exposes user-facing endpoints for document generation jobs.
...
Because the script operates directly on the unpacked OOXML package, it is independent of the `python-pptx` library used by [doc_generator](../documents/doc_generator.md) and can handle layouts and relationships that higher-level libraries may not expose.

---

## References

- [pptx_office_unpack](../documents/pptx_office_unpack.md) — unpacks a `.pptx` into the directory format this script consumes.
- [pptx_office_pack](../documents/pptx_office_pack.md) — repacks the modified directory back into a `.pptx`.
- [pptx_office_validate](../documents/pptx_office_validate.md) — validates the OOXML package after modification.
- [pptx_clean](../reference/pptx_clean.md) — removes orphaned slides and relationships after edits.
- [pptx_thumbnail](../reference/pptx_thumbnail.md) — generates thumbnail grids from a finished deck.
- [doc_generator](../documents/doc_generator.md) — high-level document/PPTX generation tool.
- [presenton_router](../api/presenton_router.md) — API router for AI presentation generation.
- [presenton_worker](../presenton_worker.md) — background worker that drives the Presenton service.
- [doc_download_router](../api/doc_download_router.md) — user-facing endpoints for document generation jobs.
