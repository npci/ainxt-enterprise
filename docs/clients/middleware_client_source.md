...
1. **Thread-local logger context** — so every log line emitted during the
   ...
   [core_infrastructure](../infrastructure/core_infrastructure.md) / `core.logger`).
...
> **See also:** [middleware_request_id](../reference/middleware_request_id.md),
...
| `core.logger.set_client_source` | Internal ([core_infrastructure](../infrastructure/core_infrastructure.md)) | Sets the `contextvars`-backed thread-local that the structured logger reads on every log emission. |
...
| **Gateway streaming handler** | [gateway](../models/gateway.md) | Uses `request.state.client_source` (and a request-shape heuristic) to guard the RAG-injection path for `browser-agent` requests. |
| **Telemetry / Metrics** | [core_infrastructure](../infrastructure/core_infrastructure.md) (`core.telemetry`) | The `client_source` dimension appears in Prometheus metrics and OpenTelemetry spans, enabling per-client dashboards. |
| **Structured Logging** | [core_infrastructure](../infrastructure/core_infrastructure.md) (`core.logger`) | Every log line emitted during the request automatically includes the `client_source` field via the thread-local context variable. |
...
| **Key consumers** | [middleware_rate_limit](../middleware_rate_limit.md), [middleware_budget](../middleware_budget.md), [gateway](../models/gateway.md), [core_infrastructure](../infrastructure/core_infrastructure.md) (logger + telemetry). |
