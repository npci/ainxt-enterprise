...
| `compliance_engine` | `agents/compliance_engine.py` | Validates input code for policy violations and redacts sensitive content from output. See compliance_engine. |
| `logger` | `core/logger.py` | Structured logging for the execution lifecycle. See logger. |
...
- **compliance_engine** — Policy enforcement and output redaction.
- **logger** — Execution observability.
...
- **agent_builder** and **react_engine** may invoke it as a code-execution tool.
- **[sdlc_coder_tools](../sdlc/sdlc_coder_tools.md)** and **[sdlc_state_machine](../sdlc/sdlc_state_machine.md)** use sandboxed execution for coding and testing tasks.
...
- **compliance_engine** supplies the policy checks that gate both input and output.
