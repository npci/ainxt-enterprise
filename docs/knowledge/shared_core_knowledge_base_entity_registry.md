...
3. workers_document_knowledge_workers (specifically `kb_entity_worker`) scan those chunks, resolve surface forms through this registry, and write `kb_edges`.
4. shared_api_routers_kb_router lets users browse entities, view lineage, diff versions, and promote entities to the global tier.
...
- workers_document_knowledge_workers — `kb_entity_worker` and `knowledge_graph_worker`
- shared_api_routers_kb_router — entity browse, lineage, version diff, and global promotion endpoints
