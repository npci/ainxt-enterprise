...
- **Offloading hot vectors** to the platform KV store (Redis DB 7, selected by `REDIS_CLIENT_CONFIG_DB7`).
...
For details on the embedders, see [embedding_service_embedders](../embedding_service_embedders.md).
...
- `core.kv.async_get_kv` resolves the concrete KV backend (Redis) using the platform-wide `REDIS_CLIENT_CONFIG_DB7` environment variable.
