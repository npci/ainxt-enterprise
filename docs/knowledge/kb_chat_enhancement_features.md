...
| Dependency | Role |
|------------|------|
| `gateway.py::enhance_prompt` ([`gateway`](../models/gateway.md)) | FastAPI route that accepts `{prompt}` and returns `{enhanced, followups}`. |
...
| `models.model_router` ([`model_router`](../models/model_routing.md)) | Routes the enhancement request to the configured model (default `mini`). |
...
- [`gateway`](../models/gateway.md) — Backend `/enhance` route and `_enhance_core` implementation.
- [`model_routing`](../models/model_routing.md) — Model router used by the enhancement backend.
