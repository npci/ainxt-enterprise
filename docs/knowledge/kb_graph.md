...
`kb_graph_drill.md` — Detailed documentation for the drill-down picker (`KbDrillGraph.jsx`).
...
`kb_graph_scope.md` — Detailed documentation for the force-directed graph picker (`KbScopeGraph.jsx`).
...
- **Shared utilities**: Uses `authFetch` / `API_BASE` from `config` and `highlightMatch` from `utils/kbFormat.js`.
