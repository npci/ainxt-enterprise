...
The backend `upload_chat_files` endpoint in [chat_router](../api/chat_router.md) performs a multi-stage pipeline for each file:
...
| `config.js` | [config](../infrastructure/config.md) | `authFetch` for authenticated requests, `API_BASE` for endpoint URLs |
...
| Dependency | Module | Usage |
|------------|--------|-------|
| `chat_router.py` | [chat_router](../api/chat_router.md) | `upload_chat_files`, `get_attachment`, `get_attachment_raw` endpoints |
| `core/storage.py` | [core_infrastructure](../infrastructure/core_infrastructure.md) | `ObjectStorage` for file persistence (local FS or MinIO) |
...
| `db/models.py` | [database](../storage/database.md) | `ChatAttachment` ORM model for persistence |
...
- [chat_router](../api/chat_router.md) — Backend API router for chat file operations
...
- [config](../infrastructure/config.md) — `authFetch` and `API_BASE` configuration
