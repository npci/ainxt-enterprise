...
| **[kafka_event_consumer](../workers/kafka_event_consumer.md)** | `_handle_embeddings` consumer processes indexing telemetry events from `TOPIC_EMBEDDINGS`. |
| **[sandbox](../sandbox/sandbox.md)** | `SandboxImageBuilder` reads `repo_build_metadata` (populated by `BuildMetadataExtractor`) to determine build tool, language version, and registry configuration for SDLC sandbox images. |
| **[sdlc_pipeline_agents](../sdlc/sdlc_pipeline_agents.md)** | SDLC agents use `sdlc:conventions:{repo}` (cached by `_derive_and_cache_conventions`) for codebase-aware code generation. Graph traversal supports impact analysis. |
