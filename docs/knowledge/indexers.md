...
|------------|------|----------------|---------------|
| Confluence Indexer | `indexers/confluence_indexer.py` | Fetches all pages from a Confluence space, normalizes HTML body text, embeds, and stores them under `docs_kb:confluence_<space>`. | indexers_confluence.md |
| Jira Indexer | `indexers/jira_indexer.py` | Fetches issues from a Jira project, extracts ADF/plain-text descriptions, embeds, and stores them under `docs_kb:jira_<project>`. | indexers_jira.md |
...
- **workers/index_worker.md** – Background worker that runs `index_repo_job` and mirrors code nodes to the knowledge graph; it also coordinates external source indexing.
- **[desktop_router.md](../cowork/desktop_router.md)** – Desktop client can request batch indexing of local files and may also trigger connector-based source indexing.
- **[gateway.md](../core/gateway.md)** – The gateway exposes `index_submit` and `index_status`, which ultimately flow into the indexing pipeline.
