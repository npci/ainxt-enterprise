...
- **[index_router.md](../api/index_router.md)** – Accepts repository index requests and can trigger the Jira/Confluence indexers for connected sources.
...
- **[desktop_router.md](../api/desktop_router.md)** – Desktop client can request batch indexing of local files and may also trigger connector-based source indexing.
- **[gateway.md](../models/gateway.md)** – The gateway exposes `index_submit` and `index_status`, which ultimately flow into the indexing pipeline.
