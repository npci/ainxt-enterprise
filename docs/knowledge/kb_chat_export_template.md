...
> **Scope note:** This module lives inside `ai-ui/src/components/KbChat.jsx`. It is the KB-chat counterpart of the regular chat export/template features found in `Chat.jsx`. See chat_export_template for the non-KB implementation.
...
- The regular chat surface (`Chat.jsx`) implements an equivalent export/template feature set. See chat_export_template.
...
| `authFetch` | [config](../core/config.md) | Authenticated HTTP client for the prompt-templates API. |
| `API` base URL | [config](../core/config.md) | Backend API root. |
| `useToast` | [ui_dialog](../ui/ui_dialog.md) | Success / error notifications. |
| `toIST` | ai-ui utils | Format export timestamp in IST. |
| `/prompt-templates` endpoint | [chat_router](../chat/chat_router.md) | Create and list user prompt templates. |
...
- **Prompt template system:** Saved templates are stored through the same `/prompt-templates` REST endpoint used by the regular chat and other AI UI features. See [chat_router](../chat/chat_router.md) for the backend contract.
- **Authentication:** All API calls use `authFetch`, which attaches the current user's JWT and handles base-URL resolution. See [config](../core/config.md) and [auth](../auth/auth.md).
- **Notifications:** Both handlers use the global toast provider. See [ui_dialog](../ui/ui_dialog.md).
...
- chat_export_template — equivalent export/template features in regular `Chat.jsx`.
- [config](../core/config.md) — `authFetch` and `API` configuration.
- [auth](../auth/auth.md) — authentication context that underlies `authFetch`.
- [ui_dialog](../ui/ui_dialog.md) — toast/notification provider (`useToast`).
- [chat_router](../chat/chat_router.md) — backend router that serves `/prompt-templates`.
