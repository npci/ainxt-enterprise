...
It does **not** handle vector embedding, semantic similarity, approval workflows, or document upload lifecycle. Those concerns live in companion modules such as `docs_store`, `hybrid_search`, and [`kb_router`](kb_router.md).
...
> **Note:** This is a simple keyword retriever. For semantic/vector retrieval, see `hybrid_search` and `local_model`.
...
- `docs_store` owns the richer document upload, approval, chunking, and embedding lifecycle. `kb_search_tools` is a lightweight sibling for filesystem-backed corpora.
- `hybrid_search` and `local_model` provide semantic/vector retrieval for production RAG.
- [`kb_router`](kb_router.md) exposes the canonical entity registry.
- [`docs_router`](../documents/docs_router.md) exposes namespace listing over HTTP.
...
2. Reimplement `search` to call a vector store (e.g., pgvector via `hybrid_search`).
...
- For fine-grained RAG ACL filtering, see [`rag_acl`](../core/core_infrastructure.md) and `docs_store`.
...
- `docs_store` – Document upload, approval, chunking, and embedding.
- [`docs_router`](../documents/docs_router.md) – HTTP routes for document and namespace management.
- [`kb_router`](kb_router.md) – Canonical entity registry routes.
- `hybrid_search` – Semantic/vector retrieval.
- `local_model` – Local LLM and embedding retriever.
