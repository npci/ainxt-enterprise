...
The `checkpoint_agent_chat_store` module provides durable persistence for **agent-runner chat conversations**. While the sibling [checkpoint_workflow_store](checkpoint_workflow_store.md) persists workflow execution threads keyed by `workflow_id`, this module is scoped to `(agent_id, owner_user_id)` so that every deployed agent can maintain a private, per-user chat history.
...
- **`PostgresAgentChatStore`** — Production backend that stores rows in the `agent_chat_threads` table. It reuses the platform-wide `SHARED_POOL` from [app.core.db_pool](core_db_pool.md) and creates its schema on startup.
...
| ---------- | ---------- | ------- |
| `ChatMessage`, `ThreadSummary`, `summarise_thread` | [checkpoint_workflow_store](checkpoint_workflow_store.md) | Shared data types and summary heuristics so the agent chat sidebar stays consistent with the workflow chat sidebar. |
| `core.logger` | [shared_core](shared_core.md) | Structured logging for load / flush diagnostics. |
| `app.core.config.postgres_enabled` | [core_config](core_config.md) | Determines whether PostgreSQL is available. |
| `app.core.db_pool.SHARED_POOL` | [core_db_pool](core_db_pool.md) | Shared connection pool used by the PostgreSQL backend. |

### Upstream consumers

| Consumer | Module doc | Usage |
| -------- | ---------- | ----- |
| `api_agent_chat` | [api_agent_chat](api_agent_chat.md) | REST endpoints for listing, reading, and deleting agent chat threads. |
| `api_factories` | [api_factories](api_factories.md) | `agent_runner_chat` and `agent_runner_chat_stream` load and persist conversation history. |
...
- **No pending-interrupt or node-output support** — Unlike [checkpoint_workflow_store](checkpoint_workflow_store.md), the agent chat store only persists messages. Agent-runner conversations do not currently require HITL snapshots, per-node outputs, loop lessons, or durable run-step replay.
...
- **Serializer parity** — `_serialise_message` mirrors the serialization logic in [checkpoint_workflow_store](checkpoint_workflow_store.md) so that `generated_files` and `usage` survive reloads and remain bit-identical when absent.
...
- Change the message schema → update `_serialise_message` / `_deserialise_message` and keep them in sync with [checkpoint_workflow_store](checkpoint_workflow_store.md).
