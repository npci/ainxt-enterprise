...
* **[core.retry](core_infrastructure.md)** — exponential-backoff retry for transient OpenAI errors.
* **[core.circuit_breaker](core_infrastructure.md)** — named `openai` circuit breaker prevents cascading failures.
* **[agents.compliance_engine](agent_system.md)** — validates input for PCI/PII/secrets and supplies findings for redaction.
* **[core.prompt_sanitizer](core_infrastructure.md)** — strips control characters and normalises line endings before sending to OpenAI.
* **[core.model_registry](model_routing.md)** — provides `OPENAI_PRIMARY_MODEL`, `BLOCKED_MODELS`, and `MODEL_COST_PER_1M`.
* **[core.proxy_tool_use](shared_core.md)** — governs web-search tools, budget checks, and proxy forwarding.
* **[pipeline.stream_events](shared_core.md)** — emits `ReasoningMarker` deltas when the model exposes reasoning text.
* **[core.logger](core_infrastructure.md)** — structured request-scoped logging.
...
* **[gateway.py](gateway.md)** — exposes `/v1/chat/completions`, `/v1/responses`, and internal `/ask` endpoints. Routes medium-tier chat to `OpenAIGateway.generate()` and delegates tool streaming to the LLM proxy.
...
* **[agents/react_orchestrator.py](agent_system.md)** — calls `generate_with_tools()` to drive ReAct-style agent loops.
* **[core.proxy_tool_use](shared_core.md)** — provides the web-search governance and proxy forwarding used inside tool-use loops.
