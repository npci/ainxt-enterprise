...
This module is intentionally **Redis-free** and **thread-safe**, making it suitable for the single-process `llm_proxy` deployment on `web02`. It exposes the same public API as the shared [`core_circuit_breaker`](core_circuit_breaker.md) used by the main gateway, so callers can switch between the two implementations without code changes.
...
- [`core_circuit_breaker`](core_circuit_breaker.md) — the Redis-backed, cluster-wide circuit breaker used by the main `gateway.py`.
...
Because this implementation is in-memory, there is no built-in `/health` endpoint specific to it. For cluster-wide breaker health, the main gateway exposes `circuit_breaker_health()` using the Redis-backed [`core_circuit_breaker`](core_circuit_breaker.md).
...
- [`core_circuit_breaker`](core_circuit_breaker.md)
- [`gateway`](gateway.md)
