...
| **Compliance gate** | Scans every outgoing message for PCI/PII/secrets via the [shared_core](shared_core.md) compliance engine and hardblock engine before any byte reaches a provider. |
...
#### 2. Budget Gate
- Calls `check_budget(user_id)` from the [shared_core](shared_core.md) budget store.
...
|---|---|---|
| **LLM Proxy** (`LLM_PROXY_URL`) | Streams Claude/OpenAI/Gemini tool-calling responses | [llm_proxy](llm_proxy.md) |
| **Local LLM Endpoint** (`LOCAL_LLM_BASE_URL`) | In-house GPU models (Kimi, Qwen, GLM, Ollama) | [local_llm_gateway](local_llm_gateway.md) |
| **Privacy Service** | PCI/PII detection via `compliance_engine.validate_input()` | [privacy_service](privacy_service.md) |
| **Postgres** | `model_usages` audit table + `user_usage_totals` budget aggregates | [shared_core](shared_core.md) |
| **Translation Service** | Multilingual translate-in/out | [translation_service](translation_service.md) |
...
- [shared_core](shared_core.md) — Core infrastructure: logger, model registry, compliance engine, hardblock engine, budget store, model router, translation wrapper.
- [llm_proxy](llm_proxy.md) — Upstream LLM proxy serving Claude/OpenAI/Gemini tool-stream endpoints.
- [local_llm_gateway](local_llm_gateway.md) — In-house GPU model gateway and catalog.
- [privacy_service](privacy_service.md) — PCI/PII detection service used by the compliance engine.
- [translation_service](translation_service.md) — Multilingual translation service for translate-in/out.
- [gateway](gateway.md) — Main gateway that registers this router under `/ainxt/v1/api`.
- [budget_router](budget_router.md) — Budget management API (shares `budget_store`).
- [coach_router](coach_router.md) — AiNxt Coach dashboard (consumes coach events emitted here).
