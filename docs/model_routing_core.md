...
|------------|--------|---------|
| `core/model_registry.py` | [core_infrastructure](core_infrastructure.md) | Approved model IDs, display labels, feature flags, blocked models |
| `core/circuit_breaker.py` | [core_infrastructure](core_infrastructure.md) | Per-provider circuit breakers |
| `core/logger.py` | [core_infrastructure](core_infrastructure.md) | Structured logging with request ID context |
| `core/proxy_tool_use.py` | [core_infrastructure](core_infrastructure.md) | LLM proxy headers |
| `models/classifier.py` | (sibling) | Complexity classification with regex + LLM fallback |
| `core/config.py` | [core_infrastructure](core_infrastructure.md) | Redis cache config (`RDB_CACHE`) |
| `core/kv` | [kv_store](kv_store.md) | Redis client for intent cache |
| `core/prompts.py` | [core_infrastructure](core_infrastructure.md) | `INTENT_CLASSIFIER_PROMPT` template |
