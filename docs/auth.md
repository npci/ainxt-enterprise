...
| -------- | ----------- |
| [`App.jsx`](ai_ui_frontend_app_core.md) | Wraps the application with `AuthProvider`; uses `session` to choose between login and main UI; calls `setSession(null)` on logout. |
...
| [`Chat.jsx`](chat.md), [`KbChat.jsx`](kb_chat.md) | May read `session` to scope requests or display user-specific data. |
...
| Module | Relationship |
| ------ | ------------ |
| [`auth_router`](shared_api_routers.md#auth_router) | Provides login, register, refresh, logout, SSO, and session management endpoints. |
| [`shared_core_authentication`](shared_core.md#authentication) | Contains RBAC, LDAP, and SSO logic used by the backend auth layer. |
...
3. **No Default Value:** `createContext(null)` means components calling `useAuth()` outside `AuthProvider` will receive `null`. In practice, the entire app is wrapped by `AuthProvider` in [`App.jsx`](ai_ui_frontend_app_core.md).
...
- [`ai_ui_frontend_app_core.md`](ai_ui_frontend_app_core.md) — Application root that wraps `AuthProvider` and handles logout.
...
- [`shared_api_routers.md#auth_router`](shared_api_routers.md#auth_router) — Backend authentication endpoints.
- [`shared_core.md#authentication`](shared_core.md#authentication) — Backend RBAC, LDAP, and SSO implementation.
