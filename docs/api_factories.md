...
- **`_find_existing_matches(kind, query, user_id)`** — Uses the `template_candidate_cache` (5-minute TTL) and `semantic_match_existing` from [core_factory_utils](core_factory_utils.md) to find matching templates. Only curated templates are recommended — never the user's own saved items.
...
- **Keyword fallback**: When `CapabilityAudit` returns no resolved tools/skills, falls back to `keyword_match_tools`/`keyword_match_skills` from [core_factory_utils](core_factory_utils.md).
...
**Key behaviors:**
- Uses `SkillIntentParser`, `SkillClarificationEngine`, `SkillBlueprintGenerator`, `SkillPlanCardGenerator` from [skill_factory_pipeline](skill_factory_pipeline.md).
...
Validates the generated SKILL.md against skill-creator spec rules via `_validate_skill_md`.
...
- **Budget enforcement**: Agent Runner endpoints enforce per-user budget via `check_budget_allowed` (see [core_governance](core_governance.md)). Local models are exempt.
...
| Module | Relationship |
|--------|-------------|
| [agent_factory_pipeline](agent_factory_pipeline.md) | Provides `IntentParser`, `ClarificationEngine`, `AgentBlueprintGenerator`, `AgentPlanCardGenerator`, `ToolSkillMatcher`, `CapabilityAudit`, `AgentAssembler`, `AgentRegistry`, `AgentRunner`, `MonitoringLogger`, session management, and LLM utilities. |
| [skill_factory_pipeline](skill_factory_pipeline.md) | Provides `SkillIntentParser`, `SkillClarificationEngine`, `SkillBlueprintGenerator`, `SkillPlanCardGenerator`, `SkillContentGenerator`, `SkillBundleDecider`, `SkillAssembler`, session management, and skill validation/lint utilities. |
| [workflow_factory_pipeline](workflow_factory_pipeline.md) | Provides `WorkflowClarificationEngine`, `WorkflowPlanCardGenerator`, `WorkflowBlueprintGenerator`, `inject_skills_into_nodes`, `inject_tools_into_nodes`, and session management. |
| [core_workflow_repo](core_workflow_repo.md) | Persistence layer for agents, skills, tools, workflows, and factory sessions. Used by all confirm endpoints and the Agent Runner for agent loading. |
| [core_factory_utils](core_factory_utils.md) | Provides `semantic_match_existing`, `keyword_match_tools`, `keyword_match_skills`, `extract_json_block`, and `call_factory_llm_with_finish_reason`. |
| [core_governance](core_governance.md) | Provides `check_budget_allowed`, `audit_event`, `_is_local_model`, and `increment_budget_usage`. Used by Agent Runner for budget preflight and cost tracking. |
| [core_governance_client](core_governance_client.md) | Provides `submit_skill_async` for governance submission of AI-created skills. |
...
| [core_config](core_config.md) | Provides `factory_model()` for default LLM model resolution. |
| [app_models](app_models.md) | Provides `AuthenticatedUser` model used across all endpoints. |
...
| [swarm](swarm.md) | The `SwarmRuntime` and `SpawnSwarmTool` are used by `AgentRunner` for adaptive sub-agent delegation when `use_subagents` is enabled. |
