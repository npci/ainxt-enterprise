...
| **Chain resolution** | Maps `loop.action.target_id` to a concrete [`ChainDefinition`](app_models.md) by loading the referenced workflow graph. |
...
| **Event streaming** | Runs [`LoopRunner`](loop_runner.md) asynchronously and yields SSE events until completion, cancellation, or client disconnect. |
...
1. Loads the [`LoopRecord`](loop_models.md) from [`loops_repo`](loop_runner.md). Returns `404` if missing, `409` if disabled.
2. Validates `user_input` is non-empty (`422` otherwise).
3. Calls `_resolve_loop_chain` to build a [`ChainDefinition`](app_models.md).
4. Optionally resolves a [`Goal`](loop_models.md) from `goal_id` (`404` if missing).
5. Builds an execution context with [`build_execution_context`](api_deps.md).
6. Instantiates [`LoopRunner`](loop_runner.md) and streams its events through an async generator.
...
Private helper that converts a `LoopRecord` into a [`ChainDefinition`](app_models.md) that [`LoopRunner`](loop_runner.md) can execute.
...
- The target workflow is loaded lazily from [`workflow_repo`](core_workflow_repo.md) to keep the router's import graph light.
...
- `edges`: list of [`ChainEdge`](app_models.md) instances built from `graphData.edges`.
...
| `app.loop.repo` | [`loop_runner.md`](loop_runner.md) | Persistence layer for [`LoopRecord`](loop_models.md) and [`Goal`](loop_models.md). |
| `app.loop.models` | [`loop_models.md`](loop_models.md) | Domain models: `LoopRecord`, `Goal`, plus `ActionSpec`, `VerifySpec`, etc. |
| `app.loop.runner` | [`loop_runner.md`](loop_runner.md) | The engine that actually executes the loop and produces the event stream. |
| `app.engine.interface` | [`engine_native_engine.md`](engine_native_engine.md) | `ChainDefinition` and `ChainEdge` data structures shared with the execution engine. |
| `app.core.workflow_repo` | [`core_workflow_repo.md`](core_workflow_repo.md) | Lazy-loaded workflow retrieval by `target_id`. |
| `app.models` | [`app_models.md`](app_models.md) | `AuthenticatedUser` and shared request/response models. |
...
While those modules target interactive or one-shot execution, `api_loops` is the entry point for **autonomous iterative execution**: a Loop can repeatedly invoke a workflow, evaluate proof/goal conditions via [`LoopRunner`](loop_runner.md), and stream progress back to the caller.
...
- [`loop_models.md`](loop_models.md) — Loop and Goal domain models.
- [`loop_runner.md`](loop_runner.md) — `LoopRunner`, proof evaluation, verifier agents, and budget metering.
- [`engine_loop_evaluator.md`](engine_loop_evaluator.md) — `LoopController` and `LLMEvaluator` used inside the runner.
- [`core_workflow_repo.md`](core_workflow_repo.md) — Workflow storage and retrieval.
- [`api_deps.md`](api_deps.md) — Authentication and execution-context helpers.
- [`app_models.md`](app_models.md) — Shared request/response models.
- [`engine_native_engine.md`](engine_native_engine.md) — Native workflow execution engine that ultimately runs the resolved chain.
