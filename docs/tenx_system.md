...
- **API route handlers** — those live in `routers/tenx_router.py` (see [shared_api_routers.md](shared_api_routers.md)).
- **Background worker orchestration** — evaluation jobs are executed by `workers/tenx_eval_worker.py` and `workers/tenx_worker_main.py` (see [workers.md](workers.md)).
...
- **Repository indexing** — indexing jobs are enqueued through `core/job_queue.py` and executed by `workers/index_worker.py` (see [shared_core.md](shared_core.md) and [workers.md](workers.md)).
...
- [shared_api_routers.md](shared_api_routers.md)
