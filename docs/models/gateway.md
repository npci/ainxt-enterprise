...
|---|---|---|
| **Health & Monitoring** | `/health`, `/metrics`, `/health/circuit-breakers`, startup/shutdown, adaptive LLM semaphore, platform kill-switch. | [health_and_monitoring](../infrastructure/health_and_monitoring.md) |
...
| **OpenAI-Compatible Endpoints** | `/v1/chat/completions`, `/v1/responses`, `/v1/models` for IDE/SDK clients. | [openai_compatible_endpoints](../reference/openai_compatible_endpoints.md) |
| **Voice & TTS** | Text-to-speech and speech-to-text endpoints, routing to LLM proxy or local Whisper. | [voice_and_tts](../reference/voice_and_tts.md) |
...
| **Audit & Tracing** | Per-request traces, telemetry spans, governance/SDLC audit log, client activity. | [audit_and_tracing](../reference/audit_and_tracing.md) |
| **File & Asset Serving** | Main SPA, ABStudio SPA, generated file downloads with TTL and path-traversal protection. | [file_and_asset_serving](../reference/file_and_asset_serving.md) |
...
- [shared_core](../reference/shared_core.md) — compliance, model routing, agent framework, memory, telemetry.
- [abstudio_backend](../ui/abstudio_backend.md) — ABStudio API routes, native engine, workflow repo, agent factory.
...
- [shared_api_routers](../api/shared_api_routers.md) — additional REST routers mounted by the gateway.