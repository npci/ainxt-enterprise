...
4. **Observability & Cost Tracking**
   ...
   - Emits `[CACHE EFFECTIVENESS]` lines that compute estimated cache savings/surcharges using the shared [`core_model_registry`](../core_model_registry.md) cost table.
...
5. **Safety & Resiliency**
   ...
   - Refuses blocked models via [`core_model_registry`](../core_model_registry.md) `BLOCKED_MODELS`.
   - Wraps non-streaming calls with [`core_circuit_breaker`](../core_circuit_breaker.md) and [`core_retry`](../core_retry.md).
...
- Binds the upstream `request_id` / `chat_id` using [`core_logger`](../core_logger.md).
...
See [`core_model_registry`](../core_model_registry.md).
...
- **Does NOT manage conversation persistence.** Chat history, thread state, and checkpointing are handled by [`abstudio_backend`](../ui/abstudio_backend.md) and [`shared_core_memory_system`](../shared_core_memory_system.md).
...
| `CLAUDE_PRIMARY_MODEL` | Default model sourced from [`core_model_registry`](../core_model_registry.md). |
...
- [`core_model_registry`](../core_model_registry.md) — Model IDs, blocked models, and pricing.
