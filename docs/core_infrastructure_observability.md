...
flowchart TB
    ...
    subgraph "External Systems"
        ...
        REDIS[("Redis / RustyCluster<br/>DB=1 trace store")]
...
- Backend (Redis or RustyCluster) is selected via `REDIS_CLIENT_CONFIG_DB1` environment variable, abstracted through the `core.kv.get_kv()` interface (see [`core_infrastructure_resilience_storage`](core_infrastructure_resilience_storage.md)).
...
| `core/kv` | [`core_infrastructure_resilience_storage`](core_infrastructure_resilience_storage.md) | `get_kv()` abstraction over Redis / RustyCluster for trace store. |
| `db/database.py` | [`database`](database.md) | `SessionLocal` for DB seeding. |
| `db/models.py` | [`database`](database.md) | `ModelUsage`, `SDLCRun` ORM models for counter seeding. |
...
| `REDIS_CLIENT_CONFIG_DB1` | - | Selects Redis vs. RustyCluster backend for DB=1. |
...
The `gateway.py` module (see [`health_and_monitoring`](health_and_monitoring.md)) is the primary consumer:
...
Background workers (see [`worker_orchestration`](worker_orchestration.md)) use:
...
The Cowork desktop app and CLI runtime (see [`cowork_desktop`](cowork_desktop.md)) use `core/otel.py`:
...
The agent framework (see [`agent_system`](agent_system.md)) uses operational metric helpers:
