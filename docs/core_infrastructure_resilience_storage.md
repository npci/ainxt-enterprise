...
**degrade gracefully** when dependencies (MinIO, Redis/RustyCluster) are
...
graph TB
    ...
    subgraph "External Backends"
        ...
        Redis[("Redis / RustyCluster<br/>DB=5 (queue), DB=7 (rate-limit)")]
    end
...
1. **Shared state across workers** — Circuit breaker state, semaphore slots, and
   rate-limit counters all live in the KV layer (Redis or RustyCluster), not in
...
`EVALSHA` with automatic `EVAL` fallback — see [kv_store](kv_store.md) for the
...
| `core.kv.get_kv()` | `CircuitBreaker`, `DistributedSemaphore` | Backend-agnostic KV access (Redis or RustyCluster) | [kv_store](kv_store.md) |
| `core.kv.KVError` | `CircuitBreaker` | Detect KV unavailability | [kv_store](kv_store.md) |
| `core.kv.KVClient.register_script()` | `DistributedSemaphore` | Atomic Lua script execution | [kv_store](kv_store.md) |
...
> The backend for each DB is independently configurable as Redis or
> RustyCluster via `REDIS_CLIENT_CONFIG_DB{n}`. See
> [core_infrastructure_config_logging](core_infrastructure_config_logging.md)
> and [kv_store](kv_store.md) for the full DB allocation table and backend
> selection logic.
...
| KV layer (Redis / RustyCluster abstraction) | [kv_store](kv_store.md) |
