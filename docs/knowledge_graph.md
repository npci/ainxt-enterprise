...
- **[config](config.md)** — Provides `authFetch` and `API_BASE` used by the frontend to call backend endpoints with correlation IDs and credential cookies.
...
- **[document_knowledge_workers](document_knowledge_workers.md)** (within `workers`) — Contains the `knowledge_graph_worker.py` that performs async graph construction, plus `kb_entity_worker.py` and `index_worker.py` that feed data into the graph.
...
- **[model_routing](model_routing.md)** (within `shared_core`) — The `model_router` used by the worker for LLM-based entity extraction and domain clustering.
- **[authentication](authentication.md)** (within `shared_core`) — RBAC enforcement via `get_current_user` dependency and ACL filters on all graph queries.
