...
1. **Spawns RQ worker subprocesses** — each child process connects to the queue backend (Redis), binds to one or more named priority queues, and processes jobs until signalled to stop.
...
The module is intentionally environment-aware: it loads `.env`, decrypts CKMS-protected secrets *before* any application code imports, and selects the queue backend (KV backend) dynamically via the KV factory — never hard-coding a connection.
...
graph TB
    ...
    RQ --> QBACKEND[("Queue Backend<br/>Redis")]
...
| Decision | Rationale |
|---|---|
| **`multiprocessing.Process` with `spawn`** | Each worker child constructs its own queue connection after the fork/spawn boundary. Connections are *not* inherited — this avoids the macOS `objc_initializeAfterForkError`. |
...
| **KV factory for worker construction** | `core.kv.queue.get_worker()` resolves the worker from `REDIS_CLIENT_CONFIG_DB5` rather than constructing an `rq.Worker` directly, so the queue backend stays a configuration concern. |
...
| `REDIS_CLIENT_CONFIG_DB5` | `REDIS` | Selects the queue backend for logical DB 5 |
...
| Module | Relationship |
|---|---|
| [kafka_event_consumer](../reference/kafka_event_consumer.md) | Launched as subprocess by `_run_kafka_consumer()`; consumes Kafka event topics for SDLC, embeddings, budget, audit, threads, agents, chat, metrics |
...
| [sdlc_pipeline_workers](sdlc_pipeline_workers.md) | Contains `sdlc_worker.py` whose `expire_stale_hitl_runs` is called by the cron scheduler every 15 minutes |
...
| [broadcast_coach_workers](broadcast_coach_workers.md) | Contains `coach_consumer.py` (launched as subprocess) and `broadcast_worker.py` |
| [kv_store](../storage/kv_store.md) | Provides `core/kv/queue.py` — the worker/connection factory that abstracts KV backend |
| [core_infrastructure](../infrastructure/core_infrastructure.md) | Provides `core/config.py`, `core/logger.py`, `core/ckms/`, `core/job_queue.py` — foundational imports |
| [gateway](../models/gateway.md) | The API gateway that enqueues jobs consumed by workers spawned here; also prepends `ABStudio/backend` to `sys.path` so worker imports resolve |
