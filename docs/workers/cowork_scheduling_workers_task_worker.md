...
| [Model Routing](../models/model_routing.md) → ModelRouter | `generate(model_hint="complex")` | Re-ask envelope repair |
| [Connector Infrastructure](../connectors/shared_integrations_connector_infrastructure.md) → MCP Bridge | `call_tool` | Send email via the same pipeline as interactive CoWork |
| [Connector Infrastructure](../connectors/shared_integrations_connector_infrastructure.md) → ConnectorEngine | `_check_user_permission` | Platform-wide permission check |
| [Connector Infrastructure](../connectors/shared_integrations_connector_infrastructure.md) → ConnectorRegistry | `get_user_status` | Resolve user's M365 email address |
| [Store Layer](../storage/store_layer.md) → Inbox Store | `publish_inbox_item` | Make headless run results visible to the user |
| [Store Layer](../storage/store_layer.md) → Skill Loop Store | `record_run_signature` | Self-improving skill loop capture |
| [Core Infrastructure](../infrastructure/core_infrastructure.md) → Logger | `logger` | Structured logging |
| [Core Infrastructure](../infrastructure/core_infrastructure.md) → Config | `ENABLE_SKILL_LOOP`, `SKILL_LOOP_SOURCES` | Feature flags for skill loop |
| [Database](../storage/database.md) | `SessionLocal` | PostgreSQL session factory |
