...
2. **Claim pass** — Select up to `_BATCH` active rows where `next_run <= now_utc()`, ordered by `next_run`. For each row:
   ...
   - Enqueue the job on `connector_queue` via `core/job_queue.md`'s `enqueue_job()`.
...
- **No auto-execution of writes**: The scheduler only enqueues. Outbound sends and connector writes are performed by the task worker through `connectors/mcp_bridge.md`, which applies the same compliance hard-block and recipient validation as interactive Cowork.
...
- `core/job_queue.md` — RQ enqueue abstraction (`enqueue_job`, `Q_CONNECTOR`).
- [`core/config.md`](../core/core_config.md) — Database DSN and feature flags.
- `connectors/mcp_bridge.md` — Connector tool execution pipeline used by the task worker.
- `agents/orchestrator.md` — Office-mode agent run invoked by the task worker.
- `routers/cowork_tasks_router.md` — REST API for creating, listing, updating, and manually running scheduled tasks.
- `ai_ui_frontend_cowork_scheduler.md` — Frontend UI for scheduled Cowork tasks.
- `workers/start_workers.md` — Worker orchestrator that can start the scheduler thread.
