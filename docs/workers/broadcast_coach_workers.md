...
- **Indexing Pipeline**: `workers/index_worker.py` calls `extract_file_edges_treesitter()` when `SDLC_GRAPH_EDGE_MODE=treesitter` to improve code-graph accuracy over the legacy regex extractor. The resulting relations are stored in the same `code_graph` JSONB shape, so downstream consumers (e.g., [shared_core models/graph_resolver](../reference/shared_core.md)) require no changes.
- **Shared Services**: The broadcast worker depends on [shared_core database](../reference/shared_core.md) models (`EmailBroadcast`, `EmailBroadcastRecipient`, `EmailBroadcastAttachment`, `EmailBroadcastAuditLog`) and the SMTP service. The coach consumer depends on [shared_core config/kv](../reference/shared_core.md) and the coach ingestor service.
