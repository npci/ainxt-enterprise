...
Each worker process calls `_worker_process()` which creates an RQ Worker via the KV factory. The backend is selected by `REDIS_CLIENT_CONFIG_DB5` environment variable. Workers run with `with_scheduler=True` for RQ's built-in job scheduling.
...
- **[chat_agent_execution_workers_sandbox](../chat_agent_execution_workers_sandbox.md)**: Code execution sandbox (`run_code_job`) — isolated Python/Docker execution.
- **[chat_agent_execution_workers_security](../chat_agent_execution_workers_security.md)**: Security scanning (`run_secure_code_gate`, `run_security_scan_job`).
- **[chat_agent_execution_workers_skill_loop](../chat_agent_execution_workers_skill_loop.md)**: Self-improving skill detection (`detect_and_propose`, `enqueue_detect`).
