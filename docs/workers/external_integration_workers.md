...
- **[shared_core](../reference/shared_core.md)** — `core.config`, `core.logger`, `core.job_queue`, `core.kv`, `models.model_router`, `agents.compliance_engine`, `core.graph_audit`, `core.platform_credentials`, `core.build_manifest_resolver`.
- **[shared_integrations](../reference/shared_integrations.md)** — `connectors` adapters and `tools.gitlab_tools` for GitLab API calls; `integrations.graph_app_client` and `services.meeting_transcript` for meeting automation.
- **[auth](../reference/shared_core.md#authentication)** — `auth/ldap_handler.py` for AD sync.
- **[db](../reference/shared_core.md#database)** — `db.database` for Postgres/pgvector sessions and SQLAlchemy engines.
