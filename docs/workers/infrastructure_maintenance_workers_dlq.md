...
Because the function is registered as an RQ job, its return value is stored by the queue backend and can be retrieved via job-status APIs.
...
flowchart LR
    ...
    subgraph QueueBackend["Queue Backend (Redis)"]
        Q1[Default Queue]
        Q2[DLQ]
    end
...
- [core_infrastructure](../infrastructure/core_infrastructure.md) — logging, configuration, and shared utilities.
- [worker_orchestration](worker_orchestration.md) — how worker processes are started and managed.
- [jobs_router](../api/jobs_router.md) — admin/job inspection HTTP endpoints.
...
> **Note:** The docstring in `record_dlq_job` references `GET /jobs/failed`. The exact admin endpoint may vary by deployment; use the queue-filtered list endpoint or the job-status endpoint described in [jobs_router](../api/jobs_router.md).
...
- [core_infrastructure](../infrastructure/core_infrastructure.md) — shared logging, configuration, and queue abstractions.
- [jobs_router](../api/jobs_router.md) — HTTP endpoints for job inspection.
