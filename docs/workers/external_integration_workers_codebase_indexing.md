...
| **Model Router** | LLM calls for code chunk enrichment (NL descriptions) | [shared_core.md](../reference/shared_core.md) |
...
| **BuildMetadataExtractor** | Post-index build tool/language detection | [shared_core.md](../reference/shared_core.md) |
| **Redis** | Distributed locks (DB=5), index status (DB=3), enrichment cache (DB=7), retrieval cache invalidation | — |
| **pgvector** | Vector storage and similarity search | [database.md](../storage/database.md) |
| **Index Router** | Submits and approves indexing requests, enqueues RQ jobs | [shared_api_routers.md](../api/shared_api_routers.md) |
| **Kafka Consumer** | Optional event-driven trigger via `_handle_embeddings` | [kafka_event_consumer.md](../reference/kafka_event_consumer.md) |

### Downstream Consumers

| Consumer | Usage | Reference |
|----------|-------|-----------|
| **hybrid_search.py** | `semantic_search` queries pgvector; `symbol_search` queries `code_symbols` for exact-match retrieval | [shared_core.md](../reference/shared_core.md) |
...
| **Knowledge Graph Router** | Queries `knowledge_graph_nodes`/`edges` for code structure exploration | [shared_api_routers.md](../api/shared_api_routers.md) |
| **Gateway codebase_search** | Serves indexed code to chat/agent workflows | [gateway.md](../models/gateway.md) |
...
| Index Router API (submit/approve/status) | [shared_api_routers.md](../api/shared_api_routers.md) |
| Embedding Service | [embedding_service.md](../knowledge/embedding_service.md) |
| Hybrid Search (semantic + symbol retrieval) | [shared_core.md](../reference/shared_core.md) |
| Database schema (pgvector, code_symbols, code_graph) | [database.md](../storage/database.md) |
| Kafka Consumer (event-driven triggers) | [kafka_event_consumer.md](../reference/kafka_event_consumer.md) |
...
| Gateway (codebase_search endpoint) | [gateway.md](../models/gateway.md) |
