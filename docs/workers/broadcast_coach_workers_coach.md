...
The consumer delegates all processing to `services.coach_ingestor.ingest()`. The pipeline is documented in detail in the [Coach System](../evaluation/coach_system.md) module; a summary is provided here for context:
...
For details on the rule predicates and evaluation logic, see [Coach System](../evaluation/coach_system.md).
...
|---|---|---|
| `core_infrastructure` | [Core Infrastructure](../infrastructure/core_infrastructure.md) | `core.config` (Kafka/Redis/Coach settings), `core.logger`, `core.kv.get_kv`, `core.ckms.load_at_boot` |
| `coach_system` | [Coach System](../evaluation/coach_system.md) | `services.coach_ingestor.ingest()`, `agents.coach_evaluator` rule engine, `agents.compliance_engine` redaction |
| `database` | [Database](../storage/database.md) | `db.models.CoachEvent`, `db.database.SessionLocal` |
| `kv_store` | [KV Store](../storage/kv_store.md) | Redis DB allocation (`RDB_QUEUE` = DB 5) and `get_kv()` abstraction |
| `ckms` | [CKMS](../reference/ckms.md) | Decrypts protected env vars at boot before config import |
...
All configuration is environment-variable driven. See [Core Infrastructure](../infrastructure/core_infrastructure.md) for the central config module.
