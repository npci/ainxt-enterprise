...
|-----------|-----------|------|
| `workflow_repo` | [core_workflow_repo](../reference/core_workflow_repo.md) | Trigger CRUD, execution persistence, workflow/agent lookups |
| `NativeEngine` | [engine_native_engine](../reference/engine_native_engine.md) | Workflow/agent execution via SSE streaming |
...
| `prompt_injection` | [core_infrastructure](../infrastructure/core_infrastructure.md) | Injection detection on trigger payloads |
| `loop.runner` | [loop_runner](../reference/loop_runner.md) | `TriageSkill` for Loop Engineering triage cycles |
| `core.config` | [core_config](../infrastructure/core_config.md) | `triage_interval_cron`, `loop_triage_enabled`, LLM endpoint helpers |
| `db_pool` | [core_db_pool](../storage/core_db_pool.md) | `SHARED_POOL` for lazy pool binding in RQ workers |
| `app.models` | [app_models](../models/app_models.md) | `TriggerCreate`, `TriggerUpdate`, `TriggerSchedule`, `TriggerOut` |
...
|----------|---------|-----------|
| `init_scheduler()` | Start APScheduler (triage only) + bootstrap triage jobs | `_lifespan` in [app_main](../reference/app_main.md) |
...
### 11.1 FastAPI Lifespan ([app_main](../reference/app_main.md))
...
### 11.4 Loop Engineering ([loop_runner](../reference/loop_runner.md))

Loop CRUD endpoints call `register_triage_job` / `deregister_triage_job` to keep APScheduler in sync with the loops table. `_fire_triage` delegates to `TriageSkill.run()` from [loop_runner](../reference/loop_runner.md).

### 11.5 Execution Engine ([engine_native_engine](../reference/engine_native_engine.md))
