...
| `core.job_queue` | [core_infrastructure](../infrastructure/core_infrastructure.md) | `release_sdlc_slot`, `refresh_sdlc_slot` for dedup/concurrency control |
| `tools.gitlab_tools` | [shared_integrations](../reference/shared_integrations.md) | `gitlab_merge_mr`, `gitlab_get_mr_diff`, `gitlab_set_mr_draft`, `gitlab_post_governance_note`, `gitlab_get_project_clone_url`, `set_token` |
...
Jobs are enqueued by the [sdlc_router](../api/sdlc_router.md) API endpoints and webhook handlers. The router validates payloads, pre-creates run records, acquires dedup slots, and enqueues the appropriate worker function via `core.job_queue.enqueue_sdlc_job` or `enqueue_hitl_resume_job`.
...
SDLC state-change events produced by `store.sdlc_store.update_run_state` are consumed by the [kafka_event_consumer](../reference/kafka_event_consumer.md) (`_handle_sdlc_events`) for audit trails and secondary writes.
...
- [core_infrastructure](../infrastructure/core_infrastructure.md) — Job queue, logging, config, model registry, platform credentials
- [shared_integrations](../reference/shared_integrations.md) — GitLab tools, Jira tools, and other connector adapters
...
- [sdlc_router](../api/sdlc_router.md) — API endpoints that enqueue these worker jobs
- [worker_orchestration](worker_orchestration.md) — RQ worker process startup and cron scheduling
- [kafka_event_consumer](../reference/kafka_event_consumer.md) — Consumes SDLC state-change events for audit
