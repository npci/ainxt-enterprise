...
`DepTable` is a leaf presentational component. It is owned by [`TriggerModal`](../ui/trigger_modal.md) and consumes shared utilities from `config` for API calls.
...
| Module | Relationship |
|--------|--------------|
| `config` | Imports `API_BASE` and `apiFetch` for backend calls. |
| [`TriggerModal`](../ui/trigger_modal.md) | Parent component that renders `DepTable` and owns the form state. |
...
`DepTable` is part of the multi-repo SDLC trigger experience in `ai-ui`. When a user configures an SDLC pipeline run through [`TriggerModal`](../ui/trigger_modal.md), the table:
...
The backend endpoint consumed by this component belongs to the SDLC subsystem (see `sdlc_router` / [`sdlc_pipeline`](../sdlc/shared_core_sdlc_pipeline.md)). The fetched dependencies are typically produced by [`agents/dep_resolver.py`](shared_core.md) or equivalent manifest parsing logic.
