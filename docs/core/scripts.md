...
- **Triggered by**: the `ai_ui_frontend` build process (typically via a `postbuild` npm script or CI step). See ai_ui_frontend.md for the broader frontend architecture.
...
- ai_ui_frontend.md — the parent frontend application that owns the build pipeline and service worker.
