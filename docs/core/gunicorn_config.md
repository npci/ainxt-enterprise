...
- **core.logger** — Provides `_configure_logging_once()` and `SizeAndTimeRotatingFileHandler`. The `post_fork` hook depends on this module to rebuild per-worker log handlers correctly.
...
- core.logger — Logging infrastructure re-initialized by `post_fork`.
