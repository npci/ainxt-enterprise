...
> middleware_budget for the sibling middlewares.
...
|---|---|---|
| **RateLimitMiddleware** | middleware_rate_limit | While it primarily keys on user_id / IP, the client source is available in `request.state` for anomaly-detection heuristics. |
| **BudgetMiddleware** | middleware_budget | Enforces token-spend limits on LLM-generating endpoints; client source helps distinguish IDE vs. platform traffic for cost attribution. |
...
| **Key consumers** | middleware_rate_limit, middleware_budget, [gateway](gateway.md), [core_infrastructure](core_infrastructure.md) (logger + telemetry). |
