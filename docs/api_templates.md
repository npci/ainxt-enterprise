...
- **`api_templates` router** defines four public endpoints and relies on `require_access` from [`api_deps`](api_deps.md) to obtain an [`AuthenticatedUser`](app_models.md).
...
- [`app.models.AuthenticatedUser`](app_models.md) — the authenticated user model.
- [`app.core.workflow_repo`](core_workflow_repo.md) — all template/workflow persistence operations.
...
| [`core_workflow_repo`](core_workflow_repo.md) | Implements `get_all_templates`, `get_template`, `use_template`, `reseed_templates`, plus the seed-management helpers used by the admin router. |
| [`app_models`](app_models.md) | Defines `AuthenticatedUser` with fields used for visibility (`role`, `department`). |
...
- [`app_core`](app_core.md) — `App.jsx` handles `handleOpenTemplate`, `handlePreviewTemplate`, and `handleEditFromTemplatePreview`, wiring template selection into the editor shell.
