...
| `core.job_queue.enqueue_job` | [core_infrastructure](core_infrastructure.md) | Enqueues scheduled workflow and trigger fire jobs. |
| `core.kv.get_kv` | [core_infrastructure](core_infrastructure.md) | Redis/RustyCluster access for throttling, run records, and chunk quality. |
| `core.storage.storage` | [core_infrastructure](core_infrastructure.md) | Deletes uploaded attachment blobs. |
| `core.ckms.load_at_boot` | [ckms](shared_core.md) | Decrypts protected env vars before imports. |
| `db.models` / `db.database` | [database](shared_core.md) | ORM models and sessions for purge and scheduling queries. |
| `memory.postgres_memory.PostgresMemory` | [memory_system](shared_core.md) | Stores and maintains memory entries and user preferences. |
| `services.feedback_processor.FeedbackProcessor` | [services](shared_core.md) | Extracts preferences and computes chunk quality penalties. |
| `app.core.workflow_repo` / `app.services.trigger_scheduler` | [abstudio_backend](abstudio_backend.md) | AB Studio trigger persistence and execution. |
