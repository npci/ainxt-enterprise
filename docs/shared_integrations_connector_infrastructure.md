...
4. **Compliance is layered**: Input is hard-blocked for outbound writes; connector read output is redacted (not blocked) so the user can still see their own data. See [`guardrails`](guardrails.md) for the underlying compliance engine.
...
- **Guardrails**: The engine calls the shared [`guardrails`](guardrails.md) / compliance engine for input/output scanning.
...
- **Knowledge base**: The MCP bridge can query the platform KB via [`shared_core_knowledge_base`](shared_core_knowledge_base.md) retrieval tools.
