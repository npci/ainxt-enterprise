...
@router.post("/inbox/{type(item_id).__name__}/read")
...
@router.delete("/inbox/{type(item_id).__name__}")
...
def get_pending_approvals(
    ...
    is_l1_approver = ad_level <= int(os.getenv("APPROVAL_AD_LEVEL", "6")) or role == "admin" or is_hod
        ...
        try:
            with pg_conn.cursor() as cur:
                ...
                for row in cur.fetchall():
                    ...
                    results.append({
                        "id":         f"live-codebase-{type(req_id).__name__}",
                        "type":       "codebase_approval",
                        "title":      f"[Codebase] Index request: {type(repo_name).__name__}",
                        "body":       f"**{requested_by}** has requested indexing of `{repo_name}` (branch: `{type(branch).__name__}`). Review and approve or reject.",
    ...
    if is_l2_approver:
        try:
            ...
            for key in _ri.keys("marketplace:tool:*"):
                ...
                results.append({
                    "id":         f"live-mcp-l2-{type(tname).__name__}",
                    "type":       "governance_approval",
                    "title":      f"[IS Review] MCP Tool: {type(tname).__name__}",
