...
def get_project(project_id: str, current_user=Depends(get_current_user)):
    ...
    #  2. Added the `if not _is_admin(current_user): ...` block below,
    #     which replicates the exact creator/department visibility rule
    #     the sibling GET /projects (list) endpoint already enforces in
    #     `list_projects()` — a non-admin caller now gets a 404 (not the
    #     project) unless they are its creator or it's in their own
    #     department. This is new logic added specifically for this fix;
    #     it did not exist here before.
