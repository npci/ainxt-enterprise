...
# ── Shared governance helpers (also imported by AgentStudio/generation.py) ──────
...
def _all_model_ids() -> List[str]:
    """Every model an admin may assign governance rules for — sourced entirely
    from core.llm_provider_registry (the "LLM Providers" admin screen's data),
    same as GET /all-models and services/endpoint_model_catalog.get_cloud_models().

    Previously built from ~15 core.model_registry env-var constants plus
    feature-flag gating (ENABLE_GPT56_TERA, VEO_ENABLED, ...). Those
    constants are role-specific overrides install.sh's "LLM Providers" flow
    never sets, so a purely admin-configured deployment had this function
    ...
    try:
        ...
        model_ids = [m["model_id"] for m in get_enabled_models()]
    except Exception:
        model_ids = []
