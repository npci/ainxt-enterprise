...
#   - base_cost_usd: the platform base allocation for EVERYONE, always —
#     default $50, configurable via the BUDGET_BASE_COST_USD env var
#     (store.budget_store.BASE_COST_USD). It is never raised by any action,
#     and the monthly reset returns it to this base value.
#   - extra_cost_usd: the POOLED extra budget, fed by approved HOD
#     budget-increase requests (My Budget → Request Increase). This portion
#     does NOT survive the monthly reset — it is fully drained back to zero.
#   - Admins and HODs have no direct "allocate/edit" action for arbitrary
#     users (POST/DELETE /budget/users are removed) — the ONLY way a user's
#     budget changes is the HOD approval flow below.
...
from services import org_budget_governor as _org_governor
...
def _audit_admin_action(current_user: dict, target_user_id: str, action: str) -> None:
    """Flat-mode (HOD_APPROVAL_ENABLED=false) counterpart of _audit_hod_action.

    Emits the same structured, logger-only audit line for admin-performed
    budget actions that, in HOD mode, would have gone through an HOD (e.g.
    the admin self-increase path in hod_self_increase() below). Deliberately
    logger.info-only, matching the existing "no DB write" pattern.
    """
    if not is_admin(current_user):
        return
    try:
        logger.info(
            "admin_action actor_email=%s target_user_id=%s action=%s",
            mask_email((current_user.get("email") or "").lower()),
            target_user_id,
            action,
        )
...
_MAX_REQUEST_EXTRA_USD:  float = float(os.getenv("BUDGET_MAX_INCREASE_PER_REQUEST_USD", "200.0"))
                                               # Max extra a user (or HOD/admin self-increase) can
                                               # request in one request. Configurable so deployments
                                               # can raise/lower the ceiling without a code change.
...
# Platform base allocation — every user's base_cost_usd, default $50,
# configurable via the BUDGET_BASE_COST_USD env var. Imported here (rather
# than re-declared) so this module and store.budget_store never drift.
...
_ROSTER_DEFAULT_BASE_COST_USD = BASE_COST_USD
_ROSTER_ROLE_LIMITS = {
...
def _invalidate_roster_cache() -> None:
    ...
    Called after mutating actions (budget increases, etc.) so the UI never
    shows post-write staleness for more than one refresh.
...
def get_user_budget(
    ...
    max_cost  = budget.get("max_cost_usd_total", _ROSTER_DEFAULT_BASE_COST_USD)
...
def get_user_usage(
    ...
    max_cost    = (budget or {}).get("max_cost_usd_total", _ROSTER_DEFAULT_BASE_COST_USD)
    remaining   = round(max(0.0, max_cost - usage_total["cost_usd_spent"]), 6)
    return {
        "user_id":      user_id,
        "budget":       budget,
...
def get_my_budget(
    ...
    # the platform base value (default $50, configurable via
    # BUDGET_BASE_COST_USD) — no band/role differentiation for the cost
    # dimension. Token/request totals still vary by role since those aren't
    # part of the base/extra cost tracking.
    if budget is None:
        try:
            from db.database import SessionLocal
            from db.models import User as _User
        ...
        # This branch only runs when get_budget() returned None, i.e. there
        # is no row at all — so the explicit extra=0.0 is a fresh seed, not
        # an overwrite of an existing balance.
    ...
    # Delegatees/other-admins the requester's budget request will be routed
    # to — shown alongside the HOD in the "Request Increase" modal so the
    # requester knows who else may approve/reject their request. Uses the
    # SAME resolve_budget_approvers_for_request() the actual submission
    # endpoint calls, so this preview never shows a set of approvers
    # different from who will really be routed.
    delegatee_emails_out: list = []
    try:
        from store.budget_store import resolve_budget_approvers_for_request
        if my_email:
            delegatee_emails_out = resolve_budget_approvers_for_request(my_email).get("delegatees") or []
    except Exception:
        pass
    ...
    return {
        ...
        # Exposed so the UI's client-side validation (max amount per
        # request) always matches the server-enforced ceiling, instead of
        # a hardcoded duplicate that would silently drift if
        # BUDGET_MAX_INCREASE_PER_REQUEST_USD is overridden.
        "max_request_extra_usd": _MAX_REQUEST_EXTRA_USD,
    }
...
def get_team_budgets(
    ...
    if rc and user_ids:
        try:
            ...
            for i, uid in enumerate(user_ids):
                ...
                if bdata:
                    _cost_total = float(bdata.get("max_cost_usd_total") or bdata.get("max_cost_usd_per_day", _ROSTER_DEFAULT_BASE_COST_USD))
        ...
        try:
            ...
            try:
                for uid in missing_budget:
                    ...
                    row = _cur.fetchone()
                    if row:
                        _cost_total = float(row[0]) if row[0] is not None else float(row[3] or _ROSTER_DEFAULT_BASE_COST_USD)
                        budget_map[uid] = {
    ...
    for entry in subtree:
        ...
        rec = {
            ...
            # For users with no seeded budget row, fall back to the platform base
            # default (BASE_COST_USD) that /budget/me and /usage auto-seed, so the
            # roster rows and the team-totals aggregate agree with each user's
            # Details drill-down.
...
def _hod_approval_email_html(requester_name: str, requester_email: str, department: str,
    ...
    HOD_APPROVAL_ENABLED=true: when `delegating_hod_email` is provided, the
    email is being sent to a delegatee (not the HOD themselves) — the copy
    names the delegating HOD explicitly and clarifies whose monthly cap the
    approval is charged against.

    HOD_APPROVAL_ENABLED=false (flat/admin-only): there is no HOD/delegation
    concept to expose to admins, so `delegating_hod_email` is ignored here —
    every admin recipient gets the identical, HOD-free copy. Callers must
    pass delegating_hod_email="" in flat mode (see call site below).
    """
    ...
    delegating_hod_email_lc = (delegating_hod_email or "").strip().lower() if HOD_APPROVAL_ENABLED else ""
...
def _apply_self_service_budget_increase(
    current_user: dict,
    caller_id: str,
    amount: float,
    justification: str,
    ...
    """Shared "apply immediately, no approval queue" budget-increase path.

    Used by both `hod_self_increase()` (HOD branch, unconditionally — HODs
    always self-apply) and `request_increase()` (admin branch, ONLY in flat
    mode — an admin's own "Request Increase" is auto-applied immediately
    instead of being fanned out to the approval queue; only non-admin
    users' requests are routed to all admins for approval).

    HOD branch: charges the HOD's own monthly allocation cap atomically via
    `reserve_and_record` (inserts a `hod_allocation_ledger` row so it shows
    in cap utilisation). Admin branch (flat mode): no cap check, no ledger
    row — just a structured audit log line via `_audit_admin_action`,
    since there's no HOD identity to charge and admin judgment is trusted.

    Raises HTTPException on failure (cap exceeded, DB error, etc.) — callers
    should let it propagate directly to the client.
    """
    is_hod_actor = is_hod(current_user)
    actor_email = (current_user.get("email") or "").strip().lower()
    if not actor_email:
        raise HTTPException(status_code=422, detail="Could not resolve your account email.")

    # HOD branch only: check remaining cap before writing anything. The admin
    # branch (flat mode) has no cap check — admin judgment only, per plan.
    if is_hod_actor:
        cap_status = _hod_governor.get_cap_status(actor_email)
    ...
    # Apply the increase directly to budget_configs and, in the HOD branch,
    # charge the HOD cap atomically via reserve_and_record (inserts a ledger
    # row with action='approve_request' so it appears in HOD cap utilisation).
    # The admin branch skips the cap charge entirely — no ledger row, no fake
    # identity, matching "user raises, admin approves" for self-increase too.
    try:
        from db.database import SessionLocal
        from sqlalchemy import text as _text
        ...
        try:
            with db.begin():
                ...
                if cfg is not None:
                    old_base  = float(cfg[0]) if cfg[0] is not None else BASE_COST_USD
                    old_extra = float(cfg[1]) if cfg[1] is not None else 0.0
                else:
                    old_base, old_extra = BASE_COST_USD, 0.0
        ...
        logger.error("_apply_self_service_budget_increase: budget_configs update failed: %s", exc)
        raise HTTPException(status_code=500, detail="Failed to apply budget increase.")

    # HOD branch: charge the HOD's own cap and write the ledger row via
    # reserve_and_record (SELECT FOR UPDATE on the cap row, inserts an
    # 'approve_request' ledger row so it shows in cap utilisation).
    # Admin branch (flat mode): no cap, no ledger row — just a structured
    # audit log line via _audit_admin_action, mirroring the HOD branch's
    # logger-only audit pattern without inventing a fake HOD identity.
    if is_hod_actor:
        try:
            import uuid as _uuid
            _hod_governor.reserve_and_record(
                hod_email=actor_email,
                ...
                justification=f"[HOD self-increase] {justification}",
            )
        except HTTPException:
            ...
            logger.error("_apply_self_service_budget_increase: reserve_and_record failed: %s", exc)
            raise HTTPException(status_code=500, detail="Budget updated but cap ledger write failed.")
    else:
        _audit_admin_action(current_user, caller_id, audit_action)

    logger.info(
        "self_service_budget_increase actor=%s admin_mode=%s amount=%.2f old_extra=%.2f new_extra=%.2f",
        actor_email, (not is_hod_actor), amount, old_extra, new_extra,
    )
    ...
    try:
        ...
        logger.warning("_apply_self_service_budget_increase: Redis sync failed (non-fatal): %s", exc)
...
def hod_self_increase(
    ...
    Constraints:
      * Caller must be an HOD (is_hod=True) — OR, when HOD_APPROVAL_ENABLED
        is False (flat/admin-only mode, the default), an admin. The admin
        branch is a separate code path: no hod_allocation_ledger row, no
        cap check, no fake HOD identity involved — the increase is applied
        directly to budget_configs and the action is recorded via
        _audit_admin_action() instead of reserve_and_record().
      * Caller may only increase their own budget (body.user_id == caller id).
      * Amount must be > 0 and ≤ $200 per request.
      * HOD branch only: amount must not exceed the HOD's remaining monthly
        cap. The admin branch has no cap check (admin judgment only).
    """
    is_hod_actor   = is_hod(current_user)
    is_admin_actor = (not HOD_APPROVAL_ENABLED) and is_admin(current_user)
    if not (is_hod_actor or is_admin_actor):
        raise HTTPException(status_code=403, detail="Only HODs may use this endpoint.")
    ...
    justification = (sanitized.get("justification") or body.justification or "").strip()
    return _apply_self_service_budget_increase(
        current_user, caller_id, amount, justification, audit_action="self_increase",
    )
...
def request_increase(
    ...
    Flat mode (HOD_APPROVAL_ENABLED=false) admin shortcut: an admin hitting
    this endpoint never goes through the approval queue — the increase is
    applied immediately, exactly like /budget/hod/self-increase. Only
    non-admin users' requests fan out to every active admin for approval in
    flat mode. (In HOD mode, this endpoint's original HOD+delegatee fan-out
    behaviour is unchanged for everyone, including admins with no HOD role.)
    """
    is_valid, field_errors, sanitized = validate_budget_request(body)
    if not is_valid:
        error_messages = []
    ...
    # Flat mode: an admin's own "Request Increase" auto-applies immediately —
    # there is no "someone else" to approve it from an admin, only from
    # another admin, and per plan the admin gets the same immediate-apply
    # shortcut here as /budget/hod/self-increase (which they could've used
    # directly instead). Only non-admin users go through the approval queue
    # below, which fans out to every active admin.
    if (not HOD_APPROVAL_ENABLED) and is_admin(current_user):
        justification = (sanitized.get("justification") or body.justification or "").strip()
        return _apply_self_service_budget_increase(
            current_user, caller_id, float(sanitized["requested_extra_cost_usd"]),
            justification, audit_action="request_increase_self_applied",
        )

    from store.budget_store import request_budget_increase, resolve_budget_approvers_for_request

    requester = _lookup_requester(caller_id)
    approvers = resolve_budget_approvers_for_request(requester["email"])
    resolved_hod = approvers.get("hod_email")
    if not resolved_hod:
        if HOD_APPROVAL_ENABLED:
            detail = "No HOD is assigned to your account. Contact your administrator to assign an HOD."
        else:
            detail = "No admin is available to approve this request. Contact your administrator."
    ...
    # the HOD themselves" (HOD mode) or "an admin" (flat mode). In flat mode,
    # resolve_budget_approvers_for_request() fans out to EVERY active admin
    # (primary + delegatees), so this loop notifies all of them, not just one
    # — each gets a plain "approve this request" notice with no delegation
    # framing, since flat mode has no HOD hierarchy for "delegated by X" to
    # apply to. HOD mode: delegatees get a copy that explicitly names their
    # delegating HOD in both the inbox body and the email.
    notify_targets: list = [(resolved_hod, "")]
    for d in delegatee_emails:
        notify_targets.append((d, resolved_hod if HOD_APPROVAL_ENABLED else ""))
                    ...
                    try:
                        item_id = publish_inbox_item(
                            ...
                            body=(
                                ...
                                f"current budget). Current budget: ${current_total:.2f} "
...
def approve_request(
    ...
    In flat mode (HOD_APPROVAL_ENABLED=false), a pending request here only
    ever comes from a NON-admin user — an admin's own request never reaches
    the approval queue (it auto-applies immediately, see
    routers/budget_router.py::request_increase). So "hod_email"/"delegatees"
    here are just every active admin, and any of them may approve.
    ...
    # Primary-slot authorization: in HOD mode the primary approver must be the
    # actual HOD (is_hod=True). In flat mode (HOD_APPROVAL_ENABLED=False), the
    # "hod_email" slot is populated by resolve_budget_approvers_for_request()
    # with one of the active admins (the requester is never an admin here —
    # see module note above), so the primary check becomes is_admin instead.
    # Every OTHER active admin is routed as a delegatee, so any of them can
    # also act first via is_delegatee_actor.
    is_primary_actor = (
        is_hod(current_user) if HOD_APPROVAL_ENABLED else is_admin(current_user)
    ) and actor_email in routed_hods
    is_delegatee_actor = bool(actor_email) and actor_email in routed_delegatees

    if not (is_primary_actor or is_delegatee_actor):
        raise HTTPException(
            status_code=403,
    ...
    # check_hod_cap=HOD_APPROVAL_ENABLED: flat mode never charges an HOD cap
    # (there's no HOD identity to charge — the "hod_email" slot holds an admin).
    result = approve_budget_request(
        request_id,
        acting_hod_email=actor_email,
        acting_hod_name=current_user.get("name", "") or "",
        is_hod_actor=True,
        check_hod_cap=HOD_APPROVAL_ENABLED,
    )
    if not result.get("success"):
    ...
    try:
        ...
        publish_inbox_item(
            ...
            body=(
                ...
                f"budget. New total budget: ${result['new_cost_usd']:.2f}."
            ),
        )
    except Exception as e:
...
def get_band_defaults(current_user: dict = Depends(get_current_user)):
    """Return the platform's flat budget default.

    Band-level differentiation has been removed: budget_configs is no longer
    seeded with band-level template rows (user_id IS NULL) — those were dead
    data, never tied to a real user. Every user's default allocation is the
    same platform base value (BASE_COST_USD, default $50, configurable via
    BUDGET_BASE_COST_USD). This endpoint is kept for backward compatibility
    with existing callers, now simply reporting that single flat value.

    SECURITY (AppSec finding — Information Disclosure / CWE-200, CWE-306):
    this endpoint previously had no auth dependency at all. Fix: added
    `current_user: dict = Depends(get_current_user)` so FastAPI rejects
    unauthenticated requests with 401. Deliberately not admin-only
    (`require_role`) — any authenticated user may still view it, as this is
    reference data every user's budget UI needs, same as /budget/model-rates.
    """
    return {
        "band_defaults": [
            {
                "band_level":           None,
                "total_allocation_usd": BASE_COST_USD,
                "model_allowlist":      [],
            }
        ]
    }
...
def _get_band_allocation(ad_level: int) -> float:
    """Return the platform's flat base budget allocation.

    Band-level differentiation has been removed: budget_configs is no longer
    seeded with band-level template rows (user_id IS NULL), so every ad_level
    (the `ad_level` parameter is accepted for call-site compatibility but no
    longer used to differentiate) resolves to the same platform base
    allocation (BASE_COST_USD, default $50, configurable via
...
def reject_request(
    ...
    # See approve_request() above for why this branches on HOD_APPROVAL_ENABLED.
    is_primary_actor = (
        is_hod(current_user) if HOD_APPROVAL_ENABLED else is_admin(current_user)
    ) and actor_email in routed_hods
    is_delegatee_actor = bool(actor_email) and actor_email in routed_delegatees
...
@router.get("/budget/admin/org-cap")
def admin_get_org_cap(current_user: dict = Depends(get_current_user)):
    """
    Admin-only: read the org-wide monthly budget cap (flat/admin-only mode).

    Used by SDLC run budget gating and managed-endpoint cloud spend when
    HOD_APPROVAL_ENABLED is False — a single global cap replaces the
    per-HOD allocation caps, with no department/HOD identity involved.
    Reads/writes ainxt.org_wide_budget_cap directly.
    """
    if not is_admin(current_user):
        raise HTTPException(status_code=403, detail="Admin access required")
    status = _org_governor.get_org_cap_status()
    config = _org_governor.get_org_cap_config()
    return {**status.to_dict(), **config}
...
class OrgCapUpdate(BaseModel):
    ...
    def _validate_cap(cls, v: float) -> float:
        if v < 0:
            raise ValueError("monthly_cap_usd must be >= 0")
        if v > 1_000_000:
            raise ValueError("monthly_cap_usd is unreasonably large")
        return v


@router.put("/budget/admin/org-cap")
def admin_set_org_cap(
        body: OrgCapUpdate,
        request: Request,
        current_user: dict = Depends(get_current_user),
):
    """
    Admin-only: set the org-wide monthly budget cap (flat/admin-only mode).

    Surfaced in the admin UI as "Org-Wide Monthly Cap" — shown only when
    HOD_APPROVAL_ENABLED is False. Not related to department_hod_mapping.
    """
    if not is_admin(current_user):
        raise HTTPException(status_code=403, detail="Admin access required")
    ...
    actor_email = (current_user.get("email") or "").strip().lower()
    result = _org_governor.set_org_cap(body.monthly_cap_usd, updated_by=actor_email or None)
    logger.info("admin_set_org_cap actor=%s new_cap=%.2f", mask_email(actor_email), body.monthly_cap_usd)
    return result
