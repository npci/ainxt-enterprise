...
from core.config import REDIS_HOST as _REDIS_HOST, REDIS_PORT as _REDIS_PORT, RDB_WORKFLOW, RDB_REGISTRY
...
def _require_approver(current_user: dict):
    ...
    if role not in APPROVER_ROLES and ad_level > int(os.getenv("APPROVAL_AD_LEVEL", "6")):
        raise HTTPException(
            status_code=403,
            detail=(
...
def approve_entity(
    ...
    is_l1       = role in APPROVER_ROLES or ad_level <= int(os.getenv("APPROVAL_AD_LEVEL", "6"))
