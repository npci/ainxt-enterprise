...
def agent_analytics(name: str, current_user: dict = Depends(_get_current_user)):
    """
    ...
    Fix: added `current_user: dict = Depends(_get_current_user)` as a
    function parameter so FastAPI rejects unauthenticated requests with
    401. Deliberately not admin-only — any authenticated user may still
    view it, mirroring the existing Agent Analytics UI tab, which is
    available to all logged-in users. No other logic changed.
...
def platform_analytics(
    ...
    function parameter so FastAPI rejects unauthenticated requests with
    401. Deliberately not admin-only — any authenticated user may still
    view it, matching the existing Analytics UI tab. No other logic changed.
...
# Agent Studio (abstudio/) persists its own agents in a dedicated `agents`
# table (see abstudio/backend/app/core/workflow_repo.py), separate from the
...
# abstudio/backend/agent_factory/pipeline.py::_record_model_usage.
...
def agent_studio_agents(
    ...
    function parameter so FastAPI rejects unauthenticated requests with
    401. Deliberately not admin-only — any authenticated user may still
    view it, matching the Analytics UI. No other logic changed.
