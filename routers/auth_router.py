...
def register(body: RegisterRequest, request: Request, response: Response):
    ...
    from core.config import ENABLE_SELF_REGISTRATION, DEFAULT_AD_LEVEL
...
def _require_director(current_user: dict = Depends(get_current_user)):
    """Require ad_level <= 2 OR admin to manage level overrides."""
    _level = int(current_user.get("ad_level") or 6)
    if _level > 2 and current_user.get("role") != "admin":
...
def create_level_override(body: LevelOverrideCreate, current_user: dict = Depends(_require_director)):
    ...
    try:
        ...
        # Deactivate any existing active overrides for this user first
        existing = (
            db.query(UserLevelOverride)
        ...
        try:
            ...
            publish_inbox_item(
                ...
                body     = (
                    ...
                    f"by `{current_user.get('email', 'admin')}`. Expires: {expires_str}.\n\n"
            ...
            try:
                ...
                publish_inbox_item(
                    ...
                    body     = (
                        ...
                        f"Your access level is restored to **L{restored_level}**."
                    ),
                    source_id= str(override.id),
                    metadata = {"entity_type": "level_override", "entity_id": str(override.id),
...
def forgot_password(body: ForgotPasswordRequest, request: Request):
    ...
    try:
        ...
        if smtp_on:
            ...
            # SMTP not configured — print to server console log
            logger.warning(
                "\n"
                "╔══════════════════════════════════════════════════════════════╗\n"
                "║  FORGOT PASSWORD — TEMPORARY PASSWORD (SMTP not configured) ║\n"
                "║  User  : %-52s ║\n"
                "║  Temp  : %-52s ║\n"
                "║  Action: User must log in and change password via Profile.  ║\n"
                "╚══════════════════════════════════════════════════════════════╝",
                email, temp_password,
            )
