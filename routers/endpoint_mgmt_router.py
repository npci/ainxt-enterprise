...
from core.security_validation import validate_endpoint_mgmt_request, _flatten_errors
...
def create_endpoint(
    ...
    is_valid, field_errors, sanitized = validate_endpoint_mgmt_request(body)
    if not is_valid:
...
def list_hods(admin: dict = Depends(require_admin)):
    ...
    Excludes HODs whose account is deactivated (users.is_active = FALSE) —
    without this, a departed/disabled HOD stayed selectable as a budget owner
    for new endpoints purely because their row lingers in
    department_hod_mapping. A HOD with no matching users row at all (e.g. the
    account hasn't been provisioned yet) is still listed, since that's not
