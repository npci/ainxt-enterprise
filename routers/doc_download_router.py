        ...
        if status == "started" and not _R.get(f"doc:result:{job_id}"):
            ...
            if _started:
                try:
                    ...
                    # Ceiling set safely ABOVE the real worst case (sandbox 120s ×
                    # up to 5 self-heal attempts + LLM structuring/preview ≈ 12 min)
                    # so we only trip on a genuinely dead worker, never a slow build.
                    _ceiling = int(os.getenv("DOC_JOB_STALL_SECONDS", "900"))  # 15 min
                    if _age > _ceiling:
