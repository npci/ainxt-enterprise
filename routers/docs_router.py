...
async def upload_doc(
    ...
    # All uploads go through the approval queue → Inbox notification fires
    # for all recipients, EXCEPT the flat-mode (HOD_APPROVAL_ENABLED=false)
    # single-admin bootstrap: if the uploader is the only active admin in
    # the system, there's nobody else to approve it, so it auto-approves.
    # HOD_APPROVAL_ENABLED=true keeps the unconditional "no auto-approve"
    # behaviour unchanged.
    from core.config import HOD_APPROVAL_ENABLED as _HOD_APPROVAL_ENABLED
    from auth.rbac import is_sole_active_admin as _is_sole_active_admin
    _auto_approve = (not _HOD_APPROVAL_ENABLED) and _is_sole_active_admin(current_user)
...
async def list_docs(
    ...
    from auth.rbac import is_admin, can_approve_governance as _can_approve
...
async def approve_doc(
    ...
    from auth.rbac import can_approve_governance as _can_approve, is_admin as _is_admin, is_request_approver as _is_request_approver
    ...
    try:
        ...
        # Authorisation:
        #   HOD_APPROVAL_ENABLED=true  — admin, OR the uploader's own HOD /
        #     one of the HOD's nominated delegatees, OR (fallback) any
        #     ad_level<=3 senior approver. Unchanged legacy behaviour.
        #   HOD_APPROVAL_ENABLED=false — admin only. Flat-mode deployments
        #     are assumed to have exactly one admin, so the uploader's doc
        #     already auto-approved at upload time via is_sole_active_admin()
        #     unless someone else uploaded it. can_approve_governance()
        #     collapses to is_admin() in this mode, so a non-admin senior
        #     employee can no longer approve — see
...
async def reject_doc(
    ...
    try:
        ...
        # Authorisation — see approve_doc() above for the HOD_APPROVAL_ENABLED
        # branching (can_approve_governance() collapses to is_admin() in flat
        # mode, so a non-admin senior employee can no longer reject either).
        if not (_is_admin(current_user)
                or _is_request_approver(current_user, doc.uploaded_by or "")
                or _can_approve(current_user)):
