    ...
    try:
        ...
        for idx, upload in enumerate(files):
            ...
            try:
                ...
                logger.warning(f"chat_router: Postgres persist failed: {e}")
    ...
    if _image_file_id and chat_id:
        try:
            ...
            produce(TOPIC_CHAT_HISTORY, {
                ...
                # attachment_ids on the USER message row: the original uploaded
                # image's attachment_ids (forwarded from /ask via the request body)
                # so the L2-img block in gateway.py can find the uploaded image's
                # caption on follow-up turns ("explain the image I attached").
                # The generated image's ChatAttachment (_generated_attachment_id)
                # is linked on the ASSISTANT message row via a separate field so
                # both the uploaded and generated images are discoverable.
                "attachment_ids":       orig_attachment_ids if orig_attachment_ids else (
                    [_generated_attachment_id] if _generated_attachment_id else []
                ),
                # Also carry the generated image's attachment id so the consumer
                # can store it separately if needed in future.
                "generated_attachment_id": _generated_attachment_id or "",
            }, key=chat_id)
...
_VEO_MAX_DURATION = 8    # Veo API maximum (supported range: 4–8 s)
_VEO_MIN_DURATION = 4    # Veo API minimum
...
def chat_generate_video(
    ...
    from core.model_registry import (
        VEO_MODEL, VEO_ENABLED, MODEL_COST_PER_SECOND, is_veo_allowed_for_user,
    )
    ...
    original_question = (body.get("original_question") or "").strip() or prompt
    try:
        duration_secs = int(body.get("duration_secs") or 8)
    ...
    cost_usd = duration_secs * float(MODEL_COST_PER_SECOND.get(VEO_MODEL, 0.0))
    try:
    ...
    try:
        ...
        logger.error(
            ...
            f"cost_usd={cost_usd:.4f} model={VEO_MODEL} → {bill_err}"
        )
    ...
    try:
        ...
        logger.error(
            ...
            f"model={VEO_MODEL} cost_usd={cost_usd:.4f} → {_mu_err}"
        )
...
def auto_title_chat(chat_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        try:
            ...
            # the runtime talks to the Neuron gateway via /v1/chat/completions (the
