...
def _gate_cloud_request(ep: dict, decision: _ModelDecision, messages: list,
    ...
    """
    Authorise a cloud call against the funding HOD's monthly cap.

    Returns (hod_email, inflight_token). Raises:
      503 — no HOD mapped (endpoint has no budget owner) or lookup unavailable
      429 — cap exhausted
    ...
    *,
    ...
    def _do():
        try:
            ...
            # ── 2. Charge the funding HOD (cloud only) ──────────────────────
            if decision.kind == "cloud" and hod_email and cost > 0:
                try:
