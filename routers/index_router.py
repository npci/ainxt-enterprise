...
def _index_worker_liveness_warning() -> Optional[str]:
    """None if at least one worker is registered to consume index_queue,
    else a user-facing warning string.

    A repo stuck at display_status="running" is otherwise indistinguishable
    from one that is merely slow -- index_repo_job can legitimately take
    minutes on a large repo. But if zero workers are currently registered
    for index_queue (the dedicated index-worker container -- see
    docker-compose.yml -- was never started, or crashed), the job will sit
    queued/running forever with no error, since nothing is there to process
    or fail it. Surfaced here so list_repos() can flag it instead of the UI
    showing an indefinite, indistinguishable "Indexing in progress...".
    Same dead end webhooks_router.py's push-triggered auto-reindex and
    external_sync_worker.py's connector doc sync silently enqueue into.
    """
    try:
        import rq
        from core.job_queue import get_queue, Q_INDEX
        queue = get_queue(Q_INDEX)
        if queue is None:
            return None  # queue backend itself unavailable — a different, already-surfaced problem
        if not rq.Worker.all(queue=queue):
            return (
                "No index-worker is currently running to process this job — "
                "it will stay queued/running indefinitely until the index-worker "
                "service is started."
            )
    except Exception as e:
        logger.warning(f"IndexRouter: index-worker liveness check failed (non-fatal): {e}")
    return None


def _index_worker_running() -> bool:
    """Boolean form of _index_worker_liveness_warning(), for the section-wide
    `infra` banner in list_repos() (shown regardless of any repo's status,
    not just ones stuck at "running")."""
    try:
        import rq
        from core.job_queue import get_queue, Q_INDEX
        queue = get_queue(Q_INDEX)
        return queue is not None and bool(rq.Worker.all(queue=queue))
    except Exception as e:
        logger.warning(f"IndexRouter: index-worker liveness check failed (non-fatal): {e}")
        return False


def _embed_svc_reachable() -> bool:
    """True if embed-svc responds to /health. Same check core/config.py's
    startup config-check already does internally (for the console/log
    summary) -- exposed here too so the Codebase/Workspace UI can show it
    proactively, not just discover it after an indexing job fails deep in
    its embedding step."""
    ...
    try:
        import httpx
        resp = httpx.get(f"{EMBED_SVC_URL}/health", timeout=3.0)
    ...
    try:
        ...
        try:
            if recipient_emails:
                ...
                # submitter IS that admin — in the latter case the request
                # already auto-approved via is_sole_active_admin() in
                # submit_index_request(), so there is nothing left to notify.
                recipients = []
            for u in recipients:
                publish_inbox_item(
                    user_id   = str(u.id),
...
def _trigger_index_from_request(req_id: str, repo_name: str, gitlab_url: str, branch: str,
    ...
    # Resolve token FIRST — before anything is marked "running". Submitter's
    # profile first, then approver's profile. Admin credentials are never
    # used. Raises PermissionError if neither has a stored token; the caller
    # is responsible for recording that failure instead of leaving the
    # request looking like indexing started (see _mark_index_request_failed).
    token = None
    try:
    ...
    job_id = _enqueue_index(payload)
    if job_id:
        # Persisted so recover_stale_index_locks() (workers/index_worker.py)
        # can cross-check this row against RQ's own job registry instead of
        # a fixed time threshold — indexing legitimately runs from seconds
        # to 10+ hours, so "stuck" can only be told apart from "still going"
        # by asking RQ whether job_id still exists, not by elapsed time.
        _jconn = _get_pg()
        try:
            with _jconn.cursor() as cur:
                cur.execute(
                    "UPDATE index_requests SET job_id=%s WHERE id=%s::uuid",
                    (job_id, req_id),
                )
            _jconn.commit()
        except Exception as exc:
            logger.warning(f"IndexRouter: could not persist job_id for {req_id}: {exc}")
        finally:
            _jconn.close()


def _mark_index_request_failed(req_id: str, error_msg: str) -> None:
    """Record that a request never actually started indexing (e.g. no SCM
    token resolvable) so /index/repos reports 'failed' with the real reason
    instead of leaving the row at 'approved'/'running' with nothing behind it."""
    conn = _get_pg()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE index_requests SET status='failed', error_msg=%s, updated_at=NOW() WHERE id=%s::uuid",
                (error_msg[:1000], req_id)
            )
        conn.commit()
    except Exception as exc:
        logger.warning(f"IndexRouter: could not mark request {req_id} failed: {exc}")
    finally:
        conn.close()
...
def submit_index_request(body: IndexSubmitRequest, current_user=Depends(get_current_user)):
    ...
    # Fail fast if the submitter has no SCM token — there is no point creating
    # a request (pending or auto-approved) that can never actually index.
    # The approval flow still has its own submitter→approver token fallback
    # for the rare case a token is revoked between submit and approve.
    from core.platform_credentials import get_scm_token
    try:
        get_scm_token(email=current_user.get("email", ""))
    ...
    from auth.rbac import is_admin as _is_admin, is_sole_active_admin as _is_sole_active_admin
    from core.config import HOD_APPROVAL_ENABLED as _HOD_APPROVAL_ENABLED
    ...
    # Auto-approve on self-submission, never bypassed for anyone but an admin:
    #   HOD_APPROVAL_ENABLED=true  (legacy HOD-hierarchy mode, unchanged) —
    #     any admin auto-approves their own submission.
    #   HOD_APPROVAL_ENABLED=false (flat/admin-only mode, default) — an admin
    #     submitter no longer auto-approves their own request; it must be
    #     approved by a DIFFERENT admin (see approve_index_request's 4-eyes
    #     check and _notify_approvers_codebase's admin-broadcast routing
    #     below). Exception: if this admin is the ONLY active admin in the
    #     system, there is nobody else to approve it, so it auto-approves.
    if _HOD_APPROVAL_ENABLED:
        submitter_auto_approves = submitter_is_admin
    else:
        submitter_auto_approves = _is_sole_active_admin(current_user)
    initial_status = "approved" if submitter_auto_approves else "pending"
    ...
    try:
        with conn.cursor() as cur:
            ...
            """, (
                ...
                submitter,
                initial_status,
                submitter if submitter_auto_approves else None,
                now if submitter_auto_approves else None,
                now, now,
            ))
        conn.commit()
    finally:
    ...
    logger.info(f"IndexRouter: new request {req_id} for {gitlab_url} (protected={is_protected}, auto_approve={submitter_auto_approves})")

    if submitter_auto_approves:
        ...
        try:
            ...
            _mark_index_request_failed(req_id, str(exc))
            raise HTTPException(status_code=403, detail=str(exc))
        return {
            "request_id":          req_id,
            "repo_name":           repo_name,
...
def list_index_requests(
    ...
    Deduped: only the latest request per (repo_name, branch) is shown --
    NOT per repo_name alone. The same repo_name can legitimately have
    independent, concurrently-pending requests on different branches (see
    submit_index_request's own duplicate-check, which is scoped to the
    exact (repo_name, product_id, branch) combination) -- deduping by
    repo_name alone silently collapsed those into one row, hiding
    whichever branch's request lost the tie-break and risking an approver
    acting on the wrong one without realising a second, distinct request
    existed for the same repo.
    """
    ...
    try:
        with conn.cursor() as cur:
            ...
            if admin:
                if status:
                    cur.execute(
                        "SELECT DISTINCT ON (repo_name, branch) * FROM index_requests WHERE status = %s ORDER BY repo_name, branch, updated_at DESC",
                        (status,)
                    )
                else:
                    cur.execute(
                        "SELECT DISTINCT ON (repo_name, branch) * FROM index_requests WHERE status != 'done' ORDER BY repo_name, branch, updated_at DESC"
                    )
            else:
                ...
                cur.execute(f"""
                    SELECT DISTINCT ON (ir.repo_name, ir.branch) ir.*
                    ...
                    {status_clause}
                    ORDER BY ir.repo_name, ir.branch, ir.updated_at DESC
                """, params)
...
def approve_index_request(req_id: str, body: ReviewAction = ReviewAction(), current_user=Depends(get_current_user)):
    ...
    Authorised:
      HOD_APPROVAL_ENABLED=true  — admin, OR the submitter's own HOD / one of
        the HOD's nominated delegatees, OR (fallback) any senior approver
        (APPROVAL_AD_LEVEL). Unchanged legacy behaviour.
      HOD_APPROVAL_ENABLED=false — admin only. Flat-mode deployments are
        assumed to have exactly one admin, so the submitter's request
        already auto-approved at submission time via is_sole_active_admin()
        unless someone else submitted it; the senior-approver fallback
        collapses to admin-only in this mode — see
        auth/rbac.py::can_approve_governance.
    """
    from auth.rbac import is_admin as _is_admin, is_request_approver as _is_request_approver, can_approve_governance as _can_approve_gov
    ...
    try:
        ...
        if not (_is_admin(current_user)
                ...
                or _can_approve_gov(current_user)):
    ...
    delete_all_by_source("codebase_approval", req_id)
...
def reject_index_request(req_id: str, body: ReviewAction = ReviewAction(), current_user=Depends(get_current_user)):
    ...
    Authorised — see approve_index_request() above for the
    HOD_APPROVAL_ENABLED branching (can_approve_governance() collapses to
    is_admin() in flat mode, so a non-admin senior employee can no longer
    reject either).
    """
    from auth.rbac import is_admin as _is_admin, is_request_approver as _is_request_approver, can_approve_governance as _can_approve_gov
...
@router.get("/index/embed-svc-status")
def embed_svc_status(current_user=Depends(get_current_user)):
    """Standalone embed-svc reachability check, for callers that aren't
    already fetching /index/repos -- e.g. Workspace chat (ai-ui/Chat.jsx),
    which needs to know up front whether retrieval will work at all, not
    just Codebase indexing's own infra block."""
    return {"embed_svc_reachable": _embed_svc_reachable()}
...
def list_repos(current_user=Depends(get_current_user)):
    ...
    try:
        ...
        # Computed once, not per row — see _index_worker_liveness_warning's
        # docstring for why a "running" repo needs this distinguished from
        # merely-slow.
        worker_warning = _index_worker_liveness_warning()
        ...
        for repo_name, branch, req_status, requested_by, product_id, product_name, updated_at, error_msg in request_rows:
            ...
            seen[dedup_key] = {
                ...
                "error":        error_msg or "",
                "warning":      worker_warning if display_status == "running" else None,
            }

        # Section-wide, not tied to any specific repo -- shown as soon as the
        # Codebase panel opens, so a user sees infra problems up front rather
        # than only discovering them via a per-repo "running" warning or a
        # job that fails deep in its embedding step.
        infra = {
            "index_worker_running": _index_worker_running(),
            "embed_svc_reachable":  _embed_svc_reachable(),
        }

        return {"repos": list(seen.values()), "infra": infra}
...
def delete_repo(
    ...
    if not branch:
        raise HTTPException(status_code=400, detail="branch is a required query parameter")
    # product_id is optional — POST /index/submit allows indexing a repo with
    # no linked product, so a repo submitted that way must still be
    # deletable. "" here means "the request/vectors with no product_id",
    # matched below via IS NULL rather than a literal string comparison.

    # Dept gate: verify the caller's department is mapped to this product.
    # A repo with no product_id has no department mapping to check against —
    # mirrors list_repos()'s rule that such repos are visible only to admin
    # + the original submitter, so restrict deletion the same way here.
    if not _is_admin(current_user):
        user_dept = current_user.get("department", "")
        try:
            conn_chk = _get_pg()
            try:
                if not product_id:
                    with conn_chk.cursor() as cur:
                        cur.execute(
                            """SELECT 1 FROM index_requests
                                WHERE repo_name = %s AND branch = %s
                                  AND product_id IS NULL
                                  AND requested_by = %s""",
                            (name, branch, current_user.get("email", ""))
                        )
                        if not cur.fetchone():
                            raise HTTPException(status_code=403, detail="Only the original submitter or an admin can delete a repo with no linked product")
                else:
                    with conn_chk.cursor() as cur:
                        cur.execute(
                            "SELECT department FROM dept_product_mappings WHERE product_id::text = %s",
