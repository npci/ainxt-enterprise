...
def ide_get_models(_u: dict = Depends(_require_auth)):
    ...
    from core.model_registry import (
        ...
        OPENAI_CODING_MODEL, OPENAI_SIMPLE_MODEL, OPENAI_LATEST_MODEL,
        ...
        CLAUDE_PRIMARY_DISPLAY, CLAUDE_HAIKU_DISPLAY,
        CLAUDE_OPUS_DISPLAY, CLAUDE_OPUS_48_DISPLAY, CLAUDE_OPUS_5_DISPLAY,
        CLAUDE_SONNET_5_DISPLAY,
        OPENAI_CODING_DISPLAY, OPENAI_SIMPLE_DISPLAY, OPENAI_LATEST_DISPLAY,
    ...
    providers = [
        ...
        },
        {
            ...
            "models": [
                {"id": "claude", "label": f"{CLAUDE_PRIMARY_DISPLAY} ({CLAUDE_PRIMARY_MODEL})", "hint": "claude"},
                {"id": "haiku",  "label": f"{CLAUDE_HAIKU_DISPLAY} ({CLAUDE_HAIKU})",           "hint": "haiku"},
                *([
                    {"id": "opus", "label": f"{CLAUDE_OPUS_DISPLAY} ({CLAUDE_OPUS_MODEL})", "hint": "opus"},
                ] if ENABLE_OPUS else []),
                *([
                    {"id": "opus-4-8", "label": f"{CLAUDE_OPUS_48_DISPLAY} ({CLAUDE_OPUS_48_MODEL})", "hint": "opus-4-8"},
                ] if (ENABLE_OPUS and ENABLE_CLI_OPUS_48) else []),
                *([
                    {"id": "opus-5", "label": f"{CLAUDE_OPUS_5_DISPLAY} ({CLAUDE_OPUS_5_MODEL})", "hint": "opus-5"},
                ] if ENABLE_CLI_OPUS_5 else []),
                *([
                    {"id": "sonnet-5", "label": f"{CLAUDE_SONNET_5_DISPLAY} ({CLAUDE_SONNET_5_MODEL})", "hint": "sonnet-5"},
                ] if ENABLE_SONNET_5 else []),
            ],
        },
        {
            "provider": "OpenAI",
            "models": [
                {"id": "gpt",      "label": f"{OPENAI_CODING_DISPLAY} ({OPENAI_CODING_MODEL})",  "hint": "gpt"},
                {"id": "deep",     "label": f"{OPENAI_LATEST_DISPLAY} ({OPENAI_LATEST_MODEL})",  "hint": "deep"},
                {"id": "gpt-mini", "label": f"{OPENAI_SIMPLE_DISPLAY} ({OPENAI_SIMPLE_MODEL})",  "hint": "gpt-mini"},
                # GPT-5.6 Tera — high-capacity variant, Chat + CLI.
                *([
                    {"id": "tera", "label": f"{OPENAI_TERA_DISPLAY} ({OPENAI_TERA_MODEL})", "hint": "tera"},
                ] if ENABLE_GPT56_TERA else []),
                # GPT-5.6 Luna — efficient variant, Chat + CLI.
                *([
                    {"id": "luna", "label": f"{OPENAI_LUNA_DISPLAY} ({OPENAI_LUNA_MODEL})", "hint": "luna"},
                ] if ENABLE_GPT56_LUNA else []),
            ],
        },
        {
            "provider": "Google",
            "models": [
                {"id": GEMINI_TEXT_MODEL,        "label": f"{GEMINI_TEXT_DISPLAY} ({GEMINI_TEXT_MODEL})",               "hint": GEMINI_TEXT_MODEL},
                {"id": GEMINI_CODING_LITE_MODEL, "label": f"{GEMINI_CODING_LITE_DISPLAY} ({GEMINI_CODING_LITE_MODEL})", "hint": GEMINI_CODING_LITE_MODEL},
                # Gemini 3.1 Flash Image model intentionally hidden from IDE /ide/models
            ],
        },
    ]
