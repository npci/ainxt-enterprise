...
Access control: all endpoints require AD level ≤ 3 or role = "admin".
This mirrors the sidebar maxLevel: 3 rule so the API cannot be called
by regular users (level 4-6) even if they know the URL.
"""
...
def _require_eval_access(current_user: dict = None):
    """
    Bug-fix: enforce the same access rule as the sidebar (maxLevel: 3).
    Raises 403 for regular users (ad_level > 3) who are not admins.
    Imported lazily to avoid circular imports at module load time.
    """
    from auth.dependencies import get_current_user as _gcu
    from fastapi import Depends as _Depends
    # This function is used as a FastAPI dependency factory — see _eval_auth below.
    pass


def _eval_auth():
    """FastAPI dependency: require ad_level ≤ 3 or role = admin."""
    from auth.dependencies import get_current_user as _gcu
    async def _check(current_user: dict = Depends(_gcu)):
        ad_level = current_user.get("ad_level", 6)
        role     = current_user.get("role", "")
        if ad_level > 3 and role != "admin":
            raise HTTPException(
                status_code=403,
                detail="Eval Observatory requires manager-level access (AD level ≤ 3).",
            )
        return current_user
    return _check
