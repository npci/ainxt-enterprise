    ...
    try:
        ...
        try:
            if recipient_emails:
                ...
                # returns the sole active admin as hod_email (delegatees is
                # always empty in flat mode). An empty recipient_emails here
                # means there is no active admin to route to, or the
                # submitter IS that admin — in the latter case create_product()
                # already auto-approved via is_sole_active_admin(), so there
...
def _require_dept_approver_or_admin(db, current_user: dict, product_id: str):
    ...
    Flat/admin-only mode (HOD_APPROVAL_ENABLED=false, the default): the
    ad_level≤3 dept-approver bypass is dropped — there is no real
    department/seniority hierarchy to have granted that authority, so
    product management (update/delete/repo changes) is admin-only.
    """
...
def _can_approve_product(db, current_user: dict, p) -> bool:
    """Non-raising: True iff current_user may approve/reject product `p`.

    Admin, or the submitter's own HOD/delegate (is_request_approver), or —
    when the submitter has no resolvable HOD at all — any admin/ad_level<=
    APPROVAL_AD_LEVEL user whose department is mapped to this product. That
    third branch mirrors _notify_approvers_product()'s own fallback routing
    (same department filter) further down this file: departments without a
    department_hod_mapping row (anything other than the couple of seeded
    departments) otherwise have no resolvable HOD for ANY submitter, so
    approve/reject always 403'd for everyone except admin — even though the
    notification fallback had already paged exactly these department-level
    approvers as the intended people to act.
    """
    from auth.rbac import is_admin, is_request_approver
    if is_admin(current_user):
        return True
    if is_request_approver(current_user, p.requested_by or ""):
        return True
    from auth.rbac import resolve_request_approvers
    approvers = resolve_request_approvers(p.requested_by or "")
    if approvers.get("hod_email"):
        return False  # submitter DOES have a resolvable HOD — only they/admin may act
    from core.config import HOD_APPROVAL_ENABLED
    if not HOD_APPROVAL_ENABLED:
        return False  # flat/admin-only mode: no dept-level fallback approver, admin only
    if not can_approve(current_user):
        return False
    depts = _get_product_depts(db, str(p.id))
    return current_user.get("department", "") in depts
...
def create_product(
    ...
    from db.database import SessionLocal
    from db.models import Product, DeptProductMapping
    from auth.rbac import is_admin, is_sole_active_admin
    from core.config import HOD_APPROVAL_ENABLED

    # Flat/admin-only mode (HOD_APPROVAL_ENABLED=false, the default): no real
    # department/HOD hierarchy exists, so the product catalog is admin-only —
    # a non-admin submitter has nobody to route a PENDING_APPROVAL product to
    # (the maker-checker in this mode only ever happens between two admins,
    # see the status logic below). HOD-hierarchy mode is unchanged: any user
    # may still submit a product for their department's HOD to approve.
    if not HOD_APPROVAL_ENABLED and not is_admin(current_user):
        raise HTTPException(403, "Admin access required to create products")
    ...
    # validate_identifier() (used for name) and validate_product_code()
    # both treat "" as valid — correct for the many genuinely-optional
    # fields that share those validators, but name/code are required here.
    # Without this, an empty name/code sailed through as a 201 (a nameless/
    # codeless product), and a second one collided on the empty-string
    # unique code and 500'd until the is_active-scoping fix above.
    if not sanitized["name"]:
        raise HTTPException(400, "name: Name is required")
    if not sanitized["code"]:
        raise HTTPException(400, "code: Code is required")

    code = sanitized["code"]

    # 4-eyes principle, never bypassed for the creator themselves:
    ...
    #     creator no longer auto-approves their own product; it must be
    #     approved by a DIFFERENT admin (see approve_product's 4-eyes check
    #     and _notify_approvers_product's admin-broadcast routing below).
    #     Exception: if this admin is the ONLY active admin in the system,
    #     there is nobody else to approve it, so it auto-approves.
    if HOD_APPROVAL_ENABLED:
        _status = "ACTIVE" if is_admin(current_user) else "PENDING_APPROVAL"
    else:
        _status = "ACTIVE" if is_sole_active_admin(current_user) else "PENDING_APPROVAL"

    db = SessionLocal()
    try:
        # NOT scoped to is_active=True: products.code/name carry a plain
        # UNIQUE constraint at the DB level that a soft-deleted (is_active=
        # False) row still occupies. Checking only active rows here let a
        # reused code/name from a deleted product pass this check and then
        # crash db.flush() below with an unhandled IntegrityError (500,
        # non-JSON body — surfaced to the user as an unhelpful generic
        # "Create failed" with no indication why).
        from db.models import Product as P
        if db.query(P).filter(P.code == code).first():
            raise HTTPException(409, f"Product code '{code}' already exists")
        if db.query(P).filter(P.name == sanitized["name"]).first():
...
def list_pending_products(current_user: dict = Depends(get_current_user)):
    from db.database import SessionLocal
    from db.models import Product, DeptProductMapping
    from auth.rbac import is_admin, can_approve_governance

    if not is_admin(current_user) and not can_approve_governance(current_user):
...
def list_departments(current_user: dict = Depends(get_current_user)):
    ...
    # Individual/OSS mode: no org chart, no HOD to route approvals to — the
    # department concept collapses to one fixed value. All three frontend
    # consumers (KB scope picker, KB private-visibility picker, Product
    # creation) render whatever this endpoint returns with no logic of their
    # own, so this one change covers all of them.
    if not HOD_APPROVAL_ENABLED:
...
def approve_product(product_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if not _can_approve_product(db, current_user, p):
            raise HTTPException(403, "Only the submitter's HOD (or their delegate), a same-department senior approver, or an admin can approve products")
    ...
    # recipient (other HODs/delegates/approvers who were notified at submit
    # time) -- otherwise they keep seeing this as still-actionable forever,
    # since product_approval items carry no reusable metadata.status the
    # way governance_approval items do (see delete_all_by_source's docstring).
    from store.inbox_store import delete_all_by_source
    delete_all_by_source("product_approval", product_id)
...
def reject_product(
    ...
    try:
        ...
        if not _can_approve_product(db, current_user, p):
            raise HTTPException(403, "Only the submitter's HOD (or their delegate), a same-department senior approver, or an admin can reject products")
...
def update_product(
    ...
    try:
        ...
        # name carries a DB-level UNIQUE constraint (like code, see
        # create_product) and validate_identifier() treats "" as valid —
        # reject empty here, and pre-check the collision so a rename to an
        # in-use name 409s cleanly instead of crashing db.commit() below.
        if "name" in sanitized:
            if not sanitized["name"]:
                raise HTTPException(400, "name: Name is required")
            from db.models import Product as P
            if db.query(P).filter(P.name == sanitized["name"], P.id != product_id).first():
                raise HTTPException(409, f"Product name '{sanitized['name']}' already exists")
...
def list_dept_mappings(product_id: str, current_user: dict = Depends(get_current_user)):
    ...
    Had no access check at all — any authenticated user could enumerate
    which departments a given product_id is mapped to, regardless of
    whether they can otherwise see that product. Apply the same
    dept-member-or-admin visibility rule GET /products/{id} already uses.
    """
    from db.database import SessionLocal
    from db.models import DeptProductMapping
    from auth.rbac import is_admin
