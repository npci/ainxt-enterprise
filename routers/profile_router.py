...
TOKEN_TYPES = {"local_llm", "atlassian", "gitlab", "github", "neuron"}  # "neuron" kept for DB compat
...
class TokenUpsert(BaseModel):
    ...
    # 'atlassian', 'gitlab' or 'neuron'" for any GitHub-provider deployment.
    token_type: Literal["local_llm", "atlassian", "gitlab", "github", "neuron"]
    value:      str
...
def get_decrypted_token(user_id: str, token_type: str) -> Optional[str]:
    """
    Internal helper — called by model_router when tier=NEURON.
