...
def _cache_key(text: str, *, rag_mode: str, history_summary: str,
    ...
    # v7: media-estimation preamble added to user message + expanded vid/img guidance.
    # Structured video production briefs (scene headings, timecodes, camera directions)
    # now correctly set vid_intent="generate". Invalidates v6 entries that incorrectly
    # cached vid_intent="none" for these prompts.
...
class UnifiedIntent:
    ...
    vid_duration_secs: int = 8           # 4..8 (Veo API limit)
...
_VID_INTENT_GUIDANCE = (
    ...
    "- vid_duration_secs: integer, must be 4–8 (Veo API hard limit). "
    "Default to 8 when the user does not mention a duration. "
    "Infer ONLY from explicit user intent words — e.g. 'short clip' or 'quick' → 4; "
    "'brief' → 5; 'medium' → 6; 'long' or 'extended' → 8. "
    "NEVER infer duration from incidental numbers in the prompt such as ages "
    "('65-year-old'), scene timings ('0–3 seconds'), counts, or any number that "
    "is not the user explicitly requesting a video of that length.\n"
)
...
def classify(text: str, *, rag_mode: str = "off",
    ...
    try:
        ...
        try:
            # Clamp to Veo API-supported range: 4–8 seconds.
            _vid_duration = max(4, min(8, int(data.get("vid_duration_secs") or 8)))
        ...
        result = UnifiedIntent(
            ...
            vid_duration_secs=_vid_duration if include_vid_intent else 8,
        )
