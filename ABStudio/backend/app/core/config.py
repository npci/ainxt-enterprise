...
def factory_agent_model() -> str:
    ...
    the *generated agents* will run on at execution time. Agents created via
    "Create with AI" default to Claude Sonnet 4.6 for strong instruction
    following; operators can override the SKU with
    ``ABSTUDIO_AGENT_DEFAULT_MODEL`` without touching the factory model.
    A user who names a model in the factory chat overrides this per-run
    (see ``workflow_factory/pipeline.py`` ``preferred_model`` handling).
    """
    return os.getenv("ABSTUDIO_AGENT_DEFAULT_MODEL", "").strip()
