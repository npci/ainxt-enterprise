...
def resolve_factory_model() -> str:
    """Resolve the factory's own LLM model from the environment at CALL time.

    Reading the env lazily (rather than binding a module-level constant at
    import) means an operator can fix ``FACTORY_MODEL`` in ``.env`` and have a
    ``--reload`` gateway pick it up without a full process restart. The old
    import-time constant could go stale when only some modules were reloaded,
    which surfaced as the agent factory still calling a decommissioned default
    (``claude-sonnet-4-6``) after the workflow factory had already switched.
    """
    return (
        os.getenv("FACTORY_MODEL")
        or os.getenv("LOCAL_LLM_MODEL")
        or os.getenv("CLAUDE_PRIMARY_MODEL", "")
    )
...
SKILL_KEYWORDS: dict[str, str] = {
    ...
    "pdf file": "pdf",
    "pdf": "pdf",
    "co-author": "doc-coauthoring",
    "coauthor": "doc-coauthoring",
    "document draft": "doc-coauthoring",
    "technical spec": "doc-coauthoring",
}
