...
def resolve_fallback_model() -> str:
    """Resolve the auto-failover model from the env at CALL time.

    Read lazily so a live ``.env`` fix (``ABSTUDIO_FALLBACK_LLM_MODEL``) takes
    effect on the next ``--reload`` without a full restart. On a local-only
    deployment this MUST be a locally-served model — the historical default
    (``claude-sonnet-4-6``) is a cloud model the ainxtlocal gateway rejects
    with a 403, which surfaced as a confusing factory error whenever a local
    primary call had a transient blip and the wrapper failed over.
    """
    return os.getenv("ABSTUDIO_FALLBACK_LLM_MODEL", "")
...
def _build_llm_client_for_model(llm_config: LLMConfig) -> BaseLLMClient:
    ...
    # Cloud — the LLM proxy is the PRIMARY endpoint for cloud models.
    # Route through the proxy's tools-stream endpoint family when configured.
    from app.core.config import (
        llm_proxy_root as _llm_proxy_root,
        openai_compatible_base_url as _compat_base_url,
        openai_compatible_api_key as _compat_api_key,
    )
    if _llm_proxy_root():
        common = dict(
            model=model,
    ...
    # No proxy configured — fall back to the OpenAI-compatible endpoint
    # (OPENAI_COMPATIBLE_BASE_URL). ``openai_compatible_base_url`` already
    # encodes the proxy-first precedence; since the proxy is absent here it
    # resolves to OPENAI_COMPATIBLE_BASE_URL → LOCAL_LLM_BASE_URL → localhost.
    # We only take this branch when the operator actually configured a
    # non-localhost compatible surface — otherwise we let the ProxyClient
    # raise its clear ``_ProxyPermanentError`` so a fully unconfigured
    # environment still fails loudly instead of silently hitting localhost.
    if os.getenv("OPENAI_COMPATIBLE_BASE_URL") or os.getenv("LOCAL_LLM_BASE_URL"):
        return OpenAIClient(
