...
# Resolve the bundled xlsx pipeline script once at import time so a missing
# script surfaces in logs at startup rather than on the first upload.
_XLSX_PIPELINE_SCRIPT = (
    Path(__file__).resolve().parent.parent.parent.parent
    / "skills" / "ainxt-skills" / "xlsx" / "scripts" / "xlsx_pipeline.py"
)

# How long the xlsx pipeline subprocess may run before we give up. Large
# workbooks legitimately take a few seconds; anything over a minute almost
# certainly means a corrupt file or pathological size.
_XLSX_PIPELINE_TIMEOUT_SEC = 60

_XLSX_EXTENSIONS = {"xlsx", "xls", "xlsm"}


def _run_xlsx_pipeline(raw_bytes: bytes, filename: str) -> Dict[str, Any]:
    """Persist the upload to a temp file and run xlsx_pipeline.py report.

    Returns the parsed JSON payload. Raises HTTPException on any failure
    so the route turns it into a clean 400/500 for the client.
    """
    if not _XLSX_PIPELINE_SCRIPT.exists():
        raise HTTPException(
            status_code=500,
            detail=(
                "Excel processing is not available — xlsx_pipeline.py is "
                f"missing at {_XLSX_PIPELINE_SCRIPT}."
            ),
        )

    # Preserve the original suffix so the pipeline's extension check passes.
    suffix = Path(filename).suffix or ".xlsx"
    tmp = tempfile.NamedTemporaryFile(suffix=suffix, delete=False)
    try:
        tmp.write(raw_bytes)
        tmp.flush()
        tmp.close()  # close so the subprocess can re-open on Windows

        try:
            # Force UTF-8 for both the child's I/O and our own decoding so
            # workbooks containing characters cp1252 can't represent (e.g.
            # U+2011 non-breaking hyphen, curly quotes, en/em-dashes,
            # non-Latin scripts) don't blow up the pipeline on Windows,
            # where the default console codec is "charmap". `errors=
            # "replace"` is a safety net — the child should always emit
            # valid UTF-8, but if anything upstream (e.g. a warning line
            # from a C extension) sneaks in with a different encoding we'd
            # rather substitute U+FFFD than 500 the whole upload.
            child_env = {**os.environ, "PYTHONIOENCODING": "utf-8"}
            proc = subprocess.run(
                [sys.executable, str(_XLSX_PIPELINE_SCRIPT), "report", tmp.name],
                capture_output=True,
                text=True,
                encoding="utf-8",
                errors="replace",
                timeout=_XLSX_PIPELINE_TIMEOUT_SEC,
                env=child_env,
            )
            ...
            raise HTTPException(
                status_code=400,
                detail=(
                    f"Excel processing timed out after "
                    f"{_XLSX_PIPELINE_TIMEOUT_SEC}s; workbook is too large or "
                    f"too complex for inline analysis."
                ),
            )

        if proc.returncode != 0:
            # rc=2 is "missing file" — shouldn't happen here since we just
            # wrote it. rc=1 means the workbook itself could not be read.
            detail = (proc.stderr or "").strip() or "xlsx pipeline failed"
            raise HTTPException(
                status_code=400,
                detail=f"Could not read Excel file: {detail}",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"xlsx pipeline produced invalid JSON: {exc}",
            )
    finally:
        try:
            os.unlink(tmp.name)
        except OSError:
            # Temp cleanup is best-effort; the OS will reap /tmp on its own.
            pass
...
def _extract_via_pipeline(
    ...
    """Single dispatch point — XLSX keeps its dedicated subprocess path,
    everything else flows through ``app.core.ocr_pipeline``.
    ...
    # Excel keeps its dedicated subprocess pipeline because the model
    # consumes both a human summary and an embedded JSON report.
