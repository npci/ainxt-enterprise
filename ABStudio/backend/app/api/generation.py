...
def _build_instructions_llm_config(
    ...
    """Build an LLMConfig sourced strictly from FACTORY_MODEL / FACTORY_BASE_URL /
    FACTORY_API_KEY — no LOCAL_LLM_* or hardcoded fallbacks.

    Generate Instructions is a meta task that must run against the dedicated
    factory model, not whatever model happens to be configured for runtime
    agent execution.
    """
    from app.models import LLMConfig, LLMProvider

    model = os.getenv("FACTORY_MODEL", "").strip()
    base_url = os.getenv("FACTORY_BASE_URL", "").strip()
    api_key = os.getenv("FACTORY_API_KEY", "").strip()
    missing = [
        name for name, val in
        (("FACTORY_MODEL", model), ("FACTORY_BASE_URL", base_url), ("FACTORY_API_KEY", api_key))
        if not val
    ]
    if missing:
        raise HTTPException(
            status_code=503,
            detail=f"Generate Instructions is not configured. Missing env: {', '.join(missing)}.",
        )
    return LLMConfig(
