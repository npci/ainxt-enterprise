...
# The DSLAR audit workflow runs ~8 agents, several of which write intermediate
# JSON / marker scratch files (control.json, diag*.json,
# image_enrichment_status.json, metadata_read_marker.txt, ...) via code_executor.
# Each of those surfaces as a download chip, burying the one deliverable the user
# wants: the final ``validation-report-complete-*.pdf`` from the renderer node.
# We detect the workflow by its unique renderer node id and, at run completion,
# collapse the accumulated file list down to just that PDF.
_DSLAR_RENDERER_NODE_ID = "report-pdf-renderer"
_DSLAR_DELIVERABLE_PREFIX = "validation-report-complete-"


def _is_dslar_workflow(gctx: "_GraphCtx") -> bool:
    return _DSLAR_RENDERER_NODE_ID in (gctx.nodes_by_id or {})


def _prune_generated_files_for_dslar(state: "_ExecState", gctx: "_GraphCtx") -> None:
    """For the DSLAR workflow, keep only the final validation-report PDF in
    ``state.generated_files`` so the UI offers a single, correct download.

    Mutates ``state.generated_files`` in place so every downstream consumer
    (the terminal ``complete`` SSE event and ``_save_history``) sees the pruned
    list. No-op for any other workflow, and a no-op if no deliverable PDF was
    produced (e.g. the renderer failed) — in that case the original list is
    left untouched so the user can still see whatever was generated."""
    if not _is_dslar_workflow(gctx):
        return
    files = state.generated_files or []
    if not files:
        return

    def _is_deliverable(f: dict) -> bool:
        name = (f.get("filename") or "").lower()
        url = (f.get("download_url") or "").lower()
        return (name.startswith(_DSLAR_DELIVERABLE_PREFIX) and name.endswith(".pdf")) \
            or (_DSLAR_DELIVERABLE_PREFIX in url and url.endswith(".pdf"))

    deliverables = [f for f in files if _is_deliverable(f)]
    if deliverables:
        state.generated_files = deliverables
...
class NativeEngine(OrchestrationEngine):
    ...
    async def execute(
        ...
        try:
            ...
            # Surface only the final deliverable for the DSLAR audit workflow,
            # not every intermediate scratch file. No-op for other workflows.
            _prune_generated_files_for_dslar(state, gctx)
