...
DEFAULTS = {
    ...
    # The env template(s) that must stay clean. env.example is deliberately NOT
    # listed: it is an unreferenced 1027-variable dump slated for removal and
    # carries 13 duplicate keys of its own. Add it here to gate on it too.
...
def check_model_hint_coverage(cfg) -> list[str]:
    ...
    # The ids in that body are import aliases, e.g.
    #     from core.model_registry import OPENAI_SIMPLE_MODEL as _OPENAI_SIMPLE
    # and the registry defines the literal as
    #     OPENAI_SIMPLE_MODEL = os.getenv("OPENAI_SIMPLE_MODEL", "gpt-5-mini")
    # so resolve alias -> registry name -> default literal. Without following that
    # chain the check found no ids at all and passed trivially.
    alias_to_name = dict(re.findall(r"^\s*([A-Z][A-Z_0-9]*)\s+as\s+(_[A-Z][A-Z_0-9]*)\s*,?\s*$",
                                    src, re.M))
    alias_to_name = {alias: name for name, alias in alias_to_name.items()}

    # Model IDs are no longer hardcoded in the registry — they come entirely from
    # env config (.env / .env.internal.example). Read the internal profile as the
    # reference for what model IDs look like; fall back to the registry var name
    # itself so the check degrades gracefully when the file is absent.
    env_vals: dict[str, str] = {}
    for env_file in (ROOT / ".env.internal.example", ROOT / ".env.example"):
        if env_file.exists():
    ...
    if registry.exists():
        rsrc = registry.read_text(encoding="utf-8", errors="replace")
        # Collect registry var names; resolve their values from env profile first,
        # then fall back to any non-empty code default (legacy, should be "").
        for var_name in re.findall(
            r'^([A-Z][A-Z_0-9]*)\s*=\s*os\.getenv\(\s*"([^"]+)"', rsrc, re.M
        ):
            py_name, env_key = var_name
            if env_key in env_vals and env_vals[env_key]:
                literals[py_name] = env_vals[env_key]
        # Also pick up any remaining non-empty code literals (display names etc.)
        literals.update(dict(re.findall(r'^([A-Z][A-Z_0-9]*)\s*=\s*"([^"]+)"\s*(?:#.*)?$', rsrc, re.M)))
    # aliases defined locally as plain literals too
    literals.update(dict(re.findall(r'^(_[A-Z][A-Z_0-9]*)\s*=\s*"([^"]+)"', src, re.M)))

    ids, unresolved = set(), []
    for alias in re.findall(r'\{"id":\s*(_[A-Z][A-Z_0-9]*)', body):
        target = alias_to_name.get(alias, alias)
        if target in literals:
            ids.add(literals[target])
        elif alias in literals:
            ids.add(literals[alias])
        else:
            unresolved.append(alias)
    ids |= set(re.findall(r'\{"id":\s*"([a-z0-9.\-]+)"', body))
    if not ids:
        return ["gateway.py: could not resolve any advertised model id — "
                "this check would pass trivially, so failing instead"]
    bad = []
    for mid in sorted(i for i in ids if i not in ("auto", "default")):
        if mid in ("local", "inhouse", "in-house"):
            continue
    ...
    for alias in unresolved:
        bad.append(f"gateway.py: could not resolve advertised id {alias} — "
                   f"add its literal to core/model_registry.py or check the alias")
    return bad
