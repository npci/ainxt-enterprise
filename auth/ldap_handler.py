...
# AD attributes to fetch on every user lookup
_USER_ATTRS = [
    "sAMAccountName",
    "mail",
    "cn",
    ...
    "memberOf",
    "userPrincipalName",
    "employeeID",          # numeric employee ID (populated in many AD schemas)
]
...
def _do_authenticate(username: str, password: str, conn: "ldap3.Connection") -> Optional[dict[str, Any]]:
    ...
    search_filter = (
        f"(&{LDAP_USER_FILTER}"
        f"(|(sAMAccountName={safe_user})(mail={safe_user})))"
    )
    conn.search(
...
def get_user_attributes(identifier: str) -> Optional[dict[str, Any]]:
    ...
    try:
        ...
        search_filter = (
            f"(&{LDAP_USER_FILTER}"
            f"(|(mail={safe_id})(sAMAccountName={safe_id})))"
        )
        conn.search(
            search_base=LDAP_BASE_DN,
            search_filter=search_filter,
