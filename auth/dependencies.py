...
def enrich_user_context(payload: dict) -> dict:
    ...
    invalidate_profile_cache(user_id) (see below), which correctly targets
    Redis DB 8 (_PROFILE_CACHE_REDIS_DB) — a bare `redis-cli DEL profile:<id>`
    on the default DB (0) or the "rl:profile:<id>" key this docstring
    previously (incorrectly) documented will silently no-op. Confirmed via
    live debugging: a stale department survived a manual cache-clear attempt
    that used the wrong key, and non-admin dept-based filtering (GET
    /products) kept using the stale value until DB 8 was targeted directly.
    """
    ...
    if rc is not None:
        try:
            ...
            if cached:
                ...
                # Tier-1 HOD-hierarchy suppression: when HOD_APPROVAL_ENABLED is
                # false (flat/admin-only mode, the default), every downstream
                # consumer of is_hod(current_user) / get_hod_departments(current_user)
                # must see a non-HOD identity, regardless of what the cached
                # profile/DB actually says.
                from core.config import HOD_APPROVAL_ENABLED as _HOD_APPROVAL_ENABLED
                payload.setdefault(
                    "is_hod",
                    _HOD_APPROVAL_ENABLED and bool(profile.get("is_hod", False)),
                )
                payload.setdefault(
                    "hod_departments",
                    list(profile.get("hod_departments") or []) if _HOD_APPROVAL_ENABLED else [],
                )
    ...
    try:
        ...
        try:
            ...
            if u:
                ...
                from core.config import HOD_APPROVAL_ENABLED as _HOD_APPROVAL_ENABLED
                hod_departments = _lookup_hod_departments(email) if _HOD_APPROVAL_ENABLED else []
                is_hod_flag = _HOD_APPROVAL_ENABLED and ((hod_check is not None) or bool(hod_departments))
