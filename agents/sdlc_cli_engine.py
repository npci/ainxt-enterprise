...
_PROFILE_PRESETS: dict[str, tuple[str, str]] = {
    ...
    # could loop on a single failed name search. Read-only-ness rests on the WITHHELD
    # toolset (no Write/Edit/MultiEdit), not on permission_mode: auto-approval is now
    # unconditional, so the mode no longer gates anything Bash could do. Same posture as
    # the "govscan" profile below, which also grants Bash under plan mode.
    "plan": ("plan", "Read,Grep,Glob,Bash"),
    "code": ("acceptEdits", "Read,Write,Edit,MultiEdit,Grep,Glob,Bash"),
    "govreview": ("acceptEdits", "Read,Grep,Glob,Bash"),
    "govscan": ("plan", "Read,Grep,Bash"),
}
...
def _build_argv(
    *,
    ...
    # `-p/--single` (no --print/--full), the toolset via `--tools` (not --allowed-tools),
    # auto-approval via `--always-approve` (not --yes/--dangerously-skip-permissions), the
    # schema INLINE via `--json-schema`, and there is NO --verbose/--no-review/stream-frame
    # flags. Output is forced to single-envelope `json` (streaming-json uses a different
    # per-token frame format we intentionally do not parse). resume/--max-turns/
    # --permission-mode are unchanged. Plugins reuse the existing seam.
    if is_v3:
...
def _is_transient_failure(exit_code: int, subtype: str, buffers: str) -> bool:
...
def run_cli(
    *,
    ...
    try:
        ...
        # ── (A)-vs-(B) tool-subset signal ────────────────────────────────────────
        # Did the CLI actually REGISTER every tool we requested via --allowed-tools?
        # A subset means MultiEdit/Grep/Glob/Write were dropped, so the coder falls
        # back to one Edit round-trip per change (the flat ~95s/Edit slowdown). This
        # warning tells you WHICH cause, straight from the run log, without deriving it:
        #   • missing == the extra tools (Write/MultiEdit/Grep/Glob) → binary honored the
        #     allow-list but this BUILD lacks them, or it filtered them by permission-mode
        #     (an ops/binary issue — a format change can't fix it).
        #   • missing == everything but a bare default → the allow-list wasn't parsed.
        # Best-effort; silent when the binary emits no init event (older build / non-stream
        # mode) — that's 'unknown', not 'empty', so we don't false-alarm.
        try:
            _registered = _extract_registered_tools(stdout)
            if _registered is not None and allowed_tools:
...
def _parse_cli_envelope(stdout: str, *, exit_code: int) -> CliResult:
    ...
    return CliResult(
        ...
        total_cost_usd=total_cost_usd, session_id=session_id,
    )
