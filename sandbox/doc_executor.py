...
def build(code: str, fmt: str, images: list | None = None) -> DocBuildResult:
    ...
    try:
        ...
        with tarfile.open(fileobj=payload, mode="w:gz") as archive:
            archive.add(src_path, arcname=spec["src"])
            for image_path in image_paths:
                archive.add(image_path, arcname=os.path.basename(image_path))

        container_script = (
            "unpack_status=0; tar -xzf - -C /work || unpack_status=$?; "
            "if [ \"$unpack_status\" -eq 0 ]; then "
            f"( {run_script} ) >/tmp/build.log 2>&1; build_status=$?; "
            "else printf 'Failed to unpack build input.\\n' >/tmp/build.log; "
            "build_status=$unpack_status; fi; "
            "tar -czf - -C /work . -C /tmp build.log; exit $build_status"
        )
        cmd = [
            "docker", "run", "--rm", "-i",
            "--network", "none",
            "--memory", "1g", "--cpus", "1",
            "--pids-limit", "256",
            "--read-only", "--tmpfs", "/tmp", "--tmpfs", "/work:rw,mode=1777",
            ...
            with tarfile.open(fileobj=io.BytesIO(proc.stdout), mode="r:gz") as archive:
                for member in archive:
                    ...
                    if not member.isfile() or not (
                        name == "build.log"
                        or name == "output.pdf"
                        or name.lower().endswith(f".{spec['deliver']}")
                        or (name.startswith("page") and name.endswith(".jpg"))
                    ):
                        continue
                    data_file = archive.extractfile(member)
                    if data_file is None:
                        continue
                    with data_file, open(os.path.join(workdir, name), "wb") as output_file:
        ...
        build_log_path = os.path.join(workdir, "build.log")
        if os.path.exists(build_log_path):
            with open(build_log_path, encoding="utf-8", errors="replace") as log_file:
                logs = log_file.read() + logs
